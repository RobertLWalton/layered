...
\ttmkey{data}{reformatter}
        \TT{(} {\em ID-sign}\TT{,} {\em equal-sign}\TT{,} {\em negator}\TT{,} 
	       {\em flags-opening}\TT{,} {\em multivalue-opening} \TT{)}
	\label{DATAT-REFORMATTER}


\begin{indpar}[0.5em]
The subexpression must be a MIN value of the form:
\begin{center}
\TT{\{data|} {\em ID} ~ {\em equal-sign} ~ {\em element-list}
     ~ {\em attribute-indented-paragraph}\QMARK{} \TT{|\}}
\end{center}
where
\begin{indpar}
\emmkey{ID}{in \TT{data} reformatter} :::=
    {\em prefix-character}\STAR{} ~ {\em ID-character} ~
    {\em ID-number}
\\[0.5ex]
\emmkey{prefix-character}{in \TT{data} reformatter} ::=
    any character allowed in a {\em numeric} lexeme
\\[0.5ex]
\emmkey{ID-character}{in \TT{data} reformatter} ::=
    first character of the {\em ID-sign} reformatter argument
    (must be ASCII)
\\[0.5ex]
...
\emkey{element-list} ::= see \pagref{ELEMENT-LIST}
\\[0.5ex]
\emmkey{attribute-indented-paragraph}{in \TT{data} reformatter} ::=
    \hspace{0.5in}
    \begin{tabular}[t]{ll}
    & \TT{:} \\
...
is `{\em ID-character} {\em ID-number}'.  The object has the
elements given in the {\em element-list} and the attributes given
in the {\em attribute-indented-paragraph}.  These are formatted
in the same manner as for {\em typed-bracketed-subexpressions}
(see \pagref{TYPED-BRACKETED-SUBEXPRESSION}).

The \TT{\{data| \ldots{} |\}} expression itself is deleted from
