...
initial segement that matches another pattern; in this case the
...
\begin{indpar}\begin{verbatim}
"<other>" translate "\u007F" output bad character;
\end{verbatim}\end{indpar}
...
A parser pass receives a stream as input and another as output.
It may move tokens from input to output by merely adjusting the
`{\tt first}' and `{\tt last}' components of the streams.
...
\end{tabular} \\
\begin{tabular}{@{}p{1.9in}p{4.0in}@{}}
explicit subexpression & Identifies explicitly bracketted subexpressions.
\\[1ex]
lexeme replacement & Replaces sequences of lexemes with other
sequences of lexemes (e.g., replaces plurals by singulars).
\end{tabular} \\
\begin{tabular}{@{}p{1.9in}p{4.0in}@{}}
radix number recognition & Recognizes numbers with non-decimal radices.
\\[1ex]
...
When one parser pass calls another, it can provide the second
pass with a stream that holds the initial tokens in the input
stream to the second pass, while also providing a function for
the second pass to call to get more tokens in its input stream.
