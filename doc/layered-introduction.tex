...
When a parser is initialized, the middle break is disabled.
The `\ttkey{concatenator}'\label{STANDARD-CONCATENATOR-1}
standard component executes the parser command:
\begin{center}
...
When a parser is initialized, the quoted string concatenator is disabled.
The `\ttkey{concatenator}'\label{STANDARD-CONCATENATOR-2}
standard component executes the parser command:
\begin{center}
...
The `\ttkey{brackets}'\label{STANDARD-BRACKETS} standard component executes
the following parser commands:
\begin{center}
\begin{tabular}{l@{\hspace*{0.5in}}l}
\multicolumn{2}{c}{All Commands Require at Least One of
                   {\tt + code, + text, + math}} \\
\multicolumn{2}{c}{Missing Selector Qualifiers are Deleted from Commands}
\\[1ex]
Parser Command				& \shortstack{
                                          Other\\{\bf Required}\\Qualifiers}
\\\hline
\tt define bracket "(" ... ")" \\
\tt ~~~~~~~[code, text, math] \\
\tt define bracket "[" ... "]" \\
