...
\\\cline{4-6}
	...
	&	& 		& \TT{line}  &            & conditional &
\\\cline{3-3}\cline{5-6}
	&	& \TT{exit}	&            & \ttkey{exit} & exit block &
\\\cline{5-6}
	&	& 		&            & \ttkey{continue} & continue loop &
\\\cline{3-6}
...
Subexpressions of a primary with \TT{.initiator} and \TT{.terminator}
...
to an element label.  This differs from other parser table
...
Some primary vector elements being scanned for symbol table
lookups are replaced before they become parts of element labels.
A quoted string \TT{"$S$"} is replaced by $S$.
A subexpression with \TT{.initiator} $I$ and \TT{.terminator}
$T$ is replaced by the label \TT{[< $I$ $T$ >]},
except that a subexpression with
\underline{no} \TT{.initiator} is replaced by \TT{[< "(" ")" >]}
(as if the \TT{.initiator} were `\TT{(}').
Optimizing assumptions are made by the actual \TT{PRIM} code that 
a subexpression with no \TT{.initiator} has no \TT{.terminator},
that a subexpression with \TT{"("} \TT{.initiator} has \TT{")"}
\TT{.terminator}, and that a subexpression with \TT{"["}
\TT{.initiator} has \TT{"]"} \TT{.terminator}.

A record is kept as to whether a quoted string appeared in a
particular lookup so that if quoted strings are not allowed
(as in a variable name) the result of the lookup can be rejected
(after a rejection a lookup can be continued to find the next
possible result).
...
the function, with replacements as indicated above.
If a subexpression (i.e., argument list) is optional (all its
arguments have default values), it and following lexemes and
subexpressions that might be in the call are omitted from the label.
However, a \TT{PRIM::func} label must not consist of just a single
subexpression replacement.

Thus if \TT{\{| \{|x|\} "+" \{|5|\} |\}} is a primary that calls
a function, the label of the function called would be: \\
\centerline{\TT{[< [< ( subexp >] + [< ( subexp >] >]}}
...
The syntax of the vector elements of a primary recognized by \TT{PRIM} is:
\begin{indpar}
...
\\[0.5ex]
\emkey{variable-name} ::= {\em name-lexeme}\PLUS{}
\\[0.5ex]
\emkey{call-term-name}\label{CALL-TERM-NAME} ::= {\em name-lexeme}\PLUS{}
\\[0.5ex]
\emkey{name-lexeme}\label{NAME-LEXEME}
                    \begin{tabular}[t]{rl}
		    ::= & {\em number} $|$
			  {\em natural} $|$
			  {\em numeric-word} \\
		    ...
		    \end{tabular}
\\[0.5ex]
\emkey{argument-list} ::= {\em subexpression}
\\[0.5ex]
...
\item
An unquoted {\em name-lexeme} is represented by a MIN string primary
vector element.  A quoted {\em name-lexeme} is represented by a MIN object
whose only attribute is a \TT{.type} attribute equal to \TT{"<Q>"} and
whose only vector element is the string quoted (e.g., {\em word},
{\em numeric}, {\em mark}, or {\em separator}).
\item
...
{\em separators}, {\em quoted-words}, {\em quoted-marks},
and {\em quoted-separators},
and to require the first lexeme to \underline{not} be a {\em natural}.
\item
...
as it is \underline{before} parsing:
...
\emkey{prototype-term} ::= {\em prototype-term-name}
                           {\em prototype-argument-list}\STAR{}
\\[0.5ex]
\emkey{prototype-term-name} ::= {\em call-term-name}
				excluding
				un-quoted {\em marks} and {\em separators}
\\[0.5ex]
\emkey{call-term-name} ::= see \pagref{CALL-TERM-NAME}
\\[0.5ex]
\emkey{prototype-argument-list} ::= \begin{tabular}[t]{@{}l}
				like {\em ()-prototype-argument-list} but with
				arbitrary \\
				(non-missing)
				\TT{.initiator} and \TT{.terminator}
				\end{tabular}
...
such that the next non-omittable {\em prototype-argument-list} in the
same {\em prototype-term} has the
\underline{same} brackets (e.g., \TT{(~)} or \TT{[~]}) as the omittable
argument list,
then the omittable argument list is said to be \key{shadowed}
by the non-omittable argument list.  Such shadowing is \underline{not}
allowed.
\item
