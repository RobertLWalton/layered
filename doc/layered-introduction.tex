...
$P'$ is the precedence of operators in an EXPRESSION token of the list
that has neither {\tt .initiator} nor {\tt .terminator} attributes
(i.e., is a non-explicitly-bracketed subexpression), then either
$P<P'$ or both $P=P'$ and either both expressions begin with a prefix
...
\item[Rule 8:] A subexpression whose precedence is lower than that
of any operator is begun at the beginning of the list.
A subexpression whose precedence is that of the operator just
found is begin before that operator if either that operator
has precedence greater than the current subexpression precedence
or both the operator has precedence equal to the current subexpression
precedence and the operator immediately follows a prefix operator.
...
is replaced when it ends by a single EXPRESSION
token with no {\tt .initiator} and no {\tt .terminator}.
\end{itemlist}
