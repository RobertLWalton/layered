...
While parsing a logical line the parser maintains the
...
\hspace*{-0.2in}\ttmkey{implied header}{line variable}\\
    ...
    a logical line.  An implied header is an implied prefix separator
    and is almost like an explicit prefix separator except that
    if it has no following {\em prefix-n-list} elements
    it is removed.  It is similar to an implied subprefix,
    see \pagref{IMPLIED-SUBPREFIX}.
...
If an explicit paragraph header is found immediately after implicit headers,
the implicit headers are deleted.

If an explicit line header is found immediately after implicit headers,
any preceeding implicit line header that was inserted at
the beginning of the logical line or following an implicit
...
computed by the implicit headers were used to parse the
explicit header and look up its
prefix definition, so care should be taken that prefix defintions
for implicit headers do not change the selectors and options sufficiently to
