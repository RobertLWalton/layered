...
scanning a special lexeme does not consume these characters, which
will appear in the non-special lexeme that follows the special lexeme.
...
input character is `\TT{|}'.
Output by either the \TT{PARAGRAPH\_\EOL CHECK} or \TT{ROW\_\EOL CHECK}
...
branch to the standard default lexical master after finding a
single special lexeme, or if there is no special lexeme in the
input.  These lexical masters are:
...
If a \TT{parser define mapped lexeme} command gives no {\em token-value},
tokens with its lexeme type are deleted, and not altered.

If a \TT{parser define mapped lexeme} command names a lexical master,
after the token has been deleted or altered, the current lexical master
is changed to the lexical master named, and the next lexeme for the
next token is scanned using that new lexical master.
