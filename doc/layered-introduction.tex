...
A numeric containing commas\label{COMMAS-IN-NUMERICS}
that has the format of a C++ number
not ending in `\TT{.}' is made into a number lexeme by removing the
...
\tt ~~~~~~~with parsing selectors [atom] \\
\tt ~~~~~~~with special reformatter \\
\tt define bracket "\{\{" \ldots~"\}\}"    & \tt + math \\
\tt ~~~~~~~~~~~~~~~[code, text, math] \\
\tt ~~~~~~~with parsing selectors \\
\tt ~~~~~~~~~~~~[+ math, - code, - text] \\
\tt define bracket "`{}`" \ldots~"'{}'"    & \tt + text \\
...
one plus the total number of dangling references with that ID number.
This information
