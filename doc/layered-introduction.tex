...
attributes at the end.  Also \TT{""} may be used as a type label to denote
a missing type.
...
(see \itemref{TYPED-PREFIX-SEPARATORS}) if the context
selectors contain a selector in common with the
...
\TT{~~~~with prefix selectors [prefix allowed]} \\
\end{tabular}
\end{center}
which specifies
\begin{center}
...
and no elements 
should be treated as a {\em typed-prefix-separator}
(see \itemref{TYPED-PREFIX-SEPARATORS})
\underline{if and only if} the context selectors
have the `\TT{prefix allowed}' selector.
\end{minipage}
\end{tabular}
\end{center}
...
*PARSER*:
    define typed bracket "{" ... "|" ... "|" ... "}"
        ...
        with attribute flags initiator "["
        with attribute multivalue initiator "{*"
        with prefix selectors [prefix allowed]
\end{verbatim}\end{indpar}
...
to scan any {\em elements}
are the same as the context options.
Similarly since
`\TT{with element selectors}' does not appear in either definition,
