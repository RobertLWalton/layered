...
A logical line that follows a blank line or the indented paragraph
beginning and itself begins with a paragraph header begins
an unindented paragraph.  The unindented paragraph ends with the
next blank line or the end of the containing indented paragraph.
Unindented paragraphs become a single MIN object whose \TT{.type}
and attributes
are taken from the paragraph header and whose elements are
the logical lines in the unindented paragraph.

The paragraph as a whole is converted to a MIN object whose elements
are the paragraph lines or unindented paragraphs,
whose \TT{.initiator} attribute
is the {\em indentation-mark} that introduced the paragraph,
and whose \TT{.terminator} attribute is \TT{min::INDENTED\_PARAGRAPH()}.
A paragraph may be empty (have no paragraph lines or unindented paragraphs).

The \TT{implied paragraph header} and \TT{lexical master} parameters
are used to initialize line variables
that affect the parsing of unindented paragraphs: see \pagref{LINE-VARIABLES}.
...
\emkey{typed-attribute-flags-opening} ::= {\em key} ~~~ [e.g., `\TT{[}', ~
    ...
    \pagref{PARSER-TYPED-BRACKET-COMMAND}]
\\[0.5ex]
\emkey{typed-attribute-flags-separator} ::= {\em key} ~~~ [e.g., `\TT{,}']
\\[0.5ex]
...
line header lexemes, this value may be used
to set the \TT{implied line header}: see \pagref{LINE-VARIABLES}.
...
\emkey{reformatter-arguments} \begin{tabular}[t]{@{}rl}
			      ...
			      $|$ & \TT{(} {\em reformatter-argument} \\
			          & ~~~~ \{ \TT{,} {\em reformatter-argument}
				         \}\STAR{} \TT{)} \\
			      \end{tabular} \\
\emkey{reformatter-argument} ::= {\em quoted-key}
\end{indpar}
