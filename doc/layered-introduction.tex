...
\\[1ex]
		& For a {\tt BRACKETED} subexpression, the value of the
		  expression is recorded as a MIN object
		  in the token {\tt value}.  This MIN object may have
		  attributes such as an {\tt .initiator} or {\tt .terminator}.
\\[1ex]  
		& Empty subexpressions are recorded as {\tt BRACKETED}
		  subexpressions whose MIN object has no elements and
		  no attributes.
\\[1ex]  
		& Subexpressions whose MIN object would have just a single
		  element and no attributes are recorded as {\tt BRACKETABLE}
		  subexpressions instead.
\\[1ex]
		& A {\tt BRACKETABLE} subexpression is just a subexpression
		  that could be represented by a {\tt BRACKETED} subexpression
		  whose object had a single element and no attributes.
		  The token {\tt value} of such a subexpression is
		  this object's single element.
\\[1ex]  
		& {\tt BRACKETABLE}
		  subexpressions are converted to {\tt BRACKETED}
		  subexpressions if attributes must be added to them.
		  Otherwise their {\tt value} is used directly as
		  an element of any containing expression.
\end{tabular}
\\[1ex]
\begin{tabular}{p{1in}p{4.5in}}
\tt value	& The token {\tt value}, as described above.  This is
...
\\[1ex]
\begin{tabular}{p{1in}p{4.5in}}
\tt position    & The positions in the input file of
...
\\[1ex]
\begin{tabular}{p{1in}p{4.5in}}
\tt indent      & The indentation of the token from the beginning
