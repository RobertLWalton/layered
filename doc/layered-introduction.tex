...
\begin{indpar}
\emkey{misplaced-horizontal} :::= {\em misplaced-horizontal-character}\PLUS{}
\\[0.3ex]
...
\TT{<NUL>}, \TT{<LF>}, \TT{<SP>}, \TT{NBSP}.  There are two other cases:
...
\\[0.5ex]
\emkey{mark} :::= \begin{tabular}[t]{l}
                  {\em mark-character}\PLUS{} but not {\em separator} \\
		  (or {\em middle-lexeme} that is not a {\em word},
...
{\em lexical-item}, by recoginzing a sequence of {\em trailing-separators}
...
(a.k.a. `alias', e.g., \TT{HT} or \TT{SOH} or \TT{NBSP}, or may be `missing'),
...
if and only if the other is in the set.
...
\TT{"}$P$\TT{<others>}$Q$\TT{"},
the prefix \TT{<others>} matches any characters not matched by
...
\begin{indpar}\begin{verbatim}
"<others>" translate to "<DEL>" output bad character;
\end{verbatim}\end{indpar}
