...
\emkey{label-component} ::=
    ...
    \end{tabular}
\\[0.5ex]
\emkey{bracket-type} ::= {\em label}\label{TYPE-LABEL}
\\[0.5ex]
...
When it is encountered, the {\em bracket-type} is looked up on the
...
and no elements 
is to be treated as a {\em typed-prefix-separator}
(see \itemref{TYPED-PREFIX-SEPARATORS})
\underline{only if} the context 
of the {\em typed-bracketed-subexpression}
has the `\TT{prefix allowed}' selector.
\end{minipage}
\end{tabular}
\end{center}
...
to scan any {\em typed-bracketed-subexpression} recognized by
these definitions are the same as the parsing options just before
this {\em typed-bracketed-subexpression}.  Similarly since
`\TT{with element selectors}' does not appear in either definition,
the parsing selectors used to scan the
{\em element-list} of any {\em typed-bracketed-subexpression}
recognized by these definitions are the same as the parsing selectors
just before the {\em typed-bracketed-subexpression}.
...
\\[0.5em]
\emkey{bracket-type} ::= see \pagref{TYPE-LABEL}
\\[0.5em]
...
A \emkey{prefix-type}\label{PREFIX-TYPE} is a
...
(see `\TT{parsing selectors}' below).
