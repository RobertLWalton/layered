...
\emkey{end-at-options} ::= {\em parser-flag-spec}
	~~~~~ [see \pagref{PARSING-END-AT-OPTIONS-1}]
\\[0.5ex]
\emkey{header} ::= {\em paragraph-header} ~~~ [see \itemref{HEADED-PARAGRAPHS}]
\\[0.5ex]
...
and from any flag character by commas.  These commas may be
separate lexemes (e.g., trailing separators, as in `\TT{*,~64}' )
or may be embedded inside lexemes (as in `\TT{*,64}').  Flag
characters can be separated from each other by commas, which are
ignored (as in `\TT{*,/}' or `\TT{*,~/}').  Some examples are:
\begin{center}
\begin{tabular}{l}
\TT{\{.format[H] = (3,4)|A B C|\}} \\
\TT{\{.format[H,64] = (3,4)|A B C|\}} \\
\TT{\{.format[64] = (3,4)|A B C|\}} \\
\TT{\{.format[64,H, 256] = (3,4)|A B C|\}} \\
\end{tabular}
\end{center}
