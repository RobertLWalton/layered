...
\newcommand{\ttsvkey}[1]{\TT{[\$~#1~\$]}\index{#1@{\tt [\$~#1~\$]}}}
\newcommand{\ttsvmkey}[2]{\TT{[\$~#1~\$]}\index{#1@{\tt [\$~#1~\$]}!#2}}
...
The bracketed parser pass also has separate symbol tables
for prefix types and prefix lexemes: see \itemref{PREFIX-DEFINITIONS}.
...
recognized if they begin at the end of a line.
...
\emkey{parser-bracketed-command}
    ...
    $|$ & {\em parser-prefix-lexeme-command}
        & see \pagref{PARSER-PREFIX-LEXEME-COMMAND} \\
    $|$ & {\em parser-print-bracket-command}
        & see \pagref{PARSER-PRINT-BRACKET-COMMAND} \\
    \end{tabular}
...
are MIN objects, or into \TT{DERIVED} tokens whose values can
be other things such as labels or special values.
...
the special value `\ttsvkey{INDENTED\_PARAGRAPH}' being the
\TT{.term\-i\-na\-tor}.  The elements of an indented paragraph are
logical lines, whose \TT{.initiator}
is the special value `\ttsvkey{LOGICAL\_LINE}' and whose \TT{.terminator}
is the line separator (e.g., `\TT{;}') if there is one,
or \TT{"<LF>"} otherwise.

For {\em typed-bracketed-subexpressions} there is \underline{no}
\TT{.initiator} or \TT{.terminator}, but there is usually a \TT{.type}
attribute.  {\em Prefix-n-lists} are just an alternate form of
{\em typed-bracketed-subexpressions}, and paragraphs and logical
lines with implied headers are just an alternate form of
{\em prefix-n-lists}.
...
\label{PARSER-INDENTATION-MARK-COMMAND} \\
    ...
    ::= & \ttikey{parser define}{define indentation mark}
	  ...
	  {}[ \TT{with implied paragraph header} {\em header} ] \\
...
\emkey{end-at-options} ::= {\em parser-flag-spec}
	~~~~~ [see \pagref{PARSING-END-AT-OPTIONS-1}]
\\[0.5ex]
\emkey{master-name} ::= {\em quoted-name}
\\[0.5ex]
\emkey{header} ::= MIN object with \TT{.type} and no elements
        (i.e., a prefix separator: see \pagref{LINE-VARIABLES})
\\[0.5ex]
...
are used to initialize line variables (see \pagref{LINE-VARIABLES})
that affect the parsing of paragraphs and lines.
...
When an implied paragraph or line header is set to a prefix
with this type, this parameter is used to
set the paragraph or line lexical master line variable
(see \pagref{LINE-VARIABLES}),
which in turn set the lexical scanner master table
immediately after the indent lexeme
that begins a logical line (see \pagref{SETTINGS-AFTER-INDENT}).
\\[2ex]
\hspace*{-0.2in}\ttmkey{options}{in prefix definition}\\
When an implied paragraph or line header is set to a prefix
with this type, this parameter is used to
set the paragraph or line options line variable
(see \pagref{LINE-VARIABLES}),
which in turn set the parser
`end-at' options (see \pagref{PARSING-END-AT-OPTIONS-1})
immediately after the indent lexeme
that begins a logical line (see \pagref{SETTINGS-AFTER-INDENT}).
...
A `\emkey{prefix-lexeme}'\label{PREFIX-LEXEME}
is a special kind of lexeme that can be
...
header prefixes can also be inserted via line variables
(see \pagref{LINE-VARIABLES}).
...
by line variables set when a implied header is set by a prefix lexeme, as per
\pagref{SETTING-IMPLICIT-HEADER}.

The `\TT{options}' parameter of a prefix type table entry is only used
to set line variables: see \pagref{LINE-VARIABLES}.
...
following `\skey{line variable}s'\label{LINE-VARIABLES}:

\begin{indpar}[0.4in]
\hspace*{-0.2in}\ttmkey{implied paragraph header}{line variable}\\
    If this value is not missing and no explicit paragraph header
    is found at the beginning of a logical line in
    paragraph beginning position, a copy of this
    implied paragraph header variable is inserted.
\\[1ex]
\hspace*{-0.2in}\ttmkey{implied line header}{line variable}\\
    If this value is not missing and no explicit line header
    is found after a paragraph header or at the beginning of
    a logical line \underline{not} in
    paragraph beginning position, a copy of this
    implied line header variable is inserted.
\\[2ex]
\hspace*{-0.2in}\ttmkey{paragraph selectors}{line variable}\\
    The parsing selectors are reset to this value just after the
    indent that begins a logical in paragraph beginning position.
\\[1ex]
\hspace*{-0.2in}\ttmkey{line selectors}{line variable}\\
    The parsing selectors are reset to this value just after the
    indent that begins a logical \underline{not}
...
When an indentation mark is recognized, it sets the above variables
to values given in the definition of the indentation mark.

Whenever an explicit (non-implied) paragraph header is recognized,
the implied paragraph header is reset to a copy of the explicit
paragraph header.   
Whenever an explicit (non-implied) line header is recognized,
the implied line header is reset to a copy of the explicit
line header.   

Whenever the implicit paragraph header is set%
\label{SETTING-IMPLICIT-HEADER}, the implicit line
header is set to the `\TT{implied subprefix}' of the new implicit
paragraph header, which may be missing.  The paragraph selectors,
options, and lexical master are then set according to values
given by the new implied paragraph header.  However, if any of these
values would be missing, they are reset to the values they were
inititially given by the indentation mark that began the indented
paragraph.

Whenever the implicit line header is set, the line selectors,
options, and lexical master are then set according to values
given by the new implied line header.  However, if any of these
values would be missing, they are reset to the values of the
paragraph selectors, options, or lexical master.
...
in paragraph beginning position is a {\em typed-prefix-separator}
with `\TT{paragraph}' group, or a {\em prefix-lexeme} that is replaced
by such, then the implied paragraph header is set to a copy of this
{\em typed-prefix-separator}.
Otherwise a copy of the implied paragraph
header is inserted just after the indent token, unless the
implied paragraph header is missing.
...
header) is a {\em typed-prefix-separator}
with `\TT{line}' group, or a {\em prefix-lexeme} that is replaced
by such, then the implied line header is set to a copy of this
{\em typed-prefix-separator}.  Otherwise
a copy of the implied line header is inserted just
