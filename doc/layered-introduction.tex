...
{\em bracket-type} becomes the \TT{.type}
...
with {\em bracket-type-marks}, if the beginning and ending
...
form `\TT{\{\}}'.  {\em Typed-prefix-separators} are syntactically the same
as {\em typed-bracketed-subexpressions} with no elements
but with no {\em typed-middle} or with only one {\em bracket-type-mark}:
see \itemref{TYPED-PREFIX-SEPARATORS}.
...
{\em Bracket-type-marks} are \underline{not} looked up in the
bracket type table for the purposes of the last paragraph.
This is because an initial {\em bracket-type-mark} may not
be the entire {\em bracket-type}.
...
\emkey{prefix-type}\label{PREFIX-TYPE} ::=
    \begin{tabular}[t]{l}
    the {\em bracket-type-label} or {\em bracket-type-mark}
    in a {\em typed-prefix-separator}
    \end{tabular}
\end{indpar}
...
In particular, the {\em prefix-type} of the {\em typed-prefix-separator}
...
if it is a {\em prefix-type} (\pagref{PREFIX-TYPE}).
