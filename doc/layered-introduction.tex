...
or is after sequence of blank and comment lines that contains
...
\hspace*{-0.2in}\ttmkey{paragraph lexical index}{line beginning variable}\\
    If not \TT{0}, the lexical analyzer master is reset to this value
    at the beginning of a logical line that is in paragraph beginning
    position, header lexeme is then read, and the string returned by
...
the line selectors are the parser's default selectors,
...
the indented paragraph.  We will call these the \key{paragraph
...
the parser may be asked once or twice produce the first token
of the yet unparsed part of the logical line.
...
\item
If the paragraph lexical index is \TT{0},
skip to Step~\ref{PARAGRAPH-HEADER-SCAN}.

\item \label{PARAGRAPH-BEGINNING}
Use the non-\TT{0} paragraph lexical index to scan
a lexeme.  In this situation the next character input to the lexical
...
(Note that going to Step~\ref{LINE-HEADER-SCAN} implicitly
takes the value of the line lexical index to be \TT{0} even if it is not.
This allows the lexical scanner to scan parts of the
logical line beyond the paragraph header that was found
before that paragraph header is processed in this step.)


\item \label{INSERT-PARAGRAPH-HEADER}

If the paragraph header is not missing,
insert a \TT{PREFIX} token containing a copy of the paragraph header
at the beginning of the logical line.  This becomes the paragraph header
token for that line.

\item \label{LOGICAL-LINE-BEGINNING}
If the line lexical index is \TT{0},
skip to Step~\ref{LINE-HEADER-SCAN}.

\item
Use the non-\TT{0} line lexical index to scan
a lexeme, in the same manner as in Step~\ref{PARAGRAPH-BEGINNING}.
...
result of this, without repeating the lexical scan.)
...
If an entry was accepted, reset just the line header
line beginning variable from the value in the
...
(ignoring the accepted entry contents),
and skip to Step~\ref{TERMINATE-LINE-BEGINNING}.

\item \label{INSERT-LINE-HEADER}

If the line header is not missing,
insert a \TT{PREFIX} token containing a copy of the line header
