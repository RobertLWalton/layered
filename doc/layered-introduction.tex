...
\\[1ex]
\begin{tabular}{p{1in}p{4.5in}}
		& A \TT{PREFIX} token is a \TT{BRACKETED} token that has
		  ...
		  It necessarily has a \TT{.type}, which is recorded in
		  the \TT{value\_type} component of the token,
		  and does \underline{not} have an \TT{.initiator}
		  or \TT{.terminator}.
\end{tabular}
\\[1ex]
\begin{tabular}{p{1in}p{4.5in}}
...
\ttmkey{value\_type}{of token}
		& For a \TT{BRACKETED} token, either the \TT{.type}
		  of the token \TT{value} if there is one, or otherwise
		  the \TT{.initiator} of the token \TT{value} if there
		  is one, or otherwise \TT{MISSING}.  For \TT{PREFIX}
		  tokens, the \TT{.type} of the token \TT{value} if
		  there is one, or otherwise \TT{MISSING}.  For
		  other token types, \TT{MISSING}.
\\[1ex]
