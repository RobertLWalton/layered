...
\ttmkey{sentence}{reformatter}
        \TT{(} {\em terminator-list}\TT{,} {\em extender-list} \TT{)}
...
\emkey{extender-list} ::=
    \TT{()}
    $|$ \TT{(} {\em extender} \{ \TT{,} {\em extender} \}\STAR{} \TT{)}
\end{indpar}
...
where
\begin{indpar}
\emkey{prefix} ::= the prefix, e.g., `\TT{p}'
\\[0.5ex]
\emkey{prefix-attributes} ::= {\em attribute-type-begin} ~ {\em attribute-list}
			      ~~~ (see \pagref{OPENING-TYPE-AND-ATTRIBUTES})
\\[0.5ex]
...
\begin{indpar}
\emkey{element-list} ::= {\em sentence-term}
			 \{ {\em sentence-prefix-list}
			    {\em sentence-term} \}\STAR{}
\\[0.5ex]
\emkey{sentence-term} ::= {\em sentence-element}\STAR{}
			 \{ {\em sentence-terminator}
			    {\em sentence-element}\STAR{} \}\STAR{}
\emkey{element-list} ::= see \pagref{ELEMENT-LIST}
\end{indpar}
