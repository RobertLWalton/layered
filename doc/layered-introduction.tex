...
A \key{comment line} is a line that contains nothing
...
characters will be skipped over while producing these header lexeme.
In short, the scanner will peek ahead but not read ahead.
...
If neither a present, the parser uses context variables as described
...
of variables called the \skey{line beginning variable}s.
...
having a selected header table entry, the some of the line beginning
variables are reset according to values in that table entry.
...
\hspace*{-0.2in}\ttmkey{paragraph header}{line beginning variable}\\
    ...
    then a bracketed header token whose value is a
...
\\[2ex]
\hspace*{-0.2in}\ttmkey{header line selectors}{line beginning variable}\\
...
\\[1ex]
\hspace*{-0.2in}\ttmkey{header line options}{line beginning variable}\\
...
\hspace*{-0.2in}\ttmkey{line selectors}{line beginning variable}\\
    Parsing selectors determined by the paragraph header and used
    as the current parsing selectors to parse the non-header
    portions of lines in the paragraph.  These default to the header
    line selectors.
\\[1ex]
\hspace*{-0.2in}\ttmkey{line options}{line beginning variable}\\
    Parsing options determined by the paragraph header and used
    as the current parsing options to parse the non-header portions
...
top level the lexical indices are \TT{0}, the headers are missing,
...
header table.  If an entry marked as a paragraph header is found
...
Then if the paragraph header variable is not missing,
replace the paragraph header lexeme
by a \TT{PREFIX} lexeme whose value is a copy of the
paragraph header variable (which may have just been changed).
Else if the paragraph header variable is missing, just
...
look up this line header lexeme's string value (which is not part of the
input, but is generated by the lexical scanner) in the parser's
header table.

If an entry marked as a line header is found
that is selected by the header line selectors, reset the
line header, line selectors, and line options
line beginning variables according to values in the table entry
(see below for details).
If no such entry is found, reset the line selectors an options
from the header line selectors and options.

Then if the line header variable is not missing,
replace the line header lexeme
by a \TT{PREFIX} lexeme whose value is a copy of the
line header variable (which may have just been changed).
Else if the line header variable is missing, just
discard the line header lexeme.

\item
If no line header lexeme was found as per the previous step,
and if the line
header variable value is not missing, insert a \TT{PREFIX}
lexeme at the beginning of the logical line whose value is a copy of the
line header variable.

\item
Skip to Step~\ref{TERMINATE-LINE-BEGINNING}.
...
in the parser's header table.  If a paragraph header entry is found
Accept an entry if it has
paragraph header type and is selected by the paragraph selectors.

If the result of this lookup is an accepted entry, reset all the
...
the value of the \TT{PREFIX} token, and
lastly go to Step~\ref{LINE-HEADER-SCAN}.

Note that going to Step~\ref{LINE-HEADER-SCAN} implies that
the entire logical line will be scanned according to the
paragraph lexical index, and the line lexical index will be
ignored.
...
\item \label{LOGICAL-LINE-BEGINNING}
If the lexical index
set in Step~\label{LEXICAL-INDEX-SET}
was \TT{0},
skip to Step~\ref{LINE-HEADER-SCAN}.

\item
Scan a lexeme (using the non-\TT{0} line lexical index)
in the same manner as in Step~\ref{PARAGRAPH-BEGINNING}.
The returned lexeme must have header lexical type,
and may have any string as a value.  (Note that if the
line lexical index equals the paragraph lexical index used in
Step~\ref{PARAGRAPH-BEGINNING}, the lexeme and its string value
returned in Step~\ref{PARAGRAPH-BEGINNING} can be used as the
result of this, without repeating the lexical scan.  Note
also that this step only executes if the parser has not
attempted to parse any part of the logical line.)

If the string is empty,
skip to Step~\ref{LINE-HEADER-SCAN}.

Otherwise look the string up in the parser's header table and
accept an entry if it is a line header entry selected by the
line selectors.

If an entry was accepted, reset the line header, line selectors,
and line options
line beginning variables from the values in the
accepted entry (see below for details).

Go to Step~\ref{INSERT-LINE-HEADER}.

\item \label{LINE-HEADER-SCAN}
Unless the beginning of the logical line was parsed
in Step~\ref{PARAGRAPH-HEADER-SCAN} and
produced a first token that was \underline{not} a paragraph
header, parse the beginning of the yet-unparsed
part of the logical line and produce the next token.
Otherwise use the first token of the logical line produced
in Step~\ref{PARAGRAPH-HEADER-SCAN}.

If this token is a \TT{PREFIX} token, look up the type of its value
in the parser's header table.  Accept an entry if it has
line header type and is selected by the line selectors.
...
Terminate the line beginning algorithm, and continue the parse
