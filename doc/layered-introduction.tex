...
preceded by an operand, an empty list preceding operand is inserted.
Similarly if first operator is not followed by an operand, an empty
list following operand is inserted.  Then the first operator is moved to the
beginning of the expression and the bracketed operators are converted to
operands.
Indented subexpressions may be treated like bracketed subexpressions.
