...
\item If the paragraph lexical index is not \TT{0}, use it to scan
...
skip to Step~\ref{PREFIX-SEPARATOR-SCAN}.

Otherwise look it up in the parser's header table and
accept an entry if it is a paragaph header entry selected by the
paragraph selectors.

If no entry is selected, skip to Step~\ref{PREFIX-SEPARATOR-SCAN}.

Otherwise reset all the line beginning variables from values in this entry
(see below for details).  Then if the paragraph header is not missing,
insert a \TT{PREFIX} token before the logical line containing a copy
of the paragraph header as its value.

\item If the lexical index is \TT{0} or the header lexeme
returned was empty, scan the beginning of the logical line to see
if it starts with a prefix separator.  If yes, look up the type
of the prefix separator in the parser's header table using
the current parsing selectors, and accepting only entries that
are paragraph headers if the logical line is in paragraph beginning
position, or are line headers otherwise.

If the result of this lookup is an accepted entry, reset all the
line beginning variables from values in this entry.

Note that it is assumed that if the lexical index is not \TT{0}
and the logical line begins with a prefix separator (e.g.,
begins with `\TT{\{}'), that the returned header lexeme will be
empty and therefore ignored.

\item If the last step found an accepted entry that is only a
paragraph header and not a line header, reset the current parsing
selectors to the line beginning variable line selectors,
scan the next part of the logical line to see if it is a second
prefix header whose type is selected as a line header in the
parser's header table, and if yes, reset all the line beginning
variables from values in this entry.

\item \label{PREFIX-SEPARATOR-SCAN}

\item \label{LOGICAL-LINE-BEGINNING}


\item Execute the instruction now in the line beginning variables.
