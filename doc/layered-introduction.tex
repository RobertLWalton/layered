...
if it contains any operators, or leaving it untouched
if it contains no operators.
...
Then processes each subexpression, innermost first, and lastly processes the
entire expression as a subexpression.
\end{enumerate}

When the operator parsing pass processes a
subexpression that contains one or more operators, the
pass first calls the reformatter associated with the first operator
...
\TT{BRACKATABLE} token with the first operator as the value of
the token value's \ttdkey{operator} attribute.

When the operator parsing pass
processes a subexpression that does \underline{not} contain an operator,
the operator parsing pass runs subsequent parsing passes on the subexpression.
Then if the subexpression has zero or more than one token, the operator
parsing pass replaces
the subexpression by a single \TT{BRACKATABLE} token with no attibutes
...
operators.  The end of expression will resolve
ambiguities at the end of the expression, but operators
