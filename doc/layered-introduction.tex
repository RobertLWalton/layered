...
\subsubsection{Unindented Paragraphs}
\label{UNINDENTED-PARAGRAPHS}

A `\key{paragraph separator}' is a maximal sequence of consecutive blank lines
in an indented paragraph, assuming that comment lines (lines
containing only a comment) are completely ignored.
Paragraph separators separate the logical lines in an indented
paragraph into `\skey{unindented paragraph}s'.

A typed prefix separator which has the `\TT{paragraph}' group can only 
appear at the very beginning of an unindented paragraph, and is called
the `\key{paragraph header}' of that paragraph.

A typed prefix separator which has the `\TT{line}' group can only 
appear at the very beginning of a logical line paragraph, or immediately
after a paragraph header that begins the line, and is called
the `\key{line header}' of that line.

Paragraph headers and line headers may be implied or be the
result of special lexeme replacement.
...
\hspace*{-0.2in}\ttmkey{implied paragraph header}{line variable}\\
    ...
    is found at the beginning of an unindented paragraph,
    a copy of this implied paragraph header variable value is inserted.
\\[1ex]
\hspace*{-0.2in}\ttmkey{implied line header}{line variable}\\
    If this value is not missing and no explicit line header
    at the beginning of a logical line or just after a paragraph
    header, a copy of this
    implied line header variable value is inserted.
\\[2ex]
\hspace*{-0.2in}\ttmkey{paragraph selectors}{line variable}\\
    The parsing selectors are reset to this value just after the
    indent token that begins an unindented paragraph.
\\[1ex]
\hspace*{-0.2in}\ttmkey{line selectors}{line variable}\\
    The parsing selectors are reset to this value just after the
    indent token that begins a logical line that is not the
    first logical line of an unindented paragraph.
\\[2ex]
...
paragraph header, or at the beginning of a logical line in which there
