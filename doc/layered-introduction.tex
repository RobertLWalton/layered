...
the lexical atom table is named `{\tt lexical}', and the
...
\begin{center}
\begin{tabular}{rcl}
\emkey{atom-table-entry} & ::= & {\em atom} {\em type} [{\em translation}] \\
\emkey{atom} & ::= & {\em quoted-string} \\
\emkey{type} & ::= & {\em identifier} \\
...
followed by the beginning of a `{\tt "L'" quoted\_string}'.
...
standard `{\tt "\BACKSLASH"" quoted\_string}' table:
...
In defining lexical atom table, we say that an atom is
...
The following are the possible types in the lexical atom table:
...
\ttkey{number}		& The atom is rejected and the state becomes
			  `{\tt number}'.
\\[1ex]
\ttkey{quoted\_string}	& The atom $A$ is rejected and the state becomes
			  `$A$ {\tt quoted\_string}'.
\\[1ex]
...
\subsection{The Identifier Atom Table}

Then the scanner is in the {\tt identifier} state, it can accumulate
...
the scanner switches to the {\tt lexical} state without accepting
any input.

The following are the possible types in the identifier atom table:
...
\ttkey{begin}	  & Accept and accumulate the atom, but only
		    recognize the atom if it is the first
...
\subsection{The Number Atom Table}

Then the scanner is in the {\tt number} state, it can accumulate
atoms, or it can reject atoms and switch to a different state.
Translations of accumulated atoms are appended to the current
accumulation string, which is output as an identifier lexeme
when the scanner leaves the {\tt identifier} state.
