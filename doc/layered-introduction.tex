...
\emkey{parser-trace-flag} \begin{tabular}[t]{@{}rl@{}}
                          ::= & \ttikey{parser input}{parser trace flag}
			  $|$ \ttikey{parser output}{parser trace flag} \\
			  $|$ & \ttikey{parser commands}%
				       {parser trace flag} \\
...
\hspace*{-0.2in}\ttmkey{subexpression lines}{parser trace flag}
    Upon recognition of a subexpression by a trace-enabled parser 
    pass, the lines containing the value of the resulting subexpression token
    are printed, with the subexpression underlined.

\hspace*{-0.2in}\ttmkey{subexpression elements}{parser trace flag}
    Upon recognition of a subexpression by a trace-enabled parser 
    pass, the value of the resulting subexpression token
    is printed.

\hspace*{-0.2in}\ttmkey{subexpression details}{parser trace flag}
    Ditto but the value is printed with non-printing string characters
    given printable representations and labels printed in the format:
    \begin{center}
    {\tt [:} {\em label-element}{\tt ,} \ldots{} {\tt :]}
    \end{center}
...
\hspace*{-0.2in}\ttdmkey{terminator}{subexpression attribute}\\
    ...
    or & the {\em line-separator-name} used to separate lines \\
...
+0000	& \ttkey{define}	& \tt nofix	& \ttnbkey{<--}	& define \\
	&			& \tt afix	& \ttkey{\LEFTBRACKET{}
							 ... \RIGHTBRACKET}
								& block \\
+1000	& \tt \bf binary right	& \tt infix	& \ttnbkey{=}	& assignment \\
     	& \tt \bf associative\ttindex{binary right associative}
				&          	& \ttnbkey{+=}	& increment \\
     	&               	&          	& \ttnbkey{-=}	& decrement \\
     	&               	&          	& \ttnbkey{*=}	& multiply by \\
     	&               	&          	& \ttnbkey{/=}	& divide by \\
+2000	& \ttkey{separator}	&		& \ttkey{,}	& separator \\
+3000	& \ttkey{unary}		& \tt nofix	& \ttkey{NOT}	& logical not \\
+3000	& \ttkey{binary}	& \tt nofix	& \ttkey{BUT NOT}
								& logical and
								  not \\
+3100	& \ttkey{infix}		& \tt nofix	& \ttkey{AND}	& logical and \\
	&			&		& \ttkey{OR}	& logical or \\
								 \\
...
\begin{indpar}[0.5em]
The operator tokens are the separator.  There must be at least one
operator token, and all the operator token values must be identical.
...
\ttmkey{binary right associative}{reformatter}
...
rightmost executes first.  For example,
...
by an operand.  The subexpression is left unchanged.  Example:
