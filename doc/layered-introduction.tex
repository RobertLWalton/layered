...
For each multiset of {\em single-attribute-values}
given for a particular {\em attribute-label} in an {\em attribute-paragraph}
...
\begin{figure*}[!p]
\begin{center}
\begin{tabular}{|l|rll@{\hspace*{2em}}l@{\hspace*{2em}}l|}
\hline
selec-  & prece-  &             &       	&               & \\
~tors   & ~dence  & reformatter	& flags		& operator	& meaning 
\\\hline
code    & 1000	& \TT{right}	& \TT{infix}	& \ttkey{=}	& assignment \\
     	&	& \TT{associative}
				& \TT{line}    	& \ttkey{+=}	& increment \\
     	&	&            	&          	& \ttkey{-=}	& decrement \\
     	&	&            	&          	& \ttkey{*=}	& multiply by \\
     	&	&            	&          	& \ttkey{/=}	& divide by
\\\hline
math    & 1000	& \TT{binary}	& \TT{infix}	& \ttkey{=}	& assignment
\\\hline
code    & 2000	& \ttkey{separator}
				& \TT{nofix}	& \ttkey{,}	& separator
\\\cline{2-6}
math    & 3000	& \ttkey{binary}
