...
precedence H+1, and postfix operators have precedence H+2.
...
\\[0.5ex]
\emkey{(H+1)-expression}
    \begin{tabular}[t]{@{}rl}
    ::= & {\em (H+2)-expression} \\
    $|$ & {\em prefix-operator} {\em (H+1)-expression} \\
    \end{tabular}
\\[0.5ex]
\emkey{(H+2)-expression}
    \begin{tabular}[t]{@{}rl}
    ::= & {\em (H+3)-expression} \\
    $|$ & {\em (H+2)-expression} {\em postfix-operator} \\
    \end{tabular}
\\[0.5ex]
\emkey{(H+3)-expression} ::= {\em non-operator}\PLUS{}
\\[2.0ex]
...
An {\em (H+1)-expression}, while it can be thought of as a sequence
of zero or more prefix operators followed by a single {\em (H+2)-expression},
is parsed so as to execute the prefix operators from right to left, each
with a single operand.
Similarly, an {\em (H+2)-expression}, while it can be thought of as
an {\em (H+3)-expression} followed by a sequence
...
much higher precedence (i.e., H+1) than the infix operator.
...
A prefix operator always has precedence H+1, and a postfix operator
always has precedence H+2.
...
Also nofix$^H$ means a nofix operator of precedence higher
than the preceeding infix-or\ldots{} operator, and
nofix$^L$ means a nofix operator of precedence equal to or lower
than the preceeding infix-or\ldots{} operator.

\begin{figure}[!p]
\newcommand{\SUP}[1]{$^\textrm{\scriptsize #1}$}
\centering
\small
...
expression-begin & infix & not-allowed\SUP{--O} \\
expression-begin & postfix & not-allowed\SUP{--O} \\
...
non-operator & prefix & not-allowed\SUP{O} \\
non-operator & infix & non-operator infix\SUP{--O} \\
...
prefix & infix & not-allowed\SUP{--O} \\
prefix & postfix & not-allowed\SUP{--O} \\
prefix & nofix & not-allowed\SUP{--O} \\
prefix & expression-end & not-allowed\SUP{--O} \\
\hline
infix & non-operator & infix non-operator \\
infix & prefix & infix prefix \\
infix & infix & not-allowed\SUP{--O} \\
infix & postfix & not-allowed\SUP{--O} \\
infix & nofix\SUP{H} & infix nofix \\
infix & nofix\SUP{--H} & not-allowed\SUP{--O} \\
infix & expression-end & not-allowed\SUP{--O} \\
\hline
postfix & non-operator & not-allowed\SUP{O} \\
postfix & prefix & not-allowed\SUP{O} \\
...
nofix\SUP{H} & infix & nofix infix \\
nofix\SUP{--H} & infix & not-allowed\SUP{--O} \\
nofix & postfix & not-allowed\SUP{--O} \\
...
\\[1ex]
\begin{tabular}{rl}
\SUP{H}   & means operator has higher precedence than \\
          & the precedence of the following operator\\[1ex]
\SUP{--H} & means operator has precedence lower than or equal to \\
          & the precedence of the following operator\\[1ex]
\end{tabular}
\\
...
nofix\SUP{H} & infix-or-prefix & unresolved \\
nofix\SUP{--H} & infix-or-prefix & nofix prefix \\
...
\hline
expression-begin & infix-or-postfix & not-allowed\SUP{--O} \\
non-operator & infix-or-postfix & unresolved \\
prefix & infix-or-postfix & not-allowed\SUP{--O} \\
infix & infix-or-postfix & not-allowed\SUP{--O} \\
postfix & infix-or-postfix & unresolved \\
nofix\SUP{H} & infix-or-postfix & nofix infix \\
nofix\SUP{--H} & infix-or-postfix & not-allowed\SUP{--O} \\
infix-or-prefix & infix-or-postfix & not-allowed\SUP{--O} \\
...
infix-or-prefix & infix & not-allowed\SUP{--O} \\
infix-or-prefix & postfix & not-allowed\SUP{--O} \\
infix-or-prefix & nofix\SUP{H} & infix nofix \\
infix-or-prefix & nofix\SUP{--H} & not-allowed\SUP{--O} \\
infix-or-prefix & expression-end & not-allowed\SUP{--O} \\
\hline
...
infix-or-postfix & nofix\SUP{H,--A} & ambiguous nofix \\
infix-or-postfix & nofix\SUP{H,A} & not-allowed \\
infix-or-postfix & nofix\SUP{--H} & postfix nofix \\
...
\SUP{A}  & means operator is afix\\[0.5ex]
\SUP{--A} & means operator is \underline{not} afix \\[0.5ex]
\SUP{O} & means fix by inserting nofix operator (see text) \\[0.5ex]
\SUP{--O} & means fix by inserting non-operator (see text)
\end{tabular}
\\
