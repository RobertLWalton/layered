...
\emkey{indentation-mark-definition} \\
    ...
    ::= & \ttikey{define}{define indentation mark}
	  ...
	  {}[ {\tt with evaluator} {\em evaluator-name} ] \\
	  \end{tabular} \\
    $|$ & \ttikey{define}{define gluing indentation mark}
	  \begin{tabular}[t]{@{}l@{}}
...
\emkey{named-bracket-definition} \\
    \hspace*{0.2in}
    \begin{tabular}[t]{rl}
    ::= & \ttikey{define}{define named bracket}
	  \begin{tabular}[t]{@{}l@{}}
	  {\tt \bf named bracket}
...
are run in the order that they appear in the stack.  To be
active, a pass must have a selector in common with the selectors
used by the top level while scanning the subexpression.



The set of passes that may be run is currently builtin, and
cannot be changed.  But the parser pass stack can be altered.
