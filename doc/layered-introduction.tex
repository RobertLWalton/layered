...
\emkey{parser-block-command}
    \begin{tabular}[t]{@{}rl@{}}
    ::= & \ttmkey{begin}{parser} {\em block-name} \\
    $|$ & \ttmkey{end}{parser} {\em block-name} \\
    $|$ & \ttmkey{print blocks}{parser} \\
    \end{tabular}
...
\emkey{parser-selector-command}
    \begin{tabular}[t]{@{}rl@{}}
    ::= & \ttmkey{define selector}{parser} {\em selector} \\
    $|$ & \ttmkey{print selector}{parser} {\em partial-name} \\
    \end{tabular}
\label{PARSER-SELECTOR-COMMAND}
\\[0.6ex]
\emkey{parser-trace-command}
    \begin{tabular}[t]{@{}rl@{}}
    ::= & \ttmkey{trace}{parser} {\em trace-flag-spec} \\
...
Parser commands are logical lines within an indented paragraph
that has the indentation mark `\TT{*PARSER*:}',
where the indented paragraph is in a top level logical line by itself
(therefore `\TT{*PARSER*:}' must appear at the beginning of a top level
logical line).
Parser commands are themselves parsed before they are executed.
...
The top level selectors and end-at options can be reset by the parser
command:
\begin{center}
    \begin{tabular}[t]{l}
    \TT{define context} \\
...
*PARSER*:
    ...
    define context with parsing selectors [TOP LEVEL, code]
\end{verbatim}\end{indpar}

Thus the \key{standard parser} creates a single initial parsing block
...
`\TT{[TOP LEVEL, code]}'.  Note that the actions of the standard parser can be
...
\hspace*{3em} \ttmkey{define string concatenator}{parser}
...
\\
    \hspace*{0.2in}
    \begin{tabular}[t]{rll}
    ::= & \multicolumn{2}{l}{\ttmkey{define bracket}{parser}
    ...
    $|$ & \multicolumn{2}{l}{\ttmkey{undefine bracket}{parser}
		    ~ {\em bracket-name} ~ {\em selectors}} \\
    \end{tabular}
\\[0.5ex]
...
\ttmkey{define indentation mark}{parser}
...
\label{PARSER-INDENTATION-MARK-COMMAND} \\
    \hspace*{0.2in}
    \begin{tabular}[t]{rl}
    ::= & \ttikey{define}{define indentation mark}
	  \begin{tabular}[t]{@{}l@{}}
	  \TT{indentation mark}
	  ...
	  \end{tabular} \\
    $|$ & \ttkey{undefine indentation mark}
	  \begin{tabular}[t]{@{}l@{}}
	  {\em indentation-name} \\
	  {\em parsing-selectors} \\
	  \end{tabular} \\
    $|$ & \ttkey{define indentation offset}
...
\ttmkey{define typed bracket}{parser}
...
\\
    ...
    ::= & \TT{parser}%
	  ...
	  {}[ \TT{with} \begin{tabular}[t]{l}
			...
			\end{tabular} \\
	  \end{tabular} \\
    $|$ & \ttmkey{undefine typed bracket}{parser}
	  \begin{tabular}[t]{@{}l@{}}
...
\label{PARSER-MAPPED-LEXEME-COMMAND} \\
    \hspace*{0.2in}
    \begin{tabular}[t]{rl}
    ::= & \ttmkey{define mapped lexeme}{parser}
    ...
    ::= & \TT{parser}%
	  ...
	  \end{tabular} \\
    $|$ & \ttmkey{undefine mapped lexeme}{parser} ~
	  {\em mapped-lexeme-name}
    \end{tabular}
...
\\
    \hspace*{0.2in}
    \begin{tabular}[t]{rl}
    ::= & \ttmkey{print bracket}{parser}
	  {\em partial-name} \\
    $|$ & \ttmkey{print prefix}{parser}
	  {\em partial-name} \\
    $|$ & \ttmkey{print mapped lexeme}{parser}
	  {\em partial-name} \\
    $|$ & \ttmkey{print indentation offset}{parser} \\
    $|$ & \ttmkey{print string concatenator}{parser} \\
    \end{tabular}
\end{indpar}
...
\\
    \hspace*{0.2in}
    \begin{tabular}[t]{rl}
    ::= & \ttmkey{print header}{parser} {\em partial-name} \\
    \end{tabular}
\end{indpar}
...
\emkey{parser-pass-command} \\
    \begin{tabular}[t]{rl}
    ::= & \ttmkey{define pass}{parser}
	  \begin{tabular}[t]{@{}l@{}}
	  {\em parser-pass-name} {\em parsing-selectors} \\
	  {\em parser-pass-stack-location} \\
	  \end{tabular} \\
    $|$ & \ttmkey{undefine pass}{parser}
	  {\em parser-pass-name} \\
    $|$ & \ttmkey{print pass}{parser}
    \end{tabular}
\\[0.5ex]
