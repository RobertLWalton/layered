...
parts are preferred, but if an initial part finds no entry
...
After an entry is found, the entire primary is matched
against the entry by \TT{PRIM::scan\_primary}.  If the match
...
Matching proceeds left to right.  After matching an {\em argument-list}
...
After this matching proceeds by looking up the
...
An output of a match is a list of arguments in the same order
as they appear in the prototype.  Initially all arguments
are marked missing.  After the primary is scanned, any
...
values are expressions to be evaluated by a call to the
...
\TT{.initializer} (in which case a {\em naked-argument} is allowed).
