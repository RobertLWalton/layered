...
\skey{bracketed subexpression}s
...
\emkey{bracketed-subexpression} ::=
	{\em opening-bracket} {\em element}\STAR{} {\em closing-bracket}
\\[0.5ex]
\emkey{opening-bracket} ::= as defined by a
    {\em parser-bracket-command} (\pagref{PARSER-BRACKET-COMMAND})
    ~~~~~ [e.g., `\TT{(}' or `\TT{[<}']
\\[0.5ex]
...
\emkey{element} ::= {\em word} $|$ {\em number} $|$ {\em mark} $|$
                    {\em separator} $|$ {\em subexpression}
\\[0.5ex]
Note: {\em element}\STAR{} is terminated by the first {\em closing-bracket}
\end{indpar}
...
command.  An example of such a command is
\begin{center}
\TT{parser define bracket "("~...~")" [operator, text]} \ldots
\end{center}
...
`\TT{|}', and `\TT{>}', then an example typed bracketed subexpression is
\begin{center}
...
A typed bracketed subexpression is converted to a
MIN object whose list elements are the components of the
subexpression except for the brackets.  The
...
If a typed bracketed subexpression ends pre-maturely, before
...
untyped bracketed subexpression, the {\em typed-closing-bracket}
of a containing typed bracketed subexpression, the end of the paragraph line
...
A \key{typed operator} has the same syntax as an {\em opening-typed-bracket}
...
no \TT{.middle} attribute.  As for a typed bracketed subexpression,
