...
the second of which is empty.  Therefore all paragraph lines
are non-empty.
...
are used to initialize line variables
that affect the parsing of paragraphs and lines: see \pagref{LINE-VARIABLES}.
...
file were an indented paragraph, and forms each line into a MIN object.
However,
...
where the \TT{.type} is `\TT{my option}',
and the following is in error because it has a different type name
