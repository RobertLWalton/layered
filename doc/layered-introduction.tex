...
In any case, the algorithm continues by looking for a line header. 
...
\hspace*{-0.2in}\ttmkey{line header}{line beginning variable}\\
    If not missing and no explicit line header is found at the
    beginning of a logical line,
    then a prefix separator token whose value is a
    copy of this line header is inserted
    at the beginning of the logical line (after any paragraph
    header if the line is in paragraph beginning position).
\\[2ex]
...
top level the indices are \TT{0}, the headers are missing,
...
Otherwise reset just the line header
...
Otherwise reset just the line header
line beginning variable from a value in the
