...
associated parser testing and selector definition
is specified on
\pagref{PARSER-COMMANDS}.
...
\\[0.5ex]
\emkey{parsing-selectors} ::= {\em parser-flag-list}
...
\label{BRACKETED-SUBEXPRESSION-RECOGNITION-DEFINITIONS-1}
\end{figure*}
...
\label{BRACKETED-SUBEXPRESSION-RECOGNITION-DEFINITIONS-2}
\end{figure*}
...
The term `\key{parser definition}' is used both in a specific
sense to mean the `{\tt parser define}' command,and in a general sense
that includes
commands such as `{\tt parser print}', `{\tt parser trace}', and
`{\tt parser test}' that test the parser, `{\tt parser undefine}'
that disables parser definitions, and `{\tt parser begin}' and
`{\tt parser end}' that define parser definition blocks.

Parser definitions begin with either 
`\ttkey{parser} \ttmkey{define}{in parser definition}' or
`{\tt \bf parser} \ttmkey{undefine}{in parser definition}'.  This is followed
...
and therefore as keys for symbol table lookups.

A `{\tt \bf parser} \ttmkey{define}{parser definition}'
line makes entries in parser symbol tables.
...
The syntax of the parser definitions that control bracketed
subexpression recognition
is specified on
\pagref{BRACKETED-SUBEXPRESSION-RECOGNITION-DEFINITIONS-1}
and
\pagref{BRACKETED-SUBEXPRESSION-RECOGNITION-DEFINITIONS-2}.
