...
end of indentation, or by a line end.
...
\\[1ex]  
		& Standardly the token lexeme types are word, mark, separator,
		  and natural number which have token {\tt value}s,
		  number and quoted string which have token {\tt string}s,
...
\\[1ex]
		& For a {\tt BRACKETED} subexpression, the token {\tt value}
		  ...
		  \end{center}
\\[1ex]  
		& Empty subexpressions are recorded as {\tt BRACKETED}
		  subexpressions whose MIN object has no elements and
		  no attributes.
\\[1ex]  
		& A {\tt BRACKTABLE} token with token {\tt value} V
		  is equivalent to (and an optimization of) a {\tt BRACKETED}
		  token whose MIN object has just a single element equal to
		  V and no attributes.  If attributes must be
		  added, the token is converted to a {\tt BRACKETED} token.
\end{tabular}
\\[1ex]
\begin{tabular}{p{1in}p{4.5in}}
...
\ttmkey{string}{of token}
		& For quoted string and non-natural number lexemes,
...
\ttmkey{indent}{of token}
		...
		\\[1ex]
		& Used by the bracketted subexpression recognizer
...
\ttmkey{level}{of symbol table entry}
		& Block nesting level of the definition block
...
not be active because they have the wrong type or wrong selectors
...
bracket is recognized.  No other mechanisms for changing the
context selectors are permitted.
...
\\[0.5ex]
\emkey{parser-trace-flag} \begin{tabular}[t]{@{}rl@{}}
                          ::= & \ttikey{input}{parser trace flag}
			        $|$ \ttikey{output}{parser trace flag}
			        $|$ \ttikey{definitions}{parser trace flag} \\
			  $|$ & \ttikey{bracketed subexpressions}%
			               {parser trace flag} \\
			  $|$ & \ttikey{operator subexpressions}%
			                   {parser trace flag} \\
			  \end{tabular}
\\[2ex]
...
sequences of {\em words} and {\em natural-numbers}, and
...
selectors.  Each set has 64 named flags.  A set of parser flags can be
...
In either the case of a {\tt parser-flag-list} or
...
for error messages.  The trace flag names are builtin: the
following gives for each flag the condition
that triggers a trace printout controlled by the flag,
and a brief description of that printout:

\begin{indpar}[0.4in]
\hspace*{-0.2in}\ttmkey{input}{parser trace flag} ~
    Upon reading a token from the input, a description of the token is printed.

\hspace*{-0.2in}\ttmkey{output}{parser trace flag}
    Upon producing a final output line, a description of the line is printed.

\hspace*{-0.2in}\ttmkey{definitions}{parser trace flag}
...
\hspace*{-0.2in}\ttmkey{bracketed subexpressions}{parser trace flag}
    Upon recognition of a subexpression by the bracketed subexpression
    recognition pass, the subexpression is printed.

\hspace*{-0.2in}\ttmkey{operator subexpressions}{parser trace flag}
    Upon recognition of a subexpression by the operator subexpression
    recognition pass, the subexpression is printed.
\end{indpar}
...
Selector names (unlike trace flag names) a \underline{not} buitin,
and must be defined by the
`\ttikey{parser define selector}{selector!parser define}'
command.  When the parser block containing selector name definition
...
Associated with each of these `{\tt parser define}' commands is a
`\ttkey{parser undefine}' command of the form:
\begin{center}
{\tt parser undefine} [ {\tt last} $|$ {\tt block} $|$ {\tt all} ]
         {\em type} {\em name} {\em selectors}
\end{center}

This command clears the designated selector bits from
symbol table entries made by their associated `{\tt parser define}'
commands.  This can be done either for just the `{\tt last}' such
entry, for all entries in the current parser block,
or for `{\tt all}' such entries.  Particular bits may be
cleared, or all bits may be cleared.  When the parser block containing
a `{\tt parser undefine}' command ends, the symbol table entry
selector bits are restored to their state at the beginning of the block.
...
Brackets, indentation marks, and indentation separators are
...
{\tt "\BACKSLASH{}n"}, and with no other syntax attributes.
(there may be non-syntax attributes such as {\tt .position}).
...
recognizer.  Quoted string and non-natural-number lexemes must be handled
differently because they have no token {\tt value}.  These are
represented by MIN objects
that have the {\tt .initiator} attribute {\tt "\BACKSLASH""} for a quoted
string and {\tt "\#"} for a non-natural number, and that have a single
element which is a MIN string equal as a character string to the
{\tt string} of the token being represented (this is the quoted
character string or the number lexeme string).
...
\hspace*{-0.2in}\ttdmkey{terminator}{subexpression attribute}\\
    ...
    or & the {\em indentation-separator-name} used to separate lines \\
...
\hspace*{-0.2in}\ttdmkey{arguments}{subexpression attribute}\\
    A MIN object whose elements are the arguments of the named
    bracket of a named bracketed subexpression, and which has
    no syntax attributes.  Each argument is
    either a bracketed subexpression or a quoted string, and is
    represented in the same way as the {\tt value} of a
    {\tt BRACKETED} token.
\\[2ex]
\hspace*{-0.2in}\ttdmkey{keys}{subexpression attribute}\\
    A MIN object whose elements are the keys of the named
    bracket of a named bracketed subexpression, and whose
    {\tt .separator} attribute is the {\em named-separator}
    that introduces each key.  The keys and the {\tt .separator}
    attribute are all either MIN strings or MIN labels with MIN string
    elements.

\end{indpar}

In addition to the attributes just given, a MIN object may have
a \ttdmkey{position}{of MIN object} attribute that gives the positions
...
\emkey{parser-bracket-command}
    \begin{tabular}[t]{@{}rl@{}}
    ::= & {\em parser-unnamed-bracket-command} \\
...
The general syntax of bracket definitions is:

\begin{indpar}[0.1in]
\emkey{parser-unnamed-bracket-command}%
...
\\
	...
	& \hspace*{3em} & [ {\tt with parsing selectors}
	                    {\em new-selectors} ] \\
        &               & [ {\tt with full lines} ] \\
    $|$ & \multicolumn{2}{l}{\ttikey{parser undefine bracket}{undefine bracket}
...
\ttmkey{full line}{in bracket definition} property.  In this case
...
associated with the full line {\em opening-bracket-name}, or an
end-of-file,
can terminate the subexpression.  During parsing of a
`{\tt full line}' subexpression, the parser only knows about
subexpressions and paragraphs that are included completely inside the
`{\tt full line}' subexpression.
...
an associated {\em indenta\-tion-separator-name} supplied by that definition.
An example is
\begin{center}
...
The general syntax of indentation mark definitions is:
...
The {\em indentation-separator-name} may be used to separate
...
Else if $P'>P$, push $P'$ into the $P$ stack, and push a pointer to the
