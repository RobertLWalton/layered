...
number of {\em line-feeds} preceeding the character.
...
preceeding the character, or after the beginning of input if there
...
the first {\em line-feed} preceeding the
...
(have no characters).  The are used to commumicate the indent of
...
\begin{tabular}{p{1in}p{4.5in}}
		& Standardly the token lexeme types are word, mark, separator,
		  ...
		  and comment, indent, line break, and end of file
...
\\[1ex]
		& Note that naturals have values that are MIN
		  strings equal to their lexemes,
		  and are \underline{not} MIN numbers.
		  This allows very large natural lexemes,
		  and does not try to exploit the 1-1 correspondence
		  between not-too-large natural lexemes and
...
\begin{itemize}
\item An indent equal to or less than the current indent.
\item An indent less than the current indent.
\item Any line break.
\item Any blank line.
...
\hspace*{-0.2in}\ttmkey{end at paragraph break}{parser option} \\
    ...
    Whether or not lines that are not indented as much as the
    current indent also end logical lines is controlled by other
    options, but if not, a logical line is the same as a unindented paragraph
    where unindented paragraphs are separated by blank lines.
...
\hspace*{-0.2in}\ttmkey{end at le indent}{parser option} \\
    This parser option causes the beginning of a line with indent less than
...
\hspace*{-0.2in}\ttmkey{end at outer closing}{parser option} \\
    ...
    If this option is off or is turned off by an opening bracket
    or indentation mark or prefix separator,
    no closing bracket outer to the bracketed
    subexpression, indented paragraph, or prefix list will
    be recognized anywhere in the the bracketed subexpression,
    indented paragraph, or prefix list.
...
\label{PARSING-END-AT-OPTIONS-1}
...
\label{PARSING-END-AT-OPTIONS-2}
...
end of a physical line, and is logically the last thing in the
...
also referred to as the \key{paragraph indent} of that paragraph.
...
with \TT{0} paragraph indent.

A more detailed description of indented paragraphs is given
in Section~\itemref{INDENTED-PARAGRAPH-SUBEXPRESIONS}.

An \key{unindented paragraph}
...
appear in the parser's \key{header table}, and be marked there
as a paragraph header.  A paragraph header must appear at the
...
`\key{default paragraph header}', which if
not \TT{MISSING}, is inserted at the beginning of
any logical line where a paragraph header can appear but does
not appear.
There is also a `\key{paragraph head scanner}' lexical
master table index, which when not \TT{0} specifies a special
lexical scan performed at the beginning of the logical
line that can identify implicit paragraph headers based on
the first characters in the logical line.

An unindented paragraph ends with the next blank line that
follows a logical line, or when the indented paragraph that
contains the unindented paragraph ends (this can be at an
end of file).

Details on paragraph headers are given in
Section~\itemref{PARAGRAPH-AND-LINE-HEADERS}.

\subsection{Parser Symbol Table}
\label{PARSER-SYMBOL-TABLE}
...
\\[1ex]
		& A \key{symbol} is a MIN string equal to the
		  ...
		  or natural lexeme.  Quoted string and numeric
...
A selector set is represented as a 48-bit unsigned integer value
with the bit in
position 1\TT{<{}<}N being on if the N+1'st selector is in the set.
Thus there can be at most 48 distinct selectors used by any set of
...
\hspace*{-0.2in}\ttmkey{parser}{parser selector} \\
    If this is on when a top level logical line is parsed, the line
    is treated as a parser command.  This selector is
    also used as a symbol table entry selector to ensure
    that the line is parsed appropriately for a parser command.
...
\hspace*{-0.2in}\ttmkey{data}{parser selector} \\
    ...
    separators and the type/attributes parts of typed
    brackets.
...
and parser contexts is specified in Figure~\itemref{PARSER-COMMANDS}.
...
\emkey{parser-test-command}
    \begin{tabular}[t]{@{}rl@{}}
    ::= & \ttikey{parsing trace}{trace!parser} {\em parser-flag-spec} \\
    $|$ & \ttikey{parser print trace}{trace!parser print} \\
    $|$ & \ttikey{parser test}{test!parser} \ldots \\
    \end{tabular}
\label{PARSER-TEST-COMMAND}
\\[0.6ex]
\emkey{parser-trace-flag} ::= see page \pageref{PARSING-TRACE-FLAGS}
\\[0.6ex]
...
The parser has serveral sets of \emkey{parser-flags}\index{flags!parser}:
\begin{center}
\begin{tabular}{lll}
parsing selectors 	& 48 bits	& \pagref{PARSING-SELECTORS} \\
parsing end at options	& 8 bits	& \pagref{PARSING-END-AT-OPTIONS-1} \\
parsing trace flags	& 64 bits	& \pagref{PARSING-TRACE-FLAGS} \\
\end{tabular}
\end{center}
...
\end{center}
is equivalent to a {\em parser-flag-list} listing all end-at options
except \TT{end at paragraph break}.  This is because the group name
...
may be given or modified by the `\ttkey{parsing trace}' command.
...
in Figure~\itemref{PARSING-TRACE-FLAGS}.
...
\hspace*{-0.2in}\ttmkey{prefix subexpressions}{parsing trace flag}
    Enables tracing for the parser prefix separator recognition pass.
...
The only builtin selector names are `\TT{parser}' and `\TT{data}'.
...
There may be at most 48 selectors defined at any one time.
...
not modify the top level selectors.  An example of this
...
consecutive quoted strings if they are in the same
logical line.  Thus if logical lines end with a line break
...
The bracketed parser pass has its own symbol table containing:
\begin{indpar}[0.5in]
...
There is also a separate symbol table for each typed opening:
see Section~\itemref{TYPED-BRACKETED-SUBEXPRESSIONS}.

In bracketed subexpression recognition, 
multi-lexeme keys in the bracketed pass symbol table
...
However the separate symbol table for a typed opening is used
during a second pass over the typed subexpression or prefix,
and so keys in that table are recognized if their lexemes
are not all on the same physical line: again
see Section~\itemref{TYPED-BRACKETED-SUBEXPRESSIONS} for details.
...
(see Section~\itemref{INDENTED-PARAGRAPH-SUBEXPRESIONS})
or a pargraph header prefix separator
...
For untyped brackets the only
syntax attributes are \TT{.initiator}, \TT{.terminator}, and
\TT{.separator}, and the
only non-syntax attribute is the \TT{.position} attribute.
For typed brackets
the only syntax attributes are \TT{.type} and \TT{.separator}, but arbitrary
non-syntax attributes may be encoded within the typed brackets,
in addition to the \TT{.position} attribute.

Sub-subexpressions may occur among the list of tokens that determine either the
object elements or the value of a non-syntax attribute.
Before their containing object is formed,
these are reduced by the parser to \TT{BACKETED} or \TT{BRACKETABLE} tokens
with a MIN object \TT{value}, or to \TT{DERIVED} tokens with a non-MIN-object
\TT{value} (e.g., a label).
...
\begin{indpar}\begin{verbatim}
{"": .initiator = [$ LOGICAL_LINE $], .terminator = "<LF>"|
     parser test {"": .initiator = "(", .terminator = ")"|
                      x 5 {"<Q>"|foo|} |} |}
\end{verbatim}\end{indpar}
...
one per top level logical line.  For each of these tokens,
its `\TT{value}' encodes its logical line.

Syntactically a {\em bracketed-subexpression} takes one of several forms:
...
recognized if they begin at the end of a line and are the last
element in their containing expression.
...
The following subsections describe the different kinds of
{\em bracketed-subexpressions} and the {\em parser-bracketed-commands}
that control their recognition.
...
\\[0.5ex]
\emkey{new-selectors} ::= {\em parser-flag-spec}
\\[0.5ex]
\emkey{end-at-options} ::= {\em parser-flag-spec}
	~~~~~ [see \pagref{PARSING-END-AT-OPTIONS-1}]
...
of non-missing component.  A component may consist of
either a single MIN object or a list of strings, numbers, and
labels which is converted into a MIN string if there is only
one token in the component and into a MIN label otherwise.
...
\emkey{indented-paragraph-subexpression} ::=
	...
	\end{tabular}
\\[0.5ex]
\emkey{paragraph-line} ::= logical line, possibly ended by
                           a {\em line-separator}
\\[0.5ex]
\emkey{paragraph-end} ::=
             beginning of a line whose indent is less than
			  the current indent
\\[0.5ex]
...
See~\pagref{PARSING-END-AT-OPTIONS-1}.  The setting of these options can be
...
\label{PARSER-INDENTATION-MARK-COMMAND} \\
    ...
    ::= & \ttikey{parser define}{define indentation mark}
	  ...
	  {}[ \TT{with paragraph master} {\em master-index} ] \\
	  {}[ \TT{with line master} {\em master-index} ] \\
	  {}[ \TT{with paragraph header} {\em header} ] \\
	  {}[ \TT{with line header} {\em header} ] \\
	  \end{tabular} \\
...
\emkey{end-at-options} ::= {\em parser-flag-spec}
	~~~~~ [see \pagref{PARSING-END-AT-OPTIONS-1}]
\\[0.5ex]
\emkey{master-index} ::= unsigned integer
	~~~~~ [see \pagref{LINE-VARIABLES} ]
\\[0.5ex]
\emkey{header} ::= MIN object with \TT{.type} and no elements
	~~~~~ [see \pagref{LINE-VARIABLES}]
\\[0.5ex]
...
(\pagref{END-AT-LINE-SEPARATOR}) option permits.  Standardly
line separators are only recognized outside subexpressions
(untyped bracketed, typed bracketed, prefix list, or indented subparagraph)
in the paragraph.
...
However, an \key{empty logical line} with no ending
...
the second of which is empty.  There is no other way of creating
an empty logical line within a paragraph, and all paragraph lines
are non-empty.

The paragraph as a whole is converted to a MIN object whose elements
are the paragraph lines, whose \TT{.initiator} attribute
is the {\em indentation-mark} that introduced the paragraph,
and whose \TT{.terminator} attribute is \TT{min::INDENTED\_PARAGRAPH()}.
A paragraph may be empty (have no paragraph lines).

The {\em master-index} and {\em header} parameters
(see \pagref{LINE-VARIABLES}) are used in the
line beginning algorithm (see \pagref{LINE-BEGINNING-ALGORITHM})
to generate implied unindented paragraph
headers and line headers.
...
Then a new selector set and new options are computed by modifying the saved set
according to the {\em new-selectors} and the saved options according
...
Note that if no {\em new-selectors} are given, the selectors
are not modified, if no {\em end-at-options} 
are given, the options are not modified, and if no
...
\TT{\{my type: my attribute = 5, my option = TRUE|} ~~~\ldots~~~
    \TT{|my type\}} \\
\end{center}
from which various parts can be omitted.  Some or all of the
...
of one or more words, naturals, numerics, and quoted strings, each
representing a MIN string (note naturals represent MIN strings and
\underline{not} MIN numbers).  In the latter case, if there is only
...
Elements must be single bracketed subexpressions, or sequences of
words, naturals, numerics, and quoted strings that are made into
...
\begin{center}
\begin{tabular}{l}
\TT{\{.position[H] = (3,4)|A B C|\}} \\
\TT{\{.position[H,65] = (3,4)|A B C|\}} \\
\TT{\{.position[65] = (3,4)|A B C|\}} \\
\TT{\{.position[65,H,256] = (3,4)|A B C|\}} \\
\end{tabular}
\end{center}
...
\emkey{typed-attribute-equal} ::= ditto
    ~~~~~ [e.g., `\TT{=}']
\\[0.5ex]
\emkey{typed-attribute-negator} ::= ditto
    ~~~~~ [e.g., `\TT{no}']
\\[0.5ex]
...
parsing selectors are standardly set to just `\TT{data}' and
`\TT{prefix}', while during the scanning of typed subexpression
{\em element-lists} parsing selectors are standardly
those of the subexpression context.
Duing both scans options are standardly set to those of the
...
and options
according to {\em element-selectors} and {\em end-at-options}
...
current parsing selector set is set to the {\em attribute-selectors}
...
\ttikey{parser define typed bracket}{typed bracket!parser define}
...
\\[0.5ex]
\emkey{typed-element-selectors} ::= {\em parser-flag-spec}
\\[0.5ex]
\emkey{end-at-options} ::= {\em parser-flag-spec}
	~~~~~ [see \pagref{PARSING-END-AT-OPTIONS-1}]
\\[0.5ex]
\emkey{typed-attribute-selectors} ::= {\em parser-flag-list}
\\[0.5ex]
...
\\[0.5ex]
\emkey{typed-attribute-negator} ::= {\em quoted-name}
\\[0.5ex]
\emkey{typed-attribute-flags-opening} ::= {\em quoted-name}
\\[0.5ex]
\emkey{typed-attribute-multivalue-opening} ::= {\em quoted-name}
...
{\em parser-typed-backet-command} (see \pagref{PARSER-TYPED-BRACKET-COMMAND})
enables use of the command
defined punctuation to construct {\em typed-prefix-separators}.
...
Note: \begin{minipage}[t]{5.8in}
      ...
      have the same type (i.e., \TT{.type} attribute), or
      same prefix group (see \pagref{PREFIX-GROUP}).
      \end{minipage}
\end{indpar}
...
The general syntax of {\em parser-prefix-commands} is:
...
\\
    ...
    \begin{tabular}[t]{rl}
    ::= & \TT{parser}%
          \ttindex{parser define prefix}%
	  \ttindex{previx!parser define}
	  \begin{tabular}[t]{@{}l@{}}
	  \TT{define prefix} {\em prefix-name} \\
	  {\em parsing-selectors} \\
	  {}[ \TT{with parsing selectors} {\em new-selectors} ] \\
	  {}[ \TT{with group} {\em prefix-group} ] \\
	  {}[ \TT{with implied subprefix} {\em implied-prefix} ] \\
	  {}[ \TT{with lexical master} {\em lexical-master} ] \\
	  {}[ \TT{with options} {\em prefix-options} ] \\
	  \end{tabular} \\
    $|$ & \ttikey{parser undefine prefix}{prefix!parser undefine}
	  \begin{tabular}[t]{@{}l@{}}
	  {\em prefix-name} \\
	  {\em parsing-selectors} \\
	  \end{tabular} \\
    \end{tabular}
\\[0.5ex]
\emkey{prefix-name} ::= {\em quoted-name}
\\[0.5ex]
\emkey{prefix-group} ::= {\em quoted-name}
\\[0.5ex]
\emkey{implied-prefix} ::= {\em typed-prefix-separator}\label{IMPLIED-PREFIX}
				~~~~~ [see \pagref{TYPED-PREFIX-SEPARATOR}]
\\[0.5ex]
\emkey{lexical-master} ::= {\em quoted-name}
\\[0.5ex]
\emkey{prefix-options}
    \begin{tabular}[t]{rl}
    ::= & {\em parser-flag-list} ~~~~~ [see \pagref{PARSER-FLAG-LIST}] \\
    ::= & \TT{[} {\em prefix-option} \{ \TT{,} {\em prefix-option} \TT{]}
    \end{tabular}
\\[0.5ex]
\emkey{prefix-option}
    \begin{tabular}[t]{rl@{~~~~~}l}
    ::= & {\em end-at-option}   & [see \pagref{PARSING-END-AT-OPTIONS-1}] \\
    $|$ & \TT{only once}
    \end{tabular}
...
of the containing {\em prefix-n-list} as modified by any \TT{parsing selectors}
...
with a non-missing {\em prefix-group}, the {\em prefix-group} is
said to be the group of the prefix.  Prefixes whose \TT{.type}
has no prefix definition, or whose prefix definition has a
missing {\em prefix-group}, are said to have no group.

In an expression of the form:
\begin{center}
\{ {\em prefix-n} {\em prefix-(n+1)-list} \}\PLUS{}
\end{center}
the {\em prefix-n}'s do not have to all have the same \TT{.type}
attribute if they all have the same group (which implies that
they all have some group).

The groups `\TT{paragraph}' and `\TT{line}' are special:
see \itemref{PARAGRAPH-AND-LINE-HEADERS}.
\\[2ex]
\hspace*{-0.2in}\ttmkey{implied subprefix}{in prefix definition}\\
If a prefix has a type with a prefix type definition that has
a non-missing {\em implied-prefix}, a copy of the {\em implied-prefix}
is inserted after the prefix, unless the first thing after the
prefix is a prefix with the same \TT{.type} attribute or group as the
{\em implied-prefix}.
\\[2ex]
\hspace*{-0.2in}\ttmkey{lexical master}{in prefix definition}\\
If a prefix is a substitution for a {\em prefix-lexeme}
(see \pagref{PREFIX-LEXEME}), the lexical master,
which determines how lexemes are scanned, will be reset to this
value immediately after the {\em prefix-lexeme}, unless this
value is missing, in which case the lexical master will be reset
to its value at the beginning of the containing logical line.

For an implied paragraph or line header, this parameter is used to
set the lexical master immediatly after the indent lexeme
that begins a logical line.  See \pagref{LEXICAL-MASTER-FOR-LOGICAL-LINE}.
\\[2ex]
\hspace*{-0.2in}\ttmkey{options}{in prefix definition}\\
The `end-at' options (see \pagref{PARSING-END-AT-OPTIONS-1}) in this
parameter, if set for the type of a paragraph or line header prefix,
determine when a logical line ends.
See \pagref{PREFIX-HEADER-END-AT-OPTIONS}.

The `\TT{only once}' option, if set for the type of a paragraph or line
header prefix, prevents the prefix from becoming the implicit paragraph
or line header (which would then be implied for subsequent paragraphs
or lines that had no explicit header prefix of their own).
See \pagref{PREFIX-HEADER-ONLY-ONCE-OPTION}.

\end{indpar}

A {\em typed-prefix-separator} can appear in or be inserted in
the parser input by three different methods:

\begin{center}
\begin{tabular}{l}
explicit bracketed subexpressions,
		   as per Section~\itemref{TYPED-PREFIX-SEPARATORS} \\
replacements for prefix lexemes  \\
copies of implicit typed prefix separators, e.g., `\TT{implicit subprefix}'es
\end{tabular}
\end{center}

A `\emkey{prefix-lexeme}' is a special kind of lexeme that can be
mapped to a {\em typed-prefix-separator} by the following
parser commands:

\begin{indpar}[0.1in]
\emkey{parser-prefix-lexeme-command}%
\label{PARSER-PREFIX-LEXEME-COMMAND} \\
    \hspace*{0.2in}
    \begin{tabular}[t]{rl}
    ::= & \ttikey{parser define}{define prefix lexeme} ~
          \begin{tabular}[t]{@{}l}
	  \ttikey{prefix lexeme}{parser define} \\
	  {\em prefix-lexeme-name} \TT{as} {\em typed-prefix-separator} \\
	  \end{tabular} \\
    $|$ & \ttikey{parser undefine}{undefine prefix lexeme} ~
	  \ttikey{prefix lexeme}{parser undefine} ~
	  {\em prefix-lexeme-name}
    \end{tabular}
\label{PREFIX-LEXEME-DEFINITION}
\\[0.5ex]
\emkey{prefix-lexeme-name} ::= {\em quoted-name}
\\[0.5ex]
{\em typed-prefix-separator} ::= see \pagref{TYPED-PREFIX-SEPARATOR}
\end{indpar}

When a {\em prefix-lexeme} is returned by the lexical scanner,
it has a name that is looked up in the parser stack managed by the
the above commands, and if found, is replaced by a copy of
the {\em typed-prefix-separator} found in the table entry.  If
not found, the {\em prefix-lexeme} is announced as an error.

If the type of any prefix has a non-missing `\ttkey{implicit subprefix}'
parameter, then a copy of the value of this parameter is inserted
after the prefix, unless it would be inserted before another prefix
with the same \TT{.type} attribute or prefix group.  Implicit
header prefixes can also be inserted as per
\pagref{IMPLICIT-HEADER-PREFIX-INSERTION}.

After a {\em prefix-lexeme} is replaced by a {\em typed-prefix-separator}
the lexical master is reset, and this can change how lexemes are
scanned following the {\em prefix-lexeme}.  If the replacement prefix
type has a non-missing `\ttkey{lexical master}'
parameter, that is used.  Otherwise
the lexical master at the beginning of the logical line is used.
The lexical master can also be reset at the beginning of a logical line
by header prefixes as per
\pagref{IMPLICIT-HEADER-PREFIX-LEXICAL-MASTERS}.

The `\TT{options}' parameter of a prefix type table entry is only used
by header prefixes: see \pagref{HEADER-PREFIX-OPTIONS}.

\subsubsection{Paragraph and Line Headers}
\label{PARAGRAPH-AND-LINE-HEADERS}

A \key{paragraph header} is a prefix separator that begins
an unindented paragraph.  A \key{line header} is a prefix
separator that begins a logical line.
Unindented paragraphs are contained in indented paragraphs.
The \key{top level} is treated as an indented paragraph with indentation \TT{0}.
...
A logical line that begins an unindented paragraph is said to be
in \key{paragraph beginning position}.
This means it is the first logical line of an indented paragraph,
or is after a sequence of blank and comment lines that contains
a blank line and that follows a logical line.
Here a \key{comment line} is a line that contains nothing
but a single comment.

A logical line in paragraph beginning position may begin with a
typed prefix separator that has the `\TT{paragraph}' group.
This is called the \key{paragraph header} of the paragraph the logical
line begins, and may be followed by a typed prefix separator that has
the `\TT{line}' group, which will be the \key{line header} of the logical
line.

Any logical line may begin with a typed prefix separator that
has the `\TT{line}' group, which will be the \key{line header}
of that logical line.

Paragraph headers and line headers may also be
specified by prefix lexemes or implicitly
by an `\mkey{implied paragraph header}{in parser context}'
and an `\mkey{implied line header}{in parser context} that
are part of the parser context.

When an indentation mark is recognized, it sets the implied
paragraph header to a {\em typed-prefix-separator} or to the
missing value.  Whenever
the implied paragraph header is set, the implied line header
is set to the \TT{implied subprefix} parameter of the implied
paragraph header, which may be missing, or to the missing value
if the implied paragraph header is set to missing.
...
in paragraph beginning position is not a {\em typed-prefix-separator}
with `\TT{paragraph}' group, or a {\em prefix-lexeme} that is replaced
by such, then a copy of the implied paragraph header is inserted just
after the indent token, unless the implied paragraph header is missing.

If the first thing after a paragraph header (either explicit, a
{\em prefix-lexeme} replacement, or a copy of the implicit paragraph
header) is not a {\em typed-prefix-separator}
with `\TT{line}' group, or a {\em prefix-lexeme} that is replaced
by such, then a copy of the implied line header is inserted just
after the paragraph header, unless the implied line header is missing.
Similarly for the first thing after the indent token that begins
a logical line \underline{not} in paragraph beginning position.

TBD

The parse context includes an `\mkey{implied header}'
that may be missing or may be an {\em implied-prefix}
(see \pagref{IMPLIED-PREFIX}).  If not missing, it (i.e., its prefix type
definition) must have either the `\TT{paragraph}' or `\TT{line}' group.
If the parser implied header has `\TT{paragraph}' group, it is
called the parser `\mkey{implied paragraph header}{of parser}',
and if its prefix type has a defined implicit subprefix, that must have
the `\TT{line}' group and is called the parser
`\mkey{implied line header}{of parser}'.  Otherwise if the implied header
has `\TT{line}' group, then it is the implied line header, and
the implied paragraph header is said to be missing.

Each logical line begins the a line indent token.  When this is read,
the lexical scanner master table index is reset as follows:

\begin{center}
\begin{tabular}{l}
\key{paragraph header lexeme} \\
\key{line header lexeme} \\
\end{tabular}
\end{center}
...
If the logical line is in paragraph beginning position, a paragraph header
lexeme followed by a line header lexeme may be produced, or either or
both may be omitted.  If the logical line is not in
paragraph beginning position, only a line header lexeme may be produced.
In any case, header lexemes can only be produced at the beginning of
a logical line and at most one of each type can be produced.
Also, if either is produced, the logical line may not begin with a typed
prefix separator opening bracket (e.g., may not begin with `\TT{\{}').

After resetting the the lexical scanner master table index,
the parser reads the beginning of the logical line and looks for
headers.  If the line begins with lexical headers, the line cannot contain
bracketed headers.  If the line does not begin with lexical headers,
the parser looks for bracketed headers which may or may not be present.
If neither are present, the parser uses context variables as described
below to optionally insert implied headers.

Lexical headers have a value that is looked up in the parser's
\key{header table}.  If there is no selected entry with
this value, then the lexical header is ignored as if it did not
exist.  Similarly the type of any prefix separator that might be
a bracketed header is looked up, and if there is no selected entry with
this type, then the prefix separator is treated as a non-header.

The parser algorithm that does all this is called
the \key{line beginning algorithm}.  This algorithm makes use
of context variables called the \skey{line variable}s.

When a bracketed or lexical paragraph header is recognized as
having a selected header table entry, the line
variables are reset according to values in that table entry.
Similarly when a bracketed or lexical line header is recognized as
having a selected header table entry, some of the line
variables are reset according to values in that table entry.

We next list the \skey{line variable}s\label{LINE-VARIABLES}, and after that
spell out the line beginning algorithm in detail.

\begin{indpar}[0.4in]

\hspace*{-0.2in}\ttmkey{paragraph master}{line variable}\\
\hspace*{-0.2in}\ttmkey{line master}{line variable}\\
    The lexical scanner master index is reset to one of these values
    at the beginning of a logical line.  If the line is in paragraph
    beginning position, the paragraph master is used.  Otherwise
    the line master is used.
\\[2ex]
\hspace*{-0.2in}\ttmkey{paragraph header}{line variable}\\
    If this value is not missing
    and no bracketed or lexical paragraph header is found at the
    beginning of a logical line that is in paragraph beginning position,
    then a \TT{PREFIX} token whose value is a
    copy of this paragraph header variable is inserted
    at the beginning of the logical line.
\\[1ex]
\hspace*{-0.2in}\ttmkey{line header}{line variable}\\
    If this value is not missing
    and no bracketed or lexical line header is found where one
    might occur,
    then a \TT{PREFIX} token whose value is a
    copy of this line header variable is inserted
    where the line header would otherwise be.
\\[2ex]
\hspace*{-0.2in}\ttmkey{header selectors}{line variable}\\
    Parsing selectors determined by the paragraph header and used
    as the current parsing selectors to parse headers in the paragraph.
    Also the default value for the line selectors below.
\\[1ex]
\hspace*{-0.2in}\ttmkey{header options}{line variable}\\
    Parsing options determined by the paragraph header and used
    as the current parsing options to parse headers in the paragraph.
    Also the default value for the line options below.
\\[2ex]
\hspace*{-0.2in}\ttmkey{line selectors}{line variable}\\
    Parsing selectors determined by the paragraph and line headers and used
    as the current parsing selectors to parse the \underline{non-header}
    portions of lines in the paragraph.  These default to the header
    selectors.
\\[1ex]
\hspace*{-0.2in}\ttmkey{line options}{line variable}\\
    Parsing options determined by the paragraph and line headers and used
    as the current parsing options to parse the \underline{non-header} portions
    of lines in the paragraph.  These default to the header
    options.
\\[1ex]
\hspace*{-0.2in}\ttmkey{instruction}{line variable}\\
    An instruction specifying what to do at the beginning of a
    logical line.  Currently this contains only one optional
    command: `\key{paragraph reset}'.

\end{indpar}

The line variables are initialized according to the
indentation mark that introduced the indented paragraph.  For
top level the paragraph master is the parser's default
master, the line master is \TT{0}, the headers are missing,
the selectors and options
are the parser's default selectors and options,
and there are no instructions.  By convention, if the
lexical scanner master index is \TT{0}, the lexical scanner
will never produce header lexemes.

In addition to the line variables, there are selectors
set by the indentation mark, which do not change while parsing
the indented paragraph, and which are used to select paragraph
header entries in the parser's header table.
We will call these the \key{paragraph
selectors}.  For top level these are the parser's default selectors.

At the start of the \key{line beginning algorithm} the parser is
at the beginning of a logical line, and none of the lexemes in
the line have been scanned.

The \key{line beginning algorithm}\label{LINE-BEGINNING-ALGORITHM}
is then as follows:

\begin{enumerate}

\item If the logical line is at the beginning of its indented paragraph,
or if the logical line is in paragraph beginning position and
the instruction variable value contains the paragraph reset command,
then reset all the line variables as specified by the indentation
mark the introduced the indented paragraph.  In particular, all
selector and option line variables are reset by modifying the
selectors and options in effect when the indentation mark was found
by the selector and option changes associated with the indentation mark:
see the parser indentation mark command on
\pagref{PARSER-INDENTATION-MARK-COMMAND}.  This means that the paragraph
selectors, header selectors, and line selectors are all set to the same
value, and the header and line options are set to the same value.

\item Set the lexical scanner master index to the paragraph lexical
master if the logical line is in paragraph beginning position,
or to the line master otherwise.

Note that a logical line is preceeded by an indent lexeme, and
the parser suspends lexical scanning after an indent lexeme is
scanned, in order to allow the lexical scanner master table index
to be reset before the first lexeme of the logical line is scanned.

\item Scan the lexemes of the logical line (through the next indent
or end-of-file lexeme).

\item If the first lexeme of the logical line is not a header lexeme,
skip to Step~\ref{BRACKETED-HEADER-SCAN}.

\item
If the first lexeme of the logical line
is a paragraph header lexeme:

\begin{enumerate}

\item
If the logical line is in paragraph beginning position,
look up the lexeme string value (which is not part of the
input, but is generated by the lexical scanner) in the parser's
header table.  If a table entry marked as a paragraph header is found
that is selected by the paragraph selectors, reset all the
line variables according to values in the table entry
(see below for details).

Then if the paragraph header variable value is not missing,
replace the paragraph header lexeme
by a \TT{PREFIX} lexeme whose value is a copy of the
paragraph header variable value (which may have just been changed).
Else if the paragraph header variable value is missing, just
discard the paragraph header lexeme.

\item
Else if the logical
line is \underline{not} in paragraph beginning position,
announce an error, and discard the paragraph header lexeme.

\end{enumerate}

\item
If the first lexeme of the logical line
is \underline{not} a paragraph header lexeme and the logical
line is in paragraph beginning position, and if the paragraph
header variable value is not missing, insert a \TT{PREFIX}
lexeme at the beginning of the logical line whose value is a copy of the
paragraph header variable value.

\item
If the first lexeme of the logical line was a paragraph header
lexeme and the second is a line header lexeme, or if the
first lexeme was a line header lexeme,
look up the line header lexeme's string value (which is not part of the
input, but is generated by the lexical scanner) in the parser's
header table.

If a table entry marked as a line header is found
that is selected by the header selectors, reset the
line header, line selectors, and line options
line variables according to values in the table entry
(see below for details).
If no such entry is found, reset the line selectors and options
from the header selectors and options.

Then if the line header variable value is not missing,
replace the line header lexeme
by a \TT{PREFIX} lexeme whose value is a copy of the
line header variable value (which may have just been changed).
Else if the line header variable value is missing, just
discard the line header lexeme.

\item
If no line header lexeme was found in the previous step,
skip to Step~\ref{INSERT-LINE-HEADER}.

Else skip to Step~\ref{TERMINATE-LINE-BEGINNING}.

\item \label{BRACKETED-HEADER-SCAN}
We come to this step only if there are no header lexemes.

Parse the beginning of the logical line and produce the first
token.

If this is \underline{not} a \TT{PREFIX} token,
skip to Step~\ref{INSERT-PARAGRAPH-HEADER} if the logical
line is in paragraph beginning position, or
skip to Step~\ref{INSERT-LINE-HEADER} otherwise.

If the line is not in paragraph beginning position,
skip to Step~\ref{LINE-HEADER-LOOKUP}.

Look up the type of the \TT{PREFIX} token value
in the parser's header table.
Accept an entry if it has
paragraph header type and is selected by the paragraph selectors.

If the result of this lookup is \underline{not} an accepted entry,
skip to Step~\ref{INSERT-PARAGRAPH-HEADER}.

Else reset all the
line variables from values in this entry (see details
below), then reset the paragraph header variable to the copy of
the value of the \TT{PREFIX} token.
Lastly go to Step~\ref{LINE-HEADER-SCAN}.

\item \label{INSERT-PARAGRAPH-HEADER}

If the paragraph header variable value is not missing,
insert a \TT{PREFIX} token containing a copy of the paragraph header
variable value at the beginning of the logical line.
This becomes the paragraph header token for that line.

Then if there is a \TT{PREFIX} token that was not accepted
as a paragraph header in Step~\ref{BRACKETED-HEADER-SCAN},
skip to Step~\ref{LINE-HEADER-LOOKUP}.
Else (if no \TT{PREFIX} token at all was found
in Step~\ref{BRACKETED-HEADER-SCAN}),
skip to Step~\ref{INSERT-LINE-HEADER}.

\item \label{LINE-HEADER-SCAN}
Parse the beginning of the yet-unparsed
part of the logical line and produce the next token.
If this token is \underline{not} a \TT{PREFIX} token,
skip to Step~\ref{INSERT-LINE-HEADER}.

\item \label{LINE-HEADER-LOOKUP}
Look up the type of the value of the \TT{PREFIX} token
previously found that was not accepted as a paragraph
header in the parser's header table.  Accept a table entry if it has
line header type and is selected by the header selectors.

If an entry was accepted, reset the line header
line variable to a copy of the value of the token,
reset the line selectors and line options
line variables from the values in the
accepted entry (see below for details),
and skip to Step~\ref{TERMINATE-LINE-BEGINNING}.

\item \label{INSERT-LINE-HEADER}

If the line header variable value is not missing,
insert a \TT{PREFIX} token containing a copy of the line header variable value
at the beginning of the logical line.  This becomes the line header
token for that line.

Copy the header selectors and options to the line selectors and options.

\item \label{TERMINATE-LINE-BEGINNING}
Terminate the line beginning algorithm, and finish the parse
of the logical line using the line selectors and line options.


\end{enumerate}


Parser header table entries are defined and undefined by
{\em parser-header-commands}:

\begin{indpar}[0.1in]
\emkey{parser-header-command}%
\label{PARSER-HEADER-COMMAND} \\
    \hspace*{0.2in}
    \begin{tabular}[t]{rl}
    ::= & \ttikey{parser define}{define header}
	  \begin{tabular}[t]{@{}l@{}}
	  {\em header-kind} \TT{header}
	  {\em header-name} {\em parsing-selectors} \\
	  {}[ \TT{with parsing selectors} {\em new-selectors} ] \\
	  {}[ \TT{with parsing options} {\em end-at-options} ] \\
	  {}[ \TT{with paragraph master} {\em master-index} ] \\
	  {}[ \TT{with line master} {\em master-index} ] \\
	  {}[ \TT{with paragraph header} {\em header} ] \\
	  {}[ \TT{with line header} {\em header} ] \\
	  {}[ \TT{with keep} {\em yes-or-no}\QMARK{} ] \\
	  \end{tabular} \\
    $|$ & \ttikey{parser undefine}{undefine header}
	  \begin{tabular}[t]{@{}l@{}}
	  {\em header-kind} \TT{header}
	  {\em header-name} \\
	  {\em parsing-selectors} \\
	  \end{tabular} \\
    \end{tabular}
\label{HEADER-DEFINITION}
\\[0.5ex]
\emkey{header-kind} ::= \TT{paragraph} $|$ \TT{line}
\\[0.5ex]
\emkey{header-name} ::= {\em quoted-name}
\\[0.5ex]
\emkey{new-selectors} ::= {\em parser-flag-spec}
\\[0.5ex]
\emkey{end-at-options} ::= {\em parser-flag-spec}
...
\emkey{header} ::= MIN object with \TT{.type} and no elements
	~~~~~ [see \pagref{LINE-VARIABLES}]
\\[0.5ex]
\emkey{yes-or-no} ::= \TT{yes} $|$ \TT{no} 
\end{indpar}

All parameters are permitted and used for paragraph headers,
but only the \TT{selectors}, \TT{options}, \TT{line header},
and \TT{keep} parameters are permitted and used for line headers.

`\TT{with keep}' without a following {\em yes-or-no} defaults to `\TT{yes}',
while the absence of any `\TT{with keep}' defaults to `\TT{no}'.

\subsubsection{The Print Bracket and Header Commands}
...
\\
    \hspace*{0.2in}
    \begin{tabular}[t]{rl}
    ::= & \ttikey{parser print header}{parser print header}
	  {\em partial-name} \\
    \end{tabular}
\end{indpar}
...
\emkey{parser-operator-definition} \\
    ...
    ::= & \TT{parser}\ttindex{parser define operator}~%
	    \begin{tabular}[t]{@{}l@{}}
	    \TT{define operator}
	        \begin{tabular}[t]{l}
	    ...
	    {}[ \TT{with} {\em reformatter-name} \TT{reformatter}
			  {\em reformatter-arguments} ] \\
	    \end{tabular} \\
    $|$ & \TT{parser}\ttindex{parser undefine operator}~%
	    \begin{tabular}[t]{@{}l@{}}
	    \TT{undefine operator}
	        \begin{tabular}[t]{l}
		{\em operator-name} {\em parsing-selectors} \\
		{\em operator-flag} {\em operator-flag}\,\STAR{} \\
		\end{tabular} \\
