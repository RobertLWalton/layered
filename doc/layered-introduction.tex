...
or \TT{.terminator} attributes.  Also no additional parser passes,
such as the operator pass, are applied to the {\em element-list},
though the bracketed subexpression pass is used to form
bracketed subexpressions in the {\em element-list}, and
other passes may be applied inside these bracketed subexpressions.
...
the parse.  Premature expression ends are recognized according
to the settings of the {\em end-at-options}.

\item Bracketed subexpressions are recognized during the scan of
the subexpression, according to the setting of the
{\em typed-attribute-selectors} before the first {\em typed-middle}
and after the second {\em typed-middle}, and the
{\em typed-element-selectors} between the {\em typed-middles}.

\item The resulting subexpression is then further analyzed by
a builtin (reformatter) pass, which recognizes, outside
the {\em element-list},
{\em typed-attribute-begin/equal/separator/negators}
and bracketed subexpressions whose \TT{.initiators} are
{\em typed-attribute-flags/multivalue-initiators}.

\item Additional passes, which are applied to
{\em untyped-bracketed-subexpressions}
and {\em indented-paragraphs}, are \underline{not}
applied to {\em typed-bracketed-subexpressions}.  However
these passes are applied to
{\em untyped-bracketed-subexpressions}
and {\em indented-paragraphs} that appear \underline{inside}
a {\em typed-bracketed-subexpression}.
