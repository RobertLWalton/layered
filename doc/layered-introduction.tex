...
\emkey{opening-bracket} ::= as defined by a
    {\em parser-untyped-bracket-command} (\pagref{PARSER-BRACKET-COMMAND})
    ~~~~~ [e.g., `\TT{(}' or `\TT{[<}']
\\[0.5ex]
\emkey{closing-bracket} ::= ditto
    ~~~~~ [e.g., `\TT{)}' or `\TT{>]}']
\\[0.5ex]
\emkey{element-list} ::= {\em simple-element-list} $|$
                         {\em prefix-0-list}
\label{ELEMENT-LIST}
\\[0.5ex]
\emkey{simple-element-list} ::= {\em element}\STAR{} but not {\em prefix-0-list}
\\[0.5ex]
\emkey{prefix-0-list} ::= specialized form of {\em element}\PLUS{};
                          see \pagref{PREFIX-0-LIST}
\\[0.5ex]
\emkey{element} ::= {\em word} $|$ {\em number} $|$ {\em mark} $|$
                    {\em separator} $|$ {\em bracketed-expression}
\\[0.5ex]
Note: An {\em untyped-bracketed-expression} {\em element-list}
is terminated by the first {\em closing-bracket}.
\end{indpar}
...
\emkey{non-continuation-line-break} ::= {\em line-break} not followed by a
				   continuation line
\\[0.5ex]
\emkey{indentation-mark} ::= as defined by a
    {\em parser-indentation-mark-command}
...
Note: \begin{tabular}[t]{@{}l}
      ...
      first {\em line-separator} \\
      or {\em non-continuation-line-break}.
      \end{tabular}
\end{indpar}
...
\emkey{element} \begin{tabular}[t]{@{}rl}
		    ...
		    & but not ({\em doubled-}){\em typed-middle}
		      if not {\em type-mark} expression \\
		    & or `{\em type-mark} {\em typed-closing}'
		      if {\em type-mark} expression \\
		\end{tabular}
\\[0.5ex]
...
\emkey{doubled-typed-middle} :::= {\em typed-middle} {\em typed-middle}
    as a single lexeme if appropriate: see text
    ~~~~~ [e.g., `\TT{||}']
\\[0.5ex]
...
\emkey{typed-attribute-negative} ::= ditto
    ~~~~~ [e.g., `\TT{no}']
\\[0.5ex]
\emkey{value-opening} ::= ditto
    ~~~~~ [e.g., `\TT{\{*}']
\\[0.5ex]
\emkey{value-separator} ::= ditto
    ~~~~~ [e.g., `\TT{\{,}']
\\[0.5ex]
\emkey{value-closing} ::= ditto
    ~~~~~ [e.g., `\TT{*\}}']
\\[0.5ex]
...
\emkey{single-attribute-value}
    \begin{tabular}[t]{@{}rl}
    ::= & {\em value} \\
    $|$ & {\em multi-value-opening} {\em value} \\
        & \{ {\em multi-value-separator} {\em value} \}\STAR{} \\
	& {\em multi-value-closing} \\
    \end{tabular}
\\[0.5ex]
...
`\TT{:}' to be a {\em typed-attribute-begin},
`\TT{=}' to be a {\em typed-attribute-equal},
`\TT{,}' to be a {\em typed-attribute-separator},
`\TT{|}' to be a {\em typed-middle},
`\TT{\}}' to be a {\em typed-closing},
`\TT{text}' to be the sole definition selector,
and
`\TT{no}' to be a {\em typed-attribute-negative}.
...
ending with
\begin{center}
{\em typed-middle} {\em typed-middle} {\em typed-closing}
\end{center}
(e.g., `\TT{||\}}') but with the two {\em typed-middles} omitted.
