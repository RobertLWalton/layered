...
\key{header table} to see if it is a paragraph or line
header.  It may or may not be.

The prefix separator can only be a paragraph header if the
logical line occurs as the first logical line of an indented
paragraph or occurs just after a blank line that itself is
just after a logical line in the indented paragraph.  Note
that here a blank line is any sequence of blank and comment
lines containing at least one blank line, where a comment
line is a line containing only a comment, but comment lines
are not considered to be blank.

In addition, to be recognized as a paragraph header, the
type of the prefix separator must be the name of an entry
in the parser header table that is selected by the current
parser selectors (see the line beginning algorithm) and
is marked as a paragraph header (as opposed to being marked
as a line header).

If an explicit paragraph header is recognized at the beginning
of a logical line, it is used to reset the context data used by the
...
recognized, the line beginning algorithm considers the possibility
that there might be an implicit paragraph header, and if so, inserts
it (see below for detail).  In any case,
the algorithm continues by looking for a line header. 
...
line header, and a line header is looked for at the beginning
of every logical line, even those not a the beginning of an
indented paragraph and not preceeded by a blank line.

A logical line is said to be in \key{paragraph beginning position}
if it is the first logical line of an indented paragraph,
or is after sequence of blank and comment lines that contains
a blank line and that follows a logical line.

The \key{line beginning algorithm} uses the following context
...
\hspace*{-0.2in}\ttmkey{paragraph lexical index}{line beginning variable}\\
    ...
    position.
    A lexeme is then read.  If this lexeme is a header lexeme, it is
    looked up in the parser's header table, and if selected and a
    paragraph header, is treated as the type of a paragraph header.
    See details below.
\\[1ex]
\hspace*{-0.2in}\ttmkey{paragraph header}{line beginning variable}\\
    ...
    copy of this paragraph header variable may be inserted
    at the beginning of the logical line.
\\[2ex]
\hspace*{-0.2in}\ttmkey{line lexical index}{line beginning variable}\\
    If not \TT{0}, the lexical analyzer master is reset to this value
    at the beginning of a logical line that is \underline{not} in
    paragraph beginning position.
    A lexeme is then read.  If this lexeme is a header lexeme, it is
    looked up in the parser's header table, and if selected and a
    line header, is treated as the type of a line header.
    See details below.
\\[1ex]
\hspace*{-0.2in}\ttmkey{line header}{line beginning variable}\\
    If not missing and no explicit paragraph header is found at the
    beginning of a logical line that is \underline{not} in paragraph beginning
    position, then a prefix separator token whose value is a
    copy of this line header may be inserted
    at the beginning of the logical line.
\\[2ex]
\hspace*{-0.2in}\ttmkey{line selectors}{line beginning variable}\\
    The parsing selectors are set to this value at the beginning
    of a logical line that is \underline{not} in paragraph beginning
    position.
\\[1ex]
...
indentation mark the introduced the indented paragraph.  For
top level the indeces are \TT{0}, the headers are missing,
...
The \key{line beginning algorithm} run at the beginning of
a logical line and is as follows:
...
\item
If the instruction contains a \ttkey{reset}, reset all the
...
If the string is empty, or the paragraph lexical index was \TT{0},
skip to Step~\ref{PARAGRAPH-HEADER-SCAN}.
...
Otherwise reset all the line beginning variables from values in the
selected entry
(see below for details), and go to Step~\ref{INSERT-PARAGRAPH-HEADER}.
...
below), and go to Step~\ref{LINE-HEADER-SCAN} (thereby implicitly
taking the value of the line lexical index to be \TT{0} even if it is not).
...
If no entry is selected, skip to Step~\ref{INSERT-LINE-HEADER}.

Otherwise reset just the line header and line selectors
line beginning variables from values in the
selected entry (see below for details), and
...
original logical line and produce the first token.
More explicitly, start the parse just after any \TT{PREFIX} token
inserted or accepted as a paragraph header, or at the beginning
of the logical line if there is no inserted or accepted
paragraph header.

If the first token found is a \TT{PREFIX} token, look up the type of its value
in the parser's header table.  Accept an entry if it has
...
Otherwise reset just the line header and line selectors
line beginning variables from values in the
selected entry (see below for details),
