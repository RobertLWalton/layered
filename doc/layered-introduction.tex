...
options control whether line continuations are allowed and how must
these must be indented relative to the continued line.  Builtin
...
\hspace*{-0.2in}\ttmkey{parser}{parser selector/option} \\
    If this is on when a top level line is parsed, the line
...
\hspace*{-0.2in}\ttmkey{end at paragraph break}{parser option} \\
    ...
    options, but if not, a logical line is the same as a paragraph
    where paragraphs are separated by blank lines.
...
\hspace*{-0.2in}\ttmkey{end at line separator}{parser option} \\
    ...
    when not inside an indented paragraph,
...
\hspace*{-0.2in}\ttmkey{end at other closing}{parser option} \\
    This parser option allows a bracketed subexpression to be
    ...
    This option is unusual in that the computation of this
    option by the `new selectors' of a bracketed or indented
    paragraph subexpression
    always assumes this option is on in the containing subexpression.

    This is the default for bracketed subexpressions, but not for
    indented paragraphs.
...
TBD Brackets, indentation marks, and line separators are
bracketed parser pass symbol table keys.
Typed bracket openings (e.g.~`\TT{\{}'),
middles (e.g.~`\TT{|}'),
and closings (e.g.~`\TT{\}}') are also 
bracketed parser pass symbol table keys, and bracketed
subexpressions are used to represent attribute flags
(e.g.~within `\TT{[~]}') and multiple values
(e.g.~within `\TT{\{*~*\}}').
