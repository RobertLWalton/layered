...
\hspace*{-0.2in}broken middle lexemes,
    whose parts need to be glued together:
...
or end with {\em middle-break-end} (see \itemref{MIDDLE-LEXEME-BREAKS}).
...
quoted string concatenation, and the output of the
bracketed subexpression parser pass, and then describe
the different kinds of
{\em brac\-keted-subexpressions} and their related
...
so the modifications should the chosen so they do not affect parsing of
{\em typed-prefix-separators} or the lookup of bracket type table definitions.
...
ignored unless one of the following is true, where the context selectors
are the parsing selectors in effect at the point where the {\em implied-prefix}
is to be inserted:
...
but within its {\em parser-bracket-type-command} it is parsed
using a definition of its {\em typed-opening} with
empty {\em prefix-selectors}, so it is recognized there
...
\hspace*{-0.2in}\ttmkey{paragraph lexical master}{in bracket type definition}\\
...
If `\TT{with lexical master}' is given in the `\TT{define bracket type}'
command, both lexical master parameters are set to the same
{\em lexical-master-name}.
