...
\emkey{prototype-argument-list} ::=
	\begin{tabular}[t]{@{}l@{}}
	a {\em subexpression} with an \TT{.initiator} and
	\TT{.terminator} \\
	whose \TT{.terminator} is not \TT{INDENTED\_PARAGRAPH} \\
	and whose \TT{.separator} is missing or `\TT{,}'
	\end{tabular}
\\[0.5ex]
\emkey{subexpression} ::= see \pagref{SUBEXPRESSION}
\\[0.5ex]
\emkey{argument-descriptor} ::= \begin{tabular}[t]{@{}l}
				...
				\end{tabular}
\\[0.5ex]
\emkey{default-value} ::= parsed {\em expression}
\\[0.5ex]
