...
\item[Step 1:] Identify operators within the expression and
replace them with \TT{OPERATOR} tokens.

This is done by a left to right scan of expression lexemes,
choosing at each position the longest operator selected by
the current parsing selectors, or skiping over the current
lexeme if there is no operator.  Note that fixity of operators
is ignored in the process.

\item[Step 2:] Determine the fixity of operators that can be
