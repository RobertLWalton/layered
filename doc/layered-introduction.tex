...
\emkey{instruction-component}
	...
	& $|$ & {\tt translate} {\em lexical-table-name} \\
	...
	& $|$ & {\tt error} {\em type} \\
	& $|$ & {\tt output} {\em type} \\
	& $|$ & {\tt goto} {\em lexical-table-name} \\
	& $|$ & {\tt call} {\em lexical-table-name} \\
	& $|$ & {\tt return} \\
	& $|$ & {\tt fail} \\
\\[1ex]
...
An {\em lexical-table-definition} specifies an {\em lexical-table-name},
a {\em mode} for the table, a set of {\em lexical-table-entries}
...
The {\em mode} of an lexical table can be either a {\em type} or can be
the special `\ttmkey{master}{lexical table mode}' mode.  If it is a
...
The {\em mode} of an lexical table is the same as the {\em lexical-table-name}
unless a non-empty {\em mode-option} is given at the beginning of the
{\em lexical-table-definition}.  The `{\tt premature end of line}' lexical
table on \pagref{EXAMPLE-LEXEME-PROGRAM} is an example
of an lexical table with a {\em mode-option}.
...
Each \emkey{lexical-table-entry} contains an {\em atom-pattern} and
...
in an {\em lexical-table} is not significant.
...
An {\em atom-pattern} in an lexical table is associated with an
...
The {\em default-instruction} at the end of an lexical table behaves as
...
`{\tt master}' lexical table.  See the example on \pagref{EXAMPLE-LEXEME-PROGRAM}.
...
in a master mode lexical table or combined with a 
{\tt goto} transferring to a master mode lexical table.
This works even if the current lexeme is of zero length.
It may be used in the {\em default-instruction} of an lexical table;
...
As an example, the `{\tt master}' lexical table at the top of
...
A `\ttmkey{call}{in lexical program} {\em lexical-table-name}\,'
...
The \key{return stack} is a stack of up to 8 lexical table identifiers
...
A {\em return-vector} is a list of {\em lexical-table-names}.
...
an lexical table other than the one containing the `{\tt call}' instruction.
For example, if `{\tt call t(x,y,z)}' called an lexical table, in that
table `{\tt return}' would return to the lexical table containing the
`{\tt call}' instruction, while `{\tt return(2)}' would return to
lexical table {\tt y}.


The `{\tt call}' and `{\tt return}' instruction components
permit a non-master lexical table to be shared among different
...
The `{\tt call}' and `{\tt return}' instruction components
also permit a non-master lexical table to be shared among different
non-master tables.  For example, a table to scan escape sequences
may be shared among several lexical tables dedicated to scanning
