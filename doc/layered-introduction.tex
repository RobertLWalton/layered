...
Lexeme tables have optional \ikey{types}{type!of lexeme table}.
When the current lexical table becomes a lexeme table with a type
other than {\tt NONE},
...
atom table.  If the
instruction contains no {\tt translate} components, the
atom table provides the atom translation, but if the atom table
invoking instruction contains a {\tt translate} component, that is
applied to the atom identified by the atom table to produce the
atom translation.
...
when applied to the atom translation produced as just described.
