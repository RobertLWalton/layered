...
\hline
\multicolumn{1}{c|}{}
       & yes    & yes     & no    & no   &  \multicolumn{1}{|l|}{prefix} \\
\cline{2-6}
\multicolumn{2}{c|}{}
...
(i.e., non-explicitly-bracketed subexpressions).
[TBD: what about consecutive postfix or prefix operators]
\item[Requirement 2:] 
If there is a prefix {\tt OPERATOR} token in the list, there is only one,
it is the first token, and there are at least two tokens in the list.
\item[Requirement 3:] 
If there is a postfix {\tt OPERATOR} token in the list, there is only one,
it is the last token, and there are at least two tokens in the list.
\item[Requirement 4:] 
If there is an infix {\tt OPERATOR} token in the list, it is neither
the first nor last token in the list.
\item[Requirement 5:] 
Infix, prefix, and postfix {\tt OPERATOR} tokens in the list are not
next to other {\tt OPERATOR} tokens in the list.
\item[Requirement 6:] 
If there is an afix {\tt OPERATOR} token in the list, it is not the
first {\tt OPERATOR} token in the list.
\end{itemlist}

A single lexeme token output from the operator parsing pass may be
thought of as the equivalent of
a single {\tt EXPRESSION} token whose value is a
list of one element, that element being a MIN string equal to the
lexeme, with the list having either no special attributes, or
having just an {\tt .initiator} attribute equal to {\tt "} if
...
following rules:
\begin{itemlist}[0.7in]
\item[Rule 1:] A prefix operator must be preceded by the beginning of
the entire token list, or by a prefix, infix, or nofix operator
of precedence less than or equal to the precedence of the prefix operator,
else the prefix operator is not recognized.
\item[Rule 2:] An infix or postfix operator
must be preceded by a non-operator token,
or by a postfix operator,
or by a prefix, infix, or nofix
operator of precedence greater than
the precedence of the infix or postfix operator,
else the infix or postfix operator is not recognized.
\item[Rule 3:] A recognized postfix operator must be followed by the end of
the entire token list or by a postfix, infix, or nofix operator,
else an `error nofix operator' is inserted after the postfix operator
and an error is announced.
\item[Rule 4:] A recognized infix or prefix
operator must be followed by a non-operator
token or by a prefix or nofix operator
of precedence greater than the precedence of the infix operator,
else an `error operand' which is not an operator
is inserted after the infix or prefix operator
and an error is announced.
\end{itemlist}

Simply by enforcing these rules the operator parser pass enforces
...
than any legal operator precedence.
\end{itemlist}

After processing an infix or nofix operator, a pointer to the token
after the operator is pushed into $F$ and the precedence of the
operator is pushed into $P$.  Similarly for a prefix operator,
except that it is the precedence of the operator minus one that
is pushed into $P$.

In the following $F$ is used to denote both the stack $F$ and
the top element of this stack.  Similarly $P$ is used to denote the
stack $P$ and the top element of this stack.

Then the operator parser pass algorithm simply repeats the following
steps in a loop:
\begin{itemlist}[1.2in]
\item[Loop Step 1:]
If $C\neq N$, find an operator beginning at token $C$, if there is one.
If $C=F$, infix and postfix operators of precedence equal to or
greater than $P$ are ignored.  If $C\neq F$, prefix operators are ignored.
...
selectors is ignored.  Among all non-ignored operators, a longest
is selected, and among all the longest, the one with the most recent
operator definition is selected.
\item[Loop Step 2:]
If an operator was found in Loop Step 1, replace its tokens by a single
OPERATOR token and point $C$ at that token.
\item[Loop Step 3:]
If the token before $C$ is a postfix operator,
$C\neq N$, and either no operator was find in Loop Step 1,
or an operator was found with precedence above that of the
postfix operator, or a non-postfix operator was found with
precedence equal to that of the postfix operator,
insert the error nofix operator of precedence
equal to $P$ as a single
OPERATOR token before $C$, set $C$ to point at it,
announce an error, and continue as if an operator were found.
\item[Loop Step 4:]
Else if $C\neq N$
...
Note that after this step either $C=N$ or an operator was found in Loop Step 1
or generated in Loop Step 2.
\item[Loop Step 5:]
If $C=N$ or the operator found in Loop Step 1 or generated by Loop Step 2
has a precedence equal to or below $P$, and if the
token before $C$ is a prefix or infix operator, insert
an error operand lexeme token before $C$,
announce an error, and then continue as if no error were found.
\item[Loop Step 6:]
If $C=N$ or the operator found in Loop Step 1 or generated by Loop Step 2
is a non-prefix operator with precedence less than or equal to $P$,
first apply Step 3 and Step 4 above to the list of tokens beginning with
$F$ and ending just before $C$, and second pop the $F$ and $P$ stacks.

Repeat this step until the operator has precedence greater than $P$
or the stacks are empty.  Emptying the stacks is only possible if
$C=N$, and in this case, terminate the algorithm (i.e., terminate the
loop).
\item[Loop Step 7:]
If a postfix operator was found in Loop Step 1,
move $C$ to the next token (after the postfix operator),
apply Step 3 and Step 4 above to the list of tokens beginning with
$F$ and ending just before $C$, point $F$
at the single non-operator
token produced by Step 4, an go to the next loop iteration.
\item[Loop Step 8:]
Move $C$ to the next token, 
push $C$ into the $F$ stack and the precedence of the operator
found into the $P$ stack.  Then go to the next loop iteration.
\end{itemlist}
