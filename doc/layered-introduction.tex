...
\hspace*{-0.2in}\ttmkey{paragraph selectors}{line beginning variable}\\
    The parsing selectors are set to this value at the beginning
    of a logical line that is in paragraph beginning position.
\\[1ex]
...
\hspace*{-0.2in}\ttmkey{paragraph header}{line beginning variable}\\
    If not \TT{MISSING} and no explicit paragraph header is found at the
...
\hspace*{-0.2in}\ttmkey{instruction}{line beginning variable}\\
    ...
    logical line.  This is only used if no explicit paragraph
    header or line header is found.

\end{indpar}

The \key{line beginning algorithm} is run at the beginning of
a logical line and is as follows:

\begin{enumerate}

\item Set the current parsing selectors to the paragraph selectors
if the line is in paragraph beginning position, or to the
line selectors otherwise.

\item Set the current lexical index to the paragraph lexical index
if the line is in paragraph beginning position, or to the
line lexical index otherwise.

\item If the current lexical index is not \TT{0}, use it to scan
...
and may have any string as a value.  If the string is empty, ignore
the lexeme.  Otherwise look it up in the parser's header table using
