...
A logical line in paragrph beginning position may begin with a
typed prefix separator that has the `\TT{paragraph}' group.
This is called the paragraph header of the paragraph the logical
line begins, and may be followed by a typed prefix separator that has
the `\TT{line}' group, which will be the line header of the logical
line.

Any logical line may begin with a typed prefix separator that
has the `\TT{line}' group, which will be the line header of that logical line.

Paragraph headers and line headers may also be
specified by special header lexemes.  A `\key{header lexeme}' is a
special lexeme that is mapped by a {\em header-lexeme-definition}
to a {\em typed-prefix-separator}, a copy of which replaces the
header lexeme in the input.

The parse context includes an `\mkey{implied header}{in parser context}'
