...
\hspace*{-0.2in}\ttmkey{line lexical index}{line beginning variable}\\
    The lexical analyzer master is reset to one of these values
...
at the beginning of a logical line.  As the algorithm executes,
the parser may be asked to produce the first one or
two tokens of the the logical line.

The line beginning algorithm is then as follows:

\begin{enumerate}

\item Reset the line selectors and options to equal the header line
selectors and options (so by default the same selectors and options
are used to scan the header and non-header portions of the line).

\item If the logical line is not in paragraph beginning position,
skip to Step~\ref{LOGICAL-LINE-BEGINNING}.

\item
If the instruction contains a \ttkey{paragraph reset}, reset all the
line beginning variables to the values they were set to
(by the indentation mark) at the
beginning of the indented paragraph.

\item \label{LEXICAL-INDEX-SET}
Set the lexical scanner master table index to the paragraph lexical
index if the line is in paragraph beginning position, and to the
line lexical index otherwise.  Then if this is \TT{0},
skip to Step~\ref{PARAGRAPH-HEADER-SCAN}.

\item \label{PARAGRAPH-BEGINNING}
Scan a lexeme (using the non-\TT{0} paragraph lexical index).
In this situation the next character input to the lexical
scanner will be the first non-whitespace character of a non-blank,
non-comment line, and the lexeme returned will depend upon the
next sequence of characters in the input, but the input itself will
not be consumed.  The returned lexeme must have header lexical type,
and may have any string as a value.

If the string is empty, skip to Step~\ref{PARAGRAPH-HEADER-SCAN}.

Note that it is assumed that if the lexical index is not \TT{0}
and the logical line begins with a prefix separator (e.g.,
begins with `\TT{\{}'), then the returned header lexeme must be
empty.

Otherwise, if the string is not empty,
look the string up in the parser's header table and
accept an entry if it is a paragaph header entry selected by the
paragraph selectors.

If an entry was selected,
reset all the line beginning variables from values in the
selected entry
(see below for details).

Go to Step~\ref{INSERT-PARAGRAPH-HEADER}.

\item \label{PARAGRAPH-HEADER-SCAN}
Parse the beginning of the logical line and produce the first
token.

If this is a \TT{PREFIX} token, look up the type of its value
