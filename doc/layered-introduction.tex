...
that begins with the {\tt ID-character} (\pagref{ID-CHARACTER}) immediately
followed by a {\em natural-number} causes a special lexeme with a special
lexeme type and empty character string to be prefaced
to the line, and the type of this special lexeme
is mapped (\itemref{MAPPED-LEXEMES}) to the prefix

\begin{center}
\TT{\{}{\em prefix} {\em prefix-attributes}\QMARK{} \TT{\}}
\end{center}
which then absorbs the elements of the logical line to become a
headed line object (\itemref{HEADED-LINES}) of the above form.
