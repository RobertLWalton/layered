...
is specified in Figure~\itemref{PARSER-COMMANDS}.

\begin{boxedfigure}[!p]
\emkey{parser-paragraph}
    \begin{tabular}[t]{@{}rlrl@{}}
    ::= & \ttkey{parser:} \\
    	& ~~~~{\em parser-command}\STAR{}
    \end{tabular}
\label{PARSER-PARAGRAPH}
\\[0.6ex]
...
\emkey{parser-block-command}
    \begin{tabular}[t]{@{}rl@{}}
    ::= & \ttikey{begin}{begin!parser block} {\em block-name} \\
    $|$ & \ttikey{end}{end!parser block} {\em block-name} \\
    $|$ & \ttikey{print blocks}{print!parser blocks} \\
    \end{tabular}
...
\emkey{parser-trace-command}
    \begin{tabular}[t]{@{}rl@{}}
    ::= & \ttikey{trace}{trace!parser} {\em trace-flag-spec} \\
    $|$ & \ttikey{print trace}{trace!parser print} \\
    \end{tabular}
\label{PARSER-TEST-COMMAND}
\\[0.6ex]
\emkey{trace-flag-spec} ::= {\em parser-flag-spec} with {\em parser-trace-flags}
\\[0.0ex]
...
Parser commands are logical lines in an indented paragraph
whose indentation mark `\TT{parser:}' appears at the beginning
of a top level logical line.  Parser commands are
themselves parsed before they are executed.

The entire `\TT{parser:}' indented
paragraph is parsed before any of its parser commands are executed.
...
\begin{tabular}{l}
\TT{parser:} \\
\TT{~~~~}\TT{begin} {\em block-name} \\
\TT{~~~~}{\em parser-command} \\
.~.~.~.~.~.~.~.~.~. \\
\TT{parser:} \\
\TT{~~~~}\TT{end} {\em block-name} \\
\end{tabular}
\end{center}
...
not possible to have one `\TT{end}' command end more than
one parser block.
...
parsing end at options	& 6 bits	& \pagref{PARSING-END-AT-OPTIONS-1} \\
...
canceled by a `\TT{end standard}' parser command.

The current set of
\skey{parsing trace flag}s\index{trace flag!parser}\index{flag!parsing trace}
may be given or modified by the `\ttkey{trace}' command.
...
\hspace*{-0.2in}headed paragraphs,
    ...
    the whole headed paragraph begins with a paragraph
    header \\
...
occurs at the beginning of an headed paragraph,
