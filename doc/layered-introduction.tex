...
\\[0.5ex]
\\[1ex]
Prefix Symbol Table. Contains information about prefix separator types:
\itemref{PREFIX-DEFINITIONS}
\\[1ex]
...
\hspace*{-0.2in}\ttdmkey{type}{subexpression attribute}\\
    ...
    or & \TT{\#} if the MIN object represents a numeric lexeme. \\[1ex]
       & The {\em type-label} \TT{""} represents
...
When a {\em typed-opening} (e.g.~`\TT{\{}') is recognized, the current parsing
...
options remain the same as those use to scan the
{\em element-list}.
...
above logical line (e.g., to \TT{\{data\}} when there are no
{\em prefix-attributes}).

The effect of reformatting is to delete the entire subexpression token list
and to define the object whose identifier
...
using its reformatter arguments to treat the lines as being
formatted
...
in place of the list containing just this element.  For example,
as long as the implied prefix to the logical line has no attributes:
\begin{center}
\begin{tabular}{lrl}
...
desired value.
But if the {\em element-list} of this object does not begin
with a preallocated stub (generated by an {\em ID} as per
\itemref{IDS}) followed by a {\em equal-sign} MIN string,
this reformatter does nothing
and the tokens are \underline{not} deleted.  On the other hand,
syntax errors in an {\em attribute-indented-paragraph} merely result in
parsing error messages indicating per-{\em attribute-line}
fixups, and do not prevent
the {\em ID} from being set or the object from being deleted.

If an {\em ID} that has been previously referenced in the input
is set to a MIN string value, instead of an object value,
the previous references will become dangling (they will reference
a MIN preallocated stub containing the {\em ID}, and will
\underline{not} cause garbage collection errors).  This situation
merely results in a parser error message.  The {\em ID} is set
to the MIN string value, the subexpression tokens are deleted, and
subsequent references to the {\em ID} in the input will work
properly.
