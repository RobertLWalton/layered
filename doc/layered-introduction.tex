...
\item[Step 1:] Determine whether a sequence of lexemes
...
operators, and amoung these operators defined later are
preferred over operators defined earlier.

Operator fixity is ignored unless the operator is
afix.  If the operator is afix, a decision is made as to
whether to keep the operator or effectively deselect
is and look for other possible operators.  This decision
is based solely on information accumulated from the scan
before the operator: i.e., on the part of the expression
to the left of the operator.  Details are below.

If an operator is found, its lexemes are collected into
an \TT{OPERATOR} token.  If no operator is found, the
current position is moved one lexeme to the right.


lexemes are an operator, choosing the longest operator
if there are alternatives.
replace them with \TT{OPERATOR} tokens.
...
operator, an infix operator of priority lower than the infix
