...
\\[1ex]
		& For a \TT{BRACKETED} token, the \TT{.initiator}
		  of the token \TT{value} if there is one.  Also,
		  as special cases, for \TT{BRACKETED} tokens that
		  are paragraph or line headers
		  (\itemref{HEADED-LINES} and \itemref{HEADED-PARAGRAPHS}),
		  the group of the
		  token \TT{value} \TT{.type}, which will be either
		  `\TT{paragraph}' or `\TT{line}' (these tokens have
		  no \TT{.initiator}).  For a
...
table from which the scan of the part of a line after its indent
starts is called the `\key{lexical master}' of the line.  The parser
can reset the lexical master immediately after reading the indent of
a line.  See \itemref{LINE-VARIABLES} for details.
...
processing.  These top level tokens appear at the
beginning of the token stream as they are created,
and as they are never again accessed
...
A logical line begins just after the first indent lexeme in the
file or the first indent token after the previous logical line.
This indent token specifies the indentation of the logical line,
called the `\key{line indent}'.
...
\item Any line break.
\item Any blank line. This is called a `\key{paragraph break}'.
\item A line separator, e.g.~`\TT{;}'.
\item The end of file.
\item Just before a closing bracket (e.g.~`\TT{)}") of a
bracketed subexpression containing the logical line.  The closing
bracket is called an `\key{outer closing}'.  This is
an abnormal way to terminate a logical line, and is a parsing
error, but the line is terminated nonetheless.
\end{itemize}
...
\emkey{quoted-key}\label{QUOTED-KEY}
    :::= \TT{"}{\em key}\TT{"}
\\[0.0ex]
\emkey{partial-name} ::= {\em quoted-key}
\\[0.6ex]
...
\emkey{parser-context-command}
    ...
    \end{tabular}
\\[0.0ex]
\emkey{context-name} ::= {\em quoted-key}
...
and {\em quoted-keys}, that are arbitrary sequences of {\em word},
...
The only builtin selector names are `\TT{top level}' and `\TT{data}'.
...
If a top level logical line begins with a \emkey{context-name}, the top level
selector set is reset to the default selectors (selectors of the default
context, initially empty at the beginning of the input file), and the
selectors used to parse the rest of the logical line are these
modified according to a {\em parser-flag-spec}
associated with the {\em context-name}.

There are two builtin
...
one per top level logical line or unindented paragraph.
...
{\em typed-bracketed-sub\-ex\-pres\-sions}, and unindented paragraphs
...
\\[0.5ex]
\emkey{bracket-options} ::= {\em parser-flag-spec}
...
\emkey{end-at-options} ::= {\em parser-flag-spec}
	~~~~~ [see \pagref{PARSING-END-AT-OPTIONS-1}]
\\[0.5ex]
\emkey{header} ::= {\em typed-prefix-separator}
                   (\pagref{TYPED-PREFIX-SEPARATOR}) with `\TT{paragraph}'
                   or `\TT{line}' \TT{.type} (\pagref{HEADED-LINES})
\\[0.5ex]
...
in the paragraph.  Line separators may not be used in headed lines
or headed paragraphs.
...
elements taken from the rest of the logical line.  See
\itemref{HEADED-LINES} for details.
...
are the MIN objects produced by paragraph lines outside headed
...
that affect the parsing of indented paragraphs: see \pagref{LINE-VARIABLES}.
...
(\pagref{BRACKETED-PARSER-PRINT-COMMAND}).
...
and options
according to {\em element-selectors} and {\em bracket-options}
...
`\TT{code}' to be the sole definition selector \\
...
\\[0.5ex]
\emkey{typed-attribute-begin} ::= {\em quoted-key}
\\[0.5ex]
...
{\em element-selectors} in either typed bracket definition,
...
If the first thing in the {\em prefix-(n+1)-list}
following a {\em prefix-n} is not a prefix in the same group
as the {\em prefix-n}'s \TT{implied subprefix} (and that is not
missing), then a copy of
the \TT{implied subprefix} is prepended to the {\em prefix-(n+1)-list}.

\hspace*{-0.2in}\ttmkey{parsing options}{in prefix definition}\\
`\TT{with parsing options}' is only permitted for prefixes that
have the `\TT{paragraph}' group.  The parsing options are
...
For prefixes with the `\TT{paragraph}' group, this is used to
set the \TT{paragraph\_lexi\-cal\_\EOL header} line variable.
...
\label{PARSER-MAPPED-LEXEME-COMMAND} \\
    \hspace*{0.2in}
    \begin{tabular}[t]{rl}
    ::= & \ttikey{parser define}{define mapped lexeme} ~
	  \ttikey{mapped lexeme}{parser define} ~ {\em mapped-lexeme-name} \\
    ::= & \TT{parser}%
          \ttindex{parser define mapped lexeme}%
	  \ttindex{mapped lexeme!parser define}
	  \begin{tabular}[t]{@{}l@{}}
	  \TT{define mapped lexeme} {\em mapped-lexeme-name} \\
	  {\em parsing-selectors} \\
	  {}[ \TT{with token type} ~ {\em token-type} ] \\
	  {}[ \TT{with token value} ~ {\em token-value} ] \\
	  {}[ \TT{with lexical master} ~ {\em lexical-master} ] \\
	  \end{tabular} \\
    $|$ & \ttikey{parser undefine}{undefine mapped lexeme} ~
	  \ttikey{mapped lexeme}{parser undefine} ~
	  {\em mapped-lexeme-name}
    \end{tabular}
...
\emkey{quoted-key} ::= see \pagref{QUOTED-KEY}
\\[0.5ex]
\emkey{token-type} ::= \TT{BRACKETED} $|$ \TT{BRACKETABLE}
                   $|$ \TT{PURELIST} $|$ \TT{PREFIX} $|$ \TT{DERIVED}
\\[0.5ex]
\emkey{token-value}
    \begin{tabular}[t]{rl}
    ::= & {\em quoted-string} \\
    ...
    \end{tabular}
\\[0.5ex]
\emkey{lexical-master} ::= {\em quoted-key}
\end{indpar}
...
A `\key{headed line}' is a line header and the part of a logical
...
taken from the rest of the line.  This output has \underline{no}
\TT{.initiator} or \TT{.terminator}, unlike the parser's output
for a logical line that has no line header.
...
appear at the beginning of a logical line.

A `\key{headed paragraph}' is a logical line that begins with paragraph header,
plus all the following logical lines up until the headed paragraph end.
A headed paragraph may be ended by a blank line (unless the paragraph
is `continuing'), by a logical line
beginning with another (non-implicit) paragraph header, or by the end
...
and whose elements are the logical lines in the headed paragraph.
...
paragraph.  The first unindented paragraph has
...
\hspace*{-0.2in}\ttmkey{implied header}{line variable}\\
    ...
    as an `\key{implied prefix}' just after the indent token that begins
...
\hspace*{-0.2in}\ttmkey{lexical master}{line variable}\\
    ...
    indent token that begins a logical line.
\\[1ex]
\hspace*{-0.2in}Note: if any of the above variables are reset
after they have been used at the beginning of a logical line,
they will not take affect until the next logical line.
\\[2ex]
...
\hspace*{-0.2in}\ttmkey{paragraph lexical master}{line variable}\\
    These values are copied to the \TT{selectors}, \TT{options},
    \TT{implied header}, and \TT{lexi\-cal master} line variables
    above just before the above variables are used for a line
    in paragraph beginning position, \underline{unless} the
    \TT{options} variable above (not the \TT{paragraph options})
    contains the `\TT{continuing}' option.
\end{indpar}

The line variables are recomputed as follows whenever
an explicit paragraph header is encountered or an implicit
paragraph header is inserted:

\begin{enumerate}

\item
The prefix definition of the paragraph header is found using
the \TT{selectors} in effect at the time the paragraph header
was parsed.  As the paragraph header must be at the beginning
of a logical line, these selectors will be the selectors at the
beginning of the line modified by implied paragraph or line
headers inserted at the beginning of the logical line (these
implied headers will then be discarded once an explicit
paragraph header is found).
The prefix definition must exist and have `\TT{paragraph}' group,
else the paragraph header would not be a paragraph header.

\item
The \TT{selectors}, \TT{options}, and \TT{lexical master}
line variables are initialized by setting them to the
values of the selectors, options, and lexical master
set for the indented paragraph (i.e., by the intentation
mark definition and the context in which the indentation mark
was found).

\item
The \TT{selectors} and \TT{options} line variables
are modified by the paragraph
prefix definition \TT{parsing selectors} and \TT{parsing options}.
The \TT{lexical master} line variable is reset to the
value of \TT{lexical master} in the paragraph prefix definition.

\item
If the \TT{options} line variable now has the `\TT{sticky}' option flag and the
paragraph header is explicit, then:
\begin{enumerate}

\item
The \TT{selectors}, \TT{options}, and \TT{lexical master} line variables
are copied to the \TT{paragraph selectors}, \TT{paragraph options},
and \TT{paragraph lexical master} line variables.

\item
The \TT{paragraph implied prefix} line variable
is set to a copy of the explicit paragraph header.
\end{enumerate}

\item
If the paragraph header prefix definition has a non-\TT{MISSING}
\TT{implied subprefix}, the \TT{.type} of this is looked up using
`new selectors' to see if it has an active prefix definition with
the `\TT{line}' group.  If yes, then:
\begin{enumerate}

\item
The \TT{selectors} and \TT{options} line variables are modified by the line
prefix definition \TT{parsing selectors} and \TT{parsing options}.
The \TT{lexical master} line variable is reset to the
value of \TT{lexical master} in the line prefix definition.

\item
\TT{implied prefix} line variable is set to a copy of the paragraph header
definition's the \TT{implied subprefix}.

\end{enumerate}

\item
The current parsing selectors and options are set to the values
of the \TT{selectors} and \TT{options} line variables for the
purpose of parsing the remainder of the logical line.  If the
\TT{implied header} line variable is not missing, a copy of
the \TT{implied header} is inserted at the beginning of this
logical line remainder.
...
If this value is missing, the indentation mark sets both
\TT{lexical master} line variables to the \TT{lexical master}
given in the definition of the indentation mark.

Whenever an explicit (non-implied) paragraph header is recognized,
the \TT{implied paragraph header} is reset to a copy of the
explicit paragraph header, with the following exceptions.  If
the explicit header has a definition with the \TT{sticky} option,
the \TT{implied paragraph header} is not reset.  If
the explicit header has a definition with the \TT{continuing}
option, the \TT{implied paragraph header} is reset to missing.

Whenever an explicit (non-implied) line header is recognized,
the \TT{implied line header} is reset to a copy of the
explicit line header, with the following exceptions.  If
the explicit header has a definition with the \TT{sticky} option,
the \TT{implied line header} is not reset.

Whenever the \TT{implied paragraph header} is reset, the values
set by its prefix definition
are used to reset the \TT{paragraph selectors},
\TT{paragraph options}, \TT{paragraph lexi\-cal master}, and
\TT{implied line header}.  The \TT{implied line header} is set
to the value of the definition's \TT{implied subprefix}.
If there is no prefix definition (no active prefix symbol table
entry for the \TT{implied paragraph header} \TT{.type}),
...
and \TT{line lexical master}.  If the \TT{implied
line header} is set to missing, or its \TT{.type} has
no active prefix definition,
all these variables are reset to missing.

\TT{Paragraph selectors} and \TT{line selectors}
are computed by starting with
the parsing selectors set by the indentation mark of the
containing indented paragraph and applying the changes indicated
by the paragraph or line header definition \TT{line selectors}.

It is in error if the \TT{implied paragraph header} is set to a
{\em typed-prefix-separator} which is not in a paragraph header,
or if the \TT{implied line header} is set to a
{\em typed-prefix-separator} which is not a line header.

If the first thing after the indent token beginning a logical line
...
is read, then the selectors and lexical master are set to
their line variable line values.

It is an error if an implied header is inserted that would change
the current parsing selectors.  This can only happen if an
implied line header is inserted just after a paragraph header.
...
\\
    ...
    ::= & \ttikey{parser print bracket}{parser print bracket}
	  {\em partial-name} \\
    $|$ & \ttikey{parser print prefix}{parser print prefix}
	  {\em partial-name} \\
    $|$ & \ttikey{parser print mapped lexeme}{parser print mapped lexeme}
	  {\em partial-name} \\
