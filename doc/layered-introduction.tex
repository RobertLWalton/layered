...
\emkey{parser-context-command}
    \begin{tabular}[t]{@{}rl@{}}
    ::= & \ttikey{parser define context}{selector!define} {\em context-name} \\
        & ~~~ [ {\tt with parsing selectors} {\em parser-flag-spec} ] \\
    \end{tabular}
\\[0.0ex]
\emkey{context-name} ::= {\em simple-name}
...
Other selector names must be defined by the
...
names `\ttikey{standard}{parsing block}' and defines in it the selectors
\begin{center}
...
not defined within the `{\tt standard}' parser block.
...
For many `{\tt parser define}' commands there is a companion
