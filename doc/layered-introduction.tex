...
\newcommand{\ttmlindex}[3]{\index{#1@\TT{#1}!#2@\TT{#2}!#3}}
...
\hspace*{-0.2in}\ttmkey{allow empty paragraphs}{parser option} \\
    Suppress the parsing error that occurs when an empty indented
    paragraph is found.

\hspace*{-0.2in}\ttmkey{allow inside paragraphs}{parser option} \\
    Suppress the parsing error that occurs when an indented
    paragraph is found inside (not at the end of) a logical line
    or headed paragraph.
...
(\itemref{BRACKET-TYPE-DEFINITIONS}),
and be marked there as a paragraph header
(i.e., must have the `\TT{*PARAGRAPH*}' group).
...
\\[1ex]
Parser Command				& Given Standard Qualifier
\\\hline
\tt define selector label & \tt + label \\
\tt define selector code  & \tt + code \\
\tt define selector text  & \tt + text \\
\tt define selector math  & \tt + math \\
\end{tabular}
\end{center}
...
\end{verbatim}\end{indpar}
will be ignored if none of the `{\tt + code}', `{\tt + text}', or `{\tt + math}'
standard qualifiers are given, and the command
\begin{indpar}[0.1in]\begin{verbatim}
define bracket "{{" ... "}}" [code, text, math]
...
\emkey{prefix-0-list}
    ...
    \end{tabular}
\\[0.5ex]
\emkey{simple-element-list}\label{SIMPLE-ELEMENT-LIST}
    ::= {\em element}\STAR{}
\\[0.5ex]
\emkey{element}
    \begin{tabular}[t]{rl}
    ::= & {\em word} $|$ {\em numeric} $|$ {\em number} $|$ {\em natural} $|$
          {\em numeric-word} \\
    $|$ & {\em mark} $|$ {\em separator} $|$ {\em quoted-string} \\
    $|$ & {\em simple-bracketed-subexpression} \\
    \end{tabular}
\\[0.5ex]
\emkey{simple-bracketed-subexpression}\label{SIMPLE-BRACKETED-SUBEXPRESSION}
    \begin{tabular}[t]{rl}
    ::= & {\em untyped-bracketed-subexpression} \\
    $|$ & {\em typed-bracketed-subexpression} \\
...
began with an explicit \TT{\{}{\em prefix}\TT{\}} that is enabled
by the \TT{enable prefix} or \TT{enable table prefix} options,
any {\em terminators}
...
\emkey{typed-elements}
	\begin{tabular}[t]{@{}rl}
	::= & {\em typed-middle}~~~{\em simple-element-list}~~~%
	      {\em typed-middle} \\
...
\\[0.5ex]
{\em simple-bracketed-subexpression} ::=
    see \pagref{SIMPLE-BRACKETED-SUBEXPRESSION}
\\[0.5ex]
...
\item
{\em Typed-prefix-separators} are \underline{not} recognized as prefixes
anywhere outside brackets in a {\em type-bracketed-expression}.  Instead
they are recognized as objects with attributes but no vector elements.
\end{enumerate}
\end{indpar}
...
If an entry is found, the selectors used to scan
the {\em elements} are computed by modifying the context selectors
according to the {\em bracket-selectors} in the
found entry.
If no entry is found, the selectors used to scan
the {\em elements} are computed by modifying the context selectors
according to the {\em bracket-selectors}
associated with the {\em typed-opening} by the
\TT{parser define typed bracket} command (see below).

In either case, the options used to scan
the {\em elements} are computed by modifying the context options
according to the {\em bracket-options}
associated with the {\em typed-opening} by the
\TT{parser define typed bracket} command (see below).
...
used to scan {\em elements} of its
...
\emkey{quoted-key} ::= see \pagref{QUOTED-KEY}
\\[0.5ex]
{\em bracket-selectors} ::= see \pagref{BRACKET-SELECTORS}
\\[0.5ex]
{\em bracket-options} ::= see \pagref{BRACKET-OPTIONS}
\\[0.5ex]
...
\emkey{bracket-type-options} ::=
    \begin{tabular}[t]{p{4in}}
    {\em option-spec} restricted to `\TT{enable \ldots}' options \\
    if {\em bracket-type-group} is \underline{not} `\TT{*PARAGRAPH*}', \\
    else unrestricted \\
    \end{tabular}
\\[0.5ex]
...
`\TT{with parsing options}' is restricted to modifying
`\TT{enable} \ldots' options unless the bracket type
has the `\TT{*PARAGRAPH*}' group.

The {\em bracket-type-options} are \underline{not} used when the
type of a {\em typed-bracketed-subexpression} has the bracket type.

The {\em bracket-type-options} \underline{do} modify the
options in effect at the beginning of a {\em prefix-n-list}
when a {\em prefix-n} has the bracket type, with the modified
options being in effect for the {\em prefix-(n+1)-list} following
the {\em prefix-n}.
...
\tt ~~~~~~~with precedence 0 \\
\tt ~~~~~~~with control reformatter ( ":", has condition ) \\
\tt define operator "else" [code] initial line & {\tt + code} \\
\tt ~~~~~~~with precedence 0 \\
\tt ~~~~~~~with control reformatter ( ":" ) \\
\\[1ex]
\tt define operator ":" [code] afix right line & {\tt + code} \\
\tt ~~~~~~~with precedence 0 \\
...
\tt ~~~~~~~with precedence 0 \\
\\[1ex]
\tt define operator "exit" [code] initial line & {\tt + code} \\
\tt ~~~~~~~with precedence 1000 \\
\tt ~~~~~~~with exit reformatter \\
\tt define operator "continue" [code] initial line & {\tt + code} \\
\tt ~~~~~~~with precedence 1000 \\
\tt ~~~~~~~with exit reformatter \\
\end{tabular}
\end{center}
...
Parser Command				& \shortstack{
                                          Given\\Qualifiers}
\\\hline
\tt define operator "do" [code] initial line & {\tt + code} \\
\tt ~~~~~~~with precedence 3000 \\
\tt define operator "repeat" [code] initial line & {\tt + code} \\
\tt ~~~~~~~with precedence 3000 \\
\tt define operator "while" [code] right line & {\tt + code} \\
\tt ~~~~~~~with precedence 3000 \\
\tt define operator "until" [code] right line & {\tt + code} \\
\tt ~~~~~~~with precedence 3000
\\[1ex]
\tt define operator "exactly" [code] afix right line & {\tt + code} \\
\tt ~~~~~~~with precedence 3000 \\
\tt define operator "at most" [code] afix right line & {\tt + code} \\
\tt ~~~~~~~with precedence 3000 \\
\\[1ex]
\tt define operator "times" [code] afix left line & {\tt + code} \\
\tt ~~~~~~~with precedence 3000 \\
\end{tabular}
\end{center}
...
\tt ~~~~~~~with unary prefix reformatter \\
\end{tabular}
\end{center}
...
\begin{tabular}{l@{\hspace*{0.2in}}l}
\multicolumn{2}{c}
{\bf Standard `\ttkey{test operators}'
     \label{STANDARD-TEST-OPERATORS} Component} \\
\multicolumn{2}{c}
{\bf Operator Definition Parser Commands}
\\[1ex]
Parser Command				& \shortstack{
                                          Given\\Qualifiers}
\\\hline
\tt define operator "is integer" [code] infix & {\tt + code} \\
\tt ~~~~~~~with precedence 12000 \\
\tt ~~~~~~~with unary postfix reformatter \\
\tt define operator "is finit" [code] infix & {\tt + code} \\
\tt ~~~~~~~with precedence 12000 \\
\tt ~~~~~~~with unary postfix reformatter \\
\tt define operator "is infinite" [code] infix & {\tt + code} \\
\tt ~~~~~~~with precedence 12000 \\
\tt ~~~~~~~with unary postfix reformatter \\
\tt define operator "is undefined number" [code] infix & {\tt + code} \\
\tt ~~~~~~~with precedence 12000 \\
\tt ~~~~~~~with unary postfix reformatter \\
\tt define operator "is number" [code] infix & {\tt + code} \\
\tt ~~~~~~~with precedence 12000 \\
\tt ~~~~~~~with unary postfix reformatter \\
\tt define operator "is true" [code] infix & {\tt + code} \\
\tt ~~~~~~~with precedence 12000 \\
\tt ~~~~~~~with unary postfix reformatter \\
\tt define operator "is false" [code] infix & {\tt + code} \\
\tt ~~~~~~~with precedence 12000 \\
\tt ~~~~~~~with unary postfix reformatter \\
\tt define operator "is truth value" [code] infix & {\tt + code} \\
\tt ~~~~~~~with precedence 12000 \\
\tt ~~~~~~~with unary postfix reformatter \\
\tt define operator "is string" [code] infix & {\tt + code} \\
\tt ~~~~~~~with precedence 12000 \\
\tt ~~~~~~~with unary postfix reformatter \\
\tt define operator "is object" [code] infix & {\tt + code} \\
\tt ~~~~~~~with precedence 12000 \\
\tt ~~~~~~~with unary postfix reformatter \\
\end{tabular}
\end{center}
...
{\bf Standard `\ttkey{arithmetic operators}'
     \label{STANDARD-ARITHMETIC-OPERATORS-1} Component: Part 1} \\
\multicolumn{2}{c}
{\bf Operator Definition Parser Commands}
\\[1ex]
Parser Command				& \shortstack{
...
\multicolumn{2}{c}
{\bf Standard `\ttkey{arithmetic operators}'
     \label{STANDARD-ARITHMETIC-OPERATORS-2} Component: Part 2} \\
\multicolumn{2}{c}
{\bf Operator Definition Parser Commands}
\\[1ex]
Parser Command				& \shortstack{
...
Parser Command				& \shortstack{
                                          Given\\Qualifiers}
\\\hline
\tt define operator "IOR =" [code] infix line & {\tt + code} \\
\tt ~~~~~~~with precedence 1000 \\
\tt ~~~~~~~with binary reformatter \\
\tt define operator "MASK =" [code] infix line & {\tt + code} \\
\tt ~~~~~~~with precedence 1000 \\
\tt ~~~~~~~with binary reformatter \\
\tt define operator "XOR =" [code] infix line & {\tt + code} \\
\tt ~~~~~~~with precedence 1000 \\
\tt ~~~~~~~with binary reformatter \\
\\[1ex]
\tt define operator "IOR" [code] infix & {\tt + code} \\
\tt ~~~~~~~with precedence 13000 \\
\tt ~~~~~~~with infix reformatter ( "IOR" ) \\
\tt define operator "MASK" [code] infix & {\tt + code} \\
\tt ~~~~~~~with precedence 13000 \\
\tt ~~~~~~~with infix reformatter ( "MASK" ) \\
\tt define operator "XOR" [code] infix & {\tt + code} \\
\tt ~~~~~~~with precedence 13000 \\
\tt ~~~~~~~with infix reformatter ( "XOR" )
\\[1ex]
\tt define operator "COMP" [code] prefix & {\tt + code} \\
\tt ~~~~~~~with precedence 100000 \\
\tt ~~~~~~~with unary reformatter \\
\end{tabular}
\end{center}
...
\TT{(} {\em condition-present} \{ \TT{,} \ttkey{afix-operator} \}\STAR{}
       \TT{,} {\em delimiter} \TT{)} \\
\hspace*{1in} \hfill \OP{if} ~~~~~ \OP{else if} ~~~~~ \OP{else}
...
\hspace*{2em} {\em op} ~ {\em condition}\QMARK{}
		       \{ {affix-operator} {\em operand} \}\STAR{}
		       {\em delimiter} ~ {\em restricted-statement}

or

\hspace*{2em}\begin{tabular}{l}
             {\em op} ~ {\em condition}\QMARK{}
	     \{ {affix-operator} {\em operand} \}\STAR{}
	     {\em delimiter} \\
	     ~~~~~{\em statement}\STAR{}
	     \end{tabular}

The {\em delimiter} is typically \TT{:}.  A {\em restricted-statement}
is an expression with no operators of precedence 0.

The {\em condition} expression must be present if and only if
{\em condition-present} is \TT{min::TRUE()}.  This is the case
for \TT{if} and \TT{else if} but not for \TT{else}.
{\em Affix-operators} given in the reformatter arguments can be omitted
in the expression along with their following operands, but if given
in the expression must appear in the same order as they appear in the
reformatter arguments.


\end{indpar}

\ttmkey{exit}{reformatter}
\hfill \OP{exit}


\begin{indpar}[0.5em]
Typically used with line exit operator
(e.g., \TT{exit}).

The expression must consist of an operator followed by
an \underline{optional} operand.
...
\ttmkey{unary prefix}{reformatter}
...
\ttmkey{unary postfix}{reformatter}
    \hfill \NBOP{is integer} ~~~ \NBOP{is finite} ~~~ \NBOP{is string}
                             ~~~ \NBOP{is object}

\begin{indpar}[0.5em]
Typically used with \TT{postfix} operators.

Checks that the subexpression has exactly two elements, an operand followed
by an operator.  Examples
where \TT{is integer} is a postfix operator with this reformatter:

\hspace*{2em}\begin{tabular}{lcl}
\TT{x is integer} & $\Longrightarrow$ & \TT{x is integer} \\
\TT{is integer} & $\Longrightarrow$ & \TT{ERROR'OPERAND is integer} \\
\end{tabular}
\end{indpar}
...
\subsection{The Primary Parsing Pass}

The \key{primary parsing pass} uses primary separators
to restructure {\em (H+1)-expressions} into a sequence of
primaries and primary separators.
Primary separators are defined by primary separator definitions
that can be added to the primary parsing definition stack.

\subsubsection{Primary Parsing}

Primary parsing occurs after operator parsing and restructures
{\em (H+1)-expressions} output by the operator parsing pass according to:
\begin{indpar}
\emkey{(H+1)-expression} \\
\hspace{3em}\begin{tabular}[t]{rl}
			::= & {\em primary} \\
			$|$ & {\em initial-primary-separator} {\em primary}
			      \{ {\em following-primary-separator}
			         {\em primary} \}\STAR{}
		    \end{tabular}
\\[0.5ex]
\emkey{primary} ::= {\em non-primary-separator}\PLUS{}
\end{indpar}

Here:
\begin{itemize}
\item An {\em (H+1)-expression} that does \underline{not} begin with a
      {\em initial-primary-separator} is a {\em primary}.
\item {\em Primary-separators} must be separated by {\em primaries}
      which cannot be empty.
\item A {\em following-primary-separator} must be enabled by a
preceeding {\em primary-separator}, else it is not recognized.
For example, in the {\em (H+1)-expression}: \\
\hspace*{3em}{\tt the cow with color brown and weight 475 lb} \\
the following primary separator `\TT{with}' is enabled by the preceding
initial primary separator `\TT{the}' and
the following primary separator `\TT{and}' is enabled by the preceding
following primary separator `\TT{with}'.
\end{itemize}

{\em Parser-primary-separator-commands} modify and print
the primary separator parsing definition stack:

\begin{indpar}[0.1in]
\emkey{parser-primary-separator-command}
    \begin{tabular}[t]{@{}rl@{}}
    ::= & {\em parser-primary-separator-definition} \\
    $|$ & {\em parser-primary-separator-print-command} \\
    \end{tabular}
\label{PARSER-PRIMARY-SEPARATOR-COMMAND}
\end{indpar}

\begin{indpar}[0.1in]
\emkey{parser-primary-separator-definition} \\
    \hspace*{0.2in}\begin{tabular}{rl}
    ::= & \TT{define}\ttmindex{define primary separator}{parser command}
	    \begin{tabular}[t]{@{}l@{}}
	    \TT{primary separator} ~
		{\em primary-separator-name} {\em parsing-selectors} \\
	    \{ \TT{following} ~ {\em primary-separator-name}\PLUS{} \}\QMARK{}
	    \end{tabular} \\
    $|$ & \TT{undefine}\ttmlindex{parser undefine}{primary separator}%
                                 {parser command}
	   ~\begin{tabular}[t]{@{}l@{}}
	    \TT{primary separator} ~
		{\em primary-separator-name} {\em parsing-selectors} \\
    ...
    \end{tabular}
\\[1ex]
{\em primary-separator-name} ::= {\em quoted-key} \\
{\em quoted-key} ::= see \pagref{QUOTED-KEY}
\end{indpar}

\begin{indpar}[0.1in]
\emkey{parser-primary-separator-print-command}%
\label{PARSER-PRIMARY-SEPARATOR-PRINT-COMMAND}
\\
    \hspace*{0.2in}
    \begin{tabular}[t]{rl}
    ::= & \ttmkey{print primary separator}{parser command} ~
	  {\em partial-primary-separator-name} \\
    \end{tabular} \\[1ex]
\emkey{partial-primary-separator-name} ::= {\em quoted-key}
\end{indpar}

A primary separator whose definition has no \TT{following} clause
is \mkey{initial}{primary separator}.  
A primary separator whose definition has a \TT{following} clause
is \mkey{following}{primary separator}.

\begin{itemize}
\item An initial separator is only recognized if it is at the beginning
of the {\em (H+1)-expression}.
\item A following separator X is only recognized if it is preceeded by
a {\em primary} that is preceded by primary separator Y and Y is in
the \TT{following} clause list of X.
\item A primary separator is not recognized if it immediately follows another
primary separator in the {\em (H+1)-expression}.
\item A primary separator is not recognized if it would be at the end of
the {\em (H+1)-expression}.
\end{itemize}

The following parser standard component
(see \itemref{PARSER-STANDARD-COMMANDS})
can be used to define standard parsing separators:


\begin{center}
\begin{tabular}{l@{\hspace*{0.2in}}l}
\multicolumn{2}{c}
{\bf Standard `\ttkey{parsing separators}'\label{STANDARD-PARSING-SEPARATORS}
     Component} \\
\multicolumn{2}{c}
{\bf Parsing Separator Definition Parser Commands}
\\[1ex]
Parser Command				& \shortstack{
                                          Given\\Qualifiers}
\\\hline
\tt define pass primary [code] after operator & {\tt + code} \\
\tt define primary separator "a" [code] & {\tt + code} \\
\tt define primary separator "an" [code] & {\tt + code} \\
\tt define primary separator "every" [code] & {\tt + code} \\
\tt define primary separator "the" [code] & {\tt + code} \\
\\[1ex]
\tt define primary separator "of" [code] & {\tt + code} \\
\tt ~~~~~~~following "the" "of the" \\
\tt define primary separator "of the" [code] & {\tt + code} \\
\tt ~~~~~~~following "the" "of the" \\
\tt define primary separator "with" [code] & {\tt + code} \\
\tt ~~~~~~~following "a" "the" "of the" "every" \\
\tt define primary separator "and" [code] & {\tt + code} \\
\tt ~~~~~~~following "with" "and"  \\
\tt define primary separator "in" [code] & {\tt + code} \\
\tt ~~~~~~~following "every"  \\
\end{tabular}
\end{center}
...
is a number or an object (including quoted strings and purelists)
that does not have
\TT{.initiator} and \TT{.terminator} `\TT{(~)}' and
does not have \TT{.terminator} \TT{INDENTED\_PARAGRAPH}.
The argument
...
Both \TT{PRIM::var} and \TT{PRIM::func} C++ object also have a
\TT{block\_level} member (common to all parser lookup tables)
which encodes a \key{lexical level} and a \key{depth}.

\subsubsection{Primary Syntax}

The syntax of the vector elements of a parsed primary
recognized by \TT{PRIM} is:
\begin{indpar}
\emkey{primary} ::= \begin{tabular}[t]{l}
		    {\em variable-primary} \\
		    {\em function-call} \\
		    \end{tabular}
\\[0.5ex]
\emkey{variable-primary} ::= {\em variable-name}
			     {\em []-subexpression}\STAR{}
\\[0.5ex]
\emkey{function-call} \begin{tabular}[t]{rl}
		      ::= & {\em argument-list}\STAR{}
			    {\em call-term}\PLUS{} \\
		      $|$ & {\em argument-list} {\em argument-list}\PLUS{}
		      \end{tabular}
\\[0.5ex]
\emkey{call-term} ::= {\em call-term-name} {\em argument-list}\STAR{}
                   $|$ {\em negator} {\em call-term-name}
\\[0.5ex]
\emkey{variable-name} ::= {\em word} \{ {\em word} $|$ {\em natural} \}\STAR{}
\\[0.5ex]
\emkey{call-term-name}\label{CALL-TERM-NAME} ::=
	       ...
	       \end{tabular}
\\[0.5ex]
\emkey{negator} ::= \TT{no} $|$ \TT{not} $|$ \TT{NO} $|$ \TT{NOT}
\\[0.5ex]
\emkey{argument-list} ::= {\em subexpression} $|$ {\em naked-argument}
\\[0.5ex]
\emkey{subexpression}\label{SUBEXPRESSION}
                      ::= \begin{tabular}[t]{@{}l@{}}
                          a MIN object whose only allowed attributes are: \\
			  ~~~ \TT{.initiator} ~~~ \TT{.terminator} ~~~
			      \TT{.separator}
			  \end{tabular}
\\[0.5ex]
\emkey{[]-subexpression} ::= \begin{tabular}[t]{@{}l@{}}
                          a {\em subexpression} whose
			  \TT{.initiator} and \TT{.terminator} \\
			  are `\TT{[}' and `\TT{]}' respectively
			  \end{tabular}
\\[0.5ex]
\emkey{naked-argument} ::= \\
\hspace*{0.5in}\begin{tabular}[t]{@{}rl@{}@{}}
                           & {\em number} \\
		       $|$ & a MIN object whose
		             \TT{.initiator} and \TT{.terminator}
			     are not `\TT{( )}', \\
			   & and whose \TT{.terminator} is not
			     \TT{INDENTED\_PARAGRAPH} \\
			   & (includes {\em quoted-strings} and purelists)
		       \end{tabular}
...
see \pagref{BOOLEAN-TERM-MATCHING}.
...
{\em Call-term-names} are scanned by \TT{PRIM::scan\_primary}.

\subsubsection{Function Prototypes}

A \TT{PRIM:func} encodes a \key{function prototype}.  The function
defined by a \TT{PRIM::func} is called when its function prototype
matches a primary that is a {\em function-call}.

\TT{PRIM} provides the function \TT{PRIM::scan\_func\_prototype}
that scans a subexpression containing a function prototype and
from that creates a \TT{PRIM::func}.  The syntax of a function prototype
is similar to that of a {\em function-call} with extra detail
for argument lists.

However, we give here the syntax of a {\em function-prototype}
as it is \underline{\bf before} parsing:

\begin{indpar}
\emkey{function-prototype} \begin{tabular}[t]{rl}
		      ::= & {\em prototype-argument-list}\STAR{}
			    {\em prototype-term}\PLUS{} \\
		      $|$ & {\em prototype-argument-list}
		            {\em prototype-argument-list}\PLUS{}
		      \end{tabular}
\\[0.5ex]
\emkey{prototype-term} ::= {\em prototype-term-name}
                           {\em prototype-argument-list}\STAR{}
\\[0.5ex]
\emkey{prototype-term-name} ::= \\
\hspace*{4em}\begin{tabular}[t]{@{}l@{}}
               \{ {\em word} $|$ {\em quoted-mark}
			     $|$ {\em quoted-separator} \} \\
               \{ {\em word} $|$ {\em natural}
	                     $|$ {\em quoted-word}
	                     $|$ {\em quoted-mark}
			     $|$ {\em quoted-separator} \}\STAR{}
	       \end{tabular}
\\[0.5ex]
\emkey{quoted-XXX} :::= \TT{"} {\em XXX} \TT{"}
\\[0.5ex]
\emkey{prototype-argument-list} ::=
	\begin{tabular}[t]{@{}rl@{}}
	    & \TT{(} {\em argument-descriptor}
	             \{ \TT{,} {\em argument-descriptor} \}\STAR{} \TT{)} \\
	$|$ & ditto with \TT{()} brackets
	      replaced by other brackets (e.g. \TT{[]})
	\end{tabular}
\\[0.5ex]
\emkey{argument-descriptor} ::= \begin{tabular}[t]{@{}l}
				{\em variable-name}
...
\\[0.5ex]
\emkey{boolean-prototype-term}\label{BOOLEAN-PROTOTYPE-TERM} ::= \\
\hspace*{4em}{\em prototype-term-name}
             \TT{(} {\em variable-name} \TT{?=} {\em truth-value} {)}
\\[0.5ex]
\emkey{truth-value} ::= \TT{*FALSE*} $|$ \TT{*TRUE*}


\begin{enumerate}
\item Except for the first {\em prototype-term}, the {\em prototype-term-name}
in the {\em prototype-term} must be followed by at least one
{\em prototype-argument-list}.
\item A special empty parenthesized argument list \TT{()} can be used
to separate the first {\em proto\-type-term} from a second
{\em prototype-term} if the first term has no other argument lists.
The \TT{()} is otherwise completely ignored.
\item \TT{?=} must be an operator with a precedence just one notch higher
than the \TT{,} operator.
\item It is assumed that words beginning and ending with \TT{*} are
reserved to the compiler and run-time system.
\item A {\em prototype-argument-list} all of whose {\em argument-descriptors}
have a {\em default-value} is called
\mkey{omittable}{\em prototype-argument-list}.
No {\em prototype-argument-list} that is before the first
{\em prototype-term} is omittable.
\item
If there is an omittable {\em prototype-argument-list} in a {\em prototype-term}
that is immediately followed by either a
non-omittable {\em prototype-argument-list} with the
\underline{same} brackets (e.g., \TT{(~)} or \TT{[~]}) as the omittable
argument list,
or by an omittable {\em prototype-argument-list} with different brackets,
then the omittable argument list is said to \key{shadow}
the following argument list.  Such shadowing is \underline{not}
allowed.
\item
It is assumed that {\em default-values} will be evaluated in the
context of the {\em function-call} that uses them, and \underline{not}
in the context of the {\em function-prototype} that defines them
(unlike C++).
\item
\label{BOOLEAN-TERM-MATCHING}
If a {\em boolean-prototype-term} is matched to a {\em call-term}
of the form `{\em negator} {\em prototype-term-name}'
the {\em call-term} will be taken to be equivalent to
`{\em prototype-term-name} \TT{( *FALSE* )}'.  If matched instead to
a {\em call-term} of the form `{\em prototype-term-name}'
it will be taken to be equivalent to
`{\em prototype-term-name} \TT{( *TRUE* )}'.
In other cases it will be matched using normal rules of matching.
...
\subsubsection{Primary Scanning}

The \TT{PRIM::scan\_primary} scans a parsed primary and returns
a primary symbol table entry, either a \TT{PRIM::var} or a
\TT{PRIM::func}.

The first part of the scan constructs the primary label from
the primary and looks that up in the symbol table to get an
entry.  Only an initial part of the primary is used.  Longer
initial parts are preferred, but if an initial part finds no entry
it is progressively shortened until an entry is found.

In constructing the label from the primary, argument lists,
including naked arguments,
are replaced by the string \TT{"(*)"} which, as it is not
a lexeme, cannot appear elsewhere in the label.  When a
label that ends with an natural fails, another attempt
is made replacing the natural with \TT{"(*)"} in case
the natural is a naked argument.  Quoted strings in the
primary are taken to be naked arguments.

After an entry is found, the \TT{PRIM::scan\_\EOL primary} matches
the entire primary against the entry.  If the match
fails, the lookup is resumed in order to find another entry
(perhaps one with a shorter label, or even one with the
same label).  If the match
succeeds, \TT{PRIM::scan\_primary} returns the entry,
but its caller may then reject the entry and resume the
lookup.

Variable labels are equal to the {\em variable-name} so that
when lookup finds a \TT{PRIM::var} the variable name is
at the beginning of the primary.  Then
\TT{PRIM::scan\_primary} checks that the reminder of the
primary consists of \TT{[]}-bracketed subexpressions
and if so, returns the \TT{PRIM::var} plus a list of
the \TT{[]}-bracketed subexpressions.

When a \TT{PRIM::func} is found, the primary is taken
to be a function call, the \TT{PRIM::func} entry encodes
a function prototype, and call-prototype matching is
used to match the \TT{PRIM:func} to the primary.
See the next section.

\subsubsection{Call/Prototype Matching}

A call consists of {\em argument-list} sequences separated by
{\em call-term-names}.  Each {\em call-term-name} must exactly
match exactly one of the {\em prototype-term-names}, except that
quotes in the prototype names are removed before the match.
The first {\em call-term-name} is part of the call label and
the first {\em prototype-term-name} is part of the \TT{PRIM::func}
label, so matches of these term names is guaranteed by lookup.

More specifically, call/prototype matching is left to right and begins
by matching
the {\em argument-list} sequence at the beginning of the call to
the {\em prototype-argument-list} sequence at the beginning of
the prototype (argument list sequences may be empty).
Then the first {\em call-term-name} is skipped over as it has
already been matched to the first {\em prototype-term-name}
by the \TT{PRIM::func} label lookup.  Then the {\em argument-list}
sequence after the first {\em call-term-name} is matched to
the {\em prototype-argument-list} sequence after the first
{\em prototype-term-name}.

Then matching proceeds by looking up the
reminder of the primary in a small symbol table attached to the
\TT{PRIM::func} that contains all the {\em prototype-term-names}
but the first.  This finds the next {\em call-term-name}
and its corresponding {\em prototype-term-name}.  The
{\em argument-list} sequence after this {\em call-term-name}
is then matched to the {\em prototype-argument-list} sequence
after the {\em prototype-term-name}.  This last process
of finding {\em call-term-names} and their matches repeats until
the end of the primary.

An output of a call/protype match is a list of arguments in the same order
as they appear in the prototype.  Initially all arguments
are marked missing.  The scan then fills in argument
expressions taken from the call.  After scanning, any
argument still missing are filled in with default values.
If after this any arguments are still missing, the
call/prototype match fails.  If during the scan any
non-missing argument is about to be given another value
(can only happen if a {\em call-term-name} is duplicated
in the call), the call/prototype match fails.  Here argument
values are expressions to be evaluated prior to a call to the
\TT{PRIM::func} function.

An {\em argument-list} matches a {\em prototype-argument-list}
list if both have the same \TT{.initializer} and \TT{.terminator}, or if
the {\em argument-list} is a {\em naked-argument} and
the {\em prototype-argument-list} has `\TT{(}' as its
\TT{.initiator} and `\TT{)}' as its \TT{.terminator} and
has at most one argument with no default value.

Matches involving {\em boolean-prototype-terms} require special
handling: see
\pagref{BOOLEAN-TERM-MATCHING}.
