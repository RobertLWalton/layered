    ...
    ~~~~\begin{tabular}[t]{l}
	   ...
	   \end{tabular} \\
        ...
        ~~~~~~~~set \TT{current} \TT{lexical} \TT{lexeme} \TT{type} to the
	        lexeme table type \\
        find the longest atom in beginning at the \TT{input} \TT{position} \\
...
\item change the \TT{current} \TT{lexeme} \TT{type} (the type can be changed
      to \TT{NONE})
\item change the \TT{current} \TT{lexical} \TT{table} - the previous
      table ID can be pushed into the \TT{return} \TT{stack}, or the
...
\emkey{mark-character} :::=
    \begin{tabular}[t]{l}
    {\em graphic-character} that is \underline{not} a {\em letter},
    {\em digit}, \\
    {\em isolated-separator-character},
    {\em repeating-separator-character}, \\
    or {\em quoting-character}
    \end{tabular}
\end{indpar}
...
\emkey{unrecognized-character} ::=
    \begin{tabular}[t]{p{4in}}
    UNICODE character with no UNICODE category
    or with a category not mentioned above
    \end{tabular}
\end{indpar}

Sequences of illegal characters can be used like {\em horizontal-space}
to separate lexemes, and can be included in {\em comment} and
{\em quoted-string} lexemes, but in all cases will generate warning
messages.  The three kinds of illegal sequences are:
\begin{indpar}
\emkey{misplaced-vertical} ::= {\em misplaced-vertical-character}\PLUS{}
\\[0.3ex]
