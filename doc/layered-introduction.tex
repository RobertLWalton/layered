...
recognizes subexpressions that are bounded by `named prefix separators' such as
...
to a single expression token.  Here indentation may or may not
be used to continue lines and paragraphs.
...
\hspace*{-0.5in}named prefix separators (e.g.~`\TT{\{p\}}')
\end{indpar}
...
symbol table keys.  However named brackets and named prefix separators are
not.  Instead, a named bracket or named prefix separator is formed by using
named openings (e.g.~`\TT{\{}'),
named property introducers (e.g.~`\TT{:}'),
named property equators (e.g.~`\TT{=}'),
named property separators (e.g.~`\TT{,}'),
...
For unnamed brackets that only
attributes are the syntax attributes \TT{.initiator} and \TT{.terminator}
plus a \TT{.type} attribute derived from these.  For named brackets
the only syntax attribute is the \TT{.type} attribute, but arbitrary
non-syntax attributes may be encoded in the named brackets.
...
\hspace*{-0.2in}\ttdmkey{initiator}{subexpression attribute}\\
    ...
    or & the {\em typed-opening} of a named bracketed subexpression, \\
    or & the {\em typed-opening} of a named operator, \\
...
\hspace*{-0.2in}\ttdmkey{terminator}{subexpression attribute}\\
    ...
    or & the {\em line-separator-name} used to separate lines of an
         indented paragraph, \\
    or & the {\em typed-closing} of a named bracketed subexpression, \\
    or & the {\em typed-closing} of a named operator. \\
    \end{tabular}
    \end{center}
\medskip
...
\hspace*{-0.2in}\ttdmkey{middle}{subexpression attribute}\\
    A MIN string, or a MIN label whose elements are MIN strings,
    that equals the {\em typed-middle} of a named bracketed subexpression.
\\[2ex]
...
\hspace*{-0.2in}\ttdmkey{arguments}{subexpression attribute}\\
    ...
    {\em opening-typed-bracket} of a named bracketed subexpression,
...
\hspace*{3em}\begin{tabular}[t]{@{}rl}
	...
	$|$ & {\em typed-opening} {\em type-label}\QMARK{}
	      ...
	      {\em typed-closing} \\
	$|$ & {\em typed-opening} {\em elements}\QMARK{} {\em typed-closing} \\
	$|$ & {\em typed-opening} {\em mark-type}
	      {\em element}\STAR{}
	      {\em mark-type} {\em typed-closing} \\
	\end{tabular}
\\[0.5ex]
\emkey{elements}
	\begin{tabular}[t]{@{}rl}
	::= & {\em typed-middle} {\em element}\STAR{} {\em typed-middle} \\
	$|$ & {\em doubled-typed-middle}
	\end{tabular}
\\[0.5ex]
\emkey{element} \begin{tabular}[t]{@{}rl}
                ::= & {\em word} $|$ {\em natural} $|$ {\em numeric} $|$
		      {\em mark} $|$ {\em separator} $|$ {\em quoted-string} \\
                $|$ & {\em bracketed-subexpression} \\
		    & but not ({\em doubled-}){\em typed-middle}
		      if not {\em mark-type} expression \\
		    & or `{\em mark-type} {\em typed-closing}'
		      if {\em mark-type} expression \\
		\end{tabular}
\\[0.5ex]
...
\emkey{attribute} \begin{tabular}[t]{@{}rl}
                  ::= & {\em attribute-label}
	                {\em typed-attribute-equal} {\em attribute-value} \\
                  ::= & {\em attribute-label} \\
                  ::= & {\em typed-attribute-negative} {\em attribute-label} \\
		  \end{tabular}
\\[0.5ex]
\emkey{typed-opening} ::= as defined by a
    {\em parser-typed-bracket-command} (\pagref{PARSER-TYPED-BRACKET-COMMAND})
    ~~~~~ [e.g., `\TT{\{}']
\\[0.5ex]
\emkey{typed-middle} ::= ditto
    ~~~~~ [e.g., `\TT{|}']
\\[0.5ex]
\emkey{doubled-typed-middle} :::= {\em typed-middle} {\em typed-middle}
    as a single lexeme
    ~~~~~ [e.g., `\TT{||}']
\\[0.5ex]
...
\\[0.5ex]
\emkey{label-component} ::= {\em word} $|$ {\em natural} $|$
                            {\em numeric} $|$ {\em quoted-string}
\\[0.5ex]
\emkey{type-label} ::= {\em label} not containing
    {\em typed-attribute-begin} or ({\em doubled-}){\em typed-middle}
\\[0.5ex]
\emkey{attribute-label} ::= \begin{tabular}[t]{l}
    {\em label} not containing
    {\em typed-attribute-equal} or
    {\em typed-attribute-separator} \\
    or ({\em doubled-}){\em typed-middle}
    \end{tabular}
\end{indpar}

If the {\em typed-opening}, {\em typed-key-separator}, {\em typed-middle},
and {\em typed-closing} are respectively `\TT{<}', `\TT{\#}',
`\TT{|}', and `\TT{>}', then an example named bracketed subexpression is
\begin{center}
\TT{<h1(2) \# Diamond Lemma|} ~~~\ldots~~~ \TT{|h>}
\end{center}

Here the {\em typed-opening-bracket} contains the {\em bracket-name}
`\TT{h1}' (note `\TT{h}' and `\TT{1}' are separate lexemes),
the {\em argument} `\TT{(2)}', and the {\em key} `\TT{Diamond Lemma}',
while the {\em typed-closing-bracket} contains just an initial
segment of the {\em bracket-name}.   The initial name segment
in the {\em typed-closing-bracket} may be empty, so
\TT{|>}, \TT{|h>}, or \TT{|h1>} could all be used as
{\em typed-closing-brackets} in the above example.

The {\em typed-opening}, {\em typed-key-separator}, {\em typed-middle},
and {\em typed-closing} are symbol table keys
...
\TT{parser define named bracket
     "<"~...~"\#"~...~"|"~...~"|"~...~">" [text]} \ldots
\end{center}
which specifies `\TT{<}' to be a {\em typed-opening},
`\TT{\#}' to be a {\em typed-key-separator},
...
symbol table keys,
{\em typed-openings},
{\em typed-key-separators},
{\em typed-middles},
and {\em typed-closings}
may not span multiple lines, but
{\em typed-opening-brackets},
{\em typed-closing-brackets},
and {\em typed-operators} (as per \itemref{TYPED-OPERATORS})
...
\emkey{typed-bracket-name} \\
    \hspace*{0.2in}
     ::= \begin{tabular}[t]{@{}l@{}}
	 {\em typed-opening} [ \TT{...} {\em typed-key-separator} ] \\
...
\\[0.5ex]
\emkey{typed-key-separator} ::= {\em quoted-name}
\\[0.5ex]
...
MIN object whose list elements are the components of the
subexpression except for the brackets.  The
{\em typed-opening} becomes the \TT{.initia\-tor}
attribute of this object, the {\em typed-middle} becomes
the \TT{.middle} attribute of this object,
the {\em typed-closing} becomes the \TT{.terminator} attribute
of the object, the {\em bracket-name} (from the {\em typed-opening-bracket})
becomes the
...
\TT{.separator} attributes are both the {\em typed-key-separator}.
...
If the {\em typed-middle} ends in a mark and the {\em typed-closing}
begins with a mark, the {\em typed-middle} may be glued to the
{\em typed-closing} to make a
{\em typed-closing-bracket} with empty {\em bracket-name-initial-segment}.
...
its {\em typed-closing-bracket} is discovered, a parsing error is
announced and the {\em typed-closing-bracket} is inserted in the input
just after the pre-maturely ended subexpression.  Such a pre-mature end can be
discovered by finding the {\em closing-bracket-name} of a containing
unnamed bracketed subexpression, the {\em typed-closing-bracket}
of a containing named bracketed subexpression, the end of the paragraph line
containing the named bracketed subexpression,
or an end of file.

\subsubsection{Named Operators}
\label{TYPED-OPERATORS}

A \key{named operator} has the same syntax as an {\em opening-typed-bracket}
except that the {\em typed-operator} ends with a
{\em typed-closing} instead of a {\em typed-middle}:

\begin{indpar}[0.1in]
\emkey{typed-operator} ::=
    \begin{tabular}[t]{@{}l@{~}l@{}}
    {\em typed-opening} & {\em operator-name} {\em argument}\STAR{} \\
	                & \{ {\em typed-key-separator} {\em key} \}\STAR{} \\
	                & {\em typed-closing} \\
	 \end{tabular}
\\[0.5ex]
...
A {\em typed-operator} is converted to a MIN object which has the
...
the \TT{.initiator} is the {\em typed-opening},
the \TT{.terminator} is the {\em typed-closing},
and the \TT{.arguments} and \TT{.keys} are set from the
contents of the {\em typed-operator} just as they would be from the
contents of a {\em typed-opening-bracket}.
