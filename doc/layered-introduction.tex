...
by a definition name, and that is followed by {\em parsing-selectors}.
...
a 64 bit unsigned integer with one bit per {\em selector}.}
...
\begin{indpar}[0.5in]
brackets (e.g.~`{\tt (}' and `{\tt )}')
    and bracketed subexpressions \\
indentation marks (e.g.~`{\tt :}'),
    \begin{tabular}[t]{@{}p{3in}@{}}
    indented paragraphs, and indented lines within a paragraph
    \end{tabular} \\
...
parsing definition.  The subexpression is converted to a
...
\index{indented paragraph subexpression}
...
paragraph lines, as long as it is outside (unnamed bracketed,
named bracketed, or indented subparagraph) subexpressions in the paragraph.
...
As a technical note, \underline{multi-lexeme}
unnamed brackets, named openings,
named middles, named closings, indentation marks, and indentation separators
may not cross line boundaries, and in
addition if they are at the end of a line may not be glued to
a line ending gluing indentation mark name.
