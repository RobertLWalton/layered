...
The `\ttkey{block}'\label{STANDARD-BLOCK} standard component executes
the parser command:
\begin{center}
...
If this is used, standard parser definitions can be
...
Whenever one of the standard qualifiers `\TT{code}', `\TT{text}',
or `\TT{math}' causes the corresponding selector to be used in a definition,
the selector
is created by the appropriate one of the parser commands:

\begin{center}
\begin{tabular}{l@{\hspace*{0.5in}}l}\label{STANDARD-SELECTORS}
\tt define selector code & If `\TT{code}' selector used. \\
\tt define selector text & If `\TT{text}' selector used. \\
\tt define selector math & If `\TT{math}' selector used. \\
\end{tabular}
\end{center}
...
by \ttkey{define top level} commands and printed by the \ttkey{print top level}
command.
...
data structures that can be printed by:
...
The `\ttkey{top level}'\label{STANDARD-TOP-LEVEL-1} standard component executes
the parser commands:
\begin{center}
\begin{tabular}{l@{\hspace*{0.5in}}l}
...
When a parser is initialized, the ID character is disabled.
The `\ttkey{top level}'\label{STANDARD-TOP-LEVEL-2} standard component executes
the parser command:
\begin{center}
\begin{tabular}{l@{\hspace*{0.5in}}l}
Parser Command				& Given Qualifiers
...
\TT{define top level} (\pagref{DEFINE-TOP-LEVEL}) (these are
...
\ttmkey{data}{reformatter}
        \TT{(} \begin{tabular}[t]{@{}l@{}}
	       {\em new-sign}\TT{,}
	       {\em new-or-same-sign}\TT{,}
	       {\em add-to-set-sign}\TT{,}
	       {\em add-to-multiset-sign}\TT{,} \\
...
\begin{center}
\TT{\{}{\em prefix}\TT{\}} {\em ID}
            \begin{tabular}[t]{l}
	    {\em assignment-sign} ~ {\em element-list} \TT{::::}\QMARK{} \\
	    \{ {\em attribute-paragraph}
	       \{ {\em assignment-sign} {\em attribute-paragraph} \}\STAR{}
	       \}\QMARK{}
	    \end{tabular}
\end{center}
where
...
\\[0.5ex]
\emkey{assignment-sign} \begin{tabular}[t]{rl}
			::= & {\em new-sign} \\
                        $|$ & {\em new-or-same-sign} \\
                        $|$ & {\em add-to-set-sign} \\
                        $|$ & {\em add-to-multiset-sign} \\
			\end{tabular}
\\[0.5ex]
\emkey{element-list} ::= see \pagref{ELEMENT-LIST}
\\[0.5ex]
...
a preallocated stub or an object (that presumed to have replaced
a preallocated stub and be identifier by an ID).
...
is invoked,
the {\em attribute-paragraph} are parsed
to become a MIN object
with \TT{min::\EOL INDENTED\_\EOL PARAGRAPH()} \TT{.terminator},
`\TT{:}' \TT{.initiator}, and logical
...
Each multiset of {\em single-attribute-values}
given for a particular {\em attribute-label} in an {\em attri\-bute-paragraph}
modifies the corresponding multiset
of {\em attribute-values} with the same {\em attribute-label} in
the object identified by the {\em ID}.
Similarly for each given multiset of {\em double-attribute-values}
and each given set of {\em attribute-flags}.

Each {\em attribute-paragraph} is preceded
by an {\em attribute-assignment} which specifies how the
given values or flags modified the corresponding values or flags,
as follows:


\begin{indpar}

\hspace*{-0.2in}\emmkey{new-sign}{in {\tt data} reformatter definition}\\
The corresponding multisets of values or sets of flags
\underline{must not} exist, and are set to the given multisets of values
or sets of flags.

\hspace*{-0.2in}\emmkey{new-or-same-sign}%
    {in {\tt data} reformatter definition}\\
The corresponding multisets of values or sets of flags
may exist only if they are equal to the given multisets or sets.
When a corresponding multiset or set does not exist,
it is set to the given multiset or set.

\hspace*{-0.2in}\emmkey{add-to-set-sign}%
    {in {\tt data} reformatter definition}\\
Each given value is added to its corresponding multiset of values if it is
not already in that multiset, and each flag is added if it is not already
in its corresponding set of flags.

\hspace*{-0.2in}\emmkey{add-to-multiset-sign}%
    {in {\tt data} reformatter definition}\\
Each given value is added to its corresponding multiset of values,
and each flag is added if it is not in its corresponding set of flags.

\end{indpar}

A simple example where the {\em new-sign} is `{\tt =}' and
the {\em add-to-set-sign} is `{\tt +=}' is:
\begin{indpar}[0.2in]\begin{verbatim}
@1 =:
    name = Jack
    wife = @2 = husband
...
and gives it the `{\tt husband = @1 = wife}' value so the line
`{\tt husband = @1 = wife}' is redundant and can either be omitted or
used with a {\tt +=} {\em add-to-set-sign}.
...
The following parser commands can be used to define these standard operators.
Each of these commands is equivalent to the given set of define commands.

\TT{define standard operators} \hfill All Standard Operator Classes
\begin{indpar}\begin{verbatim}
define standard control operators
define standard assignment operators
define standard logical operators
define standard comparison operators
define standard arithmetic operators
define standard bitwise operators
\end{verbatim}\end{indpar}

\TT{define standard control operators} \hfill Standard Operators of Class C
\begin{indpar}\begin{verbatim}
define operator "if" [code] prefix line
       with precedence 0
       with control reformatter ( ":", has condition )
define operator "else if" [code] prefix line
       with precedence 0
       with control reformatter ( ":", has condition )
define operator "while" [code] prefix line
       with precedence 0
       with control reformatter ( ":", has condition )
define operator "until" [code] prefix line
       with precedence 0
       with control reformatter ( ":", has condition )
define operator "else" [code] prefix line
       with precedence 0
       with control reformatter ( ":" )

define operator ":" [code] afix line
       with precedence 0
...
\TT{define standard assignment operators} \hfill Standard Operators of Class S
\begin{indpar}\begin{verbatim}
define operator "=" [code] infix line
       with precedence 1000
       with binary reformatter
define operator "=" [math] infix
       with precedence 1000
       with binary reformatter
...
\TT{define standard logical operators} \hfill Standard Operators of Class L
\begin{indpar}\begin{verbatim}
...
\TT{define standard comparison operators} \hfill Standard Operators of Class P
\begin{indpar}\begin{verbatim}
...
\TT{define standard arithmetic operators} \hfill Standard Operators of Class A
\begin{indpar}\begin{verbatim}
define operator "+=" [code] infix line
       with precedence 1000
       with binary reformatter
...
\TT{define standard bitwise operators} \hfill Standard Operators of Class B
\begin{indpar}\begin{verbatim}
define operator "|=" [code] infix line
       with precedence 1000
       with binary reformatter
...
\\\hline
0000    & code	& \TT{control}	& \TT{prefix} & \ttkey{if} & conditional & C \\
	&	&		& \TT{line}  & \ttkey{else if} & conditional
								       & C \\
	&	&		&	     & \ttkey{while} & loop    & C \\
	&	&		&	     & \ttkey{until} & loop    & C
\\\cline{3-7}
	&	&		& \TT{initial}
					     & \ttkey{else} & conditional & C \\
	&	&		& \TT{line}  &            &            &
\\\cline{3-7}
	&	& none		& \TT{afix } & \ttkey{:}  &	       & C \\
	&	&		& \TT{line}  &            &            &
\\\cline{4-7}
	&	& 		& \TT{postfix } & \ttkey{:} indentation & & C \\
	&	&		& \TT{afix}  & ~~~mark     &           & \\
	&	&		& \TT{line}  &            &            &
\\\cline{1-1}\cline{3-7}
...
\begin{tabular}{ll}
class A & {\tt define standard \ttkey{arithmetic operators}} \\
class B & {\tt define standard \ttkey{bitwise operators}} \\
class C & {\tt define standard \ttkey{control operators}} \\
class S & {\tt define standard \ttkey{assignment operators}} \\
\end{tabular}
...
\\\cline{1-1}\cline{3-7}
3000    & math	& \ttkey{binary}
				& \TT{infix} & \ttkey{BUT NOT}
							  & logical and not & L
\\\cline{1-1}\cline{3-7}
3100	&       & \ttkey{infix}	& \TT{infix} & \ttkey{AND} & logical and & L\\
	& 	&		&	     & \ttkey{OR}  & logical or  & L
\\\cline{1-1}\cline{3-7}
3200    &	& \ttkey{unary}	& \TT{prefix} & \ttkey{NOT} & logical not & L
\\\cline{1-1}\cline{3-7}
4000 	&	& \ttkey{infix and}
				& \TT{infix} & \ttkey{==} & equal      & P \\
	& 	&		&	     & \ttkey{!=} & not equal  & P \\
	& 	&	      	&	     & \ttkey{<}  & less than  & P \\
	...
	& 	&		&	     & \TT{>=}	& greater than or equal
	                                                               & P
\\\cline{1-1}\cline{3-7}
5000	&	& \ttkey{sum}	& \TT{infix} & \TT{+}	& addition     & A \\
	&	&		& 	     & \TT{-}	& subtraction  & A
\\\cline{2-7}
	& code	& \ttkey{infix}	& \TT{infix} & \TT{|}	& binary or    & B \\
	&	&		& 	     & \TT{\&}	& binary and   & B \\
	&	&		& 	     & \TT{\textasciitilde}
						        & binary exclusive or
							               & B
\\\hline
5100	& code	& \ttkey{binary} & \TT{infix} & \TT{/}	& division     & A
\\\cline{1-1}\cline{3-7}
5200	& math	& \ttkey{infix}	& \TT{infix} & \TT{*}   & multiplication & A
\\\cline{1-1}\cline{3-7}
5300	&	& \ttkey{binary} & \TT{infix} & \TT{\textasciicircum}
							& exponentiation & A
\\\cline{1-1}\cline{3-7}
H-1	&	& \ttkey{unary}	& \TT{prefix} & \TT{+}	& no-op        & A \\
	&	&		& 	      & \TT{-}	& negation     & A 
\\\cline{2-7}
	& code 	& \ttkey{unary}	& \TT{prefix} & \TT{\textasciitilde}
	                                                & binary complement
							               & B
\\\hline
\end{tabular}
...
class A & {\tt define standard \ttkey{arithmetic operators}} \\
class B & {\tt define standard \ttkey{bitwise operators}} \\
class L & {\tt define standard \ttkey{logical operators}} \\
class P & {\tt define standard \ttkey{comparison operators}} \\
class S & {\tt define standard \ttkey{assignment operators}} \\
\end{tabular}
...
tokens between it and the preceeding operator
...
recognized.  For example, no operator are recognized in:
\begin{center}
\begin{tabular}{l@{~~~~~~~~~~}l}
...
\begin{indpar}[0.5em]
Typcally used with a \TT{nofix} operator (e.g., \TT{<-{}-}) with
\TT{final} \TT{afix} operators following (e.g., \TT{\{\$~\$\}} or \TT{:}).
...
\begin{indpar}[0.5em]
Tyically used with \TT{prefix} operators.
...
where \TT{NOT} is a prefix operator:
