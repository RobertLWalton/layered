...
be disabled, by the parser command:
\begin{indpar}[0.1in]
\emlkey{parser-middle-break}{-command}%
\label{PARSER-MIDDLE-BREAKS-COMMAND} ::= \\
\hspace*{3em} \ttmkey{define middle break}{parser command}
...
The quoted string concatenator can be changed by the parser command:
\begin{indpar}[0.1in]
\emlkey{parser-quoted-string}{-concatenator-command}%
\label{PARSER-QUOTED-STRING-CONCATENATOR-COMMAND} ::= \\
\hspace*{3em} \ttmlkey{define quoted string}{concatenator}{parser command}
...
Also broken parts of middle lexemes (\pagref{MIDDLE-LEXEME-MERGE})
other than the first are not recognizable as part of keys.
So bracketed symbol table (see above) entries should avoid multi-character
lexemes that begin with
{\em middle-break-begin} or end with {\em middle-break-end}.
...
the token \TT{value} for numerics and the token \TT{string} for
quoted strings.
