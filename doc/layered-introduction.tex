...
\\[1ex]  
\begin{tabular}{p{1in}p{4.5in}}
		& Standardly word, mark, and separator lexeme type
...
\begin{tabular}{p{1in}p{4.5in}}
		...
		& Standardly, the lexeme characters of a
		  natural or numeric lexeme are converted to
		  a MIN number token \TT{value} using the POSIX {\tt strtod}
		  function, if that conversion is possible.
		  If the conversion is possible, and
		  the translation string contains nothing but
		  ASCII digits, and conversion by the POSIX
		  {\tt strtoll} function is possible and gives equal results
		  (i.e., the number fits into an IEEE double
		  without losing any significant digits), the token
		  is given the \TT{natural} lexeme type;
		  otherwise if conversion to a MIN number is possible
		  the token is given the \TT{NUMBER} type;
		  lastly if conversion to a MIN number is \underline{not}
		  possible the token is given the \TT{numeric}
		  lexeme type and a \TT{value} that is the
		  MIN string equal to the lexeme characters.
\end{tabular}
\\[1ex]  
\begin{tabular}{p{1in}p{4.5in}}
		& Note that {\tt strtod} cannot convert values too
		  large to represent in a \underline{finite}
		  MIN number (which is
		  an IEEE 64-bit number), nor can it
		  convert non-zero numbers too small to represent
		  in a MIN number.  For example, it cannot convert
		  {\tt 1e+500} or {\tt 1e-500}.
\end{tabular}
\\[1ex]  
\begin{tabular}{p{1in}p{4.5in}}
...
The `\TT{@}' becomes the first character of the next lexeme.
...
The `\TT{!}' is discarded and the `\TT{@}' becomes the
first character of the next lexeme.
...
\\\hline
\\[0.5ex]
\TT{PARAGRAPH-CHECK}	& \TT{@}		& \TT{DATA}	& none \\
			& \TT{!@}		& \TT{RAW-DATA} & \TT{!} \\
			& \TT{-{}-{}-{}-{}-}	& \TT{TABLE}	& none \\
			& \TT{=====}		& \TT{TABLE}	& none
\\[2ex]
\TT{DATA-CHECK}		& \TT{@}		& \TT{DATA}	& none \\
			& \TT{!@}		& \TT{RAW-DATA} & \TT{!}
\\[2ex]
...
~dence  & ~tors & reformatter	& flags	     & operator   & meaning    & class
\\\hline
4000    & code 	& \ttkey{binary}
				& \TT{infix} & \ttkey{BUT NOT}
							  & logical and not & L
\\\cline{1-1}\cline{3-6}
4100	& math  & \ttkey{infix}	& \TT{infix} & \ttkey{AND} & logical and &
\\\cline{5-6}
	& 	&		&	     & \ttkey{OR}  & logical or  &
\\\cline{1-1}\cline{3-6}
...
\\\cline{5-6}
	& 	&		&	     & \TT{>=}	& greater than or equal
	                                                               &
\\\cline{1-1}\cline{3-7}
6000	&       & \ttkey{infix}	& \TT{infix} & \TT{+}	& addition     & A
\\\cline{5-6}
	&	&		& 	     & \TT{-}	& subtraction  &
\\\cline{2-3}\cline{5-7}
...
\\\cline{5-6}
        &       &                &            & \TT{>{}>} & right shift &
\\\hline
6100	& code	& \ttkey{binary} & \TT{infix} & \TT{/}	& division     & A
\\\cline{1-1}\cline{3-6}
6200	& math	& \ttkey{infix}	& \TT{infix} & \TT{*}   & multiplication &
\\\cline{1-1}\cline{3-6}
...
\\[1ex]
\tt define operator "+" [code, math] infix & {\tt + code} or {\tt + math} \\
\tt ~~~~~~~with precedence 6000 \\
\tt ~~~~~~~with infix reformatter ( "+", "-" ) \\
\tt define operator "-" [code, math] infix & {\tt + code} or {\tt + math} \\
\tt ~~~~~~~with precedence 6000 \\
\tt ~~~~~~~with infix reformatter ( "+", "-" ) \\
\tt define operator "/" [code, math] infix & {\tt + code} or {\tt + math} \\
\tt ~~~~~~~with precedence 6100 \\
\tt ~~~~~~~with binary reformatter \\
\tt define operator "*" [code, math] infix & {\tt + code} or {\tt + math} \\
\tt ~~~~~~~with precedence 6200 \\
\tt ~~~~~~~with infix reformatter ( "*" ) \\
\tt define operator "**" [code, math] infix
                                           & {\tt + code} or {\tt + math} \\
\tt ~~~~~~~with precedence 6300 \\
\tt ~~~~~~~with binary reformatter
\\[1ex]
...
the expression, inserts parentheses into the expression from
right to left so that all operators
are binary and the \underline{rightmost}
operator is innermost (executed first).
...
\begin{indpar}[0.5em]
Identical to the \TT{infix} reformatter, except after checking
the expression, inserts parentheses into the expression from left
to right so that all operators
are binary and the \underline{leftmost}
operator is innermost (executed first).
