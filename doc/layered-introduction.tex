...
\item
Otherwise (if the `\TT{reset}' flag is off),
let $P$ denote the implied subprefix of the prefix definition
of the explicit line header ($P$ may not exist).  Then
...
\item
The \TT{paragraph implied header} is reset to the following:

\begin{enumerate}[label*=.\arabic*]
\item
To a copy of $P$, if $P$ itself has a prefix definition selected by the
modified current selectors that has the `\TT{paragraph}' group.

\item Otherwise to a copy of the explicit paragraph header,
if the current options now has the `\TT{sticky}' flag.

\item Otherwise to the value it had at the beginning of the current
indented paragraph,
or to missing if the logical line is top level.
