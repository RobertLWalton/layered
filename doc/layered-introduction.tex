...
In this case, the copied prefix must not attempt to change
the parsing selectors.

For prefixes that are \TT{.types} of implied paragraph 
header lexemes, this value may be used
to set the \TT{line implied header} line variable:
...
For prefixes with the `\TT{line}' group, this is
used to set the \TT{options} line variable.
See \pagref{LINE-VARIABLES}.
...
after a paragraph header (see \itemref{HEADED-PARAGRAPHS}).

A `\key{headed line}' is a logical line that begins with line header,
or is the part of a logical line after a line beginning
`paragraph header' (see~\itemref{HEADED-PARAGRAPHS}).
The parser's output
...
\TT{implied line header} variable to either have
a missing value or to have a line header value.  If the value of
this variable is not missing and a logical line with no line header
is encountered,
...
appear at the beginning of a logical line that is either the
first logical line of an indented paragraph, or that follows
a blank line in the indented paragraph, with intervening
...
A headed paragraph may ended by any blank line, or only by blank lines
that are immediately followed by another (non-implicit) paragraph header.
A headed paragraph is always ended by the end of its
containing indented paragraph.
...
(so there is no blank logical line).  The logical lines that
are headed paragraph elements may or may not themselves have
line headers.
...
\hspace*{-0.2in}\ttmkey{implied paragraph header}{line variable}\\
    May be missing or set equal to a paragraph header.

    If this value is not missing and no explicit paragraph header
    is found at the beginning of a logical line in paragraph beginning
    position, a copy of this implied paragraph header variable value
    is inserted at the beginning of the logical line.
\\[1ex]
\hspace*{-0.2in}\ttmkey{implied line header}{line variable}\\
    May be missing or set equal to a line header.
    ...
    However, no line header copy is inserted after a paragraph header
    that is the only element in its logical line.

    If a copy of the \TT{implied paragraph header} is to be inserted
    at the beginning of a logical line, this is done before any
    copy of the \TT{implied line header}.
\\[2ex]
\hspace*{-0.2in}\ttmkey{paragraph selectors}{line variable}\\
    The parsing selectors are reset to this value just after the
    indent token that begins a logical line in paragraph beginning
    position.
\\[1ex]
\hspace*{-0.2in}\ttmkey{line selectors}{line variable}\\
    The parsing selectors are reset to this value just after the
    indent token that begins a logical line that is \underline{not}
