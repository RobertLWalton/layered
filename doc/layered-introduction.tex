...
MIN non-objects, such as labels or special values.

For {\em untyped-bracketed-subexpressions}
the brackets become the `\skey{syntax attribute}s'
\TT{.ini\-ti\-ator} (e.g. `\TT{(}') and \TT{.terminator} (e.g., `\TT{)}'),
an additional syntax attribute \TT{.separator} (e.g., `\TT{,}')
may be added, and the `\key{non-syntax attribute}' is \TT{.position}.
...
\TT{.term\-i\-na\-tor}.  There is no \TT{.separator}.  The elements
are logical lines which are also similar with \TT{.initiator}
being the special value `\ttsvkey{LOGICAL\_LINE}' and \TT{.terminator}
being the line separator (e.g., `\TT{;}') if there is one
or \TT{"<LF>"} otherwise.

For typed brackets the attributes are
the syntax attributes \TT{.type} and \TT{.separator},
the non-syntax attribute is \TT{.position},
and in addition
arbitrary non-syntax attributes may be encoded within the typed brackets.
