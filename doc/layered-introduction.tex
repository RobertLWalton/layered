...
`\TT{TOP LEVEL}', `\TT{LINE LEVEL}', and `\TT{code}'.
...
The \TT{TOP LEVEL}\label{TOP-LEVEL-SELECTOR}
and \TT{LINE LEVEL} selectors
are always set implicitly by this command, and are
the only default selectors if no `\TT{define top level}' commands
are given.  So these selectors are always included in the
...
and the \TT{LINE LEVEL} selector is always cleared when
...
\TT{TOP LEVEL} and \TT{LINE LEVEL} selectors (\pagref{TOP-LEVEL-SELECTOR})
are implicitly set by this command, and as the parsing
