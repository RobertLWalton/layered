...
\\[1ex]  
\begin{tabular}{p{1in}p{4.5in}}
		& \TT{BRACKETED}, \TT{BRACKETABLE}, and \TT{DERIVED}
		  tokens are called
...
type \TT{BRACKETED}, \TT{BRACKETABLE}, or \TT{DERIVED})
...
\ttmkey{separator}{reformatter} \{ \TT{(} {\em separator} \TT{)} \}\QMARK{}
\hfill \OP{;} ~~~ \OP{,}
...
operands is packaged as a BRACKABLE MIN object with
a \ttmkey{.sep\-a\-ra\-tor}{produced by parsing}
attribute value equal to the {\em separator} argument to the reformatter,
if that exists, or the label of the operator otherwise.
Examples
with `\TT{,}' as both the operator and optional {\em separator} argument to the
reformatter are:
...
\ttmkey{declare}{reformatter} \{ \TT{(} {\em type} \TT{)} \}\QMARK{}
\hfill \NBOP{<-{}-} ~~~ \OP{:}
...
The result is a BRACKETED token whose value is a MIN object whose
\TT{.type} is the reformatter argument if that is given, or the
label of the final first operator otherwise.

For example, if \TT{<-{}-} is a nofix operator with this reformatter and
\TT{"<-{}-"} as an optional reformatter argument, and if
\TT{:} indented subexpressions
...
Note that because indented paragraphs are afix operators
