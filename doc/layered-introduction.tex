...
\\
    ...
    ::= & \TT{parser}%
	  ...
	  {}[ \TT{with implied subprefix} {\em implied-subprefix} ] \\
...
\\[0.5ex]
\emkey{implied-subprefix} ::= {\em typed-prefix-separator}
...
\hspace*{-0.2in}\ttkey{prefix-group}\label{PREFIX-GROUP}\\
...
\\[2ex]
\hspace*{-0.2in}\ttkey{implied-prefix}\\
...
{\em implied-prefix}.
\\[2ex]
\hspace*{-0.2in}\ttkey{lexical-master}\\
    TBD
\\[2ex]
\hspace*{-0.2in}\ttkey{at-end-options}\\
    TBD
...
contexts.  To be specific about the latter, the scanner at top level must
produce header lexemes labeled `\TT{table}' and `\TT{line}' upon
seeing `\TT{====}' at the beginning of logical line following
a blank line, and then for the remaining logical lines within this
`\TT{table}' unindented paragraph, produce a header lexeme labeled
`\TT{row}' for lines beginning with `\TT{|}' and a header lexeme
labeled `\TT{line}' for lines beginning with `\TT{-}' or `\TT{=}'.
...
At the beginning of each logical line, the lexical scanner master table
index is reset to the value of a context variable as described below.
The lexical scanner may then produce two special types of lexeme at
