...
middle lexemes if they are consecutive within a logical line,
the first ends with
a \key{middle break begin} (e.g., `\TT{\#}'), and the second begins with
a \key{middle break end} (e.g., `\TT{\#}').%
\label{MIDDLE-LEXEME-CONCATENATION}
...
The middle breaks can be changed by the parser command:
\begin{indpar}[0.1in]
\emlkey{parser-middle-breaks}{-command}%
\label{PARSER-MIDDLE-BREAKS-COMMAND} ::= \\
\hspace*{3em} \ttmkey{define middle breaks}{parser command}
...
\emkey{middle-break-name} ::= \TT{"}{\em middle-break-begin}\TT{"}
		     ...
		     $|$ \TT{disabled}
\\[0.5ex]
\emkey{middle-break-begin} :::= {\em mark}
\\[0.5ex]
\emkey{middle-break-end} :::= {\em mark}
\\[0.5ex]
...
The end of a parser block erases the effects of any
{\em define-middle-breaks-commands} executed during the block.
