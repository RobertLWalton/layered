...
Tokens of `{\tt EXPRESSION}' kind are called
\skey{subexpression token}s, or just plain `subexpressions'.
The parser begins with an explicit subexpression pass that uses
indentation and indentation marks such as `{\tt :}',
brackets such as `{\tt (}' and `{\tt )}', and
named brackets such as `{\tt <b|}' and `{\tt |b>}', to identify
subexpressions.

\subsection{Parser Passes}

After a subexpression has been read in and identified,
the parser runs parser passes on the subexpression to identify
operators and other constructs within the subexpression.

A \key{parser pass} is a function that takes as input the sequence
