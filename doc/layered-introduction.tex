...
If it is a paragraph header, the selectors, options, lexical masters,
and implied subprefix in its bracket type definition are used to
modify the current line variables selectors, options, lexical
masters, and implied header.  If this gives the \TT{implied\_header}
current line variable a non-missing value, that value must have
`\TT{line}' group when its bracket type is looked up using the
new \TT{selectors} current line variable.
