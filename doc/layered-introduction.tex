...
not all three.  A non-prefix operator may be afix.

This can be summarized in the following compatibility table:

\begin{center}
\begin{tabular}{lc|c|c|c|c|}
	& \multicolumn{5}{c}{is compatible with op} \\
\cline{2-6}
\multicolumn{1}{l|}{op} &
...
Operator fixity is ignored unless the operator is
afix.  If the operator is afix, the operator table entry
is ignored unless there is an operator to the left of
the current position that has the same precedence as the
afix operator, and there is no operator of lower precedence
between that previous operator and the current position.
Special considerations apply if the operator table entry
is afix and there is an operator immediately
before the current position that has ambiguous fixity:
