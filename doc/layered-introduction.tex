...
line, typically at the end if it containing logical line,
...
\begin{tabular}{p{1in}p{4.5in}}
		& A \TT{BRACKETABLE} token
		  ...
		  a \TT{.type}, \TT{.initiator}, and/or \TT{.terminator}
		  attribute.
		  If such a token is to become the only element of a
		  containing subexpression that will have no attributes other
		  than \TT{.type}, \TT{.initiator}, and/or \TT{.terminator}, 
		  then instead of creating a new
		  \TT{BRACKETED} token, any \TT{.type}, \TT{.initiator},
		  or \TT{.terminator} are added to the \TT{BRACKETABLE}.
		  The token type of this \TT{BRACKETABLE} token is then
...
\\[1ex]
\begin{tabular}{p{1in}p{4.5in}}
		& A \TT{PREFIX} token is essentially a
...
\ttmkey{value}{of token}
		& The token \TT{value}, as described above.  This is
		  a MIN string, label, or object.  For tokens that
		  do not have values, this is \TT{MISSING}.
\\[1ex]
...
\ttmkey{indent}{of token}
		  ...
		  Calculated assuming characeters in UNICODE category
...
delivers either top level logical line tokens
...
of the following as controlled by parser options described in
Section~\itemref{BUILTIN-PARSER-SELECTOR/OPTIONS}:
...
within the indented paragraph (which is the first token after the
indentation mark provided the indented paragraph includes any
tokens after the indentation mark).  See
...
Some of the parsing selectors are builtin, and most of these are
used not to select symbol table entries, but as options controlling
features built into the parser.  For example, some of these
options control when a logical line ends.  Builtin
selectors are described
in Section~\itemref{BUILTIN-PARSER-SELECTOR/OPTIONS}.
...
\subsubsection{Builtin Parser Selector/Options}
\label{BUILTIN-PARSER-SELECTOR/OPTIONS}

The following selectors are builtin.  Most are used to control
parser features, and these are referred to as \skey{parser option}s
(as well as being parser selectors).  One is used exclusively
to select symbol table entries, and another is used both for
control and selection and is referred to as a \key{parser
selector/\EOL option}:

\begin{indpar}[0.5in]

\hspace*{-0.2in}\ttmkey{parser}{parser selector/option} \\
    If this is on when a top level logical line is parsed, the line
    is treated as a parser command.  This selector/option is
...
\hspace*{-0.2in}\ttmkey{data}{parser selector} \\
    ...
    separators and the type/attributes parts of builtin typed
    brackets.
...
\hspace*{-0.2in}\ttmkey{end at other closing}{parser option} \\
    ...
    This is the default for standard bracketed subexpressions,
    but not for standard indented paragraphs.
...
parser selectors.  However, the bracketed subexpression recognition pass
is the only pass that can change these selectors.  Details are in
...
syntax attributes are \TT{.initiator} and \TT{.terminator}, and the
...
one per top level logical line.  The value of one of these tokens encodes
the logical line.
...
element in their containing expression or are top level.
...
\\[0.5ex]
\emkey{untyped-bracket-options} ::= {\em parser-flag-list}
