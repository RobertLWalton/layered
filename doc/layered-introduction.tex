...
\ttmkey{code}{parser standard flag} &
    Defines the `\TT{code}' selector and enables definitions using it.
\\
\ttmkey{text}{parser standard flag} &
    Defines the `\TT{text}' selector and enables definitions using it.
\\
\ttmkey{math}{parser standard flag} &
    Defines the `\TT{math}' selector and enables definitions using it.
\\
...
Standard parser definitions can be
...
Thus the standard parser executes the
equivalent of
\begin{indpar}\begin{verbatim}
*PARSER*:
...
The standard parser executes a \TT{begin block standard} parser
command that introduces a new top level indented paragraph
with its parameters modified by
...
*PARSER*:
    ...
    define top level line lexical master "DATA-CHECK"
\end{verbatim}\end{indpar}
