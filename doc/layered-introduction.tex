...
The Bracketed Subexpression Recognition Pass glues together
...
The following options apply only to prefix table entries with
...
\hspace*{-0.2in}paragraph and line headers, which are special typed prefix
    separators: \itemref{HEADED-PARAGRAPHS} and \itemref{HEADED-LINES}
\\[0.5ex]
...
\begin{indpar}
WARNING: {\em middle-break-end} and {\em middle-break-begin} must
be chosen so bracketed symbol table entries (\pagref{BRACKETED-SYMBOL-TABLE})
do not begin with a {\em middle-break-begin} or end with a
{\em middle-break-end}.
\end{indpar}
\end{indpar}
...
Also bracketed symbol table (see above) entries should avoid keys
that contain multi-character lexemes that begin with {\em middle-break-begin}
or end with {\em middle-break-end}.  If such keys are used, they may
...
\\
	...
	& \hspace*{3em} & [ \TT{with element selectors}
	                    ~ {\em bracket-selectors} ] \\
        &               & [ \TT{with element options}
	                    ~ {\em bracket-options} ] \\
        &               & [ \TT{with}
			    ~ {\em reformatter-name}
			    ~ \TT{reformatter}
...
as a (not necessarily initial) substring.  Using \TT{""} as
...
*PARSER*:
     ...
     define bracket "{{" ... "}}" [data]
     define bracket "`" ... "'" [data]
         with element selectors [ - data, + atom]
     define bracket "``" ... "''" [data]
...
*PARSER*:
     ...
     define bracket "{{" ... "}}" [code, math, text]
         with element selectors [ + math, - code, - text]
     define bracket "`" ... "'" [code, math, text, data]
...
\label{PARSER-INDENTATION-MARK-COMMAND} \\
    ...
    ::= & \TT{define}\ttmindex{define indentation mark}{parser command} ~
	  ...
	  {}[ \TT{with element options} ~ {\em indentation-mark-options} ] \\
...
the second of which is empty.  All retained paragraph lines
are non-empty.
...
they are treated as missing (for lexical masters this means the \TT{DEFAULT}
lexical master is used).
...
*PARSER*:
     define indentation mark ":" ... ";" [code]
         with element options [+ default end at options,
...
at either end to denote a missing type.
...
Lastly if there are no elements and the type is a single {\em mark},
the second copy of the {\em mark} may be omitted, as in `\TT{\{*\}}'.
...
\emkey{opening-type-and-attributes}\label{OPENING-TYPE-AND-ATTRIBUTES}
	\begin{tabular}[t]{@{}rl}
	::= & {\em bracket-type} \\
	$|$ & {\em bracket-type}~~~{\em typed-attribute-begin}~~~%
		                 {\em attribute-list}
	\end{tabular}
\\[0.5ex]
...
\emkey{label-component} ::=
    ...
    \end{tabular}
\\[0.5ex]
\emkey{bracket-type} ::= {\em label}\label{BRACKET-TYPE}
\\[0.5ex]
...
\item
An {\em attribute-label} may appear in more that one {\em attribute},
but at most one without any {\em reverse-attribute-label}, and in
at most one with any particular {\em reverse-attribute-label}.
\item
An {\em attribute-label} may appear with {\em attribute-flags}
in at most one {\em attribute}.
\end{enumerate}
\end{indpar}
...
{\em bracket-type} or {\em type-mark}(s) become(s) the \TT{.type}
...
When the {\em bracket-type} is encountered, it is looked up in the
...
{\em typed-element-selectors} and {\em bracket-options} in the
found entry.
If no entry is found, the selectors and options used to scan
the {\em elements} are computed by modifying the context selectors
and options according to the
{\em typed-element-selectors} and {\em bracket-options}
...
A {\em typed-bracketed-subexpression} with \underline{no}
elements may be a {\em typed-prefix-separator}
(see \itemref{TYPED-PREFIX-SEPARATORS}) only if
the context selectors contain a selector in common with the
{\em typed-prefix-selectors} associated with
the {\em typed-opening} by the
\TT{parser define typed bracket} command (see below) and at
least one of the following is true:
\begin{itemize}
\item The `\TT{enable prefix}' option is set in the context selectors.
\item The `\TT{enable table prefix}' option is set in the context selectors
      and a bracket table entry was found for the {\em bracket-type}.
\item The `\TT{enable header}' option is set in the context selectors
...
~~~~~~here `\TT{- end at line separator}' is implied. \\
...
\emkey{quoted-key} ::= see \pagref{QUOTED-KEY}
\\[0.5ex]
\emkey{typed-element-selectors} ::= {\em bracket-selectors}
...
\\[0.5ex]
\emkey{typed-prefix-selectors} ::= {\em selector-flags}
	~ [\pagref{SELECTOR-FLAGS}]
...
\hspace*{0.5in}\begin{tabular}[t]{@{}rl}
    ::= & {\em typed-opening}~~~{\em bracket-type}~~~%
...
\\[0.5em]
\emkey{bracket-type} ::= see \pagref{BRACKET-TYPE}
\\[0.5em]
\emkey{type-mark} ::= see \pagref{TYPE-MARK}
\\[0.5em]
\emkey{prefix-type}\label{PREFIX-TYPE} ::=
    \begin{tabular}[t]{l}
    {\em bracket-type} $|$ {\em type-mark} \\
    that is in a {\em typed-prefix-separator}
    \end{tabular}
\end{indpar}

The ~ `\TT{with prefix separators}' ~ option to a
{\em parser-typed-bracket-command} (see \pagref{PARSER-TYPED-BRACKET-COMMAND})
enables
{\em typed-bracketed-subexpressions} specified by the command
to be {\em typed-prefix-separators}.
...
A \emkey{prefix-type} is a
{\em bracket-type} or {\em type-mark} in a {\em typed-prefix-separator}.
The {\em prefix-type} of a {\em typed-prefix-separator}
may be looked up in the
bracket type table (\itemref{BRACKET-TYPE-DEFINITIONS}) and the
information found may be used to change the selectors and options
used to scan the {\em prefix-n-list} headed by
the {\em typed-prefix-separator}.
...
if it is a prefix type (\pagref{PREFIX-TYPE}).

Bracket type definitions are stored in the `\key{bracket type table}'
which is one of the parser symbol tables.

When a {\em typed-bracketed-subexpression} is parsed, its {\em bracket-type}
(i.e., its \TT{.type} if it has one) is looked up
in the bracket type table using the selectors in effect at the beginning
of the {\em typed-bracketed-subexpression} parse (the context selectors).
If a table entry is found, it is used to provide information about
that affects the parse of the
{\em elements} of the {\em typed-bracketed-subexpression},
and if the subexpression has no elements and is a
{\em typed-prefix-separator}, the parse of the {\em prefix-n-list}
following the prefix separator.
...
\\
    ...
    ::= & \TT{define}%
	  ...
	  \begin{tabular}[t]{@{}l@{}}
	  \TT{bracket type} ~ {\em prefix-name} ~ {\em parsing-selectors} \\
	  {}[ \TT{with parsing selectors} ~ {\em prefix-selectors} ] \\
    ...
    $|$ & \ttmkey{print prefix}{parser command} ~
	  {\em partial-prefix-name} \\
    \end{tabular}
\\[0.5ex]
\emkey{prefix-name} ::= {\em quoted-key}
\\[0.5ex]
\emkey{partial-prefix-name} ::= {\em quoted-key}
\\[0.5ex]
\emkey{quoted-key} ::= see \pagref{QUOTED-KEY}
\\[0.5ex]
\emkey{prefix-selectors} ::=
    \begin{tabular}[t]{l}
    {\em selector-spec} without any `\TT{TOP LEVEL}' selector
	~~~ [\pagref{SELECTOR-SPEC}]
    \end{tabular}
...
The `\TT{print prefix} {\em partial-prefix-name}' command
prints all prefix symbol table entries whose
{\em prefix-name} contains the {\em partial-prefix-name}
as a (not necessarily initial) substring.  Using \TT{""} as
a {\em partial-prefix-name} will print all prefix
symbol table entries.


The components special to a prefix definition are:
...
\begin{indpar}[0.5em]
The subexpression must be a token list that could be represented by
...
\TT{data} reformatter takes no action, and the headed logical
line is retained as is in the input.
...
above logical line, e.g., to \TT{\{data\}}.

The effect of reformatting is to delete the entire subexpression token list
and to define or add to the object whose identifier
is {\em ID}.  The object is given the
elements given in the {\em element-list} and the attributes in the
{\em indented-attributes}.
The {\em indented-attributes} are parsed before this reformatter
is invoked to become a MIN object
with \TT{INDENTED\_\EOL PARAGRAPH} \TT{.terminator} and logical
...
After a parser is initialized, it has \underline{no} prefix
definitions.
The `\TT{standard}' parser block (\pagref{STANDARD-PARSING-BLOCK})
adds the definitions:
\begin{indpar}\begin{verbatim}
*PARSER*:
    define prefix "table" [TOP LEVEL, data, code, math, text]
           with group paragraph
           with line lexical master "ROW-CHECK"
    define prefix "row" [TOP LEVEL, data, code, math, text]
           with group line
    define prefix "data" [TOP LEVEL, data, code, math, text]
           with parsing selectors [+ data paragraph]
           with data reformatter
                ( "=", "no", "[", [< "{" "*" >] )
    define prefix "raw data"
                  [TOP LEVEL, data, code, math, text]
           with parsing selectors [data, data paragraph]
           with data reformatter
                ( "=", "no", "[", [< "{" "*" >] )
    define prefix "code" [TOP LEVEL, code, text]
           with parsing options [+ reset]
    define prefix "section" [TOP LEVEL, code, text]
           with group paragraph
    ...
    define prefix "p" [TOP LEVEL, code, text]
           with group paragraph
           with parsing selectors [+ text,
                                   - code, - math, - data]
           with parsing options [+ sticky,
                                 ...
                                 - end at line separator,
                                 - end at outer closing]
    define prefix "quote" [TOP LEVEL, code, text]
           with group paragraph
           with parsing selectors [+ text,
                                   - code, - math, - data]
           with parsing options [+ sticky,
...
taken from the rest of the headed line.  This output has a \TT{.type}
but \underline{no}
...
of a paragraph header (see \itemref{BRACKET-TYPE-DEFINITIONS}
and \itemref{HEADED-PARAGRAPHS})
...
indented paragraph (or the end of file).
...
\hspace*{-0.2in}\ttmkey{implied header}{line variable}\\
    ...
    This is because in such a case the implied header will be
    deleted, as there is nothing between it and the explicit
    header.  When the implied header is deleted,
    the selectors and options will be fixed up as if
    the implied header was never inserted.  \underline{However}, the
    explicit header will have been parsed with selectors and
    options as they were modified by the implied header, and the
    explicit header is \underline{not} reparsed.
\\[1ex]
...
\\[1ex]
\hspace*{-0.2in}Note: If any current line variable is changed
...
Explicit paragraph headers with the `\TT{sticky}' or `\TT{continuing}'
options or with a \TT{line} group implied subprefix make such changes.
\\[2ex]
...
\hspace*{-0.2in}\TT{paragraph line lexical master}
    ...
    The top level paragraph line variables, with the exception of
    ...
    The top level paragraph implied header is normally missing, but
    ...
    the introduces the paragraph.  The paragraph lexical masters and
...
If the logical line is the first of a file, or the first of an
indented paragraph, or if it is in paragraph beginning position and the
...
If the logical line is the first after a blank line, and the blank
line was preceeded by a sequence of logical lines prefixed by a
paragraph header, the logical lines in this sequence (but not the
current logical line) are collected into a paragraph
headed by the paragraph header, unless the \TT{options} current line
variable has the `\TT{continuing} option \underline{and}
the current logical line does \underline{not}
begin with an explicit paragraph header.

\item The parsing selectors, options, and lexical master used at the
beginning of the logical line are taken from the current line variables.

\item If the current \TT{implied\_header} variable value is not missing,
the prefix definition for this is looked up (using selectors now
...
the logical line must be in paragraph beginning position.
...
If the implied prefix is an implied paragraph header, it is inserted at the
beginning of the logical line, and the
selectors, options, and lexical masters in the prefix definition are used to
modify the current line variables selectors, options, and lexical
masters.
If in addition the prefix definition has an implied subprefix
that itself has a prefix definition selected by the modified
selectors which has the `\TT{line}' group, this implied subprefix
is copied to the \TT{implied\_header} current line variable.

\item
If the current \TT{implied\_header} line variable is now an
implied line header, it is inserted after any implied paragraph header
or at the beginning of the logical line if there is no implied
paragraph header, and the
selectors, options, and lexical masters in the prefix definition are used to
modify the current line variables selectors, options, and lexical
masters.
...
If it begins with a prefix that has a prefix definition with either
the `\TT{paragraph}' or `\TT{line}' group, then that is an explicit
paragraph header or line header.

If an explicit paragraph header is found immediately after implied headers,
the implied headers are deleted.

If an explicit line header is found immediately after implied headers,
any preceeding implied line header that was inserted at
the beginning of the logical line or following an implied
paragraph header is deleted.

If implied headers are deleted, the current line variables are restored
to their state before the deleted implied header was inserted.
Note, however, that the current line selectors and options
computed by the implied headers were used to parse the
explicit header and look up its
prefix definition, so care should be taken that prefix definitions
for implied headers do not change the selectors and options sufficiently to
affect parsing explicit headers.
...
selectors, options, and lexical masters.  Then
\begin{enumerate}[label*=\arabic*]
\item\label{RESET-RULE}
If the `\ttkey{reset}' option flag is on in the new current line variables,
then
\begin{enumerate}[label*=.\arabic*]

\item The \TT{paragraph implied header} line variable
is reset to the value it had at the beginning of the current
indented paragraph,
or to missing if the logical line is top level.

\item The paragraph line variables are copied to the current line
variables (resetting the latter to their state before the explicit
paragraph header was parsed, except for the implied header value
which was just reset).

\item The explicit paragraph header is deleted.  Parsing is as
if the explicit paragraph header had never existed, except that
the paragraph and current line variables are set to the values
they had at the beginning of the current indented paragraph or
at top level after the last \TT{define top level} command, and
also a comment is allowed without warning
between the deleted explicit paragraph
header and any following non-comment lexeme in the current logical
line.
...
\item
Otherwise (if the `\TT{reset}' flag is off), then

\begin{enumerate}[label*=.\arabic*]

\item
Let $P$ denote the implied subprefix of the prefix definition
of the explicit paragraph header.
If $P$ exists and
itself has a prefix definition selected by the modified
current selectors that has the `\TT{line}' group, $P$
is copied to the \TT{current implied header} line variable.

\item
\label{STICKY-RULE}
The \TT{paragraph implied header} is set to
a copy of the explicit paragraph header,
if the current options now has the `\TT{sticky}' flag.
Otherwise it is reset to the value it had at the beginning of the current
indented paragraph,
or to missing if the logical line is top level.
...
If the logical line is empty except for implied headers, the
