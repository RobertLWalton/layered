...
or is a reset header.  A \key{mapped header} is a prefix
that was created at the beginning of a logical line
when a mapped lexeme was mapped to the prefix.
A \key{reset header} is a prefix with the
`\TT{reset}' group.  Isolated headers and mapped prefixes that
are headers are the only prefixes that can reset the line lexical
master.
