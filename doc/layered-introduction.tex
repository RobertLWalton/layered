...
if a comment following an indent begins a non-comment logical line,
the comment is in error and a warning message is produced.
The effect is that comments in a logical line must be at least as
indented as the logical line, unless the logical line begins
after a line separator (e.g., `\TT{;}).
...
parsing header options	& 2 bits	& \pagref{HEADER-OPTION} \\
...
\emkey{prefix-option}
    ...
    \end{tabular}
\\[0.5ex]
\emkey{header-option} ::= \ttkey{sticky} $|$ \ttkey{continuing}
          \label{HEADER-OPTION}
\\[0.5ex]
