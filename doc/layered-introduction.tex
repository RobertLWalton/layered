...
in effect at the beginning of {\em prefix-m}, and let $SO_3$ be the
selectors and options in effect just after {\em prefix-m}.
$SO_2$ is used to parse {\em prefix-m} and look up its bracket type table
entry.  Then $SO_1$ is modified by the bracket type table entry
parsing selectors and parsing options (see below)
to produce $SO_3$.  A consideration is that if
{\em prefix-m} is not at the beginning of its {\em prefix-m-list},
$SO_1$ and $SO_2$ may not be the same.
