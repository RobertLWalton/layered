...
\emkey{[]-subexpression} ::= \begin{tabular}[t]{@{}l@{}}
			  ...
			  \end{tabular}
\\[0.5ex]
\emkey{naked-argument} \begin{tabular}[t]{@{}rl@{}@{}}
                       ::= & {\em number} \\
		       $|$ & a MIN object whose
		             \TT{.initiator} is not `\TT{(}' \\
			   & and whose \TT{.terminator} is not
			     \TT{INDENTED\_PARAGRAPH} \\
			   & (includes {\em quoted-strings})
		       \end{tabular}
...
list if both have the same \TT{.initializer} and \TT{.terminator} and the
{\em argument-list} is \underline{not} a {\em naked-argument}, or if
the {\em argument-list} is a {\em naked-argument} and
the {\em prototype-argument-list} has `\TT{(}' as its
\TT{.initiator} and `\TT{)}' as its \TT{.terminator}.
