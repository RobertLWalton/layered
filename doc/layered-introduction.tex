...
line lexical master, and implied header.

If a reset header is found,\label{RESET-RULE}
the header's bracket type
definition modifies the \underline{paragraph} selectors, options,
line \underline{and paragraph}
lexical masters, and implied header.  Then the reset header is
deleted, and its line is treated as a blank line, so the next line is
in paragraph beginning position and the paragraph line variables are
copied to the current line variables.
