...
An operator must have at least one of these flags.
In addition there is an `\TT{afix}' flag.
...
A nofix operator may \underline{not} also be prefix or postfix.

An operator with the `\ttkey{afix}' flag\label{AFIX-FLAG} must follow
...
operator and the afix operator.  Infix and nofix operators may be given
the afix flag.  Prefix and postfix operators can\underline{not} be
given the afix flag.
...
This can be expressed in the following compatibility table:
...
Prefix operators always have precedent +1,000,001 (second highest), and postfix
operators always have precedence +1,000,002 (highest).  Infix and nofix
operator have precedence given in the operator definition.
...
is ignored unless there is an operator to the left of
the current position that has the same precedence as the
afix definition, and there is no \underline{non-ambiguous}
operator of lower precedence
between that previous operator and the current position.
See the end of \itemref{FIXITY-DETERMINATION} for a description
of the possible ambiguous operators that may be before the current
position.

Operator fixity is ignored during operator identification except
that determination of fixity during the left-to-right scan
determines precedence that in turn affects which afix
operator definitions are ignored.
