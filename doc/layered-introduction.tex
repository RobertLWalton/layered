...
\\[1ex]
		& The standard parser encodes the translation
		  ...
		  numbers a {\tt value}'s, but \underline{not}
...
\\[1ex]
		& For expressions, this is a MIN object which
		  encodes a list of subexpressions.  This list
		  may also have other attributes, such as
...
Tokens are organized in a doubly threaded list.  A file consists
of such a list that ends with a token containing a end-of-file
...
\\[1ex]
		& A \key{symbol} is a min::gen string equal to the
		  lexeme translation string of a word, mark, separator,
		  or natural number.  Quoted strings and non-natural
		  numbers cannot be symbols.
\\[1ex]
...
\\[1ex]
		& If a label has just one symbol, it is represented by
		  a min::gen string.  Otherwise it is represented by
		  a min::gen label whose elements are min::gen strings
...
that is part of a line (for the explicit subexpression pass)
...
mark, separator, or natural number, can match symbols.  These are just
the token types that have values which are min::gen strings equal to the
translation strings of their lexemes.  When a match is found,
