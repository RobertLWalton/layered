...
\begin{tabular}{p{1in}p{4.5in}}
		& Standardly the token lexeme types are word, mark, separator,
		  ...
		  announced when read by the parser
		  but are not translated into tokens.
\end{tabular}
\\[1ex]  
\begin{tabular}{p{1in}p{4.5in}}
...
\begin{tabular}{p{1in}p{4.5in}}
		& A \TT{BRACKETABLE} token
		  ...
		  attribute.
		  If such a token is to become the only element of a
...
\begin{tabular}{p{1in}p{4.5in}}
		& A \TT{DERIVED} token is a non-lexeme that which has
		  ...
		  For example, a label subexpression reformatter may
...
\ttmkey{position}{of token}
		& The positions in the input file of
		  ...
		  can then be computed from the line itself and the
		  byte offset.
\end{tabular}
\\[1ex]
\begin{tabular}{p{1in}p{4.5in}}
\ttmkey{indent}{of token}
	        & The indentation (beginning column)
		  of the token, for the first token of a line only.
		  Calculated assuming characters in UNICODE category
		  Zs are 1 column space characters (this includes
		  the single space and non-breaking single space
		  characters), horizontal tabs are every 8 columns,
		  and other control and space characters take 0 columns.
...
A \key{logical line} is a sequence of tokens that ends with one
of the following as controlled by parser options described below:

\begin{itemize}
\item A line break followed by a line with indent equal to or less
than the current indent.
\item A line break followed by a line with indent less
than the current indent.
\item Any line break.
\item Any blank line.
...
indent to the indent of the first token of the first logical line
within the indented paragraph.  See
Section~\itemref{INDENTED-PARAGRAPH-SUBEXPRESIONS} for indented paragraph
details.
...
of comment line and blank lines containing at least one blank line.
...
The \skey{parsering end at option}s
control when a logical line ends and are defined in
...
\hspace*{-0.2in}\ttmkey{end at le indent}{parser option} \\
    ...
    This option is on by default at top level and in indented paragraphs.
...
\hspace*{-0.2in}\ttmkey{end at line separator}{parser option} \\
    This parser option causes a line separator to end a logical line.
    The applicable line separator is that of either the top level
    when not inside an indented paragraph (default is `\TT{;}'),
    or of the smallest containing indented paragraph when inside
    an indented paragraph.

    This option is on by default at top level and also
    in indented paragraphs that have a defined line separator.
...
\begin{indpar}[0.5in]
\hspace*{-0.25in}logical lines, which may end with any line break,
...
\hspace*{-0.25in}brackets (e.g.~`\TT{(}' and `\TT{)}')
    and bracketed subexpressions \\
\hspace*{-0.25in}indentation marks (e.g.~`\TT{:}')
    and line separators (e.g.~`\TT{;}'),
    indented paragraphs, and indented paragraph logical lines \\
\hspace*{-0.25in}typed brackets (e.g.~`\TT{\{b|}' and `\TT{|b\}}')
      and typed bracketed subexpressions \\
\hspace*{-0.25in}typed prefix separators (e.g.~`\TT{\{p\}}')
      and prefix lists
\end{indpar}
...
\begin{indpar}[0.5in]
\hspace*{-0.25in}opening brackets (e.g.~`\TT{(}') and their
    corresponding closing brackets (e.g.~`\TT{)}') \\
\hspace*{-0.25in}indentation marks (e.g.~`\TT{:}')
    and any corresponding line separators (e.g.~`\TT{;}') \\
\hspace*{-0.25in}typed openings (e.g.~`\TT{\{}') and their
...
\\
	...
	& \hspace*{3em} & [ \TT{with parsing selectors}
	                    {\em new-selectors} ] \\
        &               & [ \TT{with options}
	                    {\em untyped-bracket-end-at-options} ] \\
...
\\[0.5ex]
\emkey{untyped-bracket-end-at-options} ::= {\em parser-flag-spec}
...
Similarly if the bracket definition contains {\em untyped-bracket-options},
...
An example using {\em untyped-bracket-options} to permit
\TT{`{}`{}`\ldots'{}'{}'}
...
parser define bracket "(" ... ")" [parser]
       with options [- end at line separator]
parser define bracket "[" ... "]" [parser]
       with options [- end at line separator]
\end{verbatim}\end{indpar}
are builtin.
...
consisting of zero or more logical lines called \skey{paragraph line}s.
The paragraph has a \key{paragraph indentation},
which is the indentation of the
first non-blank, non-comment physical line after the
line containing the indentation mark, if this indentation
is greater than the indentation of the physical line containing
the indentation mark.  But if this indentation is less than or equal
to the indentation of the physical line containing the indentation
mark, the paragraph is empty, and has no paragraph lines.
...
\hspace*{3em}\begin{tabular}[t]{l}
             beginning of a line whose indentation is less than
			  the paragraph indentation; \\
	     or is less than the indentation of the physical line
			  containing \\
	     the {\em indentation-mark} if the paragraph
	                  is empty
	     \end{tabular}
\\[0.5ex]
...
\TT{\{||\}} \\
\TT{\{\}} \\
\TT{\{"": my attribute = 5, no my option|} ~~~\ldots~~~ \TT{|\}} \\
\TT{\{*} ~~~\ldots~~~ \TT{*\}} \\
\end{center}
...
The type can be omitted from either beginning if there are no attributes
after it, and can be omitted from the end if there are no attributes
before it, but of course cannot be omitted from both beginning and
end.  If there are no elements, `\TT{|~|}' can be collapsed into
...
\hspace*{3em}\begin{tabular}[t]{@{}rl}
	::= & {\em typed-opening} {\em type-label}
	      {\em opening-attributes}\QMARK{}
	      {\em typed-elements} \\
	    & {\em closing-attributes}\QMARK{} {\em type-label}
	      {\em typed-closing} \\
	$|$ & {\em typed-opening} {\em type-label}
	      {\em opening-attributes}\QMARK{}
	      {\em typed-elements} {\em typed-closing} \\
	$|$ & {\em typed-opening} {\em typed-elements}
	      {\em closing-attributes}\QMARK{} {\em type-label}
	      {\em typed-closing} \\
...
\\[0.5ex]
\emkey{opening-attributes} ::= {\em typed-attribute-begin}
		               {\em attribute-list}\QMARK{}
\\[0.5ex]
\emkey{closing-attributes} ::= {\em attribute-list}\QMARK{}
...
\emkey{attribute} \begin{tabular}[t]{@{}rl}
			...
			\end{tabular} \\
                  ::= & {\em attribute-label} \\
                  ::= & {\em typed-attribute-negator} {\em attribute-label} \\
		  \end{tabular}
\\[0.5ex]
...
two {\em typed-middle} lexemes with an intervening space to
...
{\em typed-closing}, the end of the paragraph line
containing the typed bracketed subexpression,
or an end of file.

Within the {\em element-list}, only the {\em typed-middle} and
{\em typed-closing} are recognized, and both end the
{\em element-list} (but ending with a {\em typed-closing} is
a parser error).  Outside, {\em typed-attribute-begin/equal/separator}
are also recognized, and if out of place, a parsing error is announced.

When a {\em typed-opening} is recognized, the current parser
selector set is saved in a stack and
a new set is computed by modifying the saved set
according to {\em element-selectors} associated with the
{\em typed-opening} (see the \TT{parser define typed bracket}
command below).  The modified selector set becomes the current
parser selector set while scanning the {\em element-list}.
But while scanning the type label and attributes, the
current parser selector set is set to the {\em attribute-selectors}
associated with the {\em typed-opening} (also see below).
The saved selector set is restored after the subexpression ending
...
\\
    ...
    ::= & \TT{parser}%
	  ...
	  {}[ \TT{with attribute flags}
	      {\em typed-attribute-flags-punctuation} ] \\
	  {}[ \TT{with} \begin{tabular}[t]{l}
	                \TT{attribute multivalue} \\
	                {\em typed-attribute-multivalue-punctuation} ]
			\end{tabular} \\
	  {}[ \TT{with element selectors}
	      {\em typed-element-selectors} ] \\
	  {}[ \TT{with attribute selectors}
	      {\em typed-attribute-selectors} ] \\
	  {}[ \TT{with prefix separator allowed} ] \\
	  \end{tabular} \\
...
\\[0.5ex]
\emkey{typed-attribute-flags-punctuation} ::= \\
    \hspace*{0.2in}
     {\em typed-attribute-flags-opening}
     \begin{tabular}[t]{l}
     \TT{...} {\em typed-attribute-flags-separator} \\
     \TT{...} {\em typed-attribute-flags-closing}
     \end{tabular}
\\[0.5ex]
\emkey{typed-attribute-flags-opening} ::= {\em quoted-name}
\\[0.5ex]
\emkey{typed-attribute-flags-separator} ::= {\em quoted-name}
\\[0.5ex]
\emkey{typed-attribute-flags-closing} ::= {\em quoted-name}
\\[0.5ex]
\emkey{typed-attribute-multivalue-punctuation} ::= \\
    \hspace*{0.2in}
     {\em typed-attribute-multivalue-opening}
     \begin{tabular}[t]{l}
     \TT{...} {\em typed-attribute-multivalue-separator} \\
     \TT{...} {\em typed-attribute-multivalue-closing}
     \end{tabular}
\\[0.5ex]
\emkey{typed-attribute-multivalue-opening} ::= {\em quoted-name}
\\[0.5ex]
\emkey{typed-attribute-multivalue-separator} ::= {\em quoted-name}
\\[0.5ex]
\emkey{typed-attribute-multivalue-closing} ::= {\em quoted-name}
\\[0.5ex]
...
Here the `\TT{with prefix separator allowed}' option premits the use of
the typed brackets for {\em typed-prefix-separators} described in
Section \itemref{TYPED-PREFIX-SEPARATORS}.

A {\em typed-bracketed-subexpression} is first parsed by the
bracketed subexpression parser, which recognizes:

\begin{itemize}

\item The {\em typed-opening} and {\em typed-closing} that delimit
the subexpression.  The {\em typed-middle} is also recognized for
the sole purpose of changing the parsing context selectors during
the parse.

\item The {\em typed-attribute-flags-punctuation}
and {\em typed-attribute-multivalue-punctuation}, that are used
to delimit bracketed subexpressions outside the {\em element-list}.

\end{itemize}

During this parse the operator parser pass operates on subsubexpressions,
and specifically, parses {\em typed-attribute-flags/multivalue-separators}
outside the {\em element-list}.

After the bracketed subexpression parser has finished, it calls
a special typed subexpression reformatter that has its own symbol
table which includes the {\em typed-middle} and
{\em typed-attribute-punctuation}.  This reformatted makes the
MIN object adding the type, attributes, and elements it discovers.

To make all this work, the various punctuation marks defined by
a {\em parser-typed-bracket-command} are distributed among 3 symbol
tables as follows:

\begin{center}
\begin{tabular}{lll}
Symbol \\
Table & Punctuation & Selectors
\\[1ex]
Bracketed
    & {\em typed-opening} & {\em parsing-selectors} \\
Subexpression
    & {\em typed-middle} & \TT{always} \\
Parser
    & {\em typed-closing} & \TT{always} \\
    & {\em typed-attribute-flags-opening}
    & {\em typed-attribute-selectors} \\
    & {\em typed-attribute-flags-separator} & \TT{always} \\
    & {\em typed-attribute-flags-closing} & \TT{always} \\
    & {\em typed-attribute-multivalue-opening}
    & {\em typed-attribute-selectors} \\
    & {\em typed-attribute-multivalue-separator} & \TT{always} \\
    & {\em typed-attribute-multivalue-closing} & \TT{always}
\\[1ex]
Operator
    & {\em typed-attribute-flags-separator}
    & {\em typed-attribute-selectors} \\
Parser
    & {\em typed-multivalue-flags-separator}
    & {\em typed-attribute-selectors} \\
\\[1ex]
Internal
    & {\em typed-middle} & (internal) \\
Typed
    & {\em typed-attribute-begin} & (internal) \\
Subexpression
    & {\em typed-attribute-equal} & (internal) \\
Reformatter
    & {\em typed-attribute-separator} & (internal) \\

\end{tabular}
\end{center}

Alternative implementations are acceptable as long
as the punctuation is recognized where required in the
various contexts.
...
{\em element-selectors}, the parser context selectors
for the {\em element-list} of a {\em typed-bracketed-subexpression}
recognized by this definition are the same as the parser context selectors
just outside the subexpression.
...
\emkey{typed-prefix-separator}
    \begin{tabular}[t]{@{}rl}
    ::= & {\em typed-opening} {\em type-label} {\em opening-attributes}\QMARK{}
	                      {\em typed-closing} \\
...
{\em parser-typed-backet-command} enables use of the command
...
\hspace*{-0.2in}\ttmkey{paragraph header}{parser option} \\
    If a prefix separator appears in a paragraph beginning
    position (after a blank line and any following comment lines)
    and its type has this parser option, then the prefix separator
    becomes the current paragraph header.

\hspace*{-0.2in}\ttmkey{paragraph repeat}{parser option} \\
    If a paragraph does not begin with a paragraph header
    prefix separator as per the \TT{para\-graph header} option,
    and the \TT{paragraph repeat} option is in effect,
    then the current paragraph header is implicitly prepended
    to the paragraph (unless there is no current paragraph
    header).


\hspace*{-0.2in}\ttmkey{paragraph indent}{parser option} \\
    If this option is turned on by a (explicit or implicit)
    paragraph header prefix separator, the indent of the
    next token after this prefix separator becomes the new
    paragraph indent, and the paragraph ends just before
    the next physical line beginning non-comment token that
    has lesser indent than this paragraph indent.
