...
A {\tt define} parsing definition makes entries in parser symbol tables.
...
An {\tt undefine} parsing definition modifies 

TBD



Each entry made is given the 

\subsection{The Explicit Subexpression Recognition Pass}

The explicit subexpression recognition pass recognizes:
\begin{center}
brackets (e.g.~`{\tt (}' and `{\tt )}') and bracketed subexpressions \\
indentation marks (e.g.~`{\tt :}') and indented paragraphs \\
...
\\[0.5ex]
\emkey{opening-bracket-name} ::= {\em name}
\\[0.5ex]
\emkey{closing-bracket-name} ::= {\em name}
\\[0.5ex]
...
\\[0.5ex]
\emkey{indentation-mark-name} ::= {\em name}
