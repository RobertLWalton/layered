...
\emkey{parser-context-command}
    ...
    $|$ & \ttkey{parser print context} {\em context-name} \\
    \end{tabular}
\\[0.0ex]
...
For `{\tt parser define}' commands defined in the following sections,
and also for the `{\tt parser define context}' command defined above,
there is a companion
`\ttkey{parser print}' command that prints the contents of the
parser symbol table entries with the designated {\em type} and {\em name}.
Its syntax is
\begin{center}
{\tt parser print} {\em type} {\em name}
\end{center}
