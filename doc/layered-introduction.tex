...
The bracketed pass replaces purelists whose only element is a
bracketed expression by that element.  For example, if \TT{(\ldots)} and
\TT{[\ldots]} are brackets, then
\begin{center}
\begin{tabular}{rcl}
\TT{\{|(X)|\}} & is replaced by & \TT{(X)} \\
\TT{\{|[X]|\}} & is replaced by & \TT{[X]} \\
\TT{\{|\{|X|\}|\}} & is replaced by & \TT{\{|X|\}} \\
\end{tabular}
\end{center}
