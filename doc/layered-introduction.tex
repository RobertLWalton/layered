...
The operator parsing algorithm accepts as input a sequence of tokens
...
following rules:
\begin{itemlist}[0.7in]
\item[Rule a:] A prefix operator must be preceded by the beginning of
the entire token list or by a prefix, infix, or nofix operator,
of precedence not greater than the precedence of the prefix operator,
else the prefix operator is not recognized.
\item[Rule b:] A infix operator must be preceded by a non-operator token,
or by a postfix or nofix operator of precedence greater than
the infix operator,
else the infix operator is not recognized.
\item[Rule c:] A recognized postfix operator must be followed by the end of
...
\item[Rule d:] A recognized infix operator must be followed by a non-operator
...
\item[Loop Step 1:]
Find an operator beginning at token $C$.  If $C=F$, infix operators
...
\item[Loop Step 3:]
If no operator was find in Loop Step 1 and the token before $C$
...
\item[Loop Step 4:]
Else if no operator was find in Loop Step 1 or generated by Loop Step 2,
repoint $C$ to the next token after $C$ and go to the next loop iteration.
\item[Loop Step 5:]
If the operator found in Loop Step 1 or generated by Loop Step 2
has a precedence equal to or below the top of $P$, and if the
token before $C$ is a prefix or infix operator, insert
an error lexeme token before $C$, leave $C$ pointing at the operator,
announce an error, and then continue as if no error were found.
\item[Loop Step 6:]
As long as the operator found in Loop Step 1 or generated by Loop Step 2
...
\item[Loop Step 7:]
If the operator pointed at by $C$ is not a postfix operator,
push the precedence of the operator pointed at by $C$ into the $P$ stack
and the location of the token after $C$ into the $F$ stack.
\item[Loop Step 8:]
