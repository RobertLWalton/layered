...
\begin{tabular}{p{1in}p{4.5in}}
		& Standardly, the lexeme characters of a
		  natural-number or numeric lexeme are converted to
		  ...
		  the resulting number is a non-negative integer exactly
		  representable by no more than 15 digits, the token
		  is given the \TT{natural-number} lexeme type;
		  otherwise if conversion to a MIN number is possible
		  the token is given the \TT{NUMBER} type and
		  is designated to be a \key{number lexeme} token
		  (even though it is not a lexeme from the point of
		  view of the lexical scanner);
...
\\[1ex]  
\begin{tabular}{p{1in}p{4.5in}}
		& Note that if the numeric value of the string of
		  lexeme characters is too large (e.g., \TT{"1e+500"}),
		  {\tt strtod} will
		  return a MIN number value equal to plus or minus
		  infinity, and if the value is non-zero but too small
		  to be represented (e.g., \TT{"1e-500"}),
		  {\tt strtod} will return \TT{0}.
\end{tabular}
\\[1ex]  
\begin{tabular}{p{1in}p{4.5in}}
		& Note that natural-number and number lexemes have MIN number
		  values that do not precisely represent the character
		  string of the lexeme.  Thus \TT{"01"} and \TT{"1"}
		  both have MIN number \TT{1} as a value, and
		  \TT{"1e1"} and \TT{"10"} both have MIN number \TT{10}
		  as a value.  Note all these examples, even
		  \TT{"1e1"}, are natural-number lexemes and not number lexemes.
		  This allows the lexical type of a lexeme to be
		  determined from its value, which is necessary
		  when scanning lexemes into names and labels after
		  the lexemes are no longer tokens but have become
		  vector elements.
\end{tabular}
\\[1ex]  
\begin{tabular}{p{1in}p{4.5in}}
...
\\[1ex]
		& A \key{symbol} is a MIN string equal to the
		  lexeme string of a word, mark, or separator lexeme,
		  or a MIN number that is the conversion of the lexeme
		  string of a natural-number lexeme.  Quoted string, numeric,
		  and number lexemes are not symbols.
\\[1ex]
...
mark, separator, or natural-number, can match label symbols.
These symbols
...
The {\em ID-character} is a parser parameter.  It and another
parser parameter called the \emkey{ID-assignment}\label{ID-ASSIGNMENT}
(see the \TT{data} reformatter: \pagref{DATA-REFORMATTER})
can be defined
or printed by:
\begin{indpar}[0.1in]
...
\hspace*{3em} \begin{tabular}{rl}
              ...
              $|$ & \ttmkey{define ID assignment}{parser command}
	                  ~ {\em ID-assignment} \\
...
\\[0.5ex]
\emkey{ID-assignment} ::= {\em quoted-key}
\end{indpar}
...
Note that natural-number token values are MIN numbers that
...
\begin{center}
\begin{tabular}{lll}
\TT{5}	                        & MIN number	& from natural number token \\
...
{\em natural-numbers}, {\em numbers}, {\em numerics}, {\em quoted-strings},
or MIN label values.
...
(for {\em natural-numbers} and {\em numbers}),
...
which is a sequence
of one or more words, natural-numbers, numbers, numerics, and quoted strings,
not beginning with a natural-number or number.
...
Attribute values are similar but may begin with a natural-number
...
words, natural-numbers, numbers, numerics, or quoted strings that are made into
MIN strings or MIN numbers,
or sequences of words, natural-numbers, numbers, numerics, and
...
\emkey{label-component} ::=
    \begin{tabular}[t]{@{}l}
    {\em word} $|$ {\em natural-number} $|$ {\em number}
...
\TT{\{}{\em data-prefix}\TT{\}} ~ {\em ID} ~
	    {\em assignment-sign} ~ {\em element-list} ~
...
\\[0.5ex]
\emkey{assignment-sign} ::= {\em ID-assigment}
\\[0.5ex]
{\em ID-assignment} ::= see \pagref{ID-ASSIGNMENT}
\\[0.5ex]
\emkey{element-list} ::= see \pagref{ELEMENT-LIST}
\\[0.5ex]
...
and to define or add to the object denoted by
...
is invoked,
the {\em attribute-paragraph} is parsed
becoming a MIN object with
...
{\em attribute-flags}.  The object \underline{may} have
...
reference the untouched preallocated stub.  In this case the
