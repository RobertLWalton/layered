...
The `{\tt ouput} {\em type}' and `{\tt output none}'
...
line, to a single expression token.  Here lines may be continued
by indented lines.
...
\\[1ex]
		& For a lexeme, the translation of the
		  ...
		  value as a MIN string or in the token string.
\\[1ex]
...
\end{tabular} \\[1ex]
\begin{tabular}{p{1in}p{4in}}
\tt value	& For lexemes that have a {\tt value}, this is
		  lexeme translation string stored as a MIN string.
\\[1ex]
		& The standard parser encodes the translation
		  strings of words, marks, separators, and natural
		  numbers as {\tt value}'s, but \underline{not}
		  the translation strings of non-natural numbers
		  or quoted strings.
\\[1ex]
		& A token in which the translation string is
		  encoded as a value is also called a `\key{symbol token}'.
...
\\[1ex]
		& For EXPRESSION tokens, this is a MIN object which
		  ...
		  attributes, such as
		  {\tt .initiator}, {\tt .terminator},
		  {\tt .sep\-arator}, {\tt .arguments}, and
		  {\tt .keys}.
\\[1ex]
\tt string	& For lexemes that do not have {\tt value}'s,
                  the translation string of the lexeme.
		  This is stored in a manner that makes
...
\end{tabular}   & The positions of the first character of the token
		  ...
		  input text.  The position records the line,
		  column number within line,
		  and the byte offset within a UTF-8 encoding
		  of the line.  The column number depends upon
		  the format used to print the line, i.e,
		  whether or not the horizontal tab prints
		  in 1 column as {\tiny $\stackrel{\textstyle H~}{~T}$}
		  or in 1 to 8 columns as whitespace.  This
		  format must be fixed when lexemes are input.
\end{tabular}
\end{indpar}
...
tokens, may not have their translation strings recorded in the token.
...
looks up only definitions of different types.
...
an `\key{matched label}\,'.

Note that in the explicit
subexpression recognition pass matched labels may not span line bounaries.
...
A selector set is represented as 64-bit unsigned integer values with the bit in
...
Also definition activity may depend upon context.  For example,
if `{\tt<}' is a named opening and `{\tt |}' is a named
middle (see the explicit expression recognition pass below
for details), the named middle is not active unless it is proceeded by
the named opening in the current explicit subexpression.

\subsection{Parser Definitions}

The syntax of a parser definitions in general and those
associated with the explict expression recognition pass in particular
is specified on
\pagref{EXPLICIT-SUBEXPRESSION-RECOGNITION-DEFINITIONS-1}
and
\pagref{EXPLICIT-SUBEXPRESSION-RECOGNITION-DEFINITIONS-2}.

A parser definition is an input line by itself, and is
...
{\em names} that are used as labels of symbol table entries
and therefore as keys in symbol table lookups.

A \ttmkey{define}{parsing definition}
parsing definition makes entries in parser symbol tables.
One entry is made for each {\em name} in the definition name.
...
If a quoted string is used in a {\em name}, the quotes are removed
...
separator, mark, or word.

An example parsing definintion is
\begin{center}
...
Parser symbol tables are used to look up sequences of tokens which
...
has its own name, which is used to denote it in parsing definitions.
...
the parsing definitions together.\footnote{
...
An \ttmkey{undefine}{parsing definition}
parsing definition modifies the {\em parsing-selectors}
of any entries that would be added to symbols
tables by the corresponding {\tt define} parsing definition.
If the {\tt undefine} as a given {\em selector}, that is removed
from all these entries.  {\tt Undefine} parsing definitions
...
the parsing definitions in reverse order.  When a definition
is popped, its effects are removed from the symbol tables.
Both {\tt define} and {\tt undefine} parsing definitions can be
removed in this way.
...
\begin{indpar}[0.5in]
unnamed brackets (e.g.~`{\tt (}' and `{\tt )}')
    and unnamed bracketed subexpressions \\
indentation marks (e.g.~`{\tt :}'),
    \begin{tabular}[t]{@{}p{3.5in}@{}}
    indented paragraphs, and indented paragraph lines
    \end{tabular} \\
named brackets (e.g.~`{\tt <b|}' and `{\tt |b>}')
      and named bracketed subexpressions \\
named operators (e.g.~`{\tt <p>}')
\end{indpar}

The syntax of the parsing definitions that control explicit
...
\emkey{symbol} ::= {\em word} $|$ {\em mark} $|$ {\em separator}
	         $|$ {\em natural-number}
\\[0.5ex]
\emkey{name-component} ::= {\em non-special-symbol} $|$ {\em quoted-string}
\\[0.5ex]
\emkey{non-special-symbol} ::=
    {\em symbol} that is \underline{not} treated specially by the parser
\\[0.5ex]
\emkey{name} ::= {\em name-component} {\em name-component}\,$^\star$
\\[2ex]
\emkey{mark-name} ::= {\em mark-name-component}
                      {\em mark-name-component}\,$^\star$
\\[0.5ex]
\emkey{mark-name-component} ::= {\em non-special-mark-symbol}
			    $|$ {\em quoted-string}
\\[0.5ex]
\emkey{non-special-mark-symbol} ::=
    {\em mark-symbol} that is \underline{not} treated specially by the parser
\\[0.5ex]
\emkey{mark-symbol} ::= {\em mark} $|$ {\em separator}
\\[2ex]
...
\\[0.5ex]
\emkey{parsing-selector} ::= {\em name}
\\[2ex]
...
\\[0.5ex]
\emkey{opening-bracket-name} ::= {\em mark-name}
\\[0.5ex]
\emkey{closing-bracket-name} ::= {\em mark-name}
\\[0.5ex]
\emkey{reformatter-name} ::= {\em name}
\\[0.5ex]
\emkey{evaluator-name} ::= {\em name}
...
\\[0.5ex]
\emkey{indentation-mark-name} ::= {\em mark-name}
                              $|$ {\em gluing-indicator}
			          {\em gluing-indentation-mark-name}
\\[2ex]
\emkey{gluing-mark-indicator} ::= \ttmkey{*}{gluing mark}
\\[0.5ex]
\emkey{gluing-indentation-mark-name} ::= {\em mark} $|$ {\em quoted-string}
\\[2ex]
\emkey{indentation-separator-name} ::= {\em mark-name}
\\[2ex]
\emkey{named-bracket-definition} \\
    \hspace*{0.2in}
    \begin{tabular}[t]{rl}
    ::= & \ttkey{define named bracket}
	  \begin{tabular}[t]{@{}l@{}}
...
\emkey{named-bracket-name} \\
    \hspace*{0.2in}::= \begin{tabular}[t]{@{}l@{}}
	 ...
	 \end{tabular}
\\[0.5ex]
\emkey{named-opening} ::= {\em mark-name}
\\[0.5ex]
\emkey{named-separator} ::= {\em mark-name}
\\[0.5ex]
\emkey{named-middle} ::= {\em mark-name}
\\[0.5ex]
\emkey{named-closing} ::= {\em mark-name}

\label{EXPLICIT-SUBEXPRESSION-RECOGNITION-DEFINITIONS-2}
\end{figure*}

In explicit subexpression recognition, 
multi-lexeme symbol table labels are not recognized in the
...
parsing definition.  An example of such a definition is
\begin{center}
{\tt define bracket "("~...~")" [operator text]} \ldots
\end{center}
...
unnamed bracketed subexpression, or the end of the paragraph line
...
\ttkey{define indentation mark} parsing definition, and may have
an associated {\em indentation-separator-name} supplied by that definition.
An example is
\begin{center}
{\tt define indentation mark * ":"~...~";" [operator]} \ldots
\end{center}
...
An {\em indentation-mark-name} that is a single mark may be glued
to a preceeding mark at the end of its line.  For example, `{\tt :}'
may be glued to `{\tt =}' to make `{\tt =:}'.  In order for gluing
of an {\em indentation-mark-name} to be allowed, it must be identified
as a {\em gluing-indentation-mark-name} by preceding it with `{\tt *}' in the
{\tt define indentation mark} statement.

After an indentation mark is split from a previous mark at the end of
a line, the previous mark is checked to see if it by itself is
the label of a symbol table entry used by the explicit subexpression
parser, and is recognized if it is.
However, if it would be part of a \underline{multi-lexeme}
label, it is not recognized, as the parser scan does not back up
over previous lexemes after splitting the glued indentation mark.

If while parsing an indented paragraph a line is encountered whose
indentation is different from but less than $N$ (default $N=2$)
columns different from the paragraph indentation, an error is announced.
...
are parsing definitions, they are processed by the parser to make
modifications in the parser symbol tables.  Also, the initial
\key{top level paragraph indent}
is set by the first (non-comment) line of a file, but
if later on a line with smaller indent than the top level paragraph indent
is encountered, the top level paragraph indent is reset to the indent
of this line.
...
if by virtue of its indentation it
would terminate a paragraph line or paragraph that would not
be terminated by the first following non-comment line.
For example, a non-indented comment that appears between one line of
a paragraph line and an indented continuation line of that paragraph
line will announce an error but will not have any other affect.
...
\emkey{closing-named-bracket}
	\begin{tabular}[t]{@{}rl@{}}
	::= & {\em named-middle} {\em bracket-name} {\em named-closing} \\
...
and {\em named-closing} are marks from which named opening and closing
brackets are constructed.  They are specified by a
\ttkey{defined named bracket} parsing definition, an example of
which is:
\begin{center}
...
`{\tt lemma "5.1.1"}' and `{\tt lemma 5.1.1}' become idential
...
The parser runs a series of \skey{parser pass}{es} against the
input.  The top level pass is the
\key{explicit subexpression recognition pass}, which identifies
subexpressions with explicit opening and closing brackets
(such as `{\tt (}' and `{\tt )}').

Other parser passes take previously recognized subexpressions
as input, may modify these, and may recognize subexpressions of
these input subexpressions.
These other parser passes are called \skey{subexpression pass}{es}.
Each is a function that takes as input the sequence
of tokens in a subexpression and edits that token sequence.  The input does
\underline{not} include the brackets surrounding the
subexpression; an in fact, these brackets may be implicit.
...
begin unrecoginized escape seq atom table;
...
//
//      decimal integer
//          nothing but digits not begining with
//          0, with an optional suffix
//      octal integer
//          nothing but octal digits begining
