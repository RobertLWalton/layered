...
separator that begins a logical line.  These can be either
explicit or implicit.  We consider the explicit case first.

At the beginning of a logical line a \key{line beginning algorithm} described
in detail below is run to
identify paragraph and line headers.  In the absence of
implicit headers, the algorithm parses the beginning of
the logical line to see if it begins with a prefix separator.
If yes, the type of this is looked up in the parser
\key{header table} to see if the prefix separator is a paragraph or line
header.  It may or may not be.

The prefix separator can only be a paragraph header if the
logical line is in \key{paragraph beginning position}.
...
The \key{top level} is treaded as an indented paragraph with indentation \TT{0}.
A \key{comment line} is a line that contains nothing
but a single comment.

In addition, to be recognized as a paragraph header, the
prefix separator's type must be the name of an entry
in the parser header table that is selected by paragraph
selectors (see below) and
is marked as a paragraph header (as opposed to being marked
as a line header).

If an explicit paragraph header is recognized at the beginning
of a logical line, it is used to reset the line beginning variables
used by the
line beginning algorithm (see below), and then that algorithm looks for
a following line header.  If no explicit paragraph header is
recognized, the line beginning algorithm may insert
an implicit paragraph header; see below for details.
In either case, the algorithm continues by looking for a line header. 

The procedure for looking for a line header is analogous to that
of looking for a paragraph header.  Differences are that any
entry found in the parser's header table must be marked as a
line header, a line header is looked for at the beginning
of every logical line (for lines in paragraph beginning
position, the line header is looked for after any explicit
paragraph header), and the line header entry must be selected
by line selectors (see below).

The \key{line beginning algorithm} uses as context the following
\skey{line beginning variable}s:

\begin{indpar}[0.4in]

\hspace*{-0.2in}\ttmkey{paragraph lexical index}{line beginning variable}\\
    The lexical analyzer master is reset to this value
    at the beginning of a logical line that is in paragraph beginning
    position.  Then if this lexical index is not \TT{0},
    a header lexeme is read, and the string returned by
    this lexeme, if not empty, is looked up in the parser's header
    table to see if a selected paragraph header entry can be found
    and used to generate a paragraph header.  See details below.
\\[1ex]
\hspace*{-0.2in}\ttmkey{paragraph header}{line beginning variable}\\
    If not missing and no explicit paragraph header is found at the
    beginning of a logical line that is in paragraph beginning position,
    and no implicit paragraph header is generated by using the
    paragraph lexical index,
    then a prefix separator token whose value is a
...
\hspace*{-0.2in}\ttmkey{line lexical index}{line beginning variable}\\
    This is similar to the paragraph lexical index, but is used
    for all logical lines.
\\[1ex]
\hspace*{-0.2in}\ttmkey{line header}{line beginning variable}\\
    This is similar to the paragraph header, but is used
    to possibly insert an implicit line header.
\\[2ex]
