...
(prefix, infix, postfix, and nofix) to the operators.  Operators
are assigned precedence integers which are used to define
expression syntax.
...
where $P$ is the precedence of the {\em P-expression}.  For $P\ge H$,
a {\em P-expression} consists of a sequence of {\em (P+1)-expressions}
and infix and nofix operators of precedence $P$.
The case $P=H+1$ is special: an {\em (H+1)-expression} is a sequence
of zero or more prefix operators followed by a single {\em (H+2)-expression}.
An {\em (H+2)-expression} is similarly just an {\em (H+3)-expression},
which is an operand, followed by zero or more postfix operators.
