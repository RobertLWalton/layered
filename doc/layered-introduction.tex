...
\emkey{parser-flag-list}
    ...
    \end{tabular}
\\[0.5ex]
\emkey{parser-new-flag-list}
    \begin{tabular}[t]{@{}rl@{}}
    ::= & {\em parser-flag-list} \\
    $|$ & {\tt [} {\em parser-flag-op} {\em parser-flag}
          \{ \verb|,| {\em parser-flag-op} {\em parser-flag} \}\,$^\star$
	  {\tt ]} \\
    \end{tabular}
\\[0.5ex]
...
{\em parser-new-flag-list} containing {\em parser-flag-op} {\em parser-flag}
pairs.  In the last case the flag set is made by taking the current
flag set (trace flags or selectors) and modifying it according to the
instructions in the pairs.  Here `{\tt +} {\em flag-name}' is an instruction
to set the named flag, `{\tt -} {\em flag-name}' is an instruction
to clear the named flag, and `\CIRCUMFLEX {\em flag-name}' is
an instruction to complement, or flip, the named flag.
In either the case of a {\tt parser-flag-list} or
{\em parser-new-flag-list} no flag may be named more than once.
...
Also, all bracketed subexpression recognition symbol table
...
\emkey{parser-bracket-command} \\
    ...
    ::= & \multicolumn{2}{l}{\ttikey{parser define bracket}%
		    ...
		    {\em bracket-name} {\em parser-selectors}} \\
	& \hspace*{3em} & [ {\tt with parsing selectors}
	                    {\em parser-new-selectors} ] \\
        &               & [ {\tt with full lines} ] \\
...
\\[0.5ex]
\emkey{parser-new-selectors} ::= {\em parser-new-flag-list}
\\[0.5ex]
\emkey{reformatter-name} ::= {\em simple-name}

\label{BRACKET-DEFINITIONS}
\end{figure*}
