...
\label{PARSER-STRING_CONCATENATOR-COMMAND} ::= \\
\hspace*{3em} \ttikey{parser define string concatenator}%
                     {define string concatenator}
	  ~ {\em string-concatenator}
\\[0.5ex]
\emkey{string-concatenator} ::= {\em quoted-name} with a single component
		     $|$ \TT{disabled} $|$ \TT{enabled}
\end{indpar}

If the {\em string-concatenator} is `\TT{disabled}' quoted strings
...
can be printed by the \TT{parser print string concatenator} command:
...
\\
    ...
    ::= & \ttikey{parser print bracket}{parser print bracket}
	  {\em partial-name} \\
    $|$ & \ttikey{parser print indentation offset}{print indentation offset} \\
    $|$ & \ttikey{parser print string concatenator}%
