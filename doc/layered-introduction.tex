...
A parser command is a logical line that changes parser tables.
...
\emkey{parser-block-command}
    \begin{tabular}[t]{@{}rl@{}}
    ::= & \ttmkey{begin}{parser command} {\em block-name} \\
    $|$ & \ttmkey{end}{parser command} {\em block-name} \\
    $|$ & \ttmkey{print blocks}{parser command} \\
    \end{tabular}
...
and they are executed in order as soon as their containing indented
...
\label{PARSER-MAPPED-LEXEME-COMMAND} \\
    \hspace*{0.2in}
    \begin{tabular}[t]{rl}
    ::= & \ttmkey{define mapped lexeme}{parser command}
