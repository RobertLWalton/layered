...
Unlike brackets and indentation marks, prefixes types do {\bf not}
have to be defined in order to be used.  But a prefix type can be defined
so that it has special effects on parsing.  For this purpose
a `\key{prefix type}' is referred to as just a `\key{prefix}'.
...
\\
    ...
    ::= & \TT{parser}%
	  ...
	  {\em parsing-selectors} \\
	  {}[ \TT{with parsing selectors} {\em new-selectors} ] \\
	  {}[ \TT{with prefix level} {\em prefix-level} ] \\
	  {}[ \TT{with implied subprefix} {\em prefix-name} ] \\
...
\\[0.5ex]
\emkey{prefix-level} ::= {\em unsigned-integer}
\\[0.5ex]
...
\subsubsection{Implicit and Lexical Prefixes}
\label{IMPLICIT-AND-LEXICAL-PREFIXES}
