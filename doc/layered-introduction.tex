...
the entire token list, or by an infix or nofix operator
of precedence less than the precedence of the prefix operator,
or by a prefix operator of precedence less than or equal to
the precedence of the prefix operator,
...
or by an infix, nofix, or postfix
operator of precedence greater than or equal to
...
of precedence no greater than the precedence of the postfix operator,
else an `error nofix operator' is inserted after the postfix operator
and an error is announced.
\item[Rule 5:] A recognized infix operator must be followed by a non-operator
token or by a prefix, infix, or nofix operator
...
If $C=F$, infix and postfix operators of precedence greater than $P$
are ignored.  If $C\neq F$, prefix operators are ignored.
Prefix operators of precedence equal to or less than $P$ are ignored.
Any afix operator whose precedence is not in the $P$ stack is ignored.
Any prefix operator whose precedence is less than the top of the $P$
stack is ignored.
...
\item[Loop Step 3:]
If $C\neq N$, no operator was find in Loop Step 1, and the token before $C$
is a postfix operator, insert the error nofix operator as a single
...
has a precedence equal to or below the top of $P$, and if the
token before $C$ is a prefix or infix operator, insert
an error lexeme token before $C$,
announce an error, and then continue as if no error were found.
\item[Loop Step 6:]
If $C=N$ or the operator found in Loop Step 1 or generated by Loop Step 2
has precedence less than or equal to the top of the $P$ stack,
...
$C=N$, and in this case, terminate the algorithm.
\item[Loop Step 7:]
Push the location of the token after $C$ into the $F$ stack.
Into the $P$ stack push the precendence of the operator found
if that is infix or nofix, the precedence of the operator found
minus one if the operator is prefix, or the precedence of the
operator found plus one of the operator is postfix.
\item[Loop Step 8:]
Move $C$ to the next token and go to the next loop iteration.
\end{itemlist}
...
a sequence of operators and.  It uses an input list which it initializes to the parser's
