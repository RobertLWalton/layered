...
The precedence H+1 is reserved for the `error operator' that is
a nofix operator
inserted during parsing to `fix up' fixity errors (see \pagref{FIXITY-ERRORS}).
...
where N is any precedence in the range [L,H]
\end{tabular}
\end{indpar}
...
By choosing appropriate operator definition commands, one can build
...
\begin{enumerate}
\item Identifies sequences of lexemes that are operators, and
replaces each by a single \ttkey{OPERATOR} token with the operator as
its label.
...
entire expression as a subexpression.
\end{enumerate}
...
by a second left-to-right scan.
...
have ambiguous fixity or a fixity possibility-error.
\end{itemize}
...
operator, or a nofix operator of greater priority than the
infix operator.
\item[(D)]
An infix operator must be preceded by a non-operator, a postfix
operator, or a nonfix operator of greater priority than the
infix operator.
\item[(E)]
...
operator, an infix operator of priority lower than the nofix
operator, or the end of the expression.
\item[(H)]
A nofix operator will be preceded by a non-operator, a postfix
operator, an infix operator of priority lower than the nofix
operator, or the beginning of the expression.
\item[(I)]
A non-operator will be followed by a non-operator, a postfix
operator, an infix operator, a nofix operator,
or the end of the expression.
\item[(J)]
A non-operator will be preceded by a non-operator, a prefix
operator, an infix operator, a nofix operator,
or the beginning of the expression.
\item[(K)]
The beginning of expression is followed by a prefix operator,
a nofix operator, or a non-operator.
\item[(L)]
...
of operators with ambiguous fixity.  In these `ambiguous'
...
rewrite rule does not change anything (i.e., there is no
rewrite rule for the indicated case).
...
non-operator & infix & non-operator infix\OPSUP{--O} \\
...
These rewrites are arbitrary in the sense that they
