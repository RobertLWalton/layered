...
might be returned as a 1-character lexeme of kind `{\tt erroneous
...
It is incompatible with the {\tt keep} and {\tt translate}\ldots{} instruction
components.

In the absence of any {\tt accept}, {\tt keep}, or {\tt translate}
instruction components, {\tt accept} is assumed.
...
are supposed to be hexadecimal digits are not.
...
\verb|    "<letter>" else translate "" error bad character in identifier;|
\end{center}
...
announces the current atom as an erroneous atom of the given {\em kind}.
...
"\\<other>" translate "\u007f" error bad escape sequence;
\end{verbatim}\end{indpar}
There might be entries in an atom table for a quoted string.
...
also popped from from the stack.
...
type	& The type of the token.  One of: \\
        & ~~~~\begin{tabular}{l}
	      the type of a lexeme \\
	      bracketted expression \\[1ex]
	      \end{tabular} \\
\end{tabular} \\[1ex]
\begin{tabular}{p{1in}p{4in}}
string	& The character string of the token, if the token is
	  a lexeme and not a symbol.  Otherwise missing.
\\[1ex]
symbol	& A value unique to the character string of the token.
	  No two lexemes with the same character string
	  can have the same symbol.  This is used to quickly
	  compare identifiers, operators, punctuation marks, etc.
	  The character string of a token may be retrieved
	  using the symbol.
\\[1ex]
hash	& A 32 bit unsigned integer that is a randomized hash of
          the character string of a symbol.
\end{tabular} \\[1ex]
\begin{tabular}{p{1in}p{4in}}
positions
	& The positions of the first and last character of the
	  token in the input tex.
\end{tabular}
\end{indpar}
...
begin master atom table;
   ...
   // Whitespace is separated out into 3 kinds of lexeme:
   //
...
begin quoted string atom table;
    ...
    "<line-break-char>"
        goto premature end of line in quoted string;
    "\\" error bad escape sequence in quoted string;

    "<other>" accept;

    goto premature end of file in quoted string;

end quoted string atom table;


begin premature end of line in quoted string;
    mode master;
    output premature end of line in quoted string
    	   goto master;
end premature end of line in quoted string;


begin premature end of file in quoted string;
    mode master;
    output premature end of file in quoted string
    	   goto master;
end premature end of file in quoted string;
