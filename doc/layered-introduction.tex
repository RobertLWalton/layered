...
\item[Requirement 2:] 
All prefix {\tt OPERATOR} tokens in the list are preceded by {\tt OPERATOR}
tokens or by the beginning of the expression.
A prefix operator cannot be at the end of the expression.
\item[Requirement 3:] 
All postfix {\tt OPERATOR} tokens in the list are followed by {\tt OPERATOR}
tokens or by the end of the expression.
A postfix operator cannot be at the beginning of the expression.
\item[Requirement 4:] 
...
\item[Rule 2:] An infix operator
must be preceded by a non-operator,
or by an operator of precedence greater than that of the
infix operator,
else the infix operator is not recognized.
\item[Rule 3:] A postfix operator
must be preceded by a non-operator,
or by an operator of precedence greater than or equal to that of the
postfix operator,
else the postfix operator is not recognized.
\item[Rule 4:] A recognized postfix operator must be followed by the end of
the entire token list or by an operator of precedence equal to or less
than that of the postfix operator,
else an `error nofix operator' of the same precedence as the postfix
operator is inserted after the postfix operator and an error is announced.
\item[Rule 5:] A recognized prefix
operator must be followed by a non-operator
token or by an operator
of precedence equal to or greater than the precedence of the prefix operator,
else an `error operand' which is not an operator
is inserted after the prefix operator
and an error is announced.
\item[Rule 6:] A recognized infix
operator must be followed by a non-operator
token or by an operator
...
an operator of precedence $P$), infix operators of precedence
greater than or equal to $P$ are ignored,
postfix operators of precedence greater than $P$ are ignored,
and prefix operators of precedence less than $P$ are ignored.
...
postfix operator (this must be a prefix operator),
...
\item[Loop Step 5:]
If $C=N$ or the operator found in Loop Step 1 or generated by Loop Step 2
is an operator with precedence $P'$ less than or equal to $P$,
and if the list of tokens beginning with $F$ and ending just before $C$
is not empty,
...
Else if $P'$ is less than $P$, pop the $P$ and $F$ stacks and repeat
this loop step.

Else if $P'$ is greater than $P$, push $P'$ and $C$ into the $P$ and
$F$ stacks respectively.

Else if $P'$ equals $P$, set $F$ to $C$.

\item[Loop Step 6:]
Move $C$ to the next token and iterate the loop.
\end{itemlist}
