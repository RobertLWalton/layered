...
see \itemref{UNINDENTED-PARAGRAPHS}.
\\[2ex]
...
For prefixes that are \TT{.types} of implied paragraph 
or implied line header lexemes, these values may be used
to set line variables: see \pagref{LINE-VARIABLES}.

For prefixes that are values in a mapped lexeme table entry,
the \TT{lexical master} value may be used to set the mapped lexeme
table entry \TT{lexical master} value in some cases:
see \pagref{TBD}.
...
\\[0.5ex]
\emkey{mapped-lexeme-name} ::= {\em quoted-name}
\\[0.5ex]
\emkey{token-type} ::= {\em quoted-name}
\\[0.5ex]
\emkey{token-value} ::= TBD
\\[0.5ex]
\emkey{lexical-master} ::= {\em quoted-name}
\end{indpar}
