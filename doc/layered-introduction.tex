...
(the doubled quotes \verb|""| do nothing but prevent recognition of
\verb|"<lf>"| as representing a line feed)
...
\item force a containing atom table to fail to match any atom
\end{itemize}
...
an atom pattern.  An instruction group is a sequence of instructions
...
then a scan error is signalled.
...
that has the form \verb|"\/|C\ldots\verb|"| where C is a character
...
begin pp include master table;
    ...
    "\"/" translate to "" call quoted header name;
    ...
    "\"/" translate to "" call string literal;
    "L\"/" translate to "" call wide string literal;

    "<op-char-except-<>" translate to operator
    			 output operator
          else output unrecognized character;
...
begin wide string literal lexeme table;
    "\"/" translate to "" goto normal line master;
