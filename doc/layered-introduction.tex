...
*PARSER*:
    ...
    define bracket type "raw data"
           ...
           with data reformatter ( "=", "no", "[", "{*" )
    define bracket type "code" [TOP LEVEL, code, text]
           with group paragraph
...
\TT{line lexical master} or is a \key{reset header}, which is
a header with a bracket type definition that has the
`\TT{reset}' group.  An isolated header must appear at the
...
by parameters from the header bracket type definition
is used.
...
the parameters in the headers bracket type definition (one of
which must be a non-missing line lexical master).

An isolated paragraph header may be implied.  In this case it
is effectively placed on a line by itself and deleted only if
it is not followed by any (possibly headed) logical lines.

A reset header sets the paragraph line variables
to those of the containing indented paragraph or the top level
if there is no containing indented paragraph, as modified by
the parameters in the headers bracket type definition.
Then the reset header is deleted and the next logical line
is treated as being in paragraph beginning position.
...
\hspace*{-0.2in}\ttmkey{implied header}{line variable}\label{IMPLIED-HEADER}\\
    ...
    header.  This is because if the
    logical line begins with an explicit header, that will be
    parsed and its bracket type table entry will be looked up
    ...
    is \underline{not} re-parsed and its bracket type table entry
...
\item If the \TT{implied\_header} current line variable value is not missing,
...
variable selectors and options,
the lexical masters of the implied header's bracket type definition
are stored in the current line variable lexical masters, and
the implied subprefix of the bracket type definition is stored in
the current line variable implied header variable.
...
If the above resets the implied header current line variable to a non-missing
value, that value must have the `\TT{line}' group when its bracket definition
is looked up using the new selectors current line variable value.
As this new implied header current line variable value
is also the implied subprefix of the implied paragraph header that
has just been inserted at the beginning of the current logical line, it will
also be inserted as an implied subprefix just after the inserted implied
paragraph header and treated as an implied line header of the current
logical line.
...
If an explicit header is found, then after any implied headers are deleted
and state has been restored, the explicit header's bracket type definition
is used to modify the parsing selectors and options, and if the header is
a paragraph header, the current line variables selectors, options,
lexical masters, and implied header.
Here the implied header is copied from the explicit header's bracket type
definition only if it has the `\TT{line}' group when its bracket type
definition is looked up with the new selectors current line variable.

\item\label{RESET-RULE}
If an explicit paragraph header sets the `\ttkey{reset}' option flag, then
...
\item
If an explicit paragraph header does \underline{not}
set the `\TT{reset}' option flag, then

\begin{enumerate}[label*=.\arabic*]

\item\label{STICKY-RULE}
If the explicit paragraph header sets
the `\ttkey{sticky}' option flag, then
