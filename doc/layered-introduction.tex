...
\begin{tabular}{p{1in}p{4.5in}}
		& A \TT{BRACKETABLE} token
		  ...
		  either an \TT{.initiator} or \TT{.terminator} attribute.
		  If such a token is to become the only element of a
		  containing subexpression that has no attributes other
		  than \TT{.initiator} or \TT{.terminator}, 
		  then instead of creating a new
		  \TT{BRACKETED} token, any \TT{.initiator} or
		  or \TT{.terminator} are added to the \TT{BRACKETABLE}
		  token, whose type is changed to \TT{BRACKETED}, and
		  whose token position and \TT{.position} attribute are
		  updated to that of its containing subexpression.
\end{tabular}
\\[1ex]  
\begin{tabular}{p{1in}p{4.5in}}
