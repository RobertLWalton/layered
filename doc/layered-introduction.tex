...
\emkey{word} :::= {\em middle-lexeme} containing a {\em letter} that is not
                  a {\em number}
\\[0.5ex]
\emkey{mark}\label{MARK} :::= {\em middle-lexeme} that is
                              not a {\em word} or {\em number}
\\[0.5ex]
...
\\[1ex]
		& For a lexeme, the translation of the
		  ...
		  \TT{value} as a MIN string.
\end{tabular}
\\[1ex]  
\begin{tabular}{p{1in}p{4.5in}}
		& Standardly the token lexeme types are word, mark, separator,
		  natural-number, and numeric
		  whose tokens have a \TT{value} that is
		  a MIN string equal to the lexeme characters,
		  quoted string whose tokens
		  have a \TT{value} equal
		  to the lexeme translation string,
		  and comment, indent, line break, start of file,
		  and end of file whose tokens have no \TT{value}.
...
\hspace*{-0.2in}\ttmkey{enable indented paragraph}{parser option}%
    ...
    This option is on by default at top level (else parsing commands
...
Paragraph Options Continued:

\begin{indpar}

\hspace*{-0.2in}\ttmkey{reset}{parser option} \\
    A paragraph header with this option resets the line variables
    (including the implied header) to their values at the beginning
    of the current indented paragraph, or if outside an indented
    paragraph, the values at the beginning of the file or after
    the last `\TT{define top level}' parser command.  The paragraph
    header is then deleted and not used to head a paragraph.
    See \ref{RESET-RULE}, \pagref{RESET-RULE}.
...
\hspace*{0.3in}\begin{tabular}{rl}
    & \TT{define}\ttmindex{define top level}{parser command}
      ...
      ~ \begin{tabular}[t]{@{}l@{}}
       \TT{top level} \\{}
       [ \TT{with parsing selectors} ~ {\em top-level-selectors} ] \\{}
       [ \TT{with parsing options} ~ {\em top-level-options} ] \\{}
...
\emkey{top-level-selectors} ::=
    \begin{tabular}[t]{l}
    {\em selector-spec} without any `\TT{TOP LEVEL}' selector
...
are `\ttkey{TOP LEVEL}', `\ttkey{data}', and `\ttkey{atom}'.
...
named `\ttmkey{standard}{parsing block}'%
\label{STANDARD-PARSING-BLOCK}
and defines in it the selectors
\begin{center}
...
*PARSER*:
    ...
    define selector data paragraph
\end{verbatim}\end{indpar}
...
The \TT{TOP LEVEL} selector\label{TOP-LEVEL-SELECTOR}
is always set implicitly by this command, and is
...
top level selectors.  The \TT{TOP LEVEL} selector is also always cleared
...
\begin{center}
\TT{with paragraph lexical master} {\em lexical-master-name}
\end{center}
sets the paragraph lexical master,
\begin{center}
\TT{with line lexical master} {\em lexical-master-name}
\end{center}
...
\end{verbatim}\end{indpar}
is included in the `\TT{standard}' parser block
(\pagref{STANDARD-PARSING-BLOCK}).  Note that the
\TT{TOP LEVEL} selector (\pagref{TOP-LEVEL-SELECTOR})
is implicitly set by this command, and as the parsing
options are not changed, the `\TT{default}' options
set when the parser was initialized remain in effect
(\pagref{DEFAULT-OPTIONS}).
...
\\[0.5ex]
\emmkey{ID-character}{in \TT{data} reformatter}\label{ID-CHARACTER} ::=
    \begin{tabular}[t]{l}
    a unicode character that is a parser parameter (see below); \\
    it must be a non-digit that can begin a numeric
    \end{tabular}
\\[0.5ex]
...
The {\em ID-character} is a parser parameter that can be defined
or printed by:
\begin{indpar}[0.1in]
...
\hspace*{3em} \begin{tabular}{rl}
              ...
              $|$ & \ttmkey{print ID character}{parser command} \\
	      \end{tabular}
\\[0.5ex]
...
is replaced by its ID value, which is looked up in the parser's
\mkey{ID map}{of parser}.  If there is no previous value, a preallocated
stub is created and assigned as the value.

The ID map used to map IDs is defined separately for
each input stream of tokens.  {\em IDs} must be mapped to values before
they are used, although in the case of objects, which have stubs,
the values can initially be preallocated stubs that have identity but
do not yet have any content.  In this last case, object content
can be assigned by the \TT{data} reformatter: see \pagref{DATA-REFORMATTER}.

When a parser is initialized, the ID character is disabled.  The
ID definition
\begin{indpar}\begin{verbatim}
*PARSER*:
    define ID character "@"
...
scanning a special lexeme (usually) does not consume these characters,
...
\ttmkey{DATA}{special lexeme}
\begin{indpar}
Output by the \TT{DATA-\EOL CHECK} and \TT{PARAGRAPH-\EOL CHECK}
lexical masters when the next input character is `\TT{@}'.
...
\begin{indpar}
Output by the \TT{DATA-\EOL CHECK} and \TT{PARAGRAPH-\EOL CHECK}
lexical masters when the next 2 input characters are `\TT{!@}'.
Typically mapped to the `\TT{\{raw data\}}' prefix separator.
\end{indpar}

\ttmkey{TABLE}{special lexeme}
\begin{indpar}
Output by the \TT{PARAGRAPH-\EOL CHECK} lexical master
...
input.  The standard lexical masters are:
...
according to the current parser selectors.  If an entry is found,
...
If the mapped lexeme symbol table entry has no {\em token-value},
the token which selected the entry is deleted, and not altered.
If the entry has no {\em lexical-master-name}, the lexical master
...
When a parser is initialized, no lexemes are mapped.  The
lexeme mapping definitions
\begin{indpar}\begin{verbatim}
*PARSER*:
    define mapped lexeme "DATA" [TOP LEVEL]
           with token value {data}
    define mapped lexeme "RAW-DATA" [TOP LEVEL]
...
\end{verbatim}\end{indpar}
are included in the `\TT{standard}' parser block
(\pagref{STANDARD-PARSING-BLOCK}).

When a parser is initialized, its paragraph and
line top level lexical masters (\pagref{LEXICAL-MASTER-PARAMETERS})
are both set to the \TT{DEFAULT} lexical master
which does \underline{not} recognize special lexemes.  The
`\TT{standard}' parser block (\pagref{STANDARD-PARSING-BLOCK}) contains
\begin{indpar}\begin{verbatim}
*PARSER*:
    define top level
        with parsing selectors [code]
        ...
        with line lexical master "DATA-CHECK"
\end{verbatim}\end{indpar}
which sets the lexical masters as indicated so that the special
lexemes above are recognized in certain contexts.
...
When a parser is initialized, the middle break is disabled. The
`\TT{standard}' parser block (\pagref{STANDARD-PARSING-BLOCK}) contains
the definition
\begin{indpar}\begin{verbatim}
*PARSER*:
...
When a parser is initialized, the quoted string concatenator is disabled. The
`\TT{standard}' parser block (\pagref{STANDARD-PARSING-BLOCK}) contains
the definition
\begin{indpar}\begin{verbatim}
*PARSER*:
...
\emkey{reformatter-arguments}\label{REFORMATTER-ARGUMENTS}
    ...
    \end{tabular} \\
\emkey{reformatter-argument} ::= {\em empty}
                             $|$ {\em quoted-key} $|$ {\em simple-name}
			     $|$ {\em argument-sublist} \\
...
the `\TT{TOP LEVEL}' selector is always turned off.
...
{\em natural-numbers}, {\em numerics}, {\em quoted-strings},
or MIN label values.
The value of an element is taken to be the MIN string containing the
element lexeme or the MIN label value of a non-lexeme element.
...
When a parser is initialized, the following
definitions\label{UNTYPED-BRACKET-BULITINS}
are included in the \TT{TOP LEVEL} parser block:
\begin{indpar}\begin{verbatim}
*PARSER*:
     define bracket "(" ... ")" [data]
     define bracket "[" ... "]" [data]
...
\end{verbatim}\end{indpar}
The `\TT{standard}' parser block (\pagref{STANDARD-PARSING-BLOCK})
...
are not modified, if no {\em indentation-mark-options} 
...
When a parser is initialized, the following
definitions are included in the \TT{TOP LEVEL} parser block:
\begin{indpar}\begin{verbatim}
*PARSER*:
    define indentation offset 2

    define indentation mark "TOP LEVEL" []
        // Indentation mark definition implicitly governing
        // top level parsing.  May NOT be explicitly used
        // or redefined.  Has NO line separator.
...
*PARSER*:
     define indentation mark ":" ... ";" [code]
         with parsing options [+ default end at options,
                               - non-default end at options]
         with paragraph lexical master "PARAGRAPH-CHECK"
         with line lexical master "DATA-CHECK"
     define indentation mark ":" [text]
         with parsing options [+ default end at options,
                               - non-default end at options]
         with paragraph lexical master "PARAGRAPH-CHECK"
         with line lexical master "DATA-CHECK"
         with implied header {p}
     define indentation mark ":" [data paragraph]
         with parsing selectors [data]
         with parsing options [+ default end at options,
                               - non-default end at options]
         // This definition is the last defined and
         // the first examined by searches.
\end{verbatim}\end{indpar}
...
`\TT{[- data, - TOP LEVEL]}' to be the {\em typed-parsing-selectors} \\
~~~~~~here `\TT{- TOP LEVEL}' is implied. \\
`\begin{tabular}[t]{l}
...
Since `\TT{with parsing options}'
does not appear in either definition, the parsing options used
...
found.  If no defintion is found, the {\em typed-parsing-selectors} of the
...
\emmkey{indented-attributes}{in \TT{data} reformatter} ::=
    \begin{tabular}[t]{l}
    ~~~~~~~~~~{\em indentation-mark} {\em end-of-physical-line} \\
...
The {\em ID} (first element) of the headed line must be either
a preallocated stub or an object that has replaced a preallocated
stub.  In the latter case the object may not have any attributes
with flags or any single attribute values; but it may have
double attribute values.  Also the {\em prefix}
must not have any associated prefix attributes (other than
\TT{.type} or \TT{.position}).  If these requirements are not met, the
\TT{data} reformatter takes no action, and the {\em prefix-n-list}
beginning with the \TT{data} prefix
is retained as is in the input.

A subexpression of this form typically arrises when a logical line
that begins with an {\em ID} ( a lexeme consisting of an
{\em ID-character}  immediately followed by a {\em natural-number};
see \pagref{ID-CHARACTER}) causes a special lexeme with a special
lexeme type and empty character string to be prefaced
to the line, and the type of this special lexeme
is mapped (\itemref{MAPPED-LEXEMES}) to the prefix of the
above logical line, e.g., to \TT{\{data\}} or \TT{\{raw data\}}.

The effect of reformatting is to delete the entire subexpression token list
and to define or add to the object whose denoted by
{\em ID}.  The object is given the
elements given in the {\em element-list} and the attributes in the
{\em indented-attributes}.
Before this reformatter
is invoked,
the {\em indented-attributes} are parsed
to become a MIN object
with \TT{min::\EOL INDENTED\_\EOL PARAGRAPH()} \TT{.terminator} and logical
line elements.  This reformatter
...
(see \pagref{TYPED-BRACKETED-SUBEXPRESSION}).
If {\em ID} is an object with double attribute values, any
new double attribute values will add to existing double attribute value
\underline{sets} (not multi-sets).

As a special case, if there is only one {\em element}, no {\em attributes},
...
Subexpression tokens recognized by this reformatter are deleted from the input
and have no effect other than setting the {\em ID} to the
desired value.
...
\ttmkey{sentence}{reformatter} ~ \TT{(} {\em sentence-prefix}
...
\begin{indpar}
\emkey{sentence-prefix} ::=
    {\em empty} $|$ {\em simple-name} $|$ {\em quoted-key}
\\[0.5ex]
...
If the {\em prefix-n-list} does not contain a {\em terminator},
nothing is done.

If {\em sentence-prefix} is {\em empty} (i.e., missing)
and the {\em prefix-n-list} does not begin with a {\em prefix},
nothing is done.

Otherwise if {\em sentence-prefix} is \underline{not} {\em empty}
and the {\em prefix-n-list} does not begin with a {\em prefix} whose
\TT{.type} equals {\em sentence-prefix},
the {\em prefix} `\TT{\{}{\em sentence-prefix}\TT{\}}' is
pre-pended to the {\em prefix-n-list} before the following is done.

At this point the {\em prefix-n-list} has the form
`{\em prefix} {\em element-list}', and the {\em element-list}
contains at least one {\em terminator}.  The 
{\em element-list} is then parsed according to the rules:
\begin{indpar}
...
The \TT{sentence} reformatter converts the {\em prefix-n-list} to a sequence
of subexpressions, one for each {\em sentence} and one for any
non-empty final {\em phrase}.  Each {\em sentence} subexpression has
the {\em phrase-elements} of the corresponding {\em sentence} as elements,
and the attributes of the {\em prefix} as attributes,
with the addition of
a \TT{.terminator} attribute equal to the {\em terminator}
of the {\em sentence}.
Any final {\em phrase} is converted to a list of its {\em phrase-elements}
without attributes.
...
After a parser is initialized, it has \underline{no} bracket type
definitions.
The `\TT{standard}' parser block (\pagref{STANDARD-PARSING-BLOCK})
adds the definitions:
\begin{indpar}\begin{verbatim}
*PARSER*:
    define bracket type "table"
                        [TOP LEVEL, data, code, math, text]
           with group paragraph
           with line lexical master "ROW-CHECK"
    define bracket type "row"
                        [TOP LEVEL, data, code, math, text]
           with group line
    ...
    define bracket type "***" [TOP LEVEL, code, text]
           with group reset
    define bracket type "section" [TOP LEVEL, code, text]
           with group paragraph
    ...
    define bracket type "p" [TOP LEVEL, code, text]
           with group paragraph
           with parsing selectors [+ text,
                                   - code, - math, - data]
    ...
    define bracket type "s" [text]
           with sentence reformatter
                ( ".", "?", "!", ":", ";" )
    define bracket type "quote" [TOP LEVEL, code, text]
           with group paragraph
           with parsing selectors [+ text,
                                   - code, - math, - data]
           with parsing options [+ sticky,
...
lexeme, this last lexical master sees the first \TT{=} and
...
top level line lexical master is installed, which is
...
set by the `\TT{define top level}' command, and the
...
An \key{isolated header} is paragraph header that resets the
\TT{line lexical master} and is not a mapped header,
or is a reset header.  A \key{mapped header} is an explicit header
that was created at the beginning of a logical line
when a mapped lexeme was mapped to a prefix.
A \key{reset header} is a prefix with the
`\TT{reset}' group.  Isolated headers and mapped headers
...
\hspace*{-0.2in}\TT{paragraph line lexical master}
    ...
    by the \TT{define top level} parser command (\pagref{DEFINE-TOP-LEVEL}).
...
\emkey{parser-pass-command} \\
    ...
    $|$ & \ttmkey{undefine pass}{parser command}
	  {\em parser-pass-name} \\
    $|$ & \ttmkey{define standard}{parser command}
          {\em parser-pass-name}\QMARK{} \\
...
Each pass has a standard set of definitions.  The
The \TT{parser define standard} statement is equivalent to
define statements for all the standard definitions of a pass.
However this statement will put these definitions in the current
parser block, and not necessarily in the `\TT{standard}' parser block.
If no {\em parser-pass-name} is given, this statement issues all the
non-pass-specific definitions that the `\TT{standard}' block would
normally contain, including beginning a new `\TT{standard}' parser
block.
...
The operator parsing pass identifies operators, assigns fixity
(prefix, infix, postfix, and nofix) to the operators, and parses
expressions using operator fixity and operator precedence.  Operator
definitions assign precedence integers to operators.
Infix and nofix operators have precedences in the range [L,H],
where L = -1,000,000 and H = +1,000,000.  Prefix operators have
precedence H+2, and postfix operators have precedence H+3.
The precedence H+1 is reserved for the `error operator' which is
a nofix operator inserted during parsing to `fix up' errors found
while computing fixity so parsing can continue
(see \pagref{FIXITY-ERRORS}).
...
\emkey{(H+2)-expression}
    ...
    \end{tabular}
\\[0.5ex]
\emkey{(H+3)-expression}
    \begin{tabular}[t]{@{}rl}
    ::= & {\em (H+4)-expression} \\
    $|$ & {\em (H+3)-expression} {\em postfix-operator} \\
    \end{tabular}
\\[0.5ex]
\emkey{(H+4)-expression} ::= {\em non-operator}\PLUS{}
\\[2.0ex]
...
of zero or more prefix operators followed by a single {\em (H+3)-expression},
is parsed so as to execute the prefix operators from right to left, each
with a single operand.
Similarly, an {\em (H+3)-expression}, while it can be thought of as
an {\em (H+4)-expression} followed by a sequence
of zero or more postfix operators,
is parsed so as to execute the postfix operators from left to right, each
with a single operand.

An infix operator must be surrounded by operands, but a nofix operator
need not have adjacent operands.

By choosing appropriate operator definitions, one can build
a reasonable expression syntax consisting of nothing but nofix operators,
while using operator expression reformatters to check whether the
operators in an expression have sufficient operands.  However, with
nofix operators an operator cannot have different precedence when it
is infix and when it is prefix.  An operator like `\TT{-}' can be
specified as both infix and prefix, where the prefix operator has
much higher precedence (i.e., H+2) than the infix operator.
...
\emkey{operator-flag} ::= {\em operator-fixity}
                      $|$ \ttmkey{afix}{operator flag}
	\\[1ex]
\emkey{operator-fixity} ::= \ttmkey{prefix}{operator fixity}
                        $|$ \ttmkey{infix}{operator fixity}
                        $|$ \ttmkey{postfix}{operator fixity}
                        $|$ \ttmkey{nofix}{operator fixity}
	\\[1ex]
\emkey{precedence} \begin{tabular}[t]{rl}
		   :::= & {\em sign-option} {\em natural-number}
			  in the range [L, H] \\
...
Name and Selectors \\
Operator Flags \\
Precedence [for infix and nofix operators] \\
Reformatter
\end{indpar}
...
affect parsing of subexpressions of the operator.  The fixity flags are
`\TT{infix}', `\TT{prefix}', `\TT{postfix}', and `\TT{nofix}'.
An operator definition must have at least one of these fixity flags.
In addition there is an optional `\TT{afix}' flag, which in spite
of its name is \underline{not} a fixity flag.

The definition must have a {\em precedence} if and only if it is
nofix or infix.
...
An infix operator may also be either prefix or postfix.
An infix operator has the precedence given in the operator definition.
A prefix operator always has precedence H+2, and a postfix operator
always has precedence H+3.  An operator may be both prefix and postfix,
but no operator may be all three of infix, prefix, and postfix.

A \ttkey{nofix} operator may or may not be
...
A nofix operator has the precedence given in the operator definition.
A nofix operator \underline{cannot} also be either prefix or postfix.

An operator whose definition has the `\ttkey{afix}' flag\label{AFIX-FLAG}
must follow another operator that has the same precedence in a subexpression,
and will be part of the subexpression containing that previous operator.
There can be no operator of lower precedence between this previous
operator and the afix operator.  Infix and nofix operator definitions
may be given
the afix flag.  Prefix and postfix operator definitions \underline{cannot} be
given the afix flag.

As an example, standardly \TT{bracket "\{" ... "\}"} is a nofix
operator with the afix flag and the same precedence as the
as the \TT{"<-{}-"} operator.  Therefore the expression
\begin{center}
...
An operator must be prefix, infix, postfix, or nofix.
An operator may be any two of prefix, infix, and postfix, but
not all three.  An afix operator must be infix or nofix,
and not prefix or postfix.
...
The operator flag compatibility rules
are expressed by the following table:

\begin{center}
\begin{tabular}{lc|c|c|c|c|}
	& \multicolumn{5}{c}{is compatible with op-flag} \\
\cline{2-6}
\multicolumn{1}{l|}{op-flag} &
prefix	& infix	& postfix & nofix & afix \\
\hline
\multicolumn{1}{|l|}{prefix} &
	& yes	& yes	& no	& no	\\
\hline
\multicolumn{1}{|l|}{infix} &
yes	&	& yes	& no	& yes	\\
\hline
\multicolumn{1}{|l|}{postfix} &
yes	& yes	&	& no	& no	\\
\hline
\multicolumn{1}{|l|}{nofix} &
no	& no	& no	&	& yes	\\
\hline
\multicolumn{1}{|l|}{afix} &
no	& yes	& no	& yes	&	\\
\hline
\end{tabular}

\medskip
An operator must be at least one of prefix, infix, postfix, or nofix.

No operator can be all of prefix, infix, and postfix.
\end{center}
...
For an operator that is both prefix and infix, the same reformatter
is called when the operator is prefix and when the operator is infix.
Similarly for an operator that is both postfix and infix, or both
prefix and postfix.
...
The standard operators are given in Figure~\ref{STANDARD-OPERATORS}.
...
\hline
prece-  &               &       &               & \\
~dence  & reformatter	& type	& operator	& meaning 
\\\hline
0000	& \ttkey{declare}	& \TT{nofix}	& \ttkey{<-{}-}	& define \\
	&			& \TT{afix}	& \TT{bracket}  & block \\
	&			& 		& \ttkey{~~"\{\$" ...~"\$\}"}
	                                        & \\
	&			& \TT{afix}	& \TT{indentation}  & block \\
	&			& 		& \TT{~~mark} \ttkey{":"} & 
\\\hline
...
3000	& \ttkey{binary}	& \TT{nofix}	& \ttkey{BUT NOT}
								& logical and
								  not
\\\hline
3100	& \ttkey{infix}		& \TT{nofix}	& \ttkey{AND}	& logical and \\
	&			&		& \ttkey{OR}	& logical or
\\\hline
3100	& \ttkey{unary}		& \TT{nofix}	& \ttkey{NOT}	& logical not
\\\hline
4000 	& \ttkey{infix and}	& \TT{nofix}	& \ttkey{==}	& equal \\
	&			&		& \ttkey{/=}  & not equal \\
	&			&		& \ttkey{!=}	& ditto \\
	&		      	&		& \ttkey{<}	& less than \\
	&			&		& \ttkey{<=}	& less than or
	...
	&			&		& \TT{=>}	& ditto
\\\hline
5000	& \ttkey{sum}		& \TT{infix} and & \TT{+}	& addition \\
	&			& \TT{~~prefix}	& \TT{-}	& subtraction
\\\hline
...
token.
\item Determines the fixity of each operator: prefix, infix,
postfix, or nofix.
\item Identifies subexpressions using operator precedence and fixity.
...
with the first operator as it \TT{.operator} attribute.
\end{enumerate}
...
and most have had their fixity determined as per
\itemref{FIXITY-DETERMINATION}.  Subexpression recognition
is done after operators and fixity have been determined, and can be done
...
If an operator definition has the afix flag, the definition
is \underline{ignored} unless all of the following \underline{would} be true
after insertion of the afix definition's operator
and determination of fixity of any operators of ambiguous
fixity preceeding the current position:
\begin{itemize}
...
\item[(3)] The inserted
operator has not interacted with operators immediately preceding
the current position to cause an operator to
have ambiguous fixity or a fixity not-allowed error.
\end{itemize}
See \itemref{FIXITY-DETERMINATION} for a description of fixity rules.

Aside from application when an afix definition operator is considered
for insertion, operator definition fixity
is ignored during operator identification.
...
tokens between it and the preceeding operator, or expression beginning
if there is no preceeding operator,
...
the next pass is called, and if that does not make the subexpession
into a single token, the subexpression is made into a single
...
\subsubsubsection{Fixity Determination}
\label{FIXITY-DETERMINATION}

An operator definition may have ambiguous fixity.  In
...
Operator fixity is determined during the left to right scan
using the following rules:

\begin{itemize}
\item[(A)]
A prefix operator must be followed by a non-operator or another
prefix operator.
\item[(B)]
A prefix operator must be preceded by an infix operator, a nofix
operator, a prefix operator, or the beginning of the expression.
\item[(C)]
An infix operator must be followed by a non-operator, a prefix
operator, or a nofix operator of greater precedence than the
infix operator.
\item[(D)]
An infix operator must be preceded by a non-operator, a postfix
operator, or a nonfix operator of greater precedence than the
infix operator.
\item[(E)]
A postfix operator must be followed by an infix operator, a nofix
operator, a postfix operator, or the end of the expression.
\item[(F)]
A postfix operator must be preceded by a non-operator or another
postfix operator.
\end{itemize}

Recall that two non-operators next to each other.

If these rules are obeyed, then the following will be true:

\begin{itemize}
\item[(G)]
A nofix operator will be followed by a non-operator, a prefix
operator, an infix operator of precedence lower than the nofix
operator, a nofix operator, or the end of the expression.
\item[(H)]
A nofix operator will be preceded by a non-operator, a postfix
operator, an infix operator of precedence lower than the nofix
operator, a nofix operator, or the beginning of the expression.
\item[(I)]
A non-operator will be followed by a postfix
operator, an infix operator, a nofix operator,
or the end of the expression.
\item[(J)]
A non-operator will be preceded by a prefix
operator, an infix operator, a nofix operator,
or the beginning of the expression.
\item[(K)]
The beginning of expression is followed by a prefix operator,
a nofix operator, a non-operator, or the end of the expression.
\item[(L)]
The end of expression is preceded by a postfix operator,
a nofix operator, a non-operator, or the beginning of expression.
\end{itemize}

\newcommand{\OPSUP}[1]{$^\textrm{\scriptsize #1}$}
The above rules lead to the \key{fixity rewrite rules}
in Figures \ref{FIXITY-REWRITE-RULES-1},
\ref{FIXITY-REWRITE-RULES-2}, and
\ref{FIXITY-REWRITE-RULES-3} that determine the fixity
of operators with ambiguous fixity (e.g., operators that
are infix-or-prefix).  In these rules `ambiguous'
means an operator whose fixity cannot be determined by the above
rules (along with any operators before it whose fixity has not
been determined);
`not-allowed'
means there can be no way to assign fixity to the expression operators
consistent with the above rules; and `unresolved' means the
rewrite rule does not change anything.
Also nofix\OPSUP{H} means a nofix operator of precedence higher
than the preceeding or following infix-or\ldots{} operator, and
nofix\OPSUP{--H} means a nofix operator of precedence equal to or lower
than the preceeding or following infix-or\ldots{} operator.
...
\hline
expression-begin & non-operator & expression-begin non-operator \\
expression-begin & prefix & expression-begin prefix \\
expression-begin & infix & not-allowed\OPSUP{--O} \\
expression-begin & postfix & not-allowed\OPSUP{--O} \\
expression-begin & nofix & expression-begin nofix \\
expression-begin & expression-end & expression-begin expression-end \\
\hline
non-operator & non-operator & cannot-happen \\
non-operator & prefix & not-allowed\OPSUP{O} \\
non-operator & infix & non-operator infix \\
non-operator & postfix & non-operator postfix \\
non-operator & nofix & non-operator nofix \\
non-operator & expression-end & non-operator expression-end \\
\hline
prefix & non-operator & prefix non-operator \\
prefix & prefix & prefix prefix \\
prefix & infix & not-allowed\OPSUP{--O} \\
prefix & postfix & not-allowed\OPSUP{--O} \\
prefix & nofix & not-allowed\OPSUP{--O} \\
prefix & expression-end & not-allowed\OPSUP{--O} \\
\hline
infix & non-operator & infix non-operator \\
infix & prefix & infix prefix \\
infix & infix & not-allowed\OPSUP{--O} \\
infix & postfix & not-allowed\OPSUP{--O} \\
infix & nofix\OPSUP{H} & infix nofix \\
infix & nofix\OPSUP{--H} & not-allowed\OPSUP{--O} \\
infix & expression-end & not-allowed\OPSUP{--O} \\
\hline
postfix & non-operator & not-allowed\OPSUP{O} \\
postfix & prefix & not-allowed\OPSUP{O} \\
postfix & infix & postfix infix \\
postfix & postfix & postfix postfix \\
postfix & nofix & postfix nofix \\
postfix & expression-end & postfix expression-end \\
\hline
nofix & non-operator & nofix non-operator \\
nofix & prefix & nofix prefix \\
nofix\OPSUP{H} & infix & nofix infix \\
nofix\OPSUP{--H} & infix & not-allowed\OPSUP{--O} \\
nofix & postfix & not-allowed\OPSUP{--O} \\
nofix & nofix & nofix nofix \\
nofix & expression-end & nofix expression-end \\
\hline
\end{tabular}
\\[1ex]
\begin{tabular}{rl}
\OPSUP{H}   & means operator has higher precedence than \\
          & the precedence of the following operator\\[1ex]
\OPSUP{--H} & means operator has precedence lower than or equal to \\
          & the precedence of the following operator\\[1ex]
\end{tabular}
\\
\caption{Fixity Rewrite Rules: Part 1 of 3}
\label{FIXITY-REWRITE-RULES-1}
\end{figure}
...
\begin{tabular}{|r@{~~}l@{~~~$\Longrightarrow$~~~}l|}
\hline
expression-begin & infix-or-prefix & expression-begin prefix \\
non-operator & infix-or-prefix & non-operator infix \\
prefix & infix-or-prefix & prefix prefix \\
infix & infix-or-prefix & infix prefix \\
postfix & infix-or-prefix & postfix infix \\
nofix\OPSUP{H} & infix-or-prefix & unresolved \\
nofix\OPSUP{--H} & infix-or-prefix & nofix prefix \\
infix-or-prefix & infix-or-prefix & ambiguous prefix \\
infix-or-postfix & infix-or-prefix & unresolved \\
prefix-or-postfix & infix-or-prefix & unresolved \\
\hline
expression-begin & infix-or-postfix & not-allowed\OPSUP{--O} \\
non-operator & infix-or-postfix & unresolved \\
prefix & infix-or-postfix & not-allowed\OPSUP{--O} \\
infix & infix-or-postfix & not-allowed\OPSUP{--O} \\
postfix & infix-or-postfix & unresolved \\
nofix\OPSUP{H} & infix-or-postfix & nofix infix \\
nofix\OPSUP{--H} & infix-or-postfix & not-allowed\OPSUP{--O} \\
infix-or-prefix & infix-or-postfix & not-allowed\OPSUP{--O} \\
infix-or-postfix & infix-or-postfix & postfix infix-or-postfix \\
prefix-or-postfix & infix-or-postfix & postfix infix-or-postfix \\
\hline
expression-begin & prefix-or-postfix & expression-begin prefix \\
non-operator & prefix-or-postfix & non-operator postfix \\
prefix & prefix-or-postfix & prefix prefix \\
infix & prefix-or-postfix & infix prefix \\
postfix & prefix-or-postfix & postfix postfix \\
nofix & prefix-or-postfix & nofix prefix \\
infix-or-prefix & prefix-or-postfix & ambiguous prefix \\
infix-or-postfix & prefix-or-postfix & unresolved \\
prefix-or-postfix & prefix-or-postfix & unresolved \\
\hline
\end{tabular}
\\[2ex]
\begin{tabular}{rl}
...
\end{tabular}
\\
\caption{Fixity Rewrite Rules: Part 2 of 3}
\label{FIXITY-REWRITE-RULES-2}
\end{figure}
...
\begin{tabular}{|r@{~~}l@{~~~$\Longrightarrow$~~~}l|}
\hline
infix-or-prefix & non-operator & ambiguous non-operator \\
infix-or-prefix & prefix & ambiguous prefix \\
infix-or-prefix & infix & not-allowed\OPSUP{--O} \\
infix-or-prefix & postfix & not-allowed\OPSUP{--O} \\
infix-or-prefix & nofix\OPSUP{H} & infix nofix \\
infix-or-prefix & nofix\OPSUP{--H} & not-allowed\OPSUP{--O} \\
infix-or-prefix & expression-end & not-allowed\OPSUP{--O} \\
\hline
infix-or-postfix & non-operator & infix non-operator \\
infix-or-postfix & prefix & infix prefix \\
infix-or-postfix & infix & postfix infix \\
infix-or-postfix & postfix & postfix postfix \\
infix-or-postfix & nofix\OPSUP{H,--A} & ambiguous nofix \\
infix-or-postfix & nofix\OPSUP{H,A} & not-allowed \\
infix-or-postfix & nofix\OPSUP{--H} & postfix nofix \\
infix-or-postfix & expression-end & postfix expression-end \\
\hline
prefix-or-postfix & non-operator & prefix non-operator \\
prefix-or-postfix & prefix & prefix prefix \\
prefix-or-postfix & infix & postfix infix \\
prefix-or-postfix & postfix & postfix postfix \\
prefix-or-postfix & nofix & postfix nofix \\
prefix-or-postfix & expression-end & postfix expression-end \\
\hline
\end{tabular}
\\[2ex]
\begin{tabular}{rl}
\OPSUP{H}   & means operator has higher precedence than \\
          & the precedence of the preceding operator\\[0.5ex]
\OPSUP{--H} & means operator has precedence lower than or equal to \\
          & the precedence of the preceeding operator\\[0.5ex]
\OPSUP{A}  & means operator is afix\\[0.5ex]
\OPSUP{--A} & means operator is \underline{not} afix \\[0.5ex]
\OPSUP{O} & means fix by inserting nofix operator (see text) \\[0.5ex]
\OPSUP{--O} & means fix by inserting non-operator (see text)
\end{tabular}
\\
\caption{Fixity Rewrite Rules: Part 3 of 3}
\label{FIXITY-REWRITE-RULES-3}
\end{figure}

These rewrite rules can be applied during a left-to-right scan.
It can be proved that at any time, the tokens before the current
position will have one of the forms:
\begin{center}
\begin{tabular}{r}
\ldots{} \\
\ldots{} infix-or-postfix \\
\ldots{} infix-or-postfix  infix-or-prefix \\
\ldots{} infix-or-postfix\QMARK{} prefix-or-postfix\PLUS{} infix-or-prefix\QMARK{} \\
\end{tabular}
\end{center}

where \ldots{}~denotes a sequence of non-operators and non-ambiguous
operators.  The expression-end will resolve
ambiguities next to the end of the expression, but operators
designated by the rewrite rules as `ambiguous' will remain
ambiguous.

Fixity that remains ambiguous after fixity rules have been applied
is `fixed up' by applying the following rewrites while issuing
error messages for the ambiguous operators:
...
These rewrites allow parsing to continue.
They are arbitrary in the sense that they
are prefix preferring whereas an alternative set of postfix
preferring rules will also work.

\label{FIXITY-ERRORS}
If an afix operator invokes a rule that is `not-allowed',
the afix operator table entry is ignored, as if it were not in the
operator table.
Other operators or non-operators that invoke
not-allowed rules are `fixed up' by inserting an error
operator or error non-operator before the invoking operator
or non-operator during fixity determination.
An \key{error operator} is
inserted before every not-allowed non-operator or prefix
operator (as indicated by \OPSUP{O}),
and an \key{error non-operator} is inserted before
every non-allowed non-prefix operator (as indicated by \OPSUP{--O}).
The error operator is nofix with H+1 precedence.

\subsubsubsection{Subexpression Identification}
\label{SUBEXPRESSION-IDENTIFICATION}
...
there is no reformatter, the operator parsing pass
compacts the tokens now in the subexpression into a single
\TT{BRACKETABLE} token with the first operator as the value of
the token value's \ttdkey{operator} attribute.

If a subexpression does \underline{not} contain an operator,
the operator parsing pass runs subsequent parsing passes on the subexpression.
Then if the subexpression has now zero or more than one token, the operator
parsing pass replaces
the subexpression by a single \TT{BRACKETABLE} token with no attibutes
...
The {\em natural-numbers} in {\em temporaries} are assigned so no
...
\ttmkey{declare}{reformatter} \hfill \NBOP{<-{}-} ~~~ \OP{\{\$~\$\}} ~~~ \OP{:}

\begin{indpar}[0.5em]
May only be used with \TT{nofix}, \TT{infix}, or \TT{prefix} operators.

All operators but the first must be bracketed subexpressions
or indented paragraphs,
...
bracketed and indented paragraph operators are converted to operands.

The bracketed or indented paragraph operators should be both
afix and nofix.

For example, if \TT{<-{}-} is a nofix operator with this reformatter, and
\TT{\{\$~\$\}} bracketed subexpressions and \TT{:} indented paragraphs
are afix + nofix operators of the same precedence:
...
Operands and operators must alternate,
with the first and last subexpression
elements being operands.
...
elements being operands, and all
...
`{\em plus\_op} \TT{(} {\em minus\_op} {\em operand} \TT{)}', and
...
\TT{"-"} and both these operators are both infix and prefix with
