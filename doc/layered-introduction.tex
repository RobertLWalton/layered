...
are parsed by the same algorithm that is driven by parameters from
...
\item The subexpression does not contain two {\em bracket-type-marks}.
\item The subexpression contains a non-missing
      {\em bracket-type} (i.e., the subexpression is \underline{not}
      `\TT{\{\}}' or `\TT{\{""\}}').
\item The subexpression context selectors (\pagref{CONTEXT-SELECTORS})
...
If all of the above are true, the subexpression is a
{\em typed-prefix-separator} and can head a {\em prefix-N-list}.
Otherwise the subexpression is a {\em typed-bracketed-subexpression}
(without any {\em elements} if it meets the requirements 1 and 2 above)
and is a token by itself that \underline{cannot} head a {\em prefix-N-list}.
In the rest of this section we will only talk about
{\em typed-prefix-separators}.
