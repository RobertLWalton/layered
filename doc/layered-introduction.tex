...
\item A {\em following-primary-separator} must be enabled by a particular
preceeding {\em initial-primary-separator}, else it is not recognized.
For example, if `\TT{a}' and `\TT{the}' are {\em initial-primary-separators},
the {\em following-primary-separator} `\TT{of}' may be recognized in an
expression beginning with `\TT{the}' but not in one beginning
with `\TT{a}'.
\end{itemize}

The primary parser begins by recognizing the {\em primary-separators}
in an {\em (H+1)-expression} allowing empty {\em primaries}.
Then the primary parser declares an error if it finds
any empty {\em primaries}, and replaces empty primaries by an
\TT{ERROR'OPERAND}.
...
\emkey{parser-primary-separator-definition} \\
    ...
    ::= & \TT{define}\ttmindex{define primary separator}{parser command}
		...
		{\em primary-separator-flag} \\
		\end{tabular} \\
	    \TT{with group} ~ {\em primary-separator-group} \\
	    \end{tabular} \\
...
{\em primary-separator-group} ::= {\em quoted-key} \\
{\em quoted-key} ::= see \pagref{QUOTED-KEY}
\\[1ex]
\emkey{primary-separator-flag}
    ::= \ttmkey{initial}{primary separator flag}
    $|$ \ttmkey{following}{primary separator flag}
\end{indpar}
...
Parser Command				& \shortstack{
                                          Given\\Qualifiers}
\\\hline
\tt define primary separator "a" [code] initial & {\tt + code} \\
\tt ~~~~~~~with group "a" \\
\tt define primary separator "an" [code] initial & {\tt + code} \\
\tt ~~~~~~~with group "a" \\
\tt define primary separator "the" [code] initial & {\tt + code} \\
\tt ~~~~~~~with group "the" \\
\\[1ex]
\tt define primary separator "of" [code] following & {\tt + code} \\
\tt ~~~~~~~with group "the" \\
\tt define primary separator "of the" [code] following & {\tt + code} \\
\tt ~~~~~~~with group "the" \\
\tt define primary separator "with" [code] following & {\tt + code} \\
\tt ~~~~~~~with group "the" \\
\end{tabular}
\end{center}
...
A primary is a parsed subexpression.  More specifically,
a parsed subexpression is
a sequence of lexemes and subexpressions encoded
in a MIN vector object.  The vector object may or may not have
an \TT{.initiator} and \TT{.terminator}.  Quoted string lexemes
within the primary are represented by MIN objects with the
\TT{"<Q>"} \TT{.type} and a single vector element that is a MIN string
equal to the string represented by the lexeme.  Operators are
represented by their label, which is either a simple MIN string,
or is a MIN label (e.g., the MIN string \TT{+} or the MIN label \TT{[< + = >]}).
...
\subsubsection{Primary Pass}

The primary pass exists to provide two tables:
\begin{center}
\TT{primary\_table}, a symbol table containing variables and functions \\
\TT{modifying\_ops}, a hash table listing modifying operations
(e.g., \TT{[< + = >]})
\end{center}

The \key{primary table}, a.k.a.~the symbol table, stores
