...
The type can be omitted from the end unless there is preceeded
...
of one or more words, naturals, numerics, or quoted strings, each
representing a MIN string.  In the latter case, if there is only
...
The exception is that when the bracket initiator is the
`\key{multivalue initiator}' (standardly `\TT{\{*}'), the comma
...
Elements must be single bracketed expressions, or sequences of
words, naturals, numerics, or quoted strings that are made into
