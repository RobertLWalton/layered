...
replaces each by a single operator token with the operator as
its label.
\item Determines the fixity of each operator, prefix, infix,
postfix, or nofix.
\item Identifies subexpressions using operator precedence,
and turns each subexpression into a single BRACKATABLE
token.
\item If there are any operators, converts the remaining
list of operators and non-operators into a single BRACKATABLE
token which an \TT{.operator} attribute.
\end{enumerate}
...
beginning at the current position is an operator.  Only
operators selected by the current parsing selectors are
...
afix definition, and there is no operator of lower precedence
between that previous operator and the current position.
In applying this rule, if there is an operator of ambiguous
fixity immediately before the current position, the afix
definition is ignored unless the ambiguous operator can be
postfix, and if the ambiguous operator can be postfix, it
is assumed to be postfix for the purposes of determining
whether the afix definition is to be ignore (and will be
forced to be postfix if the afix definition is not in fact
ignored).
See \itemref{AFIX-OPERATORS-AND-FIXITY-DETERMINATION} below
for analysis.

Except for special consideration involving the afix flag,
operator fixity is ignored during operator identification.

If an operator is identified, its lexemes are collected into
an \TT{OPERATOR} token.  If no operator is identified, the
current position is moved one lexeme to the right.
...
If there is more than one way of assigning operator fixity obeying
the above rules, the expression is in error.  An example is the
two operator sequence:
\begin{center}
\shortstack[c]{infix\\or\\postfix} ~~~ \shortstack[c]{infix\\or\\prefix}
\end{center}
which could be resolved as either `infix prefix' or `postfix infix'.
Here are some of the ways this ambiguity can be resolved:

\begin{itemize}
\item If the two ambiguous operators are followed by a non-operator or
a prefix operator, there will be two assigments of fixity to the
two operators that obey the rules,
and the expression has a parse error.
\item If the two ambiguous operators are followed by an infix operator,
a postfix operator,
or a no-fix operator with precedence equal to or
lower than the second ambiguous operator's infix possibility,
it is not possible to assign fixity to the ambiguous operators without
violating the rules,
and the expression has a parse error.
\item If the two ambiguous operators are followed by an nofix operator,
with precedence higher than
than the second ambiguous operator's infix possibility,
the fixity assignment `postfix infix' is forced for the two ambiguous
operators.
\end{itemize}

If there is an operator immediately before the current position
whose definition has ambiguous fixity, the fixity of that
operator may be non-yet determined.

At any time can be at most one operator with ambiguous fixity.
...
postfix & infix-or-prefix & postfix prefix \\
infix-or-prefix & infix-or-prefix & ambiguity error \\
infix-or-postfix & infix-or-prefix & ambiguity error \\
prefix-or-postfix & infix-or-prefix & ambiguity error \\
\hline
...
infix-or-postfix & infix-or-postfix & ambiguity error \\
prefix-or-postfix & infix-or-postfix & ambiguity error \\
\hline
...
infix-or-prefix & prefix-or-postfix & ambiguity error \\
...
infix-or-postfix & nofix\H & ambiguity-error \\
...
\subsubsubsection{Afix Operators and Fixity Determination}
\label{AFIX-OPERATORS-AND-FIXITY-DETERMINATION}

TBD

Note first that in order for an afix flagged operator definition
to be not ignored, the current position cannot be immediately
preceded by a prefix or infix operator.  The afix definition
must be infix or nofix.  Neither fixity can be immediately preceded
by a prefix operator.  Nofix can be immediately preceded by an infix
operator of less precedence, but then an operator of less precedence
would be between the current position and any preceding operator
of precedence equal to the afix definition, so the definition
would be ignored.

So if the current position is preceded by an operator with
ambiguous fixity, then the following are the possibilities:

\begin{enumerate}
\item
The preceding operator has fixity `prefix or postfix'.
Then the afix definition is ignored.
\item
The preceding operator has fixity `prefix or infix'.
Then the afix definition is ignored.
\item
The preceding operator has fixity `postfix or prefix'.
or `postfix or infix'.
If the fixity is assumed to be postfix and there is
a preceding operator of precedence equal to the afix definition,
and no intervening operator of less precedence, then the afix
operator is \underline{not} ignored.  Otherwise the afix
definition is ignored.
\end{enumerate}



\subsubsubsection{Old Stuff}

To do this it performs a left-to-right scan of the
lexemes, performing the following steps at each position
scanned:
\begin{itemlist}[0.7in]

\item[Step 2:] If an operator is found, determine its fixity.
This determination is based on rules given below.  It is
possible that the determination is ambiguous: e.g., the
fixity could be infix or postfix.  If this is the case,
each of the two possibilities corresponds to a different
parse of the part of the expression to the left of the operator,

TBD


either prefix or infix, postfix or infix, or prefix or postfix.
If there is a unique assignment of fixity according to the
following rules, that assignment is used.  If there is no
assignment, or more than one, the entire expression is
deemed unparsable.  The rules are:
