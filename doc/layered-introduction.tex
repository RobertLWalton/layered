...
\hspace*{-0.2in}\ttmkey{implied header}{line variable}\\
    ...
    and is almost just like an explicit prefix separator except that
...
resets the current line variables which are then used to process the lines
...
We assume that prefix separators do \underline{not} change
selectors in such a way as to alter the parsing of prefix separators.
Also, implied headers should not change the selectors in such a way
...
to the corresponding current line variables.  A line is in paragraph
beginning position if it is the first line of an indented paragraph,
or if it is the first non-blank, non-comment line following a blank
non-comment line.
