...
\emkey{ID-character}\label{ID-CHARACTER} ::=
    ...
    or word \\
    should be `\TT{@}' unless the lexical analyser is changed:
...
the {\em ID-character} must be `\TT{@}' and the {\em clear-sign}
must be `\TT{@@@@}'.
...
and either to clear the symbolic ID table or to
define or add to the datum denoted by {\em ID}.  If {\em ID} denotes
an object, the object is given
elements from the {\em element-list} and attributes from the
{\em attribute-paragraph}.

An expression containing the {\em clear-sign} just clears the symbolic
ID table: see \pagref{CLEAR-SYMBOLIC-TABLE}.


Before this reformatter
is invoked,
any {\em attribute-paragraph} is parsed
