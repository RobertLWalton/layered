...
\hspace*{-0.2in}\ttmkey{implied header}{line variable}\\
    ...
    The type of a non-missing implied header must have a bracket type
...
selectors, options,
and lexical masters in the bracket type definition are used to
modify the current line variables selectors, options, and lexical
masters.
If in addition the bracket type definition has an implied subprefix
that itself has a bracket type definition selected by the modified
...
bracket type definition, so care should be taken that bracket type definitions
...
its bracket type definition is used to
...
\item
Let $P$ denote the implied subprefix of the bracket type definition
of the explicit paragraph header.
If $P$ exists and
itself has a bracket type definition selected by the modified
