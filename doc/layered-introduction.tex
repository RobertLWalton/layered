...
\begin{indpar}[0.5em]
Use only with {\tt nofix} operators.
...
\begin{indpar}[0.5em]
Use only with {\tt nofix}, {\tt prefix}, or {\tt infix} operators.


There must be exactly one operator that is not a bracketed subexpression,
that operator must be the first operator, and there must be no operands
after the first bracketed subexpression operator (so the only operands
must be just before or after the first operator).  If the first operator is not
preceded by an operand, an empty list is inserted as the preceding operand.
...
beginning of the expression and the bracketed operators are converted to
operands.
Indented subexpressions may be treated like bracketed subexpressions and
be bracketed operands.

For example, if the first operator is \verb|<--|, and the afix operators
are \verb|{ }| bracketed subexpressions and \verb|:| indented subexpressions:
