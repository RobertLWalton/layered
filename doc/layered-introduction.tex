...
\\[0.5ex]
{\em unindented-paragraph} ::= see \itemref{UNINDENTED-PARAGRAPHS}
\\[0.5ex]
\emkey{paragraph-end}
    \begin{tabular}[t]{rl}
...
{\em Un\-in\-den\-ted-paragraphs} are sequences of logical lines between
blank lines or the ends of the containing indented paragraph, where
the first logical line of the unindented paragraph begins with a
paragraph header:
see \itemref{UNINDENTED-PARAGRAPHS} for details.
...
\label{HEADED-LINES}
A `\key{headed line}'
is a logical line that begins with a typed prefix separator
in the `\TT{line}' group.  Separators in this group are only
allowed at the beginning of logical lines.  The parser's output
for a headed line is a MIN object with \TT{.type} and attributes
taken from the prefix separator beginning the line and elements
taken from the rest of the line.
