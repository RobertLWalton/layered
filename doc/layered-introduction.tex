...
\begin{indpar}
{\bf The Syntax Hypothesis}~ People reason syntactically
...
{\bf The Type Checking Segregation Hypothesis}~ A strongly typed
...
type checkable, and code that uses these functions is a is
...
in the ranges \TT{0-9}, \TT{a-f}, and \TT{A-F}.
...
might be returned as a 1-character lexeme of type `\TT{unrecognized
...
We can be a bit more precise by saying that scanning
a lexeme executes the following pseudo-code:

\begin{center}
\begin{tabular}{l}
To scan a lexeme: \\
~~~~\begin{tabular}[t]{l}
input: \TT{current} \TT{table} which must be a \TT{MASTER} table \\
set \TT{current} \TT{type} = \TT{NONE} \\
make \TT{return} \TT{stack} empty \\
make \TT{translation} \TT{buffer} empty \\
loop: \\
~~~~\begin{tabular}[t]{l}
if \TT{current} \TT{table} is a \TT{MASTER} table and 
   \TT{current} \TT{type} is \underline{not} \TT{NONE}: \\
~~~~~~~~terminate loop \\
if \TT{current} \TT{table} is a lexeme table: \\
~~~~~~~~set \TT{current} \TT{type} to the lexeme table type \\
    \end{tabular}
    \end{tabular}
\end{tabular}
\end{center}
...
\\
\emkey{control-character} ::= characters in UNICODE categories \TT{C} and \TT{Z}
\\[1ex]
\emkey{isolated-separator-character} :::=
    \begin{tabular}[t]{l}
    characters in UNICODE categories \TT{Ps}, \TT{Pi}, \TT{Pe}, and \TT{Pf}; \\
    includes \TT{\{ ( [ << >> ] ) \}}
    \end{tabular}
\\
\emkey{repeating-separator-character} :::= \TT{|}
\\
\emkey{separator-character} :::= {\em isolated-separator-character}
                             $|$ {\em repeating-separator-character}
\\[1ex]
\emkey{repeating-leading-character} :::=
	\TT{`} $|$ \TT{\$} $|$
	\TT{\textexclamdown} $|$ \TT{\textquestiondown}
\\
\emkey{repeating-trailing-character} :::=
	\TT{'} $|$ \TT{!} $|$ \TT{?} $|$ \TT{.} $|$ \TT{:} $|$ \TT{\%}
\\
\emkey{isolated-trailing-character} :::=
	\TT{,} $|$ \TT{;}
\\[1ex]
\emkey{letter} :::=
...
\emkey{digit} :::=
    ...
    (includes \TT{0}, \ldots, \TT{9})
\\
\emkey{mark-character} :::=
    \begin{tabular}[t]{l}
    characters in UNICODE categories \TT{P} and \TT{S}
    (punctuation and symbols), \\
    but \underline{not} \TT{"} or {\em separator-characters}
    \end{tabular}
\end{indpar}

Lexemes may be separated by
{\em white-space}:
\begin{indpar}
...
illegal.  {\em Control-characters} not in {\em white-space} are illegal.
Illegal characters generate warning messages and are ignored.
{\em Horizontal-space} just before {\em vertical-space}
or the end of input is just ignored.
...
and the character.
Characters in UNICODE class \TT{Mn} (combining marks)
...
\TT{<NUL>}, \TT{<LF>}, \TT{<SP>}.  There are two other cases:
\TT{<Q>} represents the doublequote \TT{"}, and \TT{<NL>} (new line)
represents a line feed (same as \TT{<LF>}).
...
\emkey{lexeme}
	::= {\em word} $|$ {\em mark} $|$ {\em numeric}
	$|$ {\em separator}
	$|$ {\em quoted-string}
	$|$ {\em comment}
\label{LEXEME}
\\[1ex]
\emkey{strict-separator} :::= {\em isolated-separator-character} $|$
                              \TT{|}\PLUS{}
\\[0.5ex]
...
\emkey{lexical-item-character} :::=
	...
	{\em isolated-separator-character} or \TT{|} or \TT{"}
	\end{tabular}
\\
...
\\[0.5ex]
\emkey{middle-lexeme} :::= see text
\\[0.5ex]
\emkey{mark} :::= {\em mark-character}\PLUS{} but not {\em separator}
\\[0.5ex]
\emkey{word} :::= {\em middle-lexeme} beginning with a {\em letter}
\\[0.5ex]
\emkey{numeric} :::= {\em middle-lexeme} containing a {\em digit}
                     and not beginning with a {\em letter}
\\[1ex]
\emkey{comment} :::=
    \TT{//} {\em comment-character}\,$^\star$
\\[1ex]
\emkey{comment-charcter} :::= {\em graphic-character}
...
The definition of a {\em token} is unusual: it is what is left over
after removing {\em leading-separators} and {\em trailing-separators}
from a {\em token-item}.  The lexical scan first scans a
{\em token-item}, and then removes
{\em leading-separators} and {\em trailing-separators} from it.
Also {\em trailing-separators} are removed
from the end of a {\em token-item} by a right-to-left scan, and not
the usual left-to-right scan which is used for everything else.
Thus the {\em token-item} `\TT{\$\$4,987,,::}' yields the
{\em leading-separator} `\TT{\$\$}', the {\em token} `\TT{4,987}',
...
\begin{indpar}
\emkey{name} ::= 
	\{ {\em word} $|$ {\em numeric} $|$ {\em quoted-string} \}\STAR{}
	{\em word}
	\{ {\em word} $|$ {\em numeric} $|$ {\em quoted-string} \}\STAR{}
\\[0.3ex]
...
\emkey{cpat-name}
	...
	::= & \TT{<}{\em letter}\PLUS{}\{\TT{-}{\em letter}\PLUS{}\}\STAR{}%
	       \TT{>} \\
	    & but not {\em character-representive} \\
	\end{tabular}
\\[0.3ex]
...
\\[0.3ex]
\emkey{type} ::= {\em name} except \TT{scan error} or \TT{none}
\end{indpar}
...
\\[0.3ex]
\emkey{repeat} ::= \TT{<repeat>} $|$ \TT{<repeat-}{\em digit}\PLUS{}\TT{>}
\\[0.3ex]
...
\TT{<digit>} and \TT{<line-break>}.  However, a {\em character-pattern-name}
...
\TT{"0"} and \TT{"+"}.  The next simplest is two {\em character-representative}
...
\TT{"}$P$\TT{<other>}$Q$\TT{"},
the prefix \TT{<other>} matches any characters not matched by
...
pattern, and that pattern \underline{cannot} be \TT{<other>}.
...
an end of file.  This fact is used in both the `\TT{main master table}'
...
~~~~~~~~ & \TT{match}, \TT{translate oct}, or \TT{translate hex} \\
~~~~~~~~ & \TT{require}
\\[2ex]
...
\hfill The \TT{translate hex/\EOL oct} \\ components are applied to a matched
...
been reinstated, so \TT{translate hex/oct} components are always
...
\TT{translate hex/oct} components, and so does not affect the
...
any \TT{match}, \TT{translate hex}, or \TT{tran\-slate oct}, or
...
\TT{translate hex}, \TT{tran\-s\-late oct}, or \TT{trans\-late to},
the translation
...
independently by \TT{match} and translate instruction components.
...
pattern other than \TT{<other>} (see \pagref{<OTHER>}).
...
lexeme of type `\TT{bad char\-ac\-ter}'.  The translation of this lexeme is
...
such as `\TT{(}' and `\TT{)}', by an indentation mark and eventual
...
\begin{tabular}{p{1in}p{4.5in}}
		& Standardly the token lexeme types are word, mark, separator,
		  and number whose tokens have a \TT{value} that is
		  a MIN string equal to the lexeme characters,
		  quoted string whose tokens have a \TT{string} equal
...
\\[1ex]  
\begin{tabular}{p{1in}p{4.5in}}
		& \TT{BRACKETED} are \TT{BRACKETABLE} tokens
...
\ttmkey{value}{of token}
		& The token \TT{value}, as described above.  This is
		  ...
		  as MIN strings, so \TT{001}, \TT{01}, and \TT{1}
		  are all distinct, as are \TT{0.10}, \TT{0.1},
		  and \TT{.10}.
\end{tabular}
\\[1ex]
\begin{tabular}{p{1in}p{4.5in}}
...
\\[1ex]
		& Note that numbers are represented by MIN
		  ...
		  As a consequence `\TT{01}' and `\TT{1}' are
		  different number symbols, as are `\TT{0.10}'
		  and `\TT{.1}'.
\end{tabular}
...
\\[0.6ex]
\emkey{simple-name} ::= {\em word} \{ {\em word} $|$ {\em numeric} \}\STAR{}
\\[0.0ex]
\emkey{quoted-name-component} ::=
    {\em word} $|$ {\em mark} $|$ {\em separator} $|$ {\em numeric}
\\[0.0ex]
\emkey{quoted-name} ::=
...
{\em mark}, {\em separator}, and {\em numeric} lexemes enclosed
...
\hspace*{-0.5in}named brackets (e.g.~`\TT{<b|}' and `\TT{|b>}')
      and named bracketed subexpressions \\
\hspace*{-0.5in}named operators (e.g.~`\TT{<p>}')
\end{indpar}
...
named openings (e.g.~`\TT{<}'),
named key separators (e.g.~`\TT{\#}'),
named middles (e.g.~`\TT{|}'),
and named closings (e.g.~`\TT{>}'), which are all symbol table
...
MIN object with a \TT{.terminator} equal to the MIN string
\TT{"<NL>"} or \TT{";"} (this is the line separator at the top level),
...
either an \TT{.initiator} attribute or
a \TT{.termina\-tor} attribute or both.
The elements of one of these MIN objects
are the \TT{value}'s of the tokens which make
up the represented
...
\begin{center}
\TT{\{|} {\em element} \ldots\TT{;} {\em attribute-name} \TT{=}
				      {\em attribute-value}\TT{;}
				      \ldots \TT{|\}}
\end{center}
...
{\em clos\-ing-bracket-name} as defined by a
\ttikey{parser define bracket}{bracket!parser define}
command.  An example of such a definition is
\begin{center}
\TT{parser define bracket "("~...~")" [operator, text]} \ldots
\end{center}
which specifies `\TT{(}' to be an {\em opening-bracket-name}
with `\TT{)}' as its corresponding {\em closing-bracket-name}
...
in which the name, in this case `\TT{b}',
...
{\em opening-bracket-name} becomes the \TT{.initiator}
...
The following is an example using `\TT{:}' as an {\em indentation-mark-name}:
\begin{indpar}\begin{verbatim}
...
\ttikey{par\-ser define gluing indentation mark}%
...
An example is
\begin{center}
\TT{define gluing indentation mark ":"~...~";" [operator]} \ldots
\end{center}
which specifies `\TT{:}' to be a {\em gluing-indentation-mark-name}
with `\TT{;}' as its corresponding {\em line-separator-name}
...
\emkey{parser-indentation-mark-command} \\
    ...
    ::= & \ttikey{parser define}{define indentation mark}
	  ...
	  \end{tabular} \\
    $|$ & \ttilkey{parser define}{define gluing}{indentation mark}
	  \begin{tabular}[t]{@{}l@{}}
	  \TT{gluing indentation mark} \\
    ...
    $|$ & \ttikey{parser undefine indentation mark}{undefine indentation mark}
	  ...
	  \end{tabular} \\
    $|$ & \ttilkey{parser undefine}{undefine gluing}{indentation mark}
	  \begin{tabular}[t]{@{}l@{}}
	  \TT{gluing indentation mark} \\
...
\emkey{gluing-indentation-name} \\
	...
	[ \TT{...} {\em line-separator-name} ]
\\[0.5ex]
\emkey{gluing-indentation-mark-name} ::= \TT{"}{\em mark}\,\TT{"}
\\[0.5ex]
...
object is given a \TT{.terminator} attribute value
...
are the paragraph lines, and whose \TT{.initiator} attribute
...
to a preceding mark at the end of its line.  For example, `\TT{:}'
may be glued to `\TT{=}' to make `\TT{=:}'.
...
glued to a preceding mark and that the former must be a single mark
...
\\[0.5ex]
\emkey{bracket-name} ::= {\em simple-name}
\\[0.5ex]
...
\\[0.5ex]
\emkey{key-component} ::= {\em word} $|$ {\em numeric} $|$ {\em quoted-string}
\\[0.5ex]
...
and {\em named-closing} are respectively `\TT{<}', `\TT{\#}',
`\TT{|}', and `\TT{>}', then an example named bracketed subexpression is
\begin{center}
\TT{<h1(2) \# Diamond Lemma|} ~~~\ldots~~~ \TT{|h>}
\end{center}

Here the {\em named-opening-bracket} contains the {\em bracket-name}
`\TT{h1}' (note `\TT{h}' and `\TT{1}' are separate lexemes),
the {\em argument} `\TT{(2)}', and the {\em key} `\TT{Diamond Lemma}',
while the {\em named-closing-bracket} contains just an initial
segment of the {\em bracket-name}.   The initial name segment
in the {\em named-closing-bracket} may be empty, so
\TT{|>}, \TT{|h>}, or \TT{|h1>} could all be used as
{\em named-closing-brackets} in the above example.
...
which is:
\begin{center}
\TT{parser define named bracket
     "<"~...~"\#"~...~"|"~...~"|"~...~">" [text]} \ldots
\end{center}
which specifies `\TT{<}' to be a {\em named-opening},
`\TT{\#}' to be a {\em named-key-separator},
`\TT{|}' to be a {\em named-middle},
`\TT{>}' to be a {\em named-closing},
and `\TT{text}' to be the sole definition selector.
...
and {\em named-operators} (as per \itemref{NAMED-OPERATORS})
...
\emkey{named-bracket-name} \\
    \hspace*{0.2in}
     ::= \begin{tabular}[t]{@{}l@{}}
	 {\em named-opening} [ \TT{...} {\em named-key-separator} ] \\
         {}[ \TT{...} {\em named-middle} \TT{...} {\em named-middle} ] \\
	 \TT{...} {\em named-closing}
	 \end{tabular}
\\[0.5ex]
...
Note that if a {\em named-middle} is given in a {\em named-bracket-name},
the {\em named-middle} must be repeated twice and both copies must
be identical.

A named bracketed subexpression is converted to a
MIN object whose list elements are the components of the
subexpression except for the brackets.  The
{\em named-opening} becomes the \TT{.initia\-tor}
attribute of this object, the {\em named-middle} becomes
the \TT{.middle} attribute of this object,
the {\em named-closing} becomes the \TT{.terminator} attribute
of the object, the {\em bracket-name} (from the {\em named-opening-bracket})
becomes the
\TT{.name} attribute of the object, the {\em arguments}
become the \TT{.arguments} attribute of the object,
and the \TT{keys} become the \TT{.keys} attribute of the object.
The \TT{.arguments} attribute value is itself a MIN object
...
The \TT{.keys} attribute is a MIN object whose list elements
are the individual {\em keys}, and whose \TT{.initiator} and 
\TT{.separator} attributes are both the {\em named-key-separator}.
...
If there are no {\em arguments}, the \TT{.arguments} attribute
is not set, and similarly if there are no {\em keys}, the \TT{.keys}
...
`\TT{lemma "5.1.1"}' and `\TT{lemma 5.1.1}' become identical
when stored in the \TT{.keys} attribute of the parser result.

If the {\em named-middle} ends in a mark and the {\em named-closing}
begins with a mark, the {\em named-middle} may be glued to the
{\em named-closing} to make a
{\em named-closing-bracket} with empty {\em bracket-name-initial-segment}.
Thus in our example the single mark `\TT{|>}' may be used in place of
the two separate marks `\TT{| >}'.
...
\emkey{named-operator} ::=
    ...
    {\em named-opening} & {\em operator-name} {\em argument}\STAR{} \\
	 ...
	 \end{tabular}
\\[0.5ex]
\emkey{operator-name} ::= {\em simple-name}
\end{indpar}
...
but with its \TT{.name} attribute equal to
the {\em operator-name} instead of a {\em bracket-name} and with
no \TT{.middle} attribute.  As for a named bracketed subexpression,
the \TT{.initiator} is the {\em named-opening},
the \TT{.terminator} is the {\em named-closing},
and the \TT{.arguments} and \TT{.keys} are set from the
