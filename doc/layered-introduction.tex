...
as prefix separators.  It is also important that prefix
...
not have this same group when computed with selectors with some
...
\TT{options} line variable does not have the `\TT{continuing}'
option, the paragraph line variables are copied
to the corresponding current line variables.

\item The selectors, options, and lexical master are set
from the corresponding current line variables.

\item If the current implied header variable is not missing,
a copy is inserted at the beginning of the line.  The
...
The prefix definition must have either the `\TT{paragraph}' or
...
The {\em prefix-0-list} headed by the inserted implied prefix
is then parsed.  If this has no elements, it must be ended
by an explicit prefix separator with a prefix definition
selected by the modified selectors that has the same group
as the inserted implied prefix (as the logical line cannot
be empty).  In this case the inserted
implied prefix is deleted, the current line variables are
restored to their values just before they were reset by the
implied header (these ara necessarily equal to the paragraph
...
must end at the end of the logical line, as the inserted implied
prefix has either the `\TT{paragraph}' or `\TT{line}' group, and
explicit prefixes with these groups cannot appear inside a
logical line (they are ignored with an error message if they do).
...
whose definition looked by the \TT{selectors} current line variable
has the `\TT{paragraph}' group, this prefix is an explicit paragraph
header.

If an empty {\em prefix-0-list} was found in the previous step,
and the prefix that ended the list does not have a prefix definition
...
Otherwise if an explicit paragraph header is found,
...
In addition, if an explicit paragraph header is found, then:
