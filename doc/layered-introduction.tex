...
\item[Requirement 3:] 
All prefix {\tt OPERATOR} tokens in the list are preceded by
the beginning of the expression or by a prefix {\tt OPERATOR} token.
A prefix operator is followed by another prefix operator or by a
non-operator.
\item[Requirement 4:] 
All infix {\tt OPERATOR} tokens in the list are both preceded by and
followed by non-{\tt OPERATOR} tokens.
\item[Requirement 5:] 
All postfix {\tt OPERATOR} tokens in the list are followed
by the end of the expression or by a postfix {\tt OPERATOR} token.
A postfix operator is preceded by another postfix operator or by a
non-operator.
\end{itemlist}
...
\item[Rule 4:] A postfix operator or precedence $P'$
must be preceded by a non-operator,
or by an operator of precedence greater than $P'$,
...
or by a postfix operator of precedence equal to $P'$,
else an `error nofix operator' of precedence one less than that of
the postfix
...
\item[Loop Step 2:]
If $C=F$ and $T=postfix$ (the token before $C$ is a postfix operator),
...
\item[Loop Step 4:]
If $C=F$, and if
$C=N$, or $T=infix$ and $P'\leq P$, or $T=prefix$ and $P'<P$,
then insert
