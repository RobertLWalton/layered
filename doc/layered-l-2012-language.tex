...
%
% File:         layered-l-2012-language.tex
% Author:       Bob Walton (walton@acm.org)
% Version:      1a (DISCONTINUED)
...
{
\Large \bf
NOTICE: Design of this language has been discontinued - R L Walton,
June 20, 2014.
}
...
\emkey{simple-variable-declaration}
...
First, cluster member names are used as prototype parameter
names, with the base of these names also being a prototype
parameter.  This specifies that some parameters are related
...
will be derived from the base, and must be omitted in calls.

When this is done for results, the cluster base may be
either an argument or a result, and the values of the designated
members of the cluster are set from the results.
If the function has the `\ttmkey{cluster}{qualifier}' qualifier,
then any returned cluster members that do not already exists
are created as variables
in the local code block, as is done by `{\tt allocate}' above.

Also, a result parameter may have the same name as an argument
parameter, as is done for the {\tt pp} parameter in
the prototype of {\tt "++"} above.  This specifies that the
result parameter is the same as the argument parameter,
and is merely being moved from argument to result in order to
...
and {\tt .get.} and/or {\tt .set.} operations to access values
pointed at by a pointer.
...
A qualifier is either a {\em name} (\pagref{QUALIFIER-NAME}),
or is a parentheses surrounded qualifier expression.
...
and `{\tt readonly}' is a location qualifier that says that the
...
Some qualifiers
are both access and body qualifiers, and when these
are put in the position of function access qualifier,
they also apply to the function body.
...
has a lifetime that lasts at least until a
current code block execution terminates.
...
an out-of-line function having a given
access qualifier (see below) is called.
For example, a `{\tt relocatable}' lifetime qualifier can specify
that a value lifetime ends when a function with
the `{\tt relocating}' access qualifier is called.
...
In addition user define `calling cache qualifiers' may be defined
that specify that software cache lifetimes will continue until
a code block having a given access qualifier (see below) is executed.
If such a code block flushes hardware caches, it can be used
in combination with these user defined qualifiers to flush both
software and hardware caches.

Cachine qualifiers, types and container types, and regions are used together
to specify aliasing (\secref{ALIASING}).
...
in a code block frame expires when the code block execution terminates.
...
at least as long as {\tt Q2} promises.  Thus the `{\tt static}'
lifetime qualifier is convertible to any other lifetime qualifier.
...
Unnested code within the function body
is considered to be at nested level {\tt 0}.
{\tt stack(n)} is automatically applied
to the addresses of locations allocated to the
current frame by code in a level {\tt n} code block (such locations
are freed when the code block of level {\tt n} terminates).

`{\tt stack(m)}' is convertable during the execution of
a code block of level {\tt n}$>${\tt m} to `{\tt stack(n)}'.
...
except in function argument and return value types.
