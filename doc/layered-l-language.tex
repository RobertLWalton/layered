...
end with the word `\TT{is}', `\TT{as}', `\TT{with}', or `\TT{without}.
\item
...
using the language syntax equations and the {\em parser-clauses}
of the current module's {\em module-declaration} and for a body
also the body's {\em body-declaration}.  The abstract syntax
...
versions of the operators in our representation of the abstract
syntax tree, e.g., {\tt +} is replaced by {\tt "+"}, so that
the operators become {\em function-term-names}.

The parser does not process {\em result-declarations} or
...
\tt function (int64 r) = (( int64 a 1 ) "+" ( int64 a 2 ))
\end{center}
Since the parser has only syntax information available to
it, the parser does not know what are the {\em qualifier-names}
and {\em type-names} in a given scope, and in fact the parse
...
\emkey{exponent-operand}\label{EXPONENT-OPERAND} ::=
    {\em mark-expression} $|$ {\em subexpression} $|$ {\em primary}
\\[0.5ex]
\emkey{mark-expression}\label{EXPONENT-OPERAND} ::= \\
\hspace*{0.5in}\begin{tabular}[t]{rl}
