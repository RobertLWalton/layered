...
Given these, the following are defined:
\begin{indpar}\begin{verbatim}
type D is int32
...
integer values consists of addining a constant, called the
...
word lexeme distinct from any other lexemes in the program.
\item[\TT{type} {\em qualifier-name}\STAR{}]
              \begin{tabular}[t]{@{}l@{}}
              {\em defined-enum-type-name}~~\TT{is}~~%
	      ...
	      \TT{with enum range [}
	         {\em first-enum} \TT{..} {\em last-enum} \TT{]} \ldots{} \\
	      \end{tabular} \\
Ditto with every `\TT{next}' replaced by `\TT{previous}'.
\end{itemlist}
\end{indpar}
...
E2 C2 = next enum 
    ...
    // range value of ( E1(C2) ) == 19 = int32(C2)
type E3 is previous E2 with enum range [1 .. 5]
    // E3(1) ---> E2(10) ---> E1(9)
    // E3(5) ---> E2(6)  ---> E1(14)
E3 C3 = next enum 
    // Now range value of (C3) == 1,
    // range value of ( E2(C3) ) == 10,
    // range value of ( E1(E2(C3)) ) = 9 = int32(C3)
\end{verbatim}\end{indpar}
...
types this is the associated base type.  For builtin types, it is
\TT{T} itself.
