...
\emkey{atomic-value} ::= {\em expression} \\
\emkey{atomic-pointer} ::= {\em expression} evaluating to a pointer

\begin{indpar}
{\em Atomic-pointers} are evaluated with \TT{pointer} target type.
{\em Atomic-values} are evaluated with target type equal to the
{\tt .type} member of the associated {\em atomic-pointer}.
This type must have a numeric type as its underlying
type.
...
{\em Atomic-statements} are executed in two passes:

\begin{indpar}[0.6in]
\hspace*{-0.3in}{\bf Pass 1, User Pass}:
...
do some checking and compile the code for pass 2.  At the end of pass 1
this pass 2 code is loaded into the kernel and checked.  Pass 1 also
compiles a parameter block for the kernel code.  If an {\em atomic-statement}
is part of an out-of-line routine that is executed many times, compilation
of the kernel code and passing this to the kernel need only be done once,
though compilation of the parameter block needs to be done for every
statement execution.

\hspace*{-0.3in}{\bf Pass 2, Kernel Pass}: First asyncrhonous interrupts are
disabled.  Then the code is executed until it terminates.  If an error
is discovered the code aborts.  Otherwise the code terminates successfully.
When the code terminates, any locks acquired are freed in the opposite order
to that in which they were acquired.  Upon termination of pass 2, the
{\em atomic-statement} terminates.  Upon successful termination, the
returned values are: {\tt count = 0}, {\tt atomic code = success},
{\tt pass = 2}.
\end{indpar}
