...
define abbreviations: \\
\TS~~~~\ttkey{false} \ABV{} `std false' \\
\TS~~~~\ttkey{true} \ABV{} `std true'
\end{indpar}
...
If \underline{any} implied conversion from $T1$ to $T2$ is defined,
...
operators cannot be mixed with summation, product, or exponent operators.
...
\emkey{logical-expression}\label{LOGICAL-EXPRESSION}
    ...
    \end{tabular}
\\[0.5ex]
\emkey{\$and\$-expression}
    \begin{tabular}[t]{rl}
    ::= & {\em logical-operand} \ttkey{and} {\em logical-operand} \\
    $|$ & {\em logical-operand} \ttkey{and} {\em \$and\$-expression} \\
    \end{tabular}
\\[0.5ex]
\emkey{\$or\$-expression}
    \begin{tabular}[t]{rl}
    ::= & {\em logical-operand} \ttkey{or} {\em logical-operand} \\
    $|$ & {\em logical-operand} \ttkey{or} {\em \$or\$-expression} \\
    \end{tabular}
\\[0.5ex]
...
\emkey{compare-expression}
    ...
    $|$ & {\em compare-operand} {\em compare-operator}
                                   {\em compare-operator} \\
    \end{tabular}
\\[0.5ex]
...
\emkey{\$*\$-expression}
    \begin{tabular}[t]{rl}
    ::= & {\em product-operand} \ttkey{\$*\$} {\em product-operand} \\
    $|$ & {\em \$*\$-expression} \ttkey{\$*\$} {\em product-operand} \\
    \end{tabular}
\\[0.5ex]
...
\emkey{\$\&\$-expression}
    ...
    $|$ & {\em \$\&\$-expression} \ttkey{\&} {\em bitop-operand} \\
    \end{tabular}
\\[0.5ex]
\emkey{\$|\$-expression}
    \begin{tabular}[t]{rl}
...
define right associative "logical" operator "and"
define right associative "logical" operator "or"
include "logical" expression in "if" right operand
...
define right associative "compare" operators
       "<", "<=", "==", "!=", ">=", ">"
...
Note that the {\tt "else"}, {\tt "and"}, {\tt "or"} operators
and the comparison operators are right associative and not
left associative.  This is because the defer evaluation of
their right operands.  Right associativity coupled with
deferred evaluation of right operands produces left to
right evaluation order.  See the example on \pagref{IF-ELSE-EXAMPLE}.
...
{\em parser-blocks} of an exporting module can be imported by the
\begin{center}
\TT{import} ~ {\em module-name} ~ \TT{parsing}
\end{center}
{\em import-clause} in the {\em module-declaration} or {\em body-declaration}
of an importing module or body.
...
Because {\tt "else"} defers the evaluation of its right operand,
it is parsed as a right associative operator, but evaluates its
operands left to right.

The following are equivalent to the builtin function definitions for
the {\tt "if"} and {\tt "else" } functions (except for context issues
as noted after the example below):
...
define type std *IF-VALUE* as void
    ...
    //                  if r.valid false.
    //
    // where T is the type of the e1 value and cannot itself
    // equal *IF-VALUE*.

// Equivalent code of "if" function:
//
...
function *IF-VALUE* r, T r.value, bool r.valid =
        ...
        "else"
        ( *DEFERRED* *IF-VALUE* arg2, T arg2.value,
                                      bool arg2.valid ):
    if:
        arg1.valid:
            r.value = arg1.value
...
function if the {\em logical-expression} was \TT{true}.
The functions also use the {\em if-assignment} statement
...
to the following code (in which we have ignored issues of context
for simplicity):
...
\begin{tabular}{l}
\tt bool tmp1 = $e2$ \\
\tt *IF-VALUE* tmp2, $T$ tmp2.value, bool tmp2.valid = do: \\
\tt ~~~~if: \\
\tt ~~~~~~~~tmp1: \\
\tt ~~~~~~~~~~~~tmp2.value = $e1$ \\
\tt ~~~~~~~~~~~~tmp2.valid = true \\
...
\end{tabular} \\
\begin{tabular}{l}
\tt *IF-VALUE* tmp3, $T$ tmp3.value, bool tmp3.valid = do: \\
\tt ~~~~if: \\
\tt ~~~~~~~~tmp2.valid: \\
\tt ~~~~~~~~~~~~tmp3.value = tmp2.value \\
\tt ~~~~~~~~~~~~tmp3.valid = true \\
\tt ~~~~~~~~else: \\
\tt ~~~~~~~~~~~~bool tmp4 = $e4$ \\
\tt ~~~~~~~~~~~~if: \\
...
the type of the result of the second {\tt "else"} whose
first argument then assumes the types given in
\begin{center}
\tt *IF-VALUE* tmp3, $T$ tmp3.value, bool tmp3.valid
\end{center}
As this is the result of the first {\tt "else"}, both
arguments to that {\tt "else"} get the same type and
therefore $T$ becomes the target type of $e1$, $e3$, and $e5$.

Note that the second arguments to both {\tt "if"} and {\tt "else"}
...
inline).  There are context issues here (see \itemref{SCOPE},
\itemref{CONTEXTS}, and the context option of the
{\em block-assignment-statement} in 
\itemref{BLOCK-ASSIGNMENT-STATEMENTS}), which we have ignored
in this section
as they would clutter both the function definitions and the compiled
code of the example.
...
skipped.  This is implemented by defining
...
Here the second arguments are {\tt *DEFERRED*} and if needed are
evaluated in the context of the caller of the {\tt "and"} or
{\tt "or"} functions.  For more on contexts see
\itemref{SCOPE}, \itemref{CONTEXTS}, and the context option of the
{\em block-assignment-statement} in 
...
are not evaluated.  This is implemented by defining comparison
function as follows:
...
define type std *COMPARE-VALUE* as void
    ...
    // Values other than bool and *COMPARE-VALUES* are
    // implicitly convertable to *COMPARE-VALUES*.
...
function Qs *COMPARE-VALUE* r, T r.right op, bool r.value =
                     (| qualifiers Qs, type T |)
		     ( Qs T v ):
    r.right op = v
    r.value = true

// The following is defined separately for various types T
...
As an example of the application of the above, consider:
