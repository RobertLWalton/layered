...
A vector address is a light weight address of a block of memory called
a vector pointer.  A vector address is used to read or write a
number.  The vector address may point to a number at an arbitrary
bit address, and the type and length of the number are encoded in the
vector pointer.

TBD\footnote{One would like to support bit addresses as well as byte addresses,
or at least bit offsets, but some modern computer architectures are not
good at this, so the Layered Languages do not support it.
Similarly it would be nice if the type
and length of numbers could be encoded as a tag in a 64-bit address,
so the same code could handle arrays with different number formats, but
current architectures do not support this.}
