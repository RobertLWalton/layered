...
unspecified `\TT{quali\-fier}' enumeration values.  The \TT{*UNCHECKED*}
...
\emkey{member-index-list} \begin{tabular}[t]{@{}rl}
    ...
    \end{tabular} \\
\emkey{member-index} ::= {\em integer-constant-expression}
\end{indpar}
...
If there is more than one replaceable subname
in a name, ones beginning first, and among these
the longest, is replaced.  After replacement, the complete name is
...
Note here that variables with names that contain {\em member-selectors}
may be cluster variables:
...
An example of a {\em block-assignment-statements} is:
\begin{indpar}\begin{verbatim}
int32 x = 1
float64 y = 1.67
int64 z, *UNCHECKED* uns8 w, next x =:
    ...
    next y = 2.6     // Illegal, y is visible outside block
                     ...
                     //          variable
    w = x            // Legal, sets w to 2 as x is now 266
\end{verbatim}\end{indpar}
...
( mem pp.type out,
  ...
  *UNCHECKED* out.offset.base.adr ) =
    ...
    ( out.offset, out.offset.base.adr ) = if:
        pp.begin < pp.end:
            out.offset = pp.begin
            out.offset.base.adr = pp.base.adr
        else:
            fatal error
...
"*" ( stack pointer pair pp,
    ...
    if:
        pp.begin < pp.end:
	    pp.type v = in
            *UNCHECKED* unsadr v.offset = pp.begin
            mem adr v.base
            *UNCHECKED* adr v.base.adr = pp.base.adr
            *WRITE* v
        else:
            fatal error
                ( "Deferencing empty pointer pair." )
...
Statements that write memory make use of the same cluster structure
as \TT{mem} qualifier variables which are used to read memory;
see Section~\itemref{THE-MEM-QUALIFIER}.

Specifically, memory needs to be addressed.  This is done by a
variable \TT{v} of some type which is called the `\key{RAM cache variable}'
and a RAM address associated with \TT{v} by either:
\begin{enumerate}
\item
A variable \TT{v.adr} of type \TT{adr} with either of the qualifiers
\TT{mem} or \TT{*UNCHECKED*} that is the RAM address associated with \TT{v}.
...
a variable \TT{v.base} of type \TT{adr} and qualifier \TT{mem}
or \TT{*UNCHECKED*}.
In this case the RAM address associated with \TT{v} is \TT{v.base+v.offset}.
If \TT{v.base} has qualifier \TT{mem} it must recursively be
the root of a subcluster obeying
these rules (so there must be either a \TT{v.base.adr} variable
or \TT{v.base.offset} and \TT{v.base\EOL .base}
variables, etc.).
\end{enumerate}
Note that for write statements the RAM cache variable \TT{v} need
\underline{not} have the \TT{mem} qualifier.
...
\begin{indpar}
\emkey{single-value-write-statement} ::= \TT{*WRITE*} {\em write-variable}
\end{indpar}

The {\em write-variable} is a RAM cache variable.
The statement `\TT{*WRITE* v}' simply writes the value of \TT{v} into
the address associated with \TT{v}.
...
\emkey{memory-copy-statement} ::=
    \TT{*COPY*} {\em source-variable} {\em destination-variable}
    		{\em length-variable}
\end{indpar}
The {\em source-variable} and {\em destination-variable} are
RAM cache variables of types and qualifiers such that values
of the {\em source-variable} may be converted to values of the
{\em destination-variable}.  Unless the {\em destination-variable}
has the \TT{*UNCHECKED*} qualifier, this conversion must be implicit.

The {\em length-variable} must be an unsigned integer type.

The `\TT{*COPY*}' statement
uses the source address associated with the {\em source-variable},
the destination address associated with the {\em destination-variable}
and the length stored in the {\em length-variable},
but does not alter any variable.
As long as the length is not zero,
the statement reads from the source address a value of
the {\em source-variable}'s type, converts that value the the
the {\em desination-variable}'s type, and writes the value to the
destination address.  It then decrements the length, adds the
length of the {\em source-variable}'s type to the source address,
and adds the length of the {\em destination-variable}'s type to the
destination address.

If the source and destination overlap the results of the copy are
undefined, for it may read multiple values from the source before
writing any to the destination, or only read one value at a time.
...
\emkey{memory-set-statement} ::=
    \TT{*SET*} {\em source-variable} {\em destination-variable}
    		{\em length-variable}
\end{indpar}
The action is similar to that of the {\em memory-copy-statement},
except that the {\em source-variable} is not a RAM cache variable,
and the value of this variable itself is used as the source value
to be converted to the {\em destination-variable}'s type and
written.
