...
\TT{*UNCHECKED*~(} ~~$t2$ ~~ \TT{<{}<++} ~~ $t1${\em -type-value} ~~\TT{)}%
\label{<<**}
...
The operator is always defined, and the result is as follows:
\begin{itemize}
...
and the $t1$-value is a number within the range values representable
by a $t2$ value.
The result is the rounded $t1$-value,
where the method of rounding is indeterminate.
\item $t2$ is a floating point type,
and the $t1$-value is a number outside the range values representable
by a $t2$ value.
...
The result is a floating point \TT{NaN}.
...
and the $t1$-value is a floating point infinity, \TT{+Inf} or \TT{-Inf},
or a floating point \TT{NaN}.
The result is an arbitrary value, but a \TT{NaN} exception flag is set.
...
as a $t2$ type value.
\item $t2$ is a floating point type,
and the {\em numeric-constant} is within the range values representable
by a $t2$ value.  The result is the rounded {\em numeric-constant},
where the method of rounding is indeterminate.
\end{itemize}
...
\begin{itemize}
\item The implicit conversion `$t2$ \TT{<{}<++} {\em numeric-constant}'
is defined.
...
equals module $2^n$ the integral part of the {\em numeric-constant},
with the fractional part discarded thereby rounding toward zero,
where $n$ is the length in bits of $t2$.
\end{itemize}

Conversions that produce IEEE NaNs set an accured exception flag,
which does not cause an immediate exception, but is separately
tested after a sequence of operations to see if any produced
a NaN, and why the NaN was produced.
...
\TT{type}~\ttkey{qualifier}\TT{~is next enum // 2}
\end{indpar}
...
foo bar int32 result = F ( foo bar int32 value ):
    ...
    // and qualifier bar.  Requires call argument to have
    // qualifier bar (but not foo), and automatically adds
    // qualifier bar (but not foo) to the call result
    // variable.
    //
...
a required qualifier, any call variable matched to this
prototype variable must have the required qualifier.

If a function prototype variable (argument or result)
does \underline{not}
have a prohibited qualifier, any call variable matched to this
...
the matching call variable will always have to have the qualifier.

If a function prototype result variable has a protected
...
any call variable matched to the argument of \TT{F} or \TT{G}
must have the `\TT{bar}' qualifier, which is required.
Any call variable matched to the result of \TT{F} will be
given the `\TT{bar}' qualifier, which is protected.  But this
last is not true of \TT{G}.

In the above example there are no rules regarding the `\TT{foo}'
qualifier, which can be attached to any variable as the
user wishes, as its not required, prohibited, or protected.

In the above example, 
any call variable matched to the argument of \TT{H} must
\underline{not} have the `\TT{oof}' qualifier, as that
is a prohibited qualifier.
Any call variable matched to the result of \TT{H} will
be given the `\TT{oof}' qualifier, as that
is a protected qualifier.
...
\TT{restricted qualifier~}\ttkey{*UNCHECKED*} \\
...
\begin{indpar}
\emkey{name} \begin{tabular}[t]{@{}rl}
    ::=  & {\em base-name} {\em member-selector}\STAR{} \\
    $|$  & {\em member-selector}\PLUS{} \\
    \end{tabular} \\[1ex]
\emkey{name-component} ::= {\em base-name} $|$ {\em member-selector} \\
\emkey{base-name} ::= {\em simple-name} not containing any `\TT{.}'s \\
\emkey{member-name} \begin{tabular}[t]{@{}rl}
    ::= & {\em simple-name} containing just one `\TT{.}'
          that begins its first lexeme \\
	& which must be a {\em word} or {\em mark} with more than one character
    \end{tabular} \\[1ex]
\emkey{member-selector} ::= {\em member-name} {\em member-subscript}\STAR{} \\
...
\emkey{member-index-list} \begin{tabular}[t]{@{}rl}
    ...
    \end{tabular} \\
\emkey{member-index} ::= {\em integer-constant-expression} \\[1ex]
\emkey{simple-name} ::= {\em simple-name-item}\PLUS{} \\
\emkey{simple-name-item} \begin{tabular}[t]{@{}rl}
    ::= & {\em word} $|$ {\em mark} $|$ {\em quoted-name-item} \\
        & [see text about splitting {\em words} with embedded `\TT{.}'s]
    \end{tabular} \\
\emkey{quoted-name-item} ::=
    double quoted (\TT{"~"}) {\em separator}, {\em mark}, or {\em word}
\end{indpar}

The difference between a {\em base-name} and a {\em member-name}
is that the first character of a {\em member-name} is `\TT{.}',
and the first character of a {\em base-name} is \underline{not}
`\TT{.}'.  Furthermore, a {\em word} of any {\em simple-name} that contains
an embedded period `\TT{.}', is split just before the period,
unless the second result lexeme is a {\em numeric} (i.e., contains
a digit not preceded by a letter within the numeric lexeme).\footnote{
Thus the lexeme is split before an embedded period if and only if
the first letter or digit after the period is a letter and not
a digit, or there is no letter or digit after the period.}

Thus \\
\centerline{\TT{bills.wife.1.weight}}
is split into \\
\centerline{\TT{bills~~~.wife.1~~~.weight}}
...
A variable may be declared to be an \key{abbreviation} by prefacing
the `\ttkey{abbreviating}' qualifier to its `\TT{name}' type.  When a name
appears in code being compiled, and the name references a
variable that is an abbreviation, the value of the variable is
subsituted for the name.

More specifically, if a contiguous part
of a {\em name} names a
variable that is an
abbreviation, the contiguous part is replaced, and then this rule
is applied recursively.
The contigous part must begin at the beginning of the {\em name} or
a the beginning of a {\em member-selector} in the name, and must end
at the end of the {\em name}, just before a {\em member-selector},
or just before a {\em member-subscript}
in the name (i.e., the part must be a contiguous sequence of the
{\em name-components} in the {\em name} with ending {\em member-subscripts}
optionally omitted).
If there is more than one contiguous part
that might be replaced, ones beginning first, and among these
the longest, is replaced.

Note that abbreviations whose names begin with a {\em base-name}
can only replace an initial part of a {\em name}, with preference
given to the longest abbreviation.  This is the most common case.

For example:
\begin{indpar}[0.1in]\begin{verbatim}
name ni = `i'
abbreviating name nj = `j'
abbreviating name nnj = `nj'
int32 nj = 5       // Legal: same as j = 5
int32 ni = 6       // Illegal variable overlay: same as ni = 6.
int32 nj = 6       // Legal: same as j = 6
int32 r = nnj      // Legal: same as r = j (nnj => nj => j)
int32 nnj.p = 4    // Legal: same as j.p = 4
name .p = `.q'
int32 nnj.p = 4    // Illegal variable overlay: same as j.p = 4
abbreviating name .r = `.q'
int32 nnj.r = 4    // Legal: same as j.q = 4
int32 nnj.r[0] = 4 // Legal: same as j.q[0] = 4
\end{verbatim}\end{indpar}

Note here that variables may have names that contain {\em member-selectors}:
see Section~\itemref{CLUSTERS} for more details.

A name declaration can be used to abbreviate a \TT{name} variable
assigment followed by an abbreviation declaration:
\begin{indpar}\begin{verbatim}
name ni is i  // same as: abbreviating name ni = `i'
\end{verbatim}\end{indpar}

A statement of the form `\TT{abbreviating name} {\em n} \TT{=} \ldots'
is an assignment statement: see Section~\itemref{ASSIGNMENT-STATEMENTS}
for more details.  The general syntax of the abbreviation declaration is:

\begin{indpar}
{\em abbreviation-declaration-statement} ::=
    \TT{name} ~ {\em abbreviating-name} ~ \TT{is} ~ {\em abbreviated-name}
\end{indpar}
...
option and the absence of a funtion prototype.
...
Note that functions are standardly inline, and not out-of-line.
...
{\em assigment-statements} or `\TT{do}' subblock {\em statements} as for
...
lasts to the end of largest block containing the declaration.
...
A result variable named `\TT{next v}' is not the same variable
or name as `\TT{v}', but, as a special case, `\TT{next v}'
...
with names the call is within scope of.
...
possible type assigments has a minimum, that is used, and if
...
type pointer pair is void
    ...
    //                 last element
    //   mem adr pp.base is the base address that is added
    //                   to the offsets
...
( stack pointer pair pp,
  ...
  *UNCHECKED* adr pp.base.adr,
  type pp.type ) =
    ...
    unsadr offset = allocate to stack ( length )
        // This sets:
        //     stack unsadr offset
        //     mem adr offset.base
...
( mem pp.type out,
  ...
  *UNCHECKED* out.offset.base.adr ) =
        "*" ( stack pointer pair pp,
              ...
              unsadr pp.end,
              adr pp.base.adr ):
    ( out.offset, out.offset.base.adr ) = if:
        pp.begin < pp.end:
            out.offset = pp.begin
            out.offset.base.adr = pp.base.addr
        else:
            fatal error
...
"*" ( stack pointer pair pp,
    ...
    if:
        pp.begin < pp.end:
            *UNCHECKED* unsadr offset = pp.begin
            mem adr offset.base
            *UNCHECKED* adr offset.base.adr = pp.base.adr
            in ---> offset
        else:
            fatal error
                ( "Deferencing empty pointer pair." )
...
or \TT{v.offset.base.offset} and \TT{v.offset\EOL .base\EOL .offset\EOL .base}
variables, etc.).
\end{enumerate}
The the
...
Because \TT{mem} and \TT{*UNCHECKED*} are protected qualifiers,
\TT{v} and the required
...
restricted qualifier that must qualify the \TT{v.adr} or
...
not allowing the variables in the {\em result-variable-list}
of a {\em call-assignment-statement},
{\em out-of-line-assigment-statement}, or
{\em compile-time-assigment-statement},
...
qualifier properties, normally has no effect on code.
...
Better matches must override worse matches.  Fewer implicit
conversions is better.  More cluster member matches is better.
