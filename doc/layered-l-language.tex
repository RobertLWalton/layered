...
\hspace*{0.5in}\begin{tabular}[t]{l}
        \{ {\em variable-list} \TT{=} \}\QMARK{} \TT{if:} \\
	\TT{~~~~}\TT{default:} \\
	\TT{~~~~~~~~~}{\em executable-statement}\STAR{} \\
	\TT{~~~~}\TT{default:} \\
	\TT{~~~~~~~~~}{\em executable-statement}\STAR{} \\
...
subblock qualified by `\TT{initially}'
executes before the first repetition, and it must
set all the variables in the {\em iteration-variable-list}.
The subblock qualified by `\TT{while}
...
subblock qualified by `\TT{finally}'
...
\TT{*REVERSE*} option reverses the order in which the argument
...
registers, by load and store assembler instructions may be used.
The assembly instructions are written in the syntax of one of the
target machine assemblers.

The variables may have additional qualifiers specifying which
...
allowing the variables in the {\em variable-list}
and {\em iteration-variable-list} 
