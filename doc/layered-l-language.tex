...
an \key{unchecked result deferred assignment} if {\tt r} is a prototype
result variable with the \TT{*UNCHECKED*} qualifier in a function
call expansion (the qualifiers
of {\tt r} are determined by call-prototype matching, see
...
(3) unchecked result deferred assignments permit value qualifiers
...
has the \TT{*MEM*} qualifier (see below), it is never
assigned a value by a deferred assignment, and is therefore free to have
any qualifiers.
\end{itemize}\end{indpar}

Unchecked result deferred assignments that change qualifiers may violate
...
\TT{*DEFERRED*}, and \TT{*OPTIONAL*} a pseudo-qualifiers that are
