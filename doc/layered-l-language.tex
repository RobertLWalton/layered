...
function.  The {\tt global} qualifier specifies that the location
...
int function1 ( int * stack * arg1 );
    // Argument is a pointer to a caller allocated location holding a
    // pointer to an int.  The caller need not provide a useful value
    // in this location, and function2 can use `* arg1 = value' to
    // return a pointer to an int value in this location, but cannot
    // use `arg1 = value' to reset the argument.
int function2 ( stack int * stack * arg1 );
    // Argument is a pointer to caller allocated location holding a 
    // pointer to a caller allocated location holding an int.  
    // Both pointers must be provided by the caller and cannot be
    // reset by function2.  The caller can use `**arg1 = value' but
    // not `*arg1 = value' or `arg1 = value'.
...
\hspace*{-1em}\ttkey{global}~~Global memory locations can be
...
Like global memory locations, collectable locations can be
accessed by any function execution and their addresses may be stored
in any place.
...
that \underline{no code} will modify the location.  In C/C++
the `{\tt const}' qualifier in an argument type is a promise that the
\underline{called code} will not modify the location, but not that
some other code will modify the location.  This difference has an
effect on how the location may be cached in registers.
...
Four protection qualifier are builtin:
`{\tt readwrite}', `{\tt readonly}', `{\tt writeonly}', and `{\tt noaccess}'.
These are used only in qualifier expressions that include declared
protection qualifiers.
