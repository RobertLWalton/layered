...
\\[0.5ex]
\emkey{exponent-operand}\label{PRODUCT-OPERAND} ::=
    {\em data-expression} $|$ {\em subexpression}
\\[0.5ex]
...
For example, given the expression `\TT{v == missing}', where \TT{v}
is of `\TT{qualifier} type, \TT{missing}, which has no natural type, will be
...
{\tt function $T$ r = ( $T$ $v1$ ) "$op$" ( unshw $v2$ )}
\begin{indpar}
...
Each operator is evaluated as the function call
`{\tt "!" $e1$}' or the function call
`{\tt $eI$ "$bop$" $e(I+1)$}' as soon as its operands have
...
Each operator is evaluated as the function call
`{\tt $eI$ "$shopI$" $e(I+1)$}' as soon as its operands have
...
The $eI$ are evaluated left to right.
Each operator is evaluated as the function call
`{\tt "$sop1$" $e1$}' or the function call
`{\tt $eI$ "$sop(I+1)$" $e(I+1)$}' as soon as its operands have
been evaluated, so the operators are
also evaluated left to right.
...
The $eI$ are evaluated left to right.
Each operator is evaluated as the function call
`{\tt "$eI$ "*" $e)I+1)$}' or the function call
`{\tt $eI$ "$dop(I+1)$" $e(I+1)$}' as soon as its operands have
been evaluated, so the operators are
also evaluated left to right.
...
\begin{indpar}
The $eI$ are evaluated left to right.
The operator is evaluated as the function call
`{\tt $eI$ "\textasciicircum" $e(I+1)$}' as soon as its operands have
been evaluated.
