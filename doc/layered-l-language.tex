...
{\em base-name} ::= {\em simple-name} $|$ {\em member-selector} \\[1ex]
...
{\em member-selector} \begin{tabular}[t]{@{}rl}
    ...
    \end{tabular} \\[1ex]
{\em member-name} ::= {\em simple-name} \\[1ex]
...
A variable of type \ttkey{name} can store either a {\em name}
or a {\em member-selectors}.  Name
constants are made by \TT{`~'} quoting a {\em name}
or {\em member-selectors}.  A {\em name} value and a
{\em member-selectors} value can be concatenated by the \TT{+} operator.  An
example is:
\begin{indpar}\begin{verbatim}
name n1 = `my name'
name n2 = `.husband'
