...
Free stacks are dynamic blocks and can be moved.
Bound stacks cannot move independently,
but move whenever their containing free stack moves.
...
the stack address.  To accommodate this stack addresses are stored
in dynamic pointers.

Free stacks may be moved when dynamic blocks are allocated or
increased in size.
...
Generally when a free stack is deallocated, its body pointer is set
to \TT{NULL}, so if unchecked code tries to use deallocated
stack memory, it will suffer a memory fault.
...
\begin{indpar}
Allocate a {\tt size} byte block of stack memory with the given
{\tt alignment} in bytes.  The block is allocated to the current
stack.

Return the offset of the block within the current stack as {\tt r.offset},
the value of the body pointer of the current stack as {\tt r.base},
and the address of this body pointer as {\tt r.base.adr}.
Here {\tt r.base} may change when the stack is moved, but the other
returned values remain fixed.

If the {\tt alignment} is omitted or is given as 0 it is taken
to be equal to \TT{size}.  The explicit or implied
alignment must be a power of two not greater than {\tt 64}
(which is the alignment of the stack itself).

The memory bytes are initialized to zero.

The `\TT{allocate to stack}' inline function may move stacks.
...
The memory containing the stack body pointer may also be
freed at this time. 
...
The offset of the first element relative to the {\tt base}.
Note that the offset can be negative.

\hspace*{-0.3in}{\tt intadr \ttdmkey{end}{of pointer}} \\
The offset just after the last element relative to the {\tt base}.
This may not be less than \TT{.begin}.

\hspace*{-0.3in}{\tt *READ* adr \ttdmkey{base}{of pointer}} \\
The base address of the vector.  The first element of a vector pointed
at by {\tt p} is at address {\tt p.base + p.begin} and the address
{\tt p.base + p.end} points just after the last element of the vector.

\hspace*{-0.3in}{\tt adr \ttdmkey{base.adr}{of pointer}} \\
The address from which {\tt .base} is read.
