...
is copied to another variable.  A variable qualifier may \underline{not}
be attached to a prototype argument variable, and any variable qualifiers
attached to an actual argument are ignored.
A variable qualifier may be attached to a prototype result variable.
...
and prototype result variables.

A qualifier must be a variable qualifier or a value qualifier,
but can not be both.

Given an assignement statement `$v1$ \TT{=} $v2$', if $v1$ has
...
that qualifier is said to have been \mkey{deleted} by the assignment.
...
compiles as
\begin{center}
\tt $Qvars$  $Qvals$ $T$ x = implied conversion ( y )
\end{center}
