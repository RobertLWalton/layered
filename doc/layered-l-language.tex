...
Qualifiers may also be applied to code blocks.  A qualifier just before the
...
specifies that the location will last as long as no out-of-line function
is called.
...
the location is valid until an out-of-line function is called.
Only out-of-line functions may deallocate or reallocate the location.
A function may return a `{\tt temp}' location to its caller,
or pass a `{\tt temp}' location to a called function.  In the
latter case the called function must not deallocate or reallocate
any location except by itself calling an out-of-line function.

\hspace*{-1em}\ikey{Collectible Qualifiers}%
{collectible qualifier}~~~~~Collectible
qualifiers are declared by the programmer and are used to tag
memory locations that can be garbage collected.

Addresses of collectible locations may only be written by checked
code into stack locations
that are declared to be addresses of
locations with the same collectible
...
to the stack.  Often addresses of collectible locations are made
traceable so the garbage collector can find them in the stack:
see \secref{TRACEABLE-QUALIFIERS}.

Functions whose bodies are given a collectible qualifier may write
addresses to locations with this qualifier to non-stack
locations declared as containing an address to a location with
the same collectible qualifier.
...
collectible qualifiers are considered to be different),
...
`{\tt caller}' qualifier.  Note that this means that a location
with collectible qualifier whose address is passed by a caller
to a called function must be protected from garbage collection
by the caller.
