...
any result variable names or block names declared by
the part of the statement before that block are declared
outside the block (\pagref{BLOCK-ASSIGNMENT-STATEMENTS}).
{\em Block-names} follow the same rules as other {\em names},
and can hide or be hidden by other kinds of {\em names}.
...
within assignment statements that have the `\TT{in context}' option.
...
\hspace*{0.5in}\begin{tabular}[t]{l}
	...
	\TT{~~~~}\{ \ttkey{do} {\em block-name}\QMARK{}
	              \{ \ttkey{in context} {\em context-expression} \}\QMARK{}
	              \}\QMARK{} \TT{:} \\
...
where `\TT{do} {\em block-name}\QMARK{}
	              \{ \TT{in context} {\em context-expression} \}\QMARK{}'
...
every one of the block's deferred result variables is given a value
...
the exit statement, the deferred result variables of this smallest
...
\emkey{initial-function-term-name}\label{INITIAL-FUNCTION-TERM-NAME} ::=
    ...
    that does not begin with a {\em pattern-argument-list} \\
...
\emkey{argument-declaration}
    ::= {\em type-name} {\em variable-name}
        \{ \TT{?} {\em default-value}\QMARK{} \}\QMARK{}
\\[0.5ex]
...
or a `\TT{[]}' bracketed {\em pattern-argument-list}, unless it
...
A {\em pattern-argument-list} that ends with `\TT{?}' is considered
...
Examples are:
\begin{indpar}\begin{verbatim}
function int32 r = fee ( int32 x ) fie ( int32 y ) foe ( int32 z ):
    ... function body omitted ...
...
int32 f = fee ( b 1 ) foe ( c 2 ) fie ( d 3 )
    // Call same as = fee ( b 1 ) fie ( d 3 ) foe ( c 2 )
int32 f = fee ( b 1 ) foe ( c 2 )
    // Call same as = fee ( b 1 ) fie () foe ( c 2 )
...
function int32 r = foo ( int32 x, int32 y ? B + C, bool z ? )
    ... function body omitted ...

int32 b 1 = ...
int32 f = foo ( b 1 )
    // Call same as:
    //     int32 tmp y, bool tmp z = do in context fooC:
    //         tmp y = B + C
    //         tmp z = false
    //     int32 f = foo ( b 1, tmp y, tmp z )
\end{verbatim}\end{indpar}
...
If the {\em qalifiers-argument-declaraiont} is
in an argument list with `\TT{(|~|)}' brackets,
...
of the {\em function-prototype}.  There may be one or more target
variables of known types, or a single target variable of unknown
type, or no target variables (as in a {\em function-call} that is
...
As a special case, if an {\em argument-de\-clar\-a\-tion} ends in
...
{\em regular-argument-list} do not have {\em default-values},
the call-prototype match fails.

The modified {\em function-call} now matches
...
and must have a compile time value which is a list of qualifiers, else
the call-prototype match fails.  If the match does not fail,
the list of qualifiers becomes the value of the {\em variable-name}
...
is used as the sole {\em qualifier-name} of prototype result variables
...
the list of qualifiers that is the assigned value
...
Otherwise if the {\em variable-name}
is used the sole {\em type-name} of several arguments whose matching
...
follows, the actual argument expression can be compiled at this
time with its target value taken from the corresponding
...
If the value assigned to the {\em variable-name} of a
{\em qualifiers-conditioner} is not a (possibly empty) list of
qualifiers, the call-prototype match fails.
Otherwise this qualifiers list value is substituted for the {\em variable-name}
throughout the {\em function-prototype}.
...
If any the statements `{\tt $QsI$ $TI$ $tmpI$ = $eI$}' cannot be compiled
...
unknown type, {\em function-prototype} must have result variables
...
type and qualifiers of the {\em function-call} expression if
