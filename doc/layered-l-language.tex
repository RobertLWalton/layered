...
\emkey{function-term}
    ...
    $|$ & \{ {\em argument-variable-list}\PLUS{}
        ...
        & ~~ {\em argument-variable-list}\PLUS{}
	     {\em function-term-modifier}\QMARK{} \\
    $|$ & \{ {\em argument-variable-list}\PLUS{}
	     {\em function-term-name} \}\PLUS{}  \\
        & ~~ {\em function-term-modifier}\QMARK{} \\
    \end{tabular}
\\[0.5ex]
\emkey{function-term-modifier} ::= \TT{?} $|$ \TT{...} $|$ \TT{...~?}
\\[0.5ex]
\emkey{function-term-name} ::= {\em name} see \pagref{NAMES}
\\[1ex]
...
At this point the function call and the resulting pattern must match,
in that
{\em function-term-names} must match exactly, after adjustments are
made as described below for alternatives and booleans, and
argument list brackets must match exactly,
while the {\em argument-variable-lists} match as described below.
To make this work,
no two {\em function-terms} in a {\em function-pattern} may begin with
the same {\em function-term-name} or {\em opening-argument-bracket}.
