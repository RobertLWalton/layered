...
must not have \TT{void} as its underling type.
...
\begin{indpar}
{\tt define implied variable qualifier \ttkey{std *MEM*}} \\
{\tt define abbreviation \ttakey{MEM} ---> `std *MEM*'} \\
{\tt define implied variable qualifier \ttkey{std *WRITABLE*}} \\
...
\TT{*OPTIONAL*} \\
\end{center}
These two are qualifiers only syntactically, and have their own semantics
...
assignment, the types of {\tt u} and {\tt w} must be identical,
...
now define the different kinds of assignments.
...
\item {\tt r = \ldots}
	\begin{tabular}[t]{l}
	is called a \key{deferred result assignment} \\
	({\tt r} is a prototype result variable)
	\end{tabular}
\item {\tt \ldots{}~u = r}
	\begin{tabular}[t]{l}
	is called a \key{final result assignment} \\
	({\tt r} is a prototype result variable)
	\end{tabular}
\item any other assignment {\tt \ldots~x = y} for variables {\tt x}
      and {\tt y} is called a \key{ordinary assignment}
\end{itemize}
\end{indpar}
...
If {\tt w} is a function prototype result variable,
the assignment a final result assignment.
If {\tt u} is a prototype argument variable,
the assignment is an argument assignment.
If {\tt u} is a prototype result variable,
the assignment is a deferred result assignment (and {\tt u}
is not preceeded by a type or qualifiers).
Otherwise the assignment is an ordinary assignment.

We distinguish these kinds of assignments because for direct
assignments:
(1) implied value qualifiers of {\tt w} are implicitly
added to {\tt u} for result final and ordinary assignments, but
not for result deferred and argument assignments; (2)
implied variable qualifiers of {\tt w} are implicitly added to
{\tt u} for result final assignments, but not for other assignments;
and (3) {\tt u} may have explicit variable qualifiers only for result final
assignments.
...
\underline{and} the assignment is a result assignment.
In the latter case
the qualifier will be implicitly added to {\tt u}.
\item[(DA4)]
If the assignment is not a result assignment,
...
However, if the qualifier is \mkey{implied}{qualifier}
...
must be assigned its value by a result assignment whose prototype
result variable {\tt w} has the qualifier.  If the qualifier is not
implied, {\tt u} must be given the qualifier explicitly.
\end{itemize}\end{indpar}

A direct assignment {\tt \ldots{}~u = w} is 
implemented by simply copying the value of {\tt w} to {\tt v}.

As specified on page \pageref{EXPLICIT-ASSIGN}, if any
assignment
\begin{center}
\tt $Qsu$ $Tu$ u = w
\end{center}
is \underline{not} a direct assignment, it is
re-written as
\begin{center}
\tt $Qsu$ $Tu$ u = assign ( w )
\end{center}
This applies to both non-result and result assignments.
If no {\tt assign} function is defined for the given types and
qualifiers, the assignment fails to compile.

Non-addable value qualifiers may be added and non-deletable value qualifiers
deleted by the following builtin un\-check\-ed function:
\begin{center} \tt
function \begin{tabular}[t]{@{}l@{}}
         \TT{*UNCHECKED*}~Qrs~T r = std \ttkey{re-qualify} \\
...
This function \underline{cannot} add
\underline{variable} qualifiers.
...
As a variable qualifier, \TT{*MEM*} can only be attached to
...
variables, the rightmost is used (e.g., prototype arguments take
precedence over prototype results).
However any actual result variable used must have
has known type, and any actual argument variable used must have
...
variables, the rightmost is used (e.g., prototype arguments take
precedence over prototype results).
However any matcing call result variable used must have
have known type, and any matching call argument variable used must have
a natural type (\pagref{NATURAL-TYPE}), else the call-prototype match
fails.
...
If $QRs1$ $TR1$ and $QCs1$ $TC1$ are not exactly equal after implied
...
function *STACK* pointer pair pp,
    ...
    *UNCHECKED* void p = allocate to stack ( size )
        // p gets protected qualifier *STACK* but this
        // is not implicitly propagated to pp.  The
        // fact that p is *UNCHECKED* indicates that it
...
The \ttakey{MEM} qualifier is a implied variable qualifier that behaves
...
Because \TT{*MEM*} and \TT{*WRITABLE*} are implied qualifiers,
