...
A declaration statement of the following form defines a type and
specifies its {\em type-name}:
\begin{indpar}
...
The simplest {\em base-type-expression} is just the name
of another type, the `\key{base type}' of the new type defined
by the {\em defined-type-declaration}.
The {\em defined-type-name} names this new `\key{defined type}.
...
or can be used to name a constant equal an enumeration value
that is the identifier of the type as in `{\tt T == int32}',
or can be used
...
use as a constant equal to an enumeration value that is the
identifier of a type (\pagref{TYPE-VALUES}).

The following builtin functions are defined:
\begin{indpar}
{\tt function $T$ r = $T$ ( \TT{*UNCHECKED*} $B$ v )} \\
{\tt function $B$ r = $B$ ( \TT{*UNCHECKED*} $T$ v )}
...
\emkey{enum-base}
    ...
    $|$ & {\em enum-direction}\QMARK{}~~~{\em enum-type-expression} \\
    \end{tabular}
\\[0.5ex]
...
This definition is recursive, so the underlying type is always an
integer type.
...
of enumeration type {\tt t} are convertable to $I$
\end{center}
...
{\tt function $B$ r = $B$ ( \TT{*UNCHECKED*} $E$ v )} \\
{\tt function $E$ r = $E$ ( \TT{*UNCHECKED*} $B$ v )}

\begin{center}
where $E$ is an {\em enum-type-name} \\
and $B$ is any {\em type-name}
for the base type of $E$
\end{center}
\begin{indpar}
These are conversions that do nothing but change the type of the value.
\end{indpar}

{\tt function $I$ r = $ma$\QMARK{} \ttkey{range value of} ( $E$ v )}

\begin{center}
where $E$ is an {\em enum-type-name}, \\
$ma$ is the {\em module-abbreviation} of $E$ if any, \\
and $I$ is the first type in the sequence \\
\TT{intwrd     unswrd     intmax     unsmax} \\
such that both the {\em first-enum} and the {\em last-enum} \\
of enumeration type $E$ are convertable to $I$
\end{center}
\begin{indpar}
The numeric value of {\tt v} is returned, where this numeric value is
the value in the range {\tt [}{\em first-enum}{\tt ,} {\em last-enum}{\tt ]}.
\end{indpar}
...
every `\TT{previous}' replaced by `\TT{next}', and
\begin{center}
\TT{enumeration offset of (}{\em enum-type-name}\TT{)} \\
{\tt =~$I$(V0)~-~}{\em last-enum}
\end{center}
...
{\tt E} must be an enumeration type.
\end{indpar}
...
conversion defined for them, or if after implied conversion the
common type is not a defined enumeration type.
Note that the common type can be an integer type if all
{\em enum-values} are implicitly convertible to integer values.
...
A type $T2$ may be implicitly convertible to type $T1$ while
$T1$ is explicitly convertible to $T2$.
...
\hspace*{0in}{\tt ~~~~~~~~~~~~~ ( *UNCHECKED* uns64 v,} \\
