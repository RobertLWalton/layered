...
If $T$ $\equiv$ \TT{bool}, the second {\tt "<"} has a
choice between the two function prototypes:
\begin{center}
{\tt function ... r = ( bool arg1 ) "<" ( bool arg2 )} \\
{\tt function ... r = \begin{tabular}[t]{l}
                      ( *COMPARE-VALUE* arg1 ) \\
		      "<" \\
		      ( *DEFERRED* bool arg2 ) \\
		      \end{tabular} }
\end{center}
The first choice requires that an implied conversion be applied to
the value of `{\tt ($e1$ "<" $e2$)}', and by rule
\ref{MORE-RESTRICTIVE-ARGUMENT-TYPES-PREFERRED} on
\pagref{MORE-RESTRICTIVE-ARGUMENT-TYPES-PREFERRED}
the prototype that does not require implied conversion is preferred.
