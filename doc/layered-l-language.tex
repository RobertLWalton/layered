...
or at least two {\em pattern-argument-lists} that have either `\TT{()}'
...
(see Rule \ref{TYPE-NAME-RULE} on \pagref{TYPE-NAME-RULE}).
\item
...
avoid conflicts, and variables declared inside the
...
are {\em expressions} with compile-time values, and such a
{\em default-value} can be replaced by a constant when the
...
If any $eI$ a {\em default-value}, then in the above
\begin{indpar}
{\tt $QsI$ $TI$ $tmpI$ = $eI$}
\end{indpar}
is replaced by
\begin{indpar}
{\tt $QsI$ $TI$ $tmpI$ = do in context }{\em prototype-context}{\tt :} \\
