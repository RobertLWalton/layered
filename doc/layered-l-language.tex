...
that the value will not be assigned until later.  The statement
assigning the value must be in the same extended module
...
comprise a `\key{block}' of code.  Normally these statements
are compiled in the context (\pagref{CONTEXT}) of the
{\em block-assignment-statement}, but if `\TT{in context }{\tt c}'
is given, they are compiled in context `{\tt c}' with the
{\em block-assignment-statement} result variables appended
to that context.
...
If the `\TT{in context} {\em context-expression}' clause is given, the
compilation is in the context given by the clause, with the
result variables appended, instead of the
...
The POSIX wildcards `\TT{*}' and `\TT{?}' can be used in the
