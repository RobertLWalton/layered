...
the compiler will check to see if there is any function with a
prototype of the form

\begin{center} \tt
function \begin{tabular}[t]{@{}l@{}}
	 \ldots~$T$ r = \\
	 $ma$\QMARK{} \ttkey{assignment}
	 \{ (| qualifiers \ldots{} |) \}\QMARK{}
	 ( \ldots~$T1$ v )
	 \end{tabular}
\end{center}

If so, the compiler will change the second statement to
\begin{center}
\tt $T$ v = assignment ( w )
\end{center}\label{ASSIGNMENT}
Otherwise the compiler will simply copy the underlying type value of
$w$ to $v$.
...
function Qs~$T2$ r =
		      \begin{tabular}[t]{@{}l@{}}
                      $ma$\QMARK{} \ttkey{implied conversion} \\
...
Context values are never needed at run-time if the program contains
no compile-and-run statements.
...
Thus the \underline{value} qualifiers of {\tt v} are treated
as prototype argument qualifiers with {\tt w} the actual argument
corresponding to {\tt v}.  The \underline{variable} qualifiers
of {\tt w} are ignored in these assignments.  Variable
qualifiers of {\tt v} are allowed only if they are not
protected.
...
In this example \TT{*MEM*}\label{MEM} is a special builtin qualifier
which can be applied to variables.
It has the special property that given a variable
...
The \TT{*STACK*} qualifier in the example has the property that values with
...
Cluster member names may be used as prototype argument
names, if the root of these names is a prototype argument.
Actual call arguments corresponding to such prototype arguments
\underline{must not} be given explicitly in the call itself, but
are implicitly taken from actual members of their root values that are
explicitly given in the call.
If a prototype argument with a members name
does not have a corresponding member of its
root value given explicitly in the call,
the function prototype does not match the call unless the prototype
member has a default value (see \pagref{DEFAULTS-TO} ).

Cluster member names (not of the form `\TT{next} \ldots')
may be used as prototype result
names, if the root of these names is either a prototype result or
a prototype argument.
Actual result variables corresponding to such prototype results
\underline{must not} be given explicitly in the call itself.
The actual result members must \underline{not exist} before the
call, and will be created by the call.


A prototype result may be of the form
`\TT{next} $v$' where $v$ is a prototype argment that may be a cluster
member.  In this case an actual call argument corresponding to $v$ must
be given as above, unless $v$ has a default argument, in which case
an actual member variable corresponding the the prototype argument $v$
is created with the given default value before the call is executed.
...
However a prototype result may be of the form `\TT{next} $v.m$' where $v$ is
the name of a prototype argument that matched to an cluster root
call actual argument and $.m$ denotes member selectors (that may be
subscripts).  In this case $v.m$ may or may not pre-exist
the call: it will be created if it does not pre-exist.

Importantly, cluster members may only be created or updated
(via `\TT{next}': see \pagref{NEXT-CALL-PROTOTYPE-MATCHING})
...
The type of the variable \TT{v} must \underline{not} have \TT{void} as its
underlying type.
