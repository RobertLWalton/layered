...
may execute (only the first can be a non-default subblock);
if this happens, the order of execution is the order
...
It is also permissible for the \TT{initially} subblock to set
both \TT{v} and `\TT{next v}' for some variable \TT{v}, in which
case the \TT{while} subblock must set `\TT{next next v}' and not
`\TT{next v}.  Here the \TT{initially} block is setting \TT{v} for
both the first and second iteration, and each iteration is setting
\TT{v} for the interation after the iteration after the current
iteration.

However, no variable may be set more than once.  If an
\TT{initially} statement sets \TT{next v}, 
no \TT{while} statement may set \TT{next v}, and vice versa.
...
When the third iteration of the loop needs to start, the first
iteration can be garbage collected, as it were, and so forth.
...
\TT{mem} qualifier in the example above.  In order for a function
...
\subsection{The Reload Command}
\label{THE-RELOAD-COMMAND}

The reload command has the syntax:
\begin{center}
\ttkey{reload} {\em qualifier-name} \{ \TT{,} {\em qualifier-name} \}\STAR{}
\end{center}

This command declares the values of all variables with the designated
qualifiers to be out-of-date and unusable.  If any of these variables
is used subsequently, statements to recompute and the members of
its cluster are automatically inserted into the code.

If \TT{q} is one of the designated qualifiers and
a variable \TT{v} has been declared by the statements:
\begin{center}
\begin{tabular}{l}
\TT{q mem} {\em type-name} \TT{v} \\
\TT{adr v.adr =} {\em expression}
\end{tabular}
\end{center}
...
are implied and \TT{v} is reloaded from memory.

If instead \TT{v} has been declared by the statement:
\begin{center}
\TT{q} {\em type-name} \TT{v =} {\em expression}
\end{center}

then the statement:
\begin{center}
\TT{next} \TT{v =} {\em expression}
\end{center}

is implied and \TT{v} is recomputed.

In many cases the compiler will be able to show that recomputation
will do nothing, the implied statements will be converted to variable
renames by the compiler, and no actual executable code will be
produced for the implied statements.
