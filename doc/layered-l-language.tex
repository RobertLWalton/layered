...
\emkey{number} \begin{tabular}[t]{rl}
    ...
    $|$ & {\em middle-lexeme} with a {\em digit} immediately after
                               a `\TT{+}' or `\TT{-}' \\
    $|$ & {\em middle-lexeme} with a `\TT{\#}' and also a {\em letter}
				or {\em digit}
	\end{tabular}
\\[0.5ex]
...
The L-Language uses several subcategories of names:
...
{\em type-name}, of a {\em variable-base-name}, or of a {\em term-name}
unless the latter follows an \TT{=} or {\em prototype-argument-list}
in a {\em function-prototype}.
\item A {\em qualifier-name} may not be an initial segment of another
{\em qualifier-name}, of a {\em type-name}, of a {\em variable-base-name},
or of a {\em term-name}.
\item
...
{\em function-call} beginning with a {\em term-name}
...
\begin{indpar}
{\tt function uns$x$~~~r = std~}\ttkey{uns$x$}{\tt ~~~( uns$y$ v )} \\
{\tt function int$x$~~~r = std~}\ttkey{int$x$}{\tt ~~~( uns$y$ v )} \\
{\tt function float$x$~r = std~}\ttkey{float$x$}{\tt ~( uns$y$ v )} \\
{\tt function int$x$~~~r = std~}\ttkey{int$x$}{\tt ~~~( int$y$ v )} \\
{\tt function float$x$~r = std~}\ttkey{float$x$}{\tt ~( int$y$ v )} \\
{\tt function float$x$~r = std~}\ttkey{float$x$}{\tt ~( float$y$ v )}
...
can be \ikey{implied}{implicit conversion}.
\end{indpar}

{\tt function uns$x$~~~r = std~}\ttkey{mod}{\tt ~~~( uns$y$ v )} \\
{\tt function uns$x$~~~r = std~}\ttkey{mod}{\tt ~~~( int$y$ v )} \\
{\tt function int$x$~~~r = std~}\ttkey{mod}{\tt ~~~( uns$y$ v )} \\
{\tt function int$x$~~~r = std~}\ttkey{mod}{\tt ~~~( int$y$ v )}
\ttindex{mod}
...
{\tt function uns$x$~~~~~r = std~}\ttkey{nearest}{\tt ~~~( uns$y$ v )} \\
{\tt function uns$x$~~~~~r = std~}\ttkey{nearest}{\tt ~~~( int$y$ v )} \\
{\tt function uns$x$~~~~~r = std~}\ttkey{nearest}{\tt ~~~( float$y$ v )} \\
{\tt function int$x$~~~~~r = std~}\ttkey{nearest}{\tt ~~~( uns$y$ v )} \\
{\tt function int$x$~~~~~r = std~}\ttkey{nearest}{\tt ~~~( int$y$ v )} \\
{\tt function int$x$~~~~~r = std~}\ttkey{nearest}{\tt ~~~( float$y$ v )} \\
{\tt function float$x$~~~r = std~}\ttkey{nearest}{\tt ~~~( uns$y$ v )} \\
{\tt function float$x$~~~r = std~}\ttkey{nearest}{\tt ~~~( int$y$ v )} \\
{\tt function float$x$~~~r = std~}\ttkey{nearest}{\tt ~~~( float$y$ v )}
\ttindex{nearest}
...
\begin{indpar}
{\tt function float$x$~~~r = std~}\ttkey{round}{\tt ~~~~~( float$x$ v )} \\
{\tt function float$x$~~~r = std~}\ttkey{ceiling}{\tt ~~~( float$x$ v )} \\
{\tt function float$x$~~~r = std~}\ttkey{floor}{\tt ~~~~~( float$x$ v )}
...
{\tt function \TT{*UNCHECKED*} uns$x$~~~r =~}%
	\ttmkey{uns$x$}{conversion function}{\tt ~~~( int$x$ v )} \\
{\tt function \TT{*UNCHECKED*} uns$x$~~~r =~}%
	\ttmkey{uns$x$}{conversion function}{\tt ~~~( float$x$ v )} \\
{\tt function \TT{*UNCHECKED*} int$x$~~~r =~}%
	\ttmkey{int$x$}{conversion function}{\tt ~~~( uns$x$ v )} \\
{\tt function \TT{*UNCHECKED*} int$x$~~~r =~}%
	\ttmkey{int$x$}{conversion function}{\tt ~~~( float$x$ v )} \\
{\tt function \TT{*UNCHECKED*} float$x$~r =~}%
	\ttmkey{float$x$}{conversion function}{\tt ~( uns$x$ v )} \\
{\tt function \TT{*UNCHECKED*} float$x$~r =~}%
	\ttmkey{float$x$}{conversion function}{\tt ~( int$x$ v )}
...
\\[0.5ex]
\emkey{decimal-constant} ::: = {\em sign}\QMARK{} {\em decimal-integer}
...
\emkey{dit} :::= \TT{0} $|$ \TT{1} $|$ \TT{2} $|$ \TT{3} $|$ \TT{4}
                     $|$ \TT{5} $|$ \TT{6} $|$ \TT{7} $|$ \TT{8} $|$ \TT{9}
 \\[0.5ex]
\emkey{binary-constant} ::: = {\em sign}\QMARK{} \TT{B\#} {\em binary-integer}
...
\hspace*{0.5in}
    {\em sign}\QMARK{} \TT{X\#} {\em hexadecimal-integer}
...
{\em variable-name}, or the first {\em term-name} of a 
{\em function-prototype} if that {\em term-name} does not
...
may be defined as desired: see \itemref{IMPLIED-CONVERSIONS}.
\end{indpar}
...
\hspace*{0.5in}
    \TT{type} {\em qualifier-name}\STAR{}
    \begin{tabular}[t]{@{}l@{}}
    {\em enum-type-name} \TT{is}~~~{\em base-enum-type-name} \\
...
type $I$.  As long as the `\TT{without implicit conversion}'
...
\begin{indpar}
{\tt function $I$ r =~}\ttkey{enumeration offset of}{\tt ~( type E )}
\begin{indpar}
Returns the enumeration offset of \TT{E}.
\end{indpar}
{\tt function $I$ r =~}\ttkey{range value of}{\tt ~( $E$ v )}
\begin{indpar}
...
\begin{center}
\begin{tabular}{l}
{\em base-enum-type-name} \TT{~V0~~=~~next enum} \\
{\em base-enum-type-name} \TT{~V1~~=~~next enum} \\
{\em base-enum-type-name} \TT{~V2~~=~~next enum} \\
\ldots\ldots\ldots \\
{\em base-enum-type-name} \TT{~V$n$~~=~~next enum} \\
\end{tabular}
\end{center}
...
\end{center}
Therefore if implicit conversions are allowed, the first statement of the form
\begin{center}
{\em enum-type-name} \TT{~V~~=~~next enum} \\
\end{center}
compiled after the {\em enum-type-declaration} will make
\TT{$I$(V) == $I$(V0)}, and the first statement of the form
\begin{center}
...
of values of type $E$ are defined as if implicit conversion
...
\begin{indpar}
{\tt type~}\ttkey{type}{\tt ~is unswrd with enum [0~..]} \\
{\tt type~}\ttmkey{missing}{type}{\tt ~is next enum // 0} \\
{\tt type~}\ttkey{type}{\tt ~is next enum // 1}
\\[1ex]
{\tt type~}\ttkey{qualifier}{\tt ~is unswrd with enum [0~..]} \\
{\tt qualifier~}\ttmkey{missing}{qualifier}{\tt ~is next enum // 0}
\\[1ex]
{\tt type~}\ttkey{bool}\begin{tabular}[t]{@{}l@{}}
                       {\tt ~is unswrd with enum [0~..~1]} \\
                       {\tt ~without implicit conversion} \\
		       \end{tabular} \\
{\tt bool~}\ttkey{false}{\tt ~is next enum // 0} \\
{\tt bool~}\ttkey{true}{\tt ~~is next enum // 1}
\end{indpar}
...
\hspace*{0.2in} \TT{type} {\em qualifier-name}\STAR{} {\em enum-set-type-name}
		\TT{is set of } {\em element-enum-type-name}
\\[0.5ex]
\emkey{enum-list-type-declaration} ::= \\
\hspace*{0.2in} \TT{type} {\em qualifier-name}\STAR{} {\em enum-list-type-name}
    \TT{is list of } {\em element-enum-type-name}
\end{indpar}
...
\begin{indpar}
{\tt type~}\ttkey{types}{\tt ~is set of type} \\
{\tt type~}\ttkey{qualifiers}{\tt ~is set of qualifier}
\end{indpar}
...
\begin{indpar}
    Defined as an implicit conversion \underline{but only if}
    an implicit conversion from $B1$ to $B2$ is defined
\end{indpar}
...
{\tt function bool r =
    ($E$ e) \ttmkey{in}{enumeration set} ($S$ s)}
\begin{indpar}
...
\begin{indpar}
{\tt type~}\ttkey{type}{\tt ~is unswrd with enum [0~..]} \\
{\tt type~}\ttkey{missing}{\tt ~is next enum // 0} \\
{\tt type~}\ttkey{type}{\tt ~is next enum // 1}
\end{indpar}
...
$T2$ if and only if \TT{$T1$~-{}-{}->~$T2$} is required to be acyclic.
...
With this in minde, the folloiwing builtin functions may be
...
\begin{indpar}
{\tt type~}\ttkey{std character}{\tt ~is uns32} \\
\ttkey{character}{\tt ~-{}-{}->~`std character'}
\end{indpar}

A character constant has the syntax:
\begin{indpar}
\emkey{character-constant} :::= \TT{C"}{\em character-representative}\TT{"} \\
...
{\tt function character r = std character ( uns32 c )}
\begin{indpar}
Returns the character with UNICODE character code equal to {\tt c}.
\end{indpar}

{\tt function uns32 r = std code ( character c )}
\begin{indpar}
Returns the UNICODE character code of the character {\tt c}.
\end{indpar}

\TT{QS uns32 r = std ( qualifiers QS, QS character c )}
\begin{indpar}
Implicit conversion of character {\tt c} to its UNICODE character code.

Because of this conversion,
characters can be compared with \TT{<}, \TT{<=}, etc.
\end{indpar}
...
A `\key{string}' is a vector of 32-bit UNICODE characters.
It is defined as a derived type:
\begin{indpar}
{\tt type~}\ttkey{string}{\tt ~is adr}
\end{indpar}
and {\em quoted-string} lexemes are constants of \TT{string} type
whose characters are those represented by the lexemes with
the addition of a \TT{NUL} character (code \TT{0}) at the end.

Strings are read-only, and their characters cannot be changed.
When computed at compile time, they cannot be garbage collected.
They may or may not be implemented at run-time, and if they are,
will have library implementation dependent memory allocation
schemes.

Strings can be accessed by `\skey{string pointer}s' which are
also derived types.  A string pointer does not have a value
but is instead a basis for a cluster of variables that
can exist only in the local memory of a frame: see
\itemref{CLUSTERS}.
...
\begin{indpar}
\emkey{name} ::= {\em initial-name-component} {\em member-selector}\STAR{} \\
\emkey{initial-name-component} ::=
    {\em name-component} \underline{not} beginning with `\TT{.}' \\
\emkey{member-selector} ::= {\em member-name} $|$ {\em member-subscript} \\
...
Then \TT{T1~-{}-{}->~T2} if and only if there is a (possibly zero size)
