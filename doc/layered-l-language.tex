...
The special type \TT{ANY-TYPE} can be used as the type of a prototype result
variable that is the root of a cluster in the prototype but which
is not being assigned a value by the function.  The actual result
variable in a call to the function can have any type.  The
`\TT{allocate to stack}' builtin function uses \TT{ANY-TYPE} as the type of its
result variable (see \pagref{ALLOCATE-TO-STACK}).
...
type \TT{void} or \TT{ANY-TYPE} or be a `\TT{next ...}' result.
...
\label{DEFAULTS-TO}
If the variable is omitted, this
