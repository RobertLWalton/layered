...
equals modulo $2^n$ the integer part of the $t1$-value,
...
by a $t2$ value.
The result is the $t2$ value that
equals modulo $2^n$ the {\em numeric-constant}.
\item $t2$ is an integer type,
...
may have \skey{enumeration value}s.  These are
...
unspecified `\TT{qualifier}' enumeration values.  The \TT{*UNCHECKED*}
conversion above can be used to find the numeric value of any
...
For all these functions it is a compile time error if their argument
does not have a compile time value.
...
int32 y10 = H ( x10 )
    ...
    // given to result y10.
int32 y11 = H ( y10 )
    // Illegal; y10 has oof qualifier, which is prohibited,
