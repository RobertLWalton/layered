...
they have a lifetime guaranteed to last
only as long as the current stack frame.
...
\begin{indpar}\begin{verbatim}
define subtype index int of int32
...
this is:
\begin{indpar}\begin{verbatim}
define subtype special adr of adr
...
\begin{center}
\ttkey{define type} {\em type-name} {\tt = (} {\em length}{\tt ,}
				              {\em alignment}{\tt ,}
				              {\em offset}
					      \{{\tt ,} {\em flag}\}$^\star$
					      {\tt )}
...
The {\em length}, {\em alignment}, {\em offset}, and {\em flags} are just
...
The {\em flags} a type definition modify the way that blocks are
...
the value must be stored in the thread stack in a particular
locatable variable:
see TBD for details.
...
in general describes the type: see \secref{NAMES-AND-TYPES}.
...
define type float64 = (8,0,0,floating)
// On some hardware:
    define type adr = (8,0,0,address)
    define type unsadr = (8,0,0)
    define type insadr = (8,0,0)
// On other hardware:
    define type adr = (4,0,0,address)
...
to which normal type checking rules do not apply.
...
Some RAM blocks are `independent'.  Two independent blocks
are guarenteed to not overlap.
...
(see \secref{CODE-AND-FRAME-BLOCKS}) are stack blocks.
A stack block is allocated by a code block execution, and is
deallocated when the code block execution terminates.
...
Independent RAM blocks may be relocated in RAM.  This applies
even to link blocks.
...
A thread stack is a sequence of RAM block.
...
frame occurs only when the frame's code block execution terminates.

Stack variables are themselves independent blocks allocated to
a frame or its extensions.  A variable whose size is determined
...
\label{NAMES-AND-TYPES}

A `\key{name}' is used to name a variable, type, or function call. 
All names are sequences of words, natural numbers, quoted strings,
and bracketted arguments or lists of arguments.

In certain contexts a type must appear before the name of a variable
or function without any intervening operator or separator.
Syntactically types, variables, and functions are all names.  The
types in these contexts are recognized because they must be declared
as types in advance.

Names may be either `\mkey{complete}{name}' or `\mkey{prototype}{name}'.
In a complete name all the arguments are specified.  In a prototype
...
induce assignment of argument names in prototypes.
...
\verb|max ( x , 2000 )| \\
assigns 
\verb|int32| to \verb|T|, \verb|x| to \verb|v|,
and \verb|.convert.(2000)| to \verb|w|. 

\end{indpar}

Matching prototypes is complex and allows omitted arguments, most
particularly type arguments, to be deduced via match generated assignments.
On the other hand, matching complete names merely involves checking for
equality of names.

TBD

A \key{type} is a first class compile-time value that merely equals
the name of the type.  Thus `{\tt int32}' is both the name of a type
and a type value.

A type name has the same syntax as a function call, and type definitions
are similar to function prototype definitions.  Some examples are:
\begin{indpar}\begin{verbatim}
define type int32
define type list ( type T )
define type list ( type T, int32 n )
\end{verbatim}\end{indpar}

According to these definitions, {\tt int32} is a type, and
for any type $T$ and {\tt int32} value $i$, {\tt list(}$T${\tt )} and
{\tt list(}$T${\tt ,}$i${\tt )} are types.
More specifically, {\tt list(int32)}, {\tt list(int32,5)}, and
{\tt list(list(int32))} are types.

Note that types can be arguments to functions and to other types.
However, when used as arguments to functions they can often be
omitted from calls, as they can be dedeuced from the types of the
non-omitted arguments.  For example:
\begin{indpar}\begin{verbatim}
// Note: because T is not pre-declared as a type, ( T z ) must
// be used for the return value to separate T from `max'.
//
( T z ) max ( type T, T x, T y ):
    if ( x > y ) z = x
    else z = y
. . . . . . .
int32 x = ...
int32 y = max ( x, 5 )  // Deduces that T = int32
\end{verbatim}\end{indpar}

In effect, the argument list contains declarations of type names,
e.g., in the example just given {\tt T} is declared as a type name.
Because type names have syntactic meaning, such declarations must
occur in an input argument list to the left of any use in any
input argument list.  If they appear in an output argument list,
the output list must be bracketted to separate it from the function
name.

Also, arguments can be omitted from function calls if the omissions can
be determined unambigously using only the types of the arguments, and
values can be supplied by other means.
In particular, all type arguments, i.e., arguments of type `{\tt type}',
can be omitted.

Type values are not very useful at runtime, as the only
operations on them are equality comparison and output.
Most runtime information about types is encoded in descriptors.

A \key{descriptor} is a run-time value that gives information
about a type.  A descriptor itself has a type, and every
descriptor type has the special property that there is only one
value possible of the descriptor type, and the compiler can
figure out how to compute that value from the type itself\,%
\footnote{This is the central idea in the Haskell programming language}.

The most basic descriptor includes the size, alignment, offset, and
mode of a type.  An example use is
\begin{indpar}\begin{verbatim}
basic(int32) b;
unsadr s    = b.length       // Length of int32
unsadr a    = b.alignment    // Alignment of int32
intadr o    = b.offset       // Offset of int32
type flags f = b.flags       // Flags of int32
\end{verbatim}\end{indpar}

If $T$ is a type, then {\tt basic(} $T$ {\tt )} is a type that has
...
It is possible to create new descriptors by supplying
a constructor that can be run by the compiler and loader to compute
the values of the new descriptors from other values known
to the compiler and loader.  For example:
\begin{indpar}\begin{verbatim}
define descriptor subtype length ( type T ) of unsadr
( constant length(T) s) .construct. ( type T ) unchecked:
    basic(T) b
    s = b.length
unsadr .convert. ( type T, length(T) s ) unchecked:
    unsadr r = s
    return r
\end{verbatim}\end{indpar}

A descriptor declaration is the same as a type declaration with
except that it begins with `{\tt define descriptor}' instead of
just `{\tt define}', which
marks the type being decleared as a `\mkey{descriptor}{type}'.
Variables of such a descritor type are not to be given values explicitly.
Instead, they are given values by their constructor, which takes
no explicit arguments.  The resulting values must be `constant'. 

Another example is:
\begin{indpar}\begin{verbatim}
define descriptor type subbase ( type T ) = ( 2*length(unsadr), 8 )
unchecked:
    disp length ( type T ) = 0 -> unsadr @ subbase (T)
    disp offset ( type T ) = length(unsadr) -> unsadr @ subbase (T)
( constant subbase(T) s) .construct. ( type T ) unchecked:
    basic(T) b
    s.length = b.length
    s.offset = b.offset
\end{verbatim}\end{indpar}

TBD: variable names like type names can be like function calls
...
float64 z = y     // Translates to .set.(&z,.get.(.get.(&x))).
                  // or to simplify z = .get.(x)
                  // .get.(x) is probably undefined
...
Displacment types are just like pointer types in so far as declaration
...
//
( void pp, unchecked adr pp.begin -> int32,
           unchecked adr pp.end -> int32 )
    allocate ( uns32 n )
//
// Prefix operator to dereference a pointer pair.
//
( adr -> int32 V ) "*"
        ( void pp, adr -> unchecked adr pp.begin -> int32,
                   adr -> unchecked adr pp.end   -> int32 )
    unchecked:
...
a stronger statement than just returning an value that
