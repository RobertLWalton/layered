...
In order to understand the following, we will look ahead at how
...
\tt $Qsr$ $Tr$ r =: \\
\tt ~~~~~ body of the inline function \\
\tt $Qsu$ $Tu$ u = r \\
\end{tabular}
\end{center}

In the above {\tt \ldots{}~u = r} is called a \key{result assignment}
and {\tt \ldots{}~v = w} is called a non-result assignment.
Also any assignment {\tt \ldots{}~u = w} in which neither {\tt u} or
{\tt w} is a function prototype variable in a function expansion is
called a non-result assignment.

Rephrasing this, consider the assignment statements
\begin{center}
\begin{tabular}{l}
\tt $Qsw$ $Tw$ w = \ldots \\
\tt $Qsu$ $Tu$ u = w \\
...
where either {\tt w} or {\tt u}, but not both, may be a function 
prototype variable in an inline function call expansion.
If {\tt w} is a function prototype result variable, then
the assignment {\tt \ldots{}~u = w} is a result assignment.  Otherwise
{\tt \ldots{}~u = w} is a non-result assignment.

Then the assignment {\tt \ldots{}~u = w} is a \key{direct assignment}
\label{DIRECT-ASSIGNMENT}
if and only if:
\begin{indpar}\begin{itemize}
\label{DIRECT-ASSIGNMENT-QUALIFIER-RULES}
\item[(DA1)]
The types of {\tt u} and {\tt w} are identical
