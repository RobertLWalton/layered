...
\begin{indpar}
Allocate {\tt size} bytes of static memory with the given
{\tt alignment} in bytes to static memory.
Return
...
{\tt alignment} in bytes to dynamic memory.  Store the
...
iteration variables
of a {\em loop-assignment-statement} may \underline{not} have the \TT{*STACK*}
qualifier.  However, stacks of loop `\TT{initially}' and `\TT{finally}'
subblocks are not free, and result variables of
a {\em loop-assignment-statement} \underline{may} have the \TT{*STACK*} qualifier.
...
the stack address.  To accommodate this stack addresses are a pair
consisting of the static address of a location containing the base
address of the containing free stack and an offset within that
free stack.
...
~~~~~~~~~*READ* adr result.base, \\
~~~~~~~~~adr result.base.adr = \\
~~~~\ttstdkey{allocate to stack} ( type T, unsadr N ) \\
