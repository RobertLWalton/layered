...
\begin{indpar}
Pointer {\tt .type}'s must have a numeric type as its underlying type.
The pointer values must point at a vector with at least one element,
and the address of that element must be a multiple of the element
size in bytes.
...
do some checkMPXCHG8B
At the end of pass 1, this kernel code is loaded into the kernel and checked.
...
underlying runtime system.  Locks are allocated to sets of processes
that share memory, so that if any process in the set acquires
...
it is \underline{not} acquired in pass 1.

In pass 2 the \emkey{lock-instruction} sets the lock and succeeds if the lock
was not already set, and otherwise fails.
...
between when the instruction execution in pass 1 and the start of pass 2.
...
conceptually of reading the values at these locations and doing
a hardware compare-and-set instruction to write the values read
back into the locations.\footnote{On INTEL X86\_64 processors
instructions typically used are {\tt CMPXCHG\ldots} instructions
with the {\tt LOCK} prefix.}

The implementation assumes that if the locks before a {\em write-instruction}
are acquired in pass 2 by an {\em atomic-statement}, the locations to be
...
first failed or operation error detecting
...
\TT{*READ*} qualifier variables:
see Section~\itemref{THE-READ-AND-WRITE-QUALIFIERS}.
...
of type \TT{adr} with either or both of the qualifiers
\TT{*READABLE*} or \TT{*WRITABLE*} that contains the RAM address.
If \TT{va} has qualifier \TT{*READ*} it must obey
the rules of Section~\itemref{THE-READ-AND-WRITE-QUALIFIERS}.


\item
An `\key{address variable}' \TT{vo} of type \TT{intadr} or \TT{unsadr} and
qualifier \TT{*UNCHECKED*} in a cluster, and
a variable \TT{vb.base} of type \TT{adr} and qualifier \TT{*READ*}
...
If \TT{vo.base} has qualifier \TT{*READ*} it must recursively obey
the rules of Section~\itemref{THE-READ-AND-WRITE-QUALIFIERS}.
