...
An {\em assignment-state\-ment} may also act as a {\em declaration} of
...
statement blocks, but must be declared at `\key{top level}', that is,
outside statement blocks.

If a declaration declares a name or prototype pattern that does
\underline{not} begin with a the {\em module-abbreviation} of the
containing module, the declaration is `\key{internal}'.  Declarations
that are not top level and all declarations in a body file
\underline{must} be internal.

The following are exceptions to the above:
\begin{itemize}
\item {\em Abbreviation-declarations} are always internal
(\pagref{ABBREVIATIONS-ARE-INTERNAL}),
even when their abbreviation begins with a {\em module-abbreviation}.

\item
\label{NON-BLOCKS}
Statement subblocks inside the following declarations
are not considered to be blocks for the purposes of program organization:
\begin{itemize}
\item[] {\em abbreviation-declarations} (\pagref{ABBREVIATION-DECLARATION})
\item[] {\em type-declarations} (\pagref{TYPE-DECLARATION})
\item[] {\em in\-clude-statements} (\pagref{INCLUDE-STATEMENT}).
\end{itemize}
These subblocks are considered to be syntactic sugar for
sequences of statements that do not contain subblocks.
\end{itemize}

A {\em name} or {\em prototype} is external if its
declaration is external, and internal if its declaration is internal.

A module may be imported into another module or body.
Any name or prototype declared to be external in a module that is imported
into a second module is
said to be `\key{imported}' within the second module, and
`\key{exported}' from the first module.
...
{\em Module-abbreviations} may not be omitted from imported names
(e.g., external types, external variables).
...
\footnote{Recall that subblocks of
{\em abbreviation-declarations},
{\em type-declarations},
and {\em in\-clude-statements} not considered to be blocks for
this purpose: see \ref{NON-BLOCKS} on \pagref{NON-BLOCKS}.}
or until the end of the file containing the {\em declaration} if
the {\em declaration} is top level.
The scope of top level {\em declarations} in a module
is extended to include all the bodies of that
module.
If the {\em declaration} is external, the declaration must be
top level in a module and not in a body, and the scope of the
...
(see \itemref{BLOCK-ASSIGNMENT-STATEMENTS}).  Block names are
internal.
...
By the last rule,
`\TT{next} {\tt v}' cannot be declared inside a block $B$ if `{\tt v}' was
...
within statements that have the context option.
...
The execution of a top level assignment statement in a module may
\underline{not} invoke an out-of-line function.
This ensures that all code needed
...
The statement executed later that
assigns the values must be a top level statement
in the same extended module
...
and there are program-initialization-time checks to catch
these errors.
...
\\[0.5ex]
\emkey{after-clause} ::= \TT{initialize after }{\em body-name}
\\[0.5ex]
...
There are program-initialization-time checks to enforce this rule.
...
\ikey{Numbers}{number} are sequences of bits that can be stored in registers
or in random access memory (RAM).
Each number has a \mkey{size}{of number}, which is
its number of bits.  For example, unsigned integers can have
...
The following is a list of all the number types, all of which are
builtin:
...
same, and that the sizes of {\tt unsmax} and {\tt intmax} be the
same.  It is required that the size of {\tt unsmax} be at least
as large as the size of {\tt unswrd}, that {\tt intmax} be at least
as large as {\tt intwrd}, and that {\tt floatmax} be at least as large
as {\tt floatwrd}.
...
\begin{indpar}
{\em number-type-name}\label{NUMBER-TYPE-NAME}
    \begin{tabular}[t]{@{}cl}
...
A {\em defined-type-declaration} declares a new \TT{type} enumeration
value and associates a type name, the
{\em defined-type-name}, with this new \TT{type} value.
The {\em base-type-expression} specifies
the `\key{base type}' of the new type.
The simplest {\em base-type-expres\-sion} is just a {\em type-name} of the
base type.  It is possible to associate more type names with the
same \TT{type} value:
see {\em type-name-declaration}, \pagref{TYPE-NAME-DECLARATION}.

Any variable {\em qualifiers} in the declaration are attached to every
variable declared with the {\em defined-type-name} (see
section \itemref{QUALIFIERS} for more on {\em qualifiers}).
Any value {\em qualifiers} in the declaration are attached to every
variable and every argument declared with the {\em defined-type-name} (see
section \itemref{QUALIFIERS} for more on {\em qualifiers}).
...
define type T2 as T1
function T2 r = T2 ( T1 v ):
    *UNCHECKED* T2 r2 = T2 ( v )
    r = r2
\end{verbatim}\end{indpar}
...
does \underline{not} define a new \TT{type} value.  It merely defines a new
{\em type-name} that references an old \TT{type} value.
...
the {\em type-name} is used to declare a variable or argument.
...
\TS~~~~*UNCHECKED* bool r2 = std bool ( 0 ) \\
\TS~~~~r = r2 \\
function std bool r = \ttstdkey{true}: \\
\TS~~~~*UNCHECKED* bool r2 = std bool ( 1 ) \\
\TS~~~~r = r2 \\
function std unswrd r = \ttstdmkey{unswrd}{of {\tt bool}} ( bool v ): \\
\TS~~~~r = 1 if v else 0
\end{indpar}
...
\begin{indpar}
\emkey{enum-assignment}\label{ENUM-ASSIGNMENT}
    \begin{tabular}[t]{rl}
...
to the constant named in the first `\TT{next enum}'
{\em enum-assignment} statement for the given type,
and then for subsequent `\TT{next enum}' statements for that type,
assigns consecutive integer values in increasing order.
...
to the constant named in the first `\TT{previous enum}'
{\em enum-assignment} statement for the given type.
Then for subsequent `\TT{previous enum}' statements for that type,
the compilation assigns consecutive integer values in decreasing order.
...
An enumeration value also has a numeric value in the range
...
and the range value of an enumeration value is constant
for a given enumeration type, and
called the `\mkey{enumeration offset}{of enumeration type}' of
that enumeration type.

Thus for a value $V$ of enumeration type $E$:
\begin{center}
underlying value of $V$ = range value of $V$ + enumeration offset of $E$
\end{center}

If the base type of an enumeration type is an integer type,
...
The effects of {\em enumeration-type-declarations} with
{\em enum-type-expression} base types are as follows:
...
\item[\TT{define type} {\em qualifier-name}\STAR{}]
	      ...
	      \TT{with enum range [}
	         {\em first-enum} \TT{..} {\em last-enum} \TT{]} \\
	      \end{tabular} \\
...
\begin{center}
enumeration offset of {\em enum-type-name} =
underlying value of V0 - {\em first-enum}
\end{center}
\item[\TT{define type} {\em qualifier-name}\STAR{}]
              \begin{tabular}[t]{@{}l@{}}
              {\em enum-type-name}~~\TT{as}~~%
...
\begin{center}
enumeration offset of {\em enum-type-name} =
underlying value of V0 - {\em last-enum}
\end{center}
\end{itemlist}
\end{indpar}
...
For an enumeration type $E$ defined with range
{\tt [{\em first-enum}~..~{\em last-enum}]}, the
`\mkey{range type}{of enumeration type}'\label{RANGE-TYPE}
of $E$ is defined as the
most specific type $R$ (\pagref{MORE-SPECIFIC-RELATION})
in the set
\begin{center}
\TT{unswrd     intwrd     unsmax     intmax}
\end{center}
such that {\em first-enum}, {\em last-enum}, and all values of the
underlying type of $E$ are convertible to $R$.
It is a compilation error if the range type of an enumeration type
does not exist.

Given this, information about an enumeration type $E$ with
range type $R$ can be discovered by the compile time functions:

\begin{indpar}

{\tt function $R$ r = $ma$\QMARK{} \ttkey{underlying value of} ( $E$ v )} \\
{\tt function $R$ r = $ma$\QMARK{} \ttkey{range value of} ( $E$ v )}
\begin{indpar}
Returns the underlying value or range value of {\tt v}.
\end{indpar}

{\tt function $R$ r = \ttstdkey{first enum of} ( type t )} \\
{\tt function $R$ r = \ttstdkey{last enum of} ( type t )} \\
{\tt function $R$ r = \ttstdkey{enumeration offset of} ( type t )}

\begin{indpar}
Returns the {\em first-enum}, {\em last-enum}, or enumeration offset
of the enumeration type {\tt t}.
\end{indpar}
...
\begin{indpar}
Returns the base type, underlying type, or range type
of the enumeration type {\tt t}.
\end{indpar}
...
\end{center}
\begin{indpar}
These conversions do nothing but change the type of the underlying value.
The result may be an illegal $E1$ value, that is, a value whose range
value is outside the range of $E1$.
\end{indpar}
...
\TS~~~~\ttstdkey{error code}~as unswrd with enum [0~..] \\
define abbreviations: \\
\TS~~~~\ttkey{type} \ABV{} `std type' \\
\TS~~~~\ttkey{qualifier} \ABV{} `std qualifier' \\
\TS~~~~\ttkey{error code} \ABV{} `std error code'
\\[1ex]
...
\\[1ex]
error code~\ttstdkey{NULL ERROR CODE}~= next enum // 0%
\label{NULL-ERROR-CODE} \\
function std error code r = \ttstdmkey{NULL}{error code}: \\
\TS~~~~r = std NULL ERROR CODE
\end{indpar}
...
and builtin error codes, e.g.~\TT{BAD ARG\-U\-MENT},
are given unspecified `\TT{error code}' enumeration values.
...
of that enumeration type are assigned in a single code file.
...
non-negative integer that gives some information as to the kind of error.
There is also an \TT{error code} type, defined as a builtin enumeration
type, that is implicitly convertable to every number type.

The following \TT{error code} enumeration values have builtin definitions:
\begin{center}
\begin{tabular}{lp{3.5in}}
\ttstdkey{NULL ERROR CODE} &  Used to indicate a value that is missing, but not
necessarily associated with any error.  This value is returned by the
{\tt NULL} function described below, and is defined to have range value
\TT{0}: see \pagref{NULL-ERROR-CODE}.
\\[0.5ex]
\ttstdkey{TOO LARGE ERROR CODE} &  If an error code is to be stored in a number,
and the error code range value
is too large for the number type to store the error code, this value
is stored instead.
\\[0.5ex]
\ttstdkey{NOT ERROR VALUE} &
If an error code is to be read from a number, and
the number value is not an error value, this error code is read instead.
\\[0.5ex]
\ttstdkey{BAD RANGE} &  Argument is out of range.
\\[0.5ex]
\ttstdkey{BAD ALIGNMENT} &  Alignment argument is out of range.
\\[0.5ex]
\ttstdkey{INSUFFICIENT MEMORY} &  Not enough memory is available.
\end{tabular}
\end{center}

The maximum error code range value allowed is determined by the number type
as follows:
\begin{center}
\begin{tabular}{l@{~~~~~}l}
\bf Type & \bf Maximum Error Code Range Value
\\[1ex]
\TT{int$x$} & $x/4-1$ \\
\TT{uns$x$} & $x/4-1$ \\
\TT{float$x$} & $x/2-1$ \\
\TT{\ldots wrd} & same as \TT{\ldots $x$}
                  where $x$ is size of \TT{\ldots wrd} \\
\TT{\ldots max} & same as \TT{\ldots $x$}
                  where $x$ is size of \TT{\ldots max} \\
\TT{adr} &  $2^{20}-1$ if addresses are 32 bits, else $2^{30}-1$ \\
\TT{\ldots adr} & same as \TT{adr} \\
\end{tabular}
\end{center}

Error codes are computed as follows for an integer type $T$ and an error
code range value $c$ not greater than the maximum allowed for $T$:
\begin{center}
\begin{tabular}{l@{~~~~~~~~~~}l}
$T$ is unsigned integer & \tt ERROR(c) == $T$(max value) - $c$
\\[1ex]
$T$ is signed integer & \tt ERROR(c) == $T$(min value) + $c$
\\[1ex]
$T$ is \TT{adr} & \tt ERROR(c) == adr(NULL) + $c$  ~~~~~ [see text]\\
\end{tabular}
\end{center}

For the \TT{adr} type, \TT{adr(NULL)} is the address
...
with 48 or more bit virtual addresses.  Thus using an \TT{adr} error code
as an address will cause a memory fault.

For a \TT{float$x$} type $T$,
the high order $x/2$ bits of every error value of type $T$
is the same and designates the error value as a non-signaling NaN.
The low order $x$ bits are the error code range value.

The following builtin functions manipulate error values for
any number type $T$:\label{ERROR-CODE-FUNCTIONS}
\begin{indpar}
{\tt function $T$ r = $T$ ( error code c )}
\begin{indpar}
Converts {\tt c} to type $T$.  If {\tt c} is out of the
range of error codes permissible for $T$,
\TT{std TOO LARGE ERROR CODE} is used in place of {\tt c}.
\end{indpar}
{\tt function error code r = \ttstdkey{NULL}}\label{NULL}
\begin{indpar}
Returns {\tt NULL ERROR CODE}.  Permits \TT{std} to be omitted
when using {\tt NULL ERROR CODE} (but \TT{std} cannot be omitted with other
error codes).
\end{indpar}
{\tt function bool r = \TT{std} ( $T$ value) \ttkey{is} ( error code c )}
\begin{indpar}
Returns \TT{true} iff {\tt value} equals {\tt c} converted to type $T$.
Differs from \TT{==} only for floating point types $T$, for which
\TT{==} will always return \TT{false} when NaNs are input.
\end{indpar}
{\tt function error code r = \ttstdkey{error code of} ( $T$ value ) }
\begin{indpar}
Returns error code of {\tt value} if that is an error value, or
returns {\tt std BAD ERROR CODE} if {\tt value} is not an error value.
\end{indpar}
{\tt function bool r = \TT{std} ( $T$ value )\ttkey{is error code} }
\begin{indpar}
Returns \TT{true} if {\tt value} is an error value and therefore
encodes an error code.
\end{indpar}
{\tt function unswrd r = \ttstdkey{max error code} ( type t )}
\begin{indpar}
Returns the maximum allowed error code range value for a number type $T$.
\end{indpar}
\end{indpar}
...
{\tt function type r = $ma$\QMARK{} \ttkey{set of} ( type E )} \\
{\tt function type r = $ma$\QMARK{} \ttkey{list of} ( type E )}
\begin{indpar}
Returns the type whose values are sets or lists
of elements of type {\tt E}, where
{\tt E} must have an integer underlying type, and
$ma$ is the {\em module-abbreviation} of the original type name
of $E$ if any.
\end{indpar}
...
an integer type, an enumeration type, or a non-enumeration defined type
with an integer underlying type.
...
\emkey{enum-range}
    ...
    $|$ & \TT{..}~~~{\em enum-value} \\
    \end{tabular} \\
\emkey{enum-value} ::= \begin{tabular}[t]{p{4.5in}}
                       {\em expression} evaluating at compile-time to
		        a value of a defined type with integer underlying type,
			an enumeration type, or an integer type
			\end{tabular}
\end{indpar}

If the {\em enum-values} in an {\em enum-set-constant} or
{\em enum-list-constant} have different
types, implied conversions from these to their base types
...
values in between.  For ranges the values cannot have defined types,
but must have enumeration or integer types.
...
limit of the enumeration or integer type of the non-omitted {\em enum-value}.
...
for the function definition to match a function call, and therefore
the type argument {\tt T}, which is implied and need not be
given in the function call, must satisfy the logical expression
by being in the given set of
...
{\tt function type r = \ttstdkey{element type of} ( type T )}
\begin{indpar}
...
\begin{indpar} \tt
define types: \\
\TS~~~~\ttstdkey{types} as type name of std set of ( std type ) \\
\TS~~~~\ttstdkey{qualifiers} as type name of\\
\TS~~~~~~~~~~~~~~~~~~~~~~std set of ( std qualifier ) \\
define abbreviations: \\
...
Variables with compile-time enumeration set or list values can be defined.
If $S$ is the type name of an enumeration set type with element type $E$,
$L$ is the type name of an enumeration list type with element type $E$,
...
If the size of \TT{unsmax} is \underline{larger than} the
size of \TT{unswrd}, the builtin exceptions are
\begin{center}
\begin{tabular}{lll}
...
\end{center}
If \TT{unsmax} \underline{equals} \TT{unswrd}, the builtin exceptions are
\begin{center}
\begin{tabular}{lll}
\tt unswrd	& $\CNV$ & \tt intwrd \\
\tt intwrd	& $\CNV$ & \tt floatwrd \\
\end{tabular}
\end{center}
These exceptions are used in the following:
\begin{itemize}
\item
definition of range type, \pagref{RANGE-TYPE}
\item
definition of common element type, \pagref{COMMON-ELEMENT-TYPE}
\item
comparison operator operand conversion, \pagref{COMPARISON-OPERATOR-TYPE}
\item
...
type name $T$, if any, and $S$ is the size in 8-bit bytes of values of type $T$.
...
declaration.
The default declaration returns {\tt std missing type}.
\end{indpar}
...
The alignment of builtin types is their size in bytes.
...
and for enumeration types the values returned are the
\underline{range value} limits of
...
\begin{indpar}
Returns the minimum or maximum \underline{range values} not yet assigned to an
enumeration type $T$ by an {\em enum-assignment} (\pagref{ENUM-ASSIGNMENT}).
Values assigned by unchecked conversions are not considered.  If
...
These are defined only for enumeration types $T$.
...
Returns \TT{$T$(NULL)} for numeric types $T$.
May be defined by user for enumeration and defined types.
\end{indpar}
...
function Qs~uns32 r = \begin{tabular}[t]{@{}l@{}}
                      \ttstdmkey{assign}{of character} \\
...
\TT{character} values can be compared using \TT{<}, \TT{<=}, \TT{==},
\TT{!=}, \TT{=>}, or \TT{>}, differences of \TT{character} values can be
computed (e.g., \TT{C"7" - C"0"}),
and \TT{character} values can be
tested by the error code functions (\pagref{ERROR-CODE-FUNCTIONS})
that test \TT{uns32} values.
\end{indpar}

{\tt function character r = \ttstdmkey{NUL}{character value}}
\begin{indpar}
Returns \TT{C"<0>"},
...
{\tt function character r = \ttstdkey{character} ( error code c )}
\begin{indpar}
Returns {\tt c} converted to a \TT{character}.  UNICODE
character codes to not overlap \TT{error code} values when
stored in an \TT{uns32} integer, as that former
are less than $2^{24}$ and the latter are not less than
$2^{30}$.
\end{indpar}
...
\emkey{string-constant}\label{STRING-CONSTANT}
    :::= \TT{"}{\em string-character-representative\STAR{}}\TT{"} \\
{\em string-character-representative} :::=
    {\em character-representative} not representing \TT{NUL} \\
{\em character-representative} :::= see \pagref{CHARACTER-REPRESENTATIVE}
\end{indpar}
...
{\tt function string r = \ttkey{string} ( error code c )}
\begin{indpar}
Returns {\tt c} converted to an error value of \TT{string} type.
\TT{String} values
have \TT{adr} as their underlying type and can store any error value
that an \TT{adr} value can. 
If {\tt c} is out of the
range of error codes permissible for \TT{adr},
\TT{std TOO LARGE ERROR CODE} is used in place of {\tt c}.
\end{indpar}

{\tt function error code r = \ttkey{error code} ( string s )}
\begin{indpar}
Returns the \TT{error code} encoded by the error value stored in {\tt s}, or 
returns \TT{std NOT ERROR VALUE} if {\tt s} is not storing an error value.
\end{indpar}
...
{\tt function name r =
	\TT{std} ( name n1 ) "\ttmkey{+}{of names}" ( name n2 )}
\begin{indpar}
...
{\tt function unswrd r = \ttstdmkey{length of}{name} ( name n )}
\begin{indpar}
Returns the length of {\tt n}, the number of strings in {\tt n}.
\end{indpar}

{\tt function string r = \TT{std} ( name n ) \TT{[} unswrd i \TT{]}}%
\ttmindex{[]}{of string}
\begin{indpar}
...
{\tt function name r = \ttkey{name} ( error code c )}
\begin{indpar}
Returns {\tt c} converted to an error value of \TT{name} type.
\TT{Name} values
have \TT{adr} as their underlying type and can store any error value
that an \TT{adr} value can. 
If {\tt c} is out of the
...
{\tt function error code r = \ttkey{error code} ( name n )}
\begin{indpar}
Returns the \TT{error code} encoded by the error value stored in {\tt n}, or 
returns \TT{std NOT ERROR VALUE} if {\tt n} is not storing an error value.
\end{indpar}
...
\begin{indpar}
{\tt function name set r = \TT{std} (name set s1)
...
{\tt function bool r = \TT{std} (name e) \ttmkey{is in}{name set} ( name set s )}
\begin{indpar}
Returns \TT{true} if {\tt e} is an element of the set {\tt s}, and
\TT{false} otherwise.
\end{indpar}

{\tt function name set r = \TT{std} (name n)
                           "\ttmkey{+}{name set union}" ( name set s )}
\begin{indpar}
Returns union of \{{\tt n}\} and {\tt s}.
\end{indpar}

{\tt function name set r = \TT{std} ( name set s )
...
{\tt function unswrd r = \TT{std} \ttmkey{size of}{name set} ( name set s )}
\begin{indpar}
Returns the size of {\tt s}, the number of names in {\tt s}.
\end{indpar}

{\tt function name r = \TT{std} ( name set s ) \TT{[} unswrd i \TT{]}}%
\ttmindex{[]}{of string}
\begin{indpar}
...
{\tt function name set r = \ttkey{name set} ( name list ls )}
\begin{indpar}
Converts {\tt ls} from a list to a set, removing duplicate elements.
\end{indpar}

{\tt function name set r = \ttkey{name set} ( error code c )}
\begin{indpar}
Returns {\tt c} converted to an error value of \TT{name set} type.
\TT{Name set} values
have \TT{adr} as their underlying type and can store any error value
that an \TT{adr} value can. 
If {\tt c} is out of the
...
{\tt function error code r = \ttkey{error code} ( name set s )}
\begin{indpar}
Returns the \TT{error code} encoded by the error value stored in {\tt s}, or 
returns \TT{std NOT ERROR VALUE} if {\tt s} is not storing an error value.
\end{indpar}
...
{\tt function unswrd r = \TT{std} \ttmkey{length of}{name list} ( name list ls )}
\begin{indpar}
Returns the length of {\tt ls}, the number of names in {\tt ls}.
\end{indpar}

{\tt function name r = \TT{std} ( name list ls ) \TT{[} unswrd i \TT{]}}%
\ttmindex{[]}{of string}
\begin{indpar}
...
{\tt function name list r = \ttkey{name list} ( name set s )}
\begin{indpar}
Converts {\tt s} from a set to a list, where elements of {\tt s}
are listed in sorted order in the result.
\end{indpar}

{\tt function name list r = \ttkey{name list} ( error code c )}
\begin{indpar}
Returns {\tt c} converted to an error value of \TT{name list} type.
\TT{Name list} values
have \TT{adr} as their underlying type and can store any error value
that an \TT{adr} value can. 
If {\tt c} is out of the
...
{\tt function error code r = \ttkey{error code} ( name list ls )}
\begin{indpar}
Returns the \TT{error code} encoded by the error value stored in {\tt ls}, or 
returns \TT{std NOT ERROR VALUE} if {\tt ls} is not storing an error value.
\end{indpar}
...
There are context and variable renaming issues here, as there are
whenever we give examples of expanded function calls.  For details on contexts,
see \itemref{SCOPE},
...
Given these definitions the example
\begin{center}\label{IF-ELSE-EXAMPLE}
\tt $T$ $v$ = ($e1$ "if" $b1$) "else" ( ($e2$ "if" $b2$) "else" $e3$ )
\end{center}
where
\begin{center}
compiling {\tt $T$ ...~= $eI$} adds qualifiers $QsI$ to the result

none of the $bI$ have \TT{false} as a compile time value
\end{center}

is expanded by the compiler
to the following \LSTAR-language (\pagref{LSTAR-LANGUAGE}) code
(module variable renaming issues):
...
\tt *IF-VALUE* tmp2, $Qs1$ $T$ tmp2.value, bool tmp2.valid = \\
\tt ~~~~do ( if-context ): \\
\tt ~~if: \\
\tt ~~~~tmp1: \\
\tt ~~~~~~do ( tmp0 ): \\
...
\tt $Qs1$ $Qs2$ $Qs3$ $T$ $v$ = do ( else-context ): \\
\tt ~~if: \\
\tt ~~~~tmp2.valid: \\
\tt ~~~~~~$v$ = tmp2.value \\
\tt ~~~~else: \\
...
\tt ~~~~~~~~*IF-VALUE* tmp4, $Qs2$ $T$ tmp4.value, \\
\tt ~~~~~~~~~~~~~~~~~~~~~~~~~bool tmp4.valid = \\
\tt ~~~~~~~~~~do ( if-context ): \\
\tt ~~~~~~~~~~~~if: \\
\tt ~~~~~~~~~~~~~~tmp3: \\
...
the target type of the result of the first {\tt "else"} whose
...
If $b1$ has the compile time value \TT{false}, the first {\tt "if"}
compiles instead to:
\begin{indpar}
\begin{tabular}{l}
\tt // Evaluate ($e1$ "if" false) - left operand of first "else" \\
...
\end{tabular}
\end{indpar}
so that $e1$ is never compiled and $Qs1$ is in effect the empty set of
qualifiers.
...
can be implicitly converted to that type:\label{COMPARISON-OPERATOR-TYPE}
\begin{center}
...
(modulo variable renaming issues):
...
\item
\label{DEFAULT-ARGUMENT-INSERTION}
...
\item
For any {\em type-argument-declaration} in a {\em prototype-pattern}
{\em qualifiers-type-argument-list} or {\em regular-argument-list}
the {\em variable-name} in the declaration may be used syntactically as
a type throughout the rest of the prototype and in any associated
{\em function-declaration} body.  If it is so used, then
any actual argument corresponding to the {\em type-argument-declaration}
must evaluate with the `\TT{type}'
target type to a compile-time value, else
the call-prototype match fails.    If the match does not fail,
this value is substituted for the {\em variable-name} throughout
the rest of the prototype and in any associated {\em function-declaration}
body.
(The corresponding actual argument will exist if
the {\em type-argument-declaration} is in a
{\em regular-argument-list}, but may be the default
argument from Step \ref{DEFAULT-ARGUMENT-INSERTION}.)
...
prototype and in any associated {\em function-declaration} body.

\item
For any {\em qualifiers-argument-declaration} in a {\em prototype-pattern}
{\em qualifiers-type-argument-list} or {\em regular-argument-list}
the {\em variable-name} in the declaration may be used syntactically as
a qualifier throughout the rest of the prototype and in any associated
{\em function-declaration} body.  If it is so used, then
any actual argument corresponding to the {\em qualifiers-argument-declaration}
must evaluate with the `\TT{qualifiers}'
target type to a compile-time value, else
the call-prototype match fails.    If the match does not fail,
the set of qualifiers denoted by this value
is substituted for the {\em variable-name} throughout
the rest of the prototype and in any associated {\em function-declaration}
body.
(The corresponding actual argument will exist if
the {\em qualifiers-argument-declaration} is in a
{\em regular-argument-list}, but may be the default
argument from Step \ref{DEFAULT-ARGUMENT-INSERTION}.)
...
(\pagref{DIRECT-ASSIGNMENT}).  Also, if any of these assignments
are direct assignments, they have the special semantics of
result direct assignments described on page \pagref{RESULT-DIRECT-ASSIGNMENT}
(so that cluster members are assigned correctly).
...
qualifiers.  A variable with both may be both written and used in
expressions, but a variable with just \TT{*WRITE*} may only be written,
and may \underline{not} be used in expressions (that is, writting
`{\tt next v}' does \underline{not} give {\tt v} a value that can be used in
an expression).

If a RAM cache variable {\tt v}'s value is not used,
it will not actually be loaded from RAM, even if {\tt v} is written
by assigning a value to `{\tt next v}'.  Thus if {\tt v} is given
both the \TT{*READ*} and \TT{*WRITE*} qualifiers, but it is not used,
it will not be loaded from RAM.  Then if {\tt next v} is written and
subsequently {\tt v} is used in an expression, the
value of {\tt v} will be the value written.

Since an unused RAM cache variable {\tt v} is not loaded from RAM,
members of {\tt v} used to compute its address may be given the unchecked value
\TT{*NO* *VALUE*} (\pagref{NO-VALUE}).
