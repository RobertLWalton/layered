...
\begin{indpar}
\emkey{name-constant} :::= \TT{`}{\em name-item}\STAR{}\TT{'} \\
...
The transitive closure of the \TT{\ABV} relation between types is
required to be a partial order,
or equivalently, the directed graph whose nodes are types
which has an edge from $T1$ to
$T2$ if and only if \TT{$T1$~\ABV~$T2$} is required to be acyclic.
...
\begin{indpar}
Implied conversion (\pagref{IMPLIED-CONVERSIONS})
...
{\tt bool r = (name e) \ttmkey{in}{name set} ( name set s )}
\begin{indpar}
Returns \TT{true} if {\tt e} is an element of the set {\tt s}, and
\TT{false} otherwise.
\end{indpar}

{\tt name set r = (name n) "\ttmkey{+}{name set union}" ( name set s )}
\begin{indpar}
Returns union of \{{\tt n}\} and {\tt s}.
\end{indpar}

{\tt name set r = ( name set s )
...
\hspace*{0.5in}
    \begin{tabular}[t]{rl}
    ::= & \TT{define} {\em qualifier-attribute}\STAR{} \ttkey{qualifier}
...
\TT{T1}$\succ$\TT{T2} mean that there is an implied conversion from
values of type \TT{T1} to values of type \TT{T2}.  Let \TT{T1~\ABV~T2}
be the transitive closure of \TT{T1}$\succ$\TT{T2}, which means
that a value of type \TT{T1} may be converted to a value
of type \TT{T2} by applying zero or more implied conversions.
Then we \underline{require}
that \TT{T1~\ABV~T2} be a partial order, which means that:
\begin{itemize}
\item \TT{\ABV} is reflexive: \TT{T1~\ABV~T1}.
\item \TT{\ABV} is transitive: \TT{T1~\ABV~T2} and \TT{T2~\ABV~T3}
imply that \TT{T1~\ABV~T3}.
\item \TT{\ABV} is anti-symmetric:
\TT{T1~\ABV~T2} and \TT{T2~\ABV~T1}
imply that \TT{T1~==~T2}.
\end{itemize}
...
Then \TT{T1~\ABV~T2} if and only if there is a (possibly zero size)
...
User defined implied conversions are assumed to obey these rules.

Various builtin implied conversions are described
in this document.

Note that \TT{T1~\ABV~T2} does \underline{not} imply that every
value of type \TT{T1} can be fully represented by a value of type
\TT{T2}.  For example, an implied conversion \TT{T1~\ABV~bool}
...
and \TT{T-src~\ABV~T-des},
meaning that an implied conversions are defined that convert values
...
If \TT{T-src} and \TT{T-des} are not the same and \TT{T-src~\ABV~T-des}
...
allowed.  Only implied conversions are allowed.
\end{indpar}
...
float64 s = tmp s
\end{verbatim}\end{indpar}
using an implied conversion of the result of \TT{"+"}.
...
float64 s = tmp s
\end{verbatim}\end{indpar}
using implied conversions of the arguments of \TT{"+"}?
...
In this case we prefer \TT{T == ins32} because \TT{ins32 \ABV float64},
as per the implied conversion ordering of types introduced in
...
than a matching $k$ if and only if \TT{T$_{j,i}$~\ABV~T$_{k,i}$}
...
SAFE expressions are compiled into simpler expressions by introducing
new variables, called `\skey{temporary variable}s'.  For example, in:
\begin{indpar}\begin{verbatim}
int32 x = ...
int64 y = 34,587 + 5 * x
\end{verbatim}\end{indpar}
the second statement is compiled to:
\begin{indpar}\begin{verbatim}
int64 T$1 = 34,587
int64 T$2 = 5
int64 T$3 = x
...
int64 y = T$1 + T$4
\end{verbatim}\end{indpar}
The statements in this last code are statements of SAFE-AL, the
SAFE Assembly Language.
Operators in SAFE-AL only appear in statements that contain just
one operator and some variables, and do not contain subexpressions
or constants.
The variables \TT{T\TMP\ldots{}} are temporary variables.
...
However precedence is only a rough guide to the syntax of SAFE
...
\emkey{selection-operand}\label{SELECTION-OPERAND}
    ...
    ::= & {\em bitop-expression} $|$ {\em summation-expression}
                                 $|$ {\em product-expression} \\
    $|$ & {\em data-expression} $|$ {\em call-expression}
                                $|$ {\em subexpression}
    \end{tabular}
\\[0.5ex]
...
\emkey{logical-operand}\label{LOGICAL-OPERAND}
    ...
    $|$ & {\em call-expression} $|$ {\em subexpression}
    \end{tabular}
\\[0.5ex]
...
\hspace*{0.5in}\begin{tabular}[t]{rl}
    ...
    \end{tabular}
\\[0.5ex]
\emkey{shift-operator}\label{SHIFT-OPERATOR} ::= \TT{<{}<} $|$ {\em >{}>}
\\[0.5ex]
\emkey{bitop-operand}\label{BITOP-OPERAND} ::=
    {\em data-expression} $|$ {\em call-expression} $|$ {\em subexpression}
\\[0.5ex]
...
\emkey{summation-operand}\label{SUMMATION-OPERAND}
    \begin{tabular}[t]{rl}
    ::= & {\em product-expression} $|$ {\em data-expression}
                                 $|$ {\em call-expression} \\
    $|$ & {\em subexpression} \\
    \end{tabular}
\\[0.5ex]
...
\emkey{data-expression}\label{DATA-EXPRESSION}
    ...
    ::= & {\em constant} \\
    $|$ & {\em variable-name} \\
    $|$ & {\em data-primary} ~ {\em selector}\PLUS{} \\
    $|$ & {\em type-name} ~ \TT{(} {\em expression} \TT{)} \\
...
\emkey{constant}\label{CONSTANT} ::=
    {\em number} $|$ {\em quoted-string} $|$ {\em quoted-name}
\\[0.5ex]
\emkey{quoted-name}\label{QUOTED-NAME} ::= \TT{`} {\em name} \TT{'}
\\[0.5ex]
\emkey{data-primary}\label{DATA-PRIMARY} ::=
	{\em variable-name} $|$ {\em subexpression}
\\[0.5ex]
\emkey{selector} ::= {\em element-selector} $|$ {\em subscripts}
\\[0.5ex]
\emkey{element-selector} :::= \TT{.} {\em element-name}
\\[0.5ex]
{\em element-name} :::= see \pagref{ELEMENT-NAME}
\\[0.5ex]
\emkey{subscripts} ::= \TT{[} {\em expression}
		      \{ \TT{,} {\em expression} \}\STAR{} \TT{]}
\\[0.5ex]
\emkey{variable-name}\label{VARIABLE-NAME} ::=
    \begin{tabular}[t]{l}
...
\\[0.5ex]
\emkey{subexpression}\label{SUBEXPRESSION} ::= \TT{(} {\em expression} \TT{)}
...
In `\TT{v >{}> s}' and `\TT{v <{}< s}', \TT{v} is converted to the
target type which may be any integer type $T$, but \TT{s} is converted
to \TT{unshw} and only its low order $x$ bits are used as the
shift amount, where $x$ is the size of $T$.
In other words, the shift amount is \TT{s mod 2$^x$}.
...
The operator \TT{/} is strictly floating point.
...
*UNCHECKED* int8 u = z  // *UNCHECKED* conversion used:
                        // Now u == 1
float64 m = 1.7
int32 i = m             // Illegal: undefined implied
                        // conversion
