...
can be referenced in code.  {\em Function-declarations}
...
and \skey{module extender}s.  Names shared
...
never in module extender files.
...
Declarations are statements. A declaration has
...
External names and declarations are said to be `skey{export}{ed}' to
other modules.

An example is:
\begin{indpar}\begin{verbatim}
module `my own module' as mom
mom x = ...
function int32 y = mom my external function ( int32 x )
function int32 y = my internal function ( int32 x )
function int32 y = mom ( int32 x ) another external function
function int32 y = ( int32 x ) another internal function
...
all modules that import the module containing the declaration.
Note that the {\em module-abbreviation} used to name the
module in an importing module may differ from the {\em module-abbreviation}
...
Thus given the above example and
\begin{indpar}\begin{verbatim}
int32 z1 = mom 5 another external function
int32 z2 = 5 another external function
\end{verbatim}\end{indpar}
both statements are correct if there is no other function declaration
in scope to match the function usage in the second of these statements.
...
\emkey{module-declaration}
    ...
    \end{tabular} \\
\emkey{simple-module-declaration} ::= \TT{module} \TT{`}{\em module-name}\TT{'}
        \TT{as} {\em module-abbreviation} \\
\emkey{module-name} ::= \TT{name} \\
\emkey{module-abbreviation} ::= \TT{word} not containing any `\TT{.}'s \\
\emkey{module-import-clause} ::= \TT{import} \TT{`}{\em module-name}\TT{'}
        \TT{as} {\em module-abbreviation}
\end{indpar}

A {\em module-name} is transcribed to a file name by replacing any
spaces between words and numbers by `\TT{\_}', omitting other spaces,
and replacing any `\TT{\_}'s by `\TT{\_\_}'.
For example, the name
\begin{center}
`\TT{fee / fie foe fum}'
\end{center}
transcribes to `\TT{fee/fie\_foe\_fum}'.
The transcribed {\em module-name} of a {\em module-declaration} must match the
...
A \key{module extender}
is a file that begins with an {\em extender-declaration}:

\begin{indpar}
\emkey{extender-declaration}
    \begin{tabular}[t]{rl}
    ::= & {\em simple-extender-declaration} \\
    $|$ & {\em simple-extender-declaration}\TT{:} \\
	& \TT{~~~~}{\em module-import-clause}\STAR{} \\
    \end{tabular} \\
\emkey{simple-extender-declaration} ::= \TT{extender}
        \TT{`}{\em module-name}\TT{'}
        \TT{as} {\em module-abbreviation}
\end{indpar}

A module extender must have an associated module with the
same {\em module-name} (but usually a different extension).
The {\em module-abbreviations} associated with this name
by the extender and by the module proper must match.

The code in the extender is compiled as if it were appended
to the code in the module proper, but with additions to
the imported modules made by the {\em module-import-clauses} of
the {\em extender-declaration}.

A module extender may not contain external declarations.
Its purpose is to provide out of line function code.

[Global variable initialization?].

If we consider the conceptual directed graph whose nodes
are modules and whose arrows connect each module to the
modules it or its extenders import, then this graph
\underline{must be acyclic}.
...
{\em extender-declaration} defines a context in which the
code of the module extender is compiled.

Contexts can also be computed and code compiled at run time
can be compiled in a computed context.  The only expression
that can be used to compute a new context is:

\begin{indpar}
\emkey{context-constant}
    \begin{tabular}[t]{rl}
    ::= & \TT{new context:} \\
