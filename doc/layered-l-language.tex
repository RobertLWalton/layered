...
using `\key{direct assignment}'\label{DIRECT-ASSIGNMENT} if possible, or
...
invoked the inline function that contains
...
\begin{indpar}
{\tt define protected variable qualifier~}\ttakey{MEM} \\
{\tt define protected variable qualifier~}\ttakey{WRITABLE} \\
{\tt define protected addable value qualifier~}\ttakey{STACK} \\
{\tt define variable qualifier~}\ttakey{UNCHECKED}
\end{indpar}
...
where $Qsw$ denotes the qualifiers of {\tt w} and $Tw$ is the type
of {\tt w}, and $Qsu$, $Tu$ similarly for {\tt u},
the compiler will assign the value of {\tt w} to the variable {\tt v}
using direct assignment if possible: see \pagref{DIRECT-ASSIGNMENT}.
However a direct assignment must obey the following rules:
\begin{indpar}\begin{itemize}
...
must also be a qualifier of {\tt w} unless
the qualifier is addable.
\item[(DA3)]
A \underline{value} qualifier of {\tt w}
must also be a qualifier of {\tt u} unless
the qualifier is deletable.
\item[(DA4n)]
If the assignment is not a result assignment (see below),
all the \underline{variable} qualifiers of {\tt u} must be both
\underline{un-protected} and \underline{addable}, regardless of
whether they are qualifiers of {\tt w}.
\item[(DA5n)]
If the assignment is not a result assignment (see below),
{\tt w} may have any \underline{variable} qualifier.
\item[NOTE]
If the assignment is not a result assignment (see below),
then as far as {\tt u} is concerned, the qualifiers of {\tt w} are
ignored.
\end{itemize}\end{indpar}

Function calls are implemented with implied assignments of actual
arguments to function prototype arguments and function prototype
results to actual results.
Function prototypes and function
calls are described in detail later in this document
(\itemref{FUNCTION-PROTOTYPES-AND-CALLS}), but for the
purposes of this section reference to the example at the beginning
of this section may suffice.
...
\end{tabular}
\end{center}
an \key{argument assignment}
\begin{center}
\begin{tabular}{l}
\tt $Qsv$ $Tv$ v = w \\
\end{tabular}
\end{center}
is generated which behaves just like any other assignment.
Also a \key{result assignment}
\begin{center}
\begin{tabular}{l}
\tt $Qsu$ $Tu$ u = r \\
\end{tabular}
\end{center}
is generated, but result assignments differ from other assignments
in that result assignments obey the rules:
\begin{indpar}\begin{itemize}
\item[(DA4r)]
If the assignment is a result assignment,
a \underline{variable} qualifier
of {\tt u} must also be a qualifier of {\tt r} unless
the qualifier is addable and un-protected.
\item[(DA5r)]
If the assignment is a result assignment,
a \underline{variable} qualifier
of {\tt r} must also be an explicit qualifier of {\tt u} unless
the qualifier is deletable or protected.
If the qualifier is protected but not an explicit qualifier of {\tt u},
the qualifier will be implicitly added to {\tt u}.
\item[NOTE]
If the assignment is a result assignment,
the \underline{variable} qualifiers of {\tt u} must be the same as the
qualifiers of {\tt r}, except that (1) an addable, un-protected qualifier
of {\tt u} need not be a qualifier of {\tt r}, (2) a deletable, un-protected
qualifier of {\tt r} need not be a qualifier of {\tt u}, and (3)
a protected qualifier of {\tt r} need not be an explicit qualifier of {\tt u}
but will be implicitly added to {\tt u}.

Thus result assignments treat variable qualifiers as if they
were value qualifiers, except for protected qualifiers, which
are added by implication.
\end{itemize}\end{indpar}
...
\tt $Qsu$ $Tu$ u = w
\end{center}
does \underline{not} obey the rules of direct assignment, it is
re-written as
\begin{center}
\tt $Qsu$ $Tu$ u = assign ( w )
\end{center}
...
\underline{protected} or non-addable \underline{variable} qualifiers.
