...
\TT{type} {\em qualifier-name}\STAR{} {\em defined-enum-type-name}
    ...
    \{ \TT{without comparison operators} \}\QMARK
    \end{tabular}
\end{center}
where
\begin{center}
...
\emkey{base-enum-type-name}
    & ::= & {\em enum-direction}\QMARK{}~~~{\em builtin-integer-type-name} \\
    & $|$ & {\em enum-direction}\QMARK{}~~~{\em defined-enum-type-name} \\
\emkey{enum-direction}
    & ::= & \TT{next} $|$ \TT{previous} \\
\emkey{builtin-integer-type-name}
    & ::= & {\em builtin-number-type-name} naming an integer type \\
\emkey{enum-range}
...
Here {\em first-enum} and {\em last-enum} are integer values assigned
to the newly defined enum type (and not integer values assigned to
base enum type values).
