...
A qualifier has a name that is an identifier, just as variable and functions
have names that are identifiers.

There are several types of qualifiers.

\ikey{Caching qualifiers}{caching qualifier}
control the movement of memory locations into registers.  A register
that holds the contents of a memory location is refered to in this
document as a software cache (of the memory location).  Such software
caches have to be flushed appropriately.  Software caching qualifiers
contain information necessary to do this.

For example, the `{\tt local}' qualifier states that a memory datum
is allocated to a function frame and is only accessed by the function
execution associated with that frame.  Such a datum may be cached
in a register for the entire lifetime of the function execution.

Another example is the `{\tt stack}' qualifier, which states that a memory
datum is allocated to a function frame and can only be accessed by the
function execution associated by that frame or by functions called by this
function execution which are passed the address of the memory location
as a argument.  Such a memory location can be cached in a register, but
this software cache must be flushed when a function is called that has
the address of the memory location as an argument.

A third example is the `{\tt constant}' qualifier which states that a memory
location will no longer be modified.  Such a memory location can be cached
in registers that need never be flushed.

A different kind of qualifier is the \key{protection qualifier}.  Most of
these are specifically declared in the program and not built into the
langauge.  A memory location with protection qualifier $X$ can only be
...
Yet another kind of qualifier is the \key{alias qualifier}.  In general,
memory locations that are referenced differently are assumed by the
L-Language to be different memory locations.  But they might not be,
and to specify this, two memory locations may be given the same alias
qualifier, indicating they might be the same location.  Aliased locations
may not be able to use software caching.

A last kind of qualifier is the \key{parallel qualifier}.  This qualifier
is attached to memory locations that can be accessed in parallel by
several hardware central processing units.  Parallel qualifiers are used
to identify memory locations whose software \underline{and hardware}
caches are to be flushed by explicit cache flusing statements in the
L-Language.
...
The following are the caching qualifiers.
...
restriction, and there are no restrictions on where an address to the
location can be stored or whether it can be passed as an argument or result,
as long as the address type indicates the location is constant.
However a `{\tt constant}' location may also have other qualifiers that
place restrictions on the location.
...
`{\tt constant}' when they are passed to
initialization functions, such as constructors.

The following code has some examples:
\begin{indpar}\begin{verbatim}
int function1 ( constant & arg1 );
int function2 ( & arg1 );
int function3 ( void )
{
   constant int value1 = 5;
       // Permitted: constructor(int) ( int & value1, 5 ) is called.
   value1 = 10; 
       // Erroneous: assign(int) ( constant & value1, 10 ) is called.
   function1 ( value1 );  // Permitted.
   function2 ( value1 );  // Erroneous.
}
\end{verbatim}\end{indpar}

\hspace*{-1em}\ttkey{local}~~The memory location must be allocated to
a function frame (see Frames, \pagref{FRAMES}) and the location
may be accessed only by the function execution associated with the frame.

The address of the memory location cannot passed as an argument or result
value to an out-of-line function or stored in global memory.

The `{\tt local}' qualifier is assigned by the default to memory locations
that are part of a function frame.  The `{\tt local}' qualifier cannot
be assigned to a memory location which has the `{\tt stack}' or
`{\tt global}' qualifiers (see below).

The following code has some examples (see `{\tt stack}' below):
\begin{indpar}\begin{verbatim}
int function1 ( local int & arg1 );
    // `local' forces function1 to be inline.
extern int function2 ( stack int & arg1 );
    // `extern' forces function2 to be outline.
int function3 ( void )
{
   int value1;  // value1 is `local' by default.
   function1 ( value1 );  // Inline call.
   function2 ( value1 );  // Erroneous.
   int & value3 = value1;  // value3 is `local' by default.
   stack int & value4 = value1;  // Erroneous.
   function2 ( value4 );  // OK.
}
\end{verbatim}\end{indpar}
...
\begin{center}
\end{center}
is in error as {\tt arg1} would be local to the caller.

\hspace*{-1em}\ttkey{stack}~~If the memory location is being allocated
to a function frame, this qualifier specifies that the location may be
accessed by the function execution associated with the frame, or by
functions called by this function execution.  The address of the location
may be passed as an argument but may not be returned as a result.

If the memory location is being passed into a function execution via an address
datum (e.g., an argument),
the qualifier specifies that the memory location will only
be accessed by the function execution or by functions called by this function
execution.  The memory location, if it is allocated to a function frame,
must have that `{\tt stack}' qualifier when it is allocated.  But the
memory location may also be `{\tt global}'.  In this case the memory location
may also be returned as a function result (of a function execution that did
not allocate the location).

TBD: should be two different qualifiers?
