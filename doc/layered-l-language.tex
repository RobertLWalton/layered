...
where
\begin{enumerate}
\item A {\em type-name} may not be an initial segment of
an {\em initial-function-term-name}.
\item A {\em qualifier-name} may not be an initial segment
of a {\em type-name},
...
{\em qualifier-names},
{\em type-names}, {\em variable-base-names}, and
{\em member-names} have been identified.
...
and \ikey{bodies}{body}.  There is a one-to-one correspondence
between modules and module files.  Each body file is associated
with a particular module, the `\key{module of the body}',
and extends that module.  A module
may have zero or more body files.
A module file and all its body files together form
an `\key{extended module}'.

Module and body files consist of a sequence
...
A {\em module-name} is a POSIX file name without its final extension.
...
If two different declarations of the same name have overlapping
overlapping scope, one of these scopes must include the other,
...
As a consequence, it is unwise to place function declarations
...
as the deferred assignment statement, but may be in a separate body
of that module, and may use out-of-line functions in that module or in modules
imported by that module.  It is a programming error to access the
result variable values before they are assigned.
...
\end{verbatim}\end{indpar}
are equivalent if no imported module defines `\TT{my external function}'
with an \TT{int32} result and one \TT{int32} argument.
...
A {\em body-name} is a POSIX file name without its final extension.
The {\em body-name} of a {\em body-de\-clar\-a\-tion} must match the
non-extension part of the name of the file containing the
...
by executing top level assignment statements in the body file.
...
The {\em function-declaration's} {\em result-variable-list} becomes the 
{\em result-variable-list} of the {\em block-as\-sign\-ment-statement} and the
{\em function-declaration's} {\em statements} become the 
...
(i.e., module file, see \itemref{MODULES}).
