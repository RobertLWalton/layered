...
The enumeration value also has a numeric value in the range
...
Thuse for a value $V$ of enumeration type $E$:
\begin{center}
...
The effect of an {\em enumeration-type-declaration} with an
{\em enum-type-expression} base type is as follows:
...
\\[0.5ex]
\ttstdkey{BAD ERROR CODE} &  When an error code that is too large is
converted to a number type that cannot store it, the error value stored
corresponds to this error code instead.
\\[0.5ex]
...
The maximum error code range value allowed determined by the number type
as follows:
\begin{center}
\begin{tabular}{l@{~~~~~}l}
...
\TT{uns$x$} & $x/4-1$ \\
\TT{float$x$} & $x/2-1$ \\
\TT{\ldots wrd} & same as \TT{\ldots $x$} where $x$ is size of type \\
\TT{\ldots max} & same as \TT{\ldots $x$} where $x$ is size of type \\
...
\\[1ex]
$T$ is \TT{adr} & \tt ERROR(c) == NULL + $c$  ~~~~~ [see text]\\
\end{tabular}
\end{center}

For the \TT{adr} type, \TT{NULL} is the address
...
error codes).
\end{indpar}
{\tt function bool r = ( $T$ value) \ttstdkey{is} ( error code c )}
\begin{indpar}
...
\end{indpar}
{\tt function bool r = ( $T$ value )\ttstdkey{is error code} }
\begin{indpar}
Returns \TT{true} if {\tt value} encodes an error code.
Same as: \\
\hspace*{0.3in}{\tt error code of ( value ) != std BAD ERROR CODE}.
\end{indpar}
