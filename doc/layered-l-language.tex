...
number if $M=N$.  If instead there are $M$ {\tt \&}'s in the declaration,
$M\leq N$,
...
Here $x${\tt @}$y$ is an unchecked operation that takes the byte address of
...
The standard pointer type is denoted by `{\tt *}', but
additional pointer types may be defined by
\begin{center}
...
with that same syntax as `{\tt *}'.  An example is:

\begin{indpar}\begin{verbatim}
defptr "<-" ( 64, 64, 0 );
void function1 ( int32 <- x, mystruct <-& y );
\end{verbatim}\end{indpar}

Here the first argument to {\tt function1},
{\tt x}, is a \verb|<-| type pointer to an {\tt int32} value,
and the second argument is similarly a \verb|<-| type pointer
...
"<-" .conversion. ( "*" p );
uns32 x;
uns32 <- xp = & x;  // Implied conversion from "*" to "<-".
uns32 y = * xp;     // Implied conversion from "<-" to "*".
\end{verbatim}\end{indpar}

Pointer types can be used to store pointers in special formats.
They can also be used to attach qualifiers to some pointers
that are stored in standard format: see \pagref{TRACEABLE-QUALIFIERS}.
...
adding either `{\tt .}' followed by a member name or
an integer constant subscript in `{\tt []}' brackets to
...
{
    // Vector of 2 element is allocated and the elements
...
readonly unsptr *& .n ( myarray *& @ a ) unchecked = a@0;
readonly unsptr *& .max_n ( myarray *& @ a ) unchecked = a@8;
readonly unsptr *& .inc_max ( myarray *& @ a ) unchecked = a@16;
...
Qualifiers may be applied to locations of memory.  For example,
...
`\skey{code qualifier}s'.  A qualifier just before the
...
a `\key{call qualifier}' of the function.  Such qualifiers are
generally protection qualifiers, inline qualifiers, or the
`{\tt pure}' qualifier.  For example,
\begin{indpar}[0.5em]\begin{verbatim}
int32 Q function3 ();
int32 *& Q function4 ();
\end{verbatim}\end{indpar}
...
must not be stored by checked code in a place that will exist after the
...
a function.
The `{\tt temp}' qualifier
specifies that the location will last as long as no `{\tt impure} function
is called.  The `{\tt pure}' and `{\tt impure}' call qualifiers specify
how function calls interact with `{\tt temp}' locations.
...
whose bodies have qualifier $X$.  There is one builtin protection
...
Qualifiers may \ikey{imply}{qualifiers}
other qualifiers.  There can be qualifiers that
are not themselves in any of the above categories, but which imply
other qualifiers that are.

Qualifiers may also be \ikey{inherited}{qualifiers}.  For example, in
\begin{indpar}\begin{verbatim}
readonly unsptr *& .n ( *& myarray @ a ) unchecked = a@0;
\end{verbatim}\end{indpar}
the `\verb|@|' in the argument declaration `\verb|*& myarray @ a|'
specifies that inheritable qualifiers applied to the argument location
should become qualifiers of the returned value.
...
The four lifetime qualifiers are all builtin:  `{\tt static}', `{\tt stack}',
`{\tt caller}', and `{\tt temp}'.
...
Other locations are given the `{\tt temp}' lifetime qualifier by default.
The `{\tt temp}' qualifier makes use of two associated function 
call qualifiers, `{\tt impure}' and `{\tt pure}.

\begin{indpar}

\hspace*{-1em}\ttkey{static}~~~~~Specifies
the location lives forever and may not be relocated.
Locations allocated at load time
...
function or one of its code blocks, and may not be relocated.
Such locations are precisely known
...
used in a function $F$'s argument or return value types to qualify a location
whose address is passed to the function.  Specifies the function's caller
must provide the `{\tt caller}' location, and the location cannot be
deallocated or relocated until after control returns to the caller.
Within the caller the location may be `{\tt stack}',
`{\tt static}', or `{\tt caller}'.
Asserting that such a location is `{\tt caller}' is a promise by the
...
\hspace*{-1em}\ttkey{temp}~~~~~Specifies
the location is valid until an `{\tt impure}' function is called.
Only `{\tt impure}' functions may deallocate or reallocate the location.
A function may return a `{\tt temp}' location to its caller,
or pass a `{\tt temp}' location to a called function.

Out-of-line functions are `{\tt impure}' by default.  A `{\tt pure}'
function may \underline{not} call an `{\tt impure}' function, and
in particular, cannot suffer an interrupt or exception.  Inlined
functions are by default neither `{\tt pure}' or `{\tt impure}'; they
are simply inserted as if they were part of their caller, and may
contain calls to either `{\tt pure}' or `{\tt impure}' functions.

The assumption is that only `{\tt impure}' functions can deallocate
or reallocate locations.  It is further assumed that interrupts
are the equivalent to calls to out-of-line `{\tt impure}' functions.
In actual practice,
only `{\tt unchecked}' functions can deallocate or reallocate locations,
so it is up to the user to declare all such functions `{\tt impure}'
or `{\tt outline}'.
...
with a different lifetime qualifier,
with the exception that the address of a location with
a `{\tt static}', `{\tt stack}', or collectible qualifier
\underline{may} be re-cast as the an address of a location with a
`{\tt caller}' qualifier, and the address of a location with
any lifetime qualifier may be re-cast as the an address of a location
with a `{\tt temp}' qualifier.

\subsubsection{Traceable Qualifiers}
\label{TRACEABLE-QUALIFIERS}

There are two builtin traceable qualifiers: 

\begin{indpar}

\hspace*{-1em}\ttkey{traceable location}~~~~~Specifies
the location can be found by the appropriate trace operations.
The location must also have either the `{\tt static}' or `{\tt stack}'
qualifiers.  The traceable location qualifier is used to find
data that must be modified in certain circumstances.

\hspace*{-1em}\ttkey{traceable value}~~~~~Specifies that if the value
of the location is stored in some `{\tt static}', `{\tt stack}',
or `{\tt caller}' location, then the value
can be found by the appropriate trace operations.
In this case a traceable location datum is established implicitly that holds
the value.  Once stored in a single implicit traceable location datum,
a traceable value can be copied to many other locations
...
each central processing unit has its own hardware cache that is
...
... my_function ( ... )
{
    value1 = 10; 
       // Erroneous:
       //   caller constant int32 *& .assign. ( 10 ) is called.
...
\hspace*{-1em}\ikey{Code Block Qualifiers}%
{code block qualifier}~~~~~Code block
qualifiers may be declared by the programmer and used to qualify
code blocks.
They may then be used in `\skey{qualifier expression}s'
that serve as function call qualifiers or memory location qualifiers.

\end{indpar}

Examples of protection qualifiers are given in the next section.
...
each of the function's call qualifier expressions
or location's qualifier expressions
must evaluate to true
according to the following rules.  First, each code block qualifier
is given the value true if it qualifies the code block and false otherwise.
Second, every non-code-block qualifier is given the value true if and only if
doing so is necessary and sufficient to make all
the subexpressions containing the non-code-block
qualifier true.  This last rule is applied from left to right.
Any non-code-block qualifiers
...
non-code-block qualifiers given the value false are ignored.

For example, given the code
\begin{indpar}[0.5em]\begin{verbatim}
deftype type1 (24,4);
declare code block qualifier constructor(type1);
...
Note the left-to-right clause in the rule for evaluating non-code-block
qualifiers.  For example, given the code:
\begin{indpar}[0.5em]\begin{verbatim}
deftype type1 (24,4);
declare code block qualifier A;
((A&readonly)|constant) type1 my_data1;
((A&readonly)|(A&readwrite)|constant) type1 my_data2;
\end{verbatim}\end{indpar}

then to a function with code block qualifier {\tt A}, {\tt my\_data1}
...
\begin{enumerate}
\item
A code block qualifier evaluates to true or false according to whether
the invoking code has or does not have the qualifier.
\item
Any other qualifier evaluates to true when it is evaluated.
When this happens the qualifier is asserted for the function
...
is called the look ahead window, and is a subarray of the full window.
...
The way we accommodate this is to use the selector codes of {\tt y}
to specify that the address of {\tt y} is the value of {\tt x}, so
that if the value of {\tt x} is changed by adding $\Delta${\tt x}
then the address of {\tt y} should be changed by adding $\Delta${\tt x}.
And similarly the selector of {\tt z} specifies that the address of {\tt z}
...
`\verb|c*d|' will be reused comparatively very rarely code
