...
Most fo these have special effects
...
is copied to another variable.  If a variable qualifier is attached
to a prototype argument, any actual argument matching the prototype
argument must be a variable name that has the variable qualifier.
If no variable qualifiers are attached to a prototype argument,
variable qualifiers on actual arguments matched to that prototype
argument are ignored as if they did not exist.
...
The \ttkey{*MEM*} qualifier is a protected variable qualifier that behaves
...
\begin{enumerate}
\item
A member {\tt v.adr} of type {\tt adr} 
...
and because a variable cannot be a RAM cache variable unless it is
a cluster root,
...
required qualifier that must qualify the {\tt v.adr} or
{\tt v.offset} variables
associated with a variable {\tt v} that has the \TT{*MEM*} qualifier.
