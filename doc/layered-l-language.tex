...
modules it imports and bodies it is after,
then this graph \underline{must be acyclic}.
Modules and bodies are compiled in an order consistent with the partial
order implied by this graph.  They are also loaded in such an order.

When a body is loaded, the statements in it are effectively executed
in order.  Global area assignment statements, including those of
deferred variables declared in modules, are executed in order.
Out of line code is defined when it is encountered.

When a body is compiles statements that execute at compile time
