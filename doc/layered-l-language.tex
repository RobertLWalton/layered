...
\emkey{function-declaration}\label{FUNCTION-DECLARATION}
    ::= \begin{tabular}[t]{l}
        \{ \ttkey{out of line} \}\QMARK{} ~ {\em function-prototype} \TT{:} \\
...
\item
The first {\em function-term-name} of a {\em function-prototype}
should not have a {\em type-name} as an initial segment
unless it is preceded by a \TT{=} in
the {\em function-prototype}
...
A function is inline unless a {\em function-prototype}
for the function precedes the {\em function-declaration} in a compilation, or
the {\em function-declaration} begins with `\TT{out of line}'.
...
If the {\em function-call} is in a statement of the form
\begin{center}
{\tt $TC1$ $VC1$, $TC2$ $VR2$, \ldots{}~=~}{\em function-call}
\end{center}
the {\em function-prototype} must be of the form
\begin{center}
{\tt function $TR1$ $VR1$, $TR2$ $VR2$, \ldots{}~=~}{\em function-pattern}
\end{center}
with at least as many result variables in the prototype as in
the {\em function-call} statement.  The statement is then
replaced by:
\begin{indpar}
{\tt $TR1$ $tmp1$, $TR2$ $tmp2$, \ldots{}~=~}{\em function-call} \\
{\tt $TC1$ $VC1$ = $tmp1$} \\
{\tt $TC2$ $VC2$ = $tmp2$} \\
\ldots\ldots\ldots\ldots
\end{indpar}
If any of the `{\tt $TCI$ $VC1$ = $tmpI$}' statements cannot be
compiled because $TRI$ is not implicitly convertible
to $TCI$, the match fails.  Note that extra prototype result variables
are never used and are ultimately ignored.

If $TR1$ and $TC1$ are not equal (but $TR1$ values are implicitly
convertible to $TC1$ values), the match is marked
...
{\em argument-declarations} `$T1$ $v1$', `$T2$ $v2$', \ldots{}
read from left to right, then the statement containing the
...
If all the statements `{\tt $TI$ $tmpI$ = $eI$}' can be compiled
without error, the match succeeds.

If any of the statements `{\tt $TI$ $tmpI$ = $eI$}' cannot be compiled
for a reason other then an ambiguity error, the match fails.
