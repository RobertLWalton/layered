...
\begin{indpar}\begin{verbatim}
defsubtype indexint of int32

external:
    constant int32 max index

inline indexint .convert. ( int32 i ) unchecked:
    assert ( 0 <= i && i < max index )
    return i

inline int32 .convert. ( indexint i ) unchecked:
    int32 j = i
...
conversion of {\tt i} from {\tt indexint} to {\tt int32}
...
this is:
\begin{indpar}\begin{verbatim}
defsubtype specialadr of adr
    // A specialadr is either an adr or an integer
    // in the range [0,4096).

inline bool is int ( specialadr a ) unchecked:
    unsadr x = a
    return x < 4096

inline bool is adr ( specialadr a ) unchecked:
    unsadr x = a
    return x >= 4096

inline adr .convert. ( specialadr a ) unchecked:
    unsadr x = a
    assert ( x >= 4096 )
    return a

inline unsadr .convert. ( specialadr a ) unchecked:
    unsadr x = a
    assert ( x < 4096 )
    return x

inline specialadr .convert. ( unsadr x ) unchecked:
    assert ( x < 4096 )
    adr a = x
    return a

inline specialadr .convert. ( adr a ) unchecked:
    unsadr x = a
    assert ( x >= 4096 )
...
\begin{indpar}\begin{verbatim}
deftype int32 = (4,0,0)
deftype float64 = (8,0,0,floating)
// On some hardware:
    deftype adr = (8,0,0,address)
    deftype unsadr = (8,0,0)
    deftype insadr = (8,0,0)
// On other hardware:
    deftype adr = (4,0,0,address)
    deftype unsadr = (4,0,0)
    deftype insadr = (4,0,0)
\end{verbatim}\end{indpar}
...
\subsection{Types and Type Descriptors}
\label{TYPES-AND-TYPE-DESCRIPTORS}
...
Most runtime information about types is encoded in type descriptors.

A \key{type descriptor} is a run-time value that gives information
about a type.  A type descriptor itself has a type, and every type
descriptor type has the special property that there is only one
value possible of the type descriptor type, and the compiler can
...
It is possible to create new type descriptors by supplying
a function that can be run by the compiler and loader to compute
the values of the new type descriptors from other values known
to the compiler and loader.  For example:
\begin{indpar}\begin{verbatim}
defsubtype length ( type T ) of unsadr
...
A type descriptor declaration is similar to a function declaration,
except that the name of the function becomes the type name of the
type descriptor.  The type descriptor type in turn becomes
a subtype of the value returned by the function.
