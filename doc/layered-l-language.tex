...
$t2$ ~~ \TT{*UNCHECKED*~<{}<++} ~~ $t1${\em -type-value}%
\label{<<**}
...
\begin{center}
$t2$ ~~ \TT{*UNCHECKED*~<{}<++} ~~ {\em numeric-constant}
\end{center}
...
{\em builtin-type-name}
	& \TT{*UNCHECKED*~<{}<++} & {\em defined-type-value} \\
{\em defined-type-name}
	& \TT{*UNCHECKED*~<{}<++} & {\em builtin-type-value} \\
\end{tabular}
\end{center}
...
{\em base-enum-type-name}
	& \TT{*UNCHECKED*~<{}<++} & {\em defined-enum-type-value} \\
{\em defined-enum-type-name}
	& \TT{*UNCHECKED*~<{}<++} & {\em base-enum-type-value} \\
\end{tabular}
\end{center}
...
any actual variable matched to the argument of \TT{F} or \TT{G}
must have the `\TT{bar}' qualifier, which is required.
Any actual variable matched to the result of \TT{F} will be
...
In the above example, 
any actual variable matched to the argument of \TT{H} must
\underline{not} have the `\TT{oof}' qualifier, as that
is a prohibited qualifier.
Any actual variable matched to the result of \TT{H} will
...
\TT{protected qualifier~}\ttkey{*UNCHECKED*} \\
...
( stack pointer pair pp,
  ...
  *UNCHECKED* adr pp.base.adr,
  type pp.type ) =
    ...
    pp.base.adr *UNCHECKED* = offset.base.adr
...
( mem pp.type out,
  ...
  *UNCHECKED* out.offset.base.adr ) =
    ...
    ( out.offset, out.offset.base.adr ) = if:
        pp.begin < pp.end:
            out.offset *UNCHECKED* = pp.begin
            out.offset.base.adr *UNCHECKED* = pp.base.addr
        else:
            fatal error
...
"*" ( stack pointer pair pp,
    ...
    if:
        pp.begin < pp.end:
            unsadr offset *UNCHECKED* = pp.begin
            mem adr offset.base
            adr offset.base.adr
                *UNCHECKED* = pp.base.adr
            in *UNCHECKED* ---> offset
        else:
            fatal error
                ( "Deferencing empty pointer pair." )
...
( stack pointer pair pp2,
  ...
  *UNCHECKED* adr pp2.base.adr,
  type pp2.type ) =
      ...
      pp2.end = pp.end
      pp2.base.adr *UNCHECKED* = pp.base.adr
      pp2.type = pp.type
...
protected qualifier that must qualify the \TT{v.adr} or
\TT{v.offset} variables
associated with a variable \TT{v} that has the \TT{mem} qualifier.
See Section~\itemref{THE-MEM-QUALIFIER}.

The lexeme \TT{*UNCHECKED*} lexeme is commonly the initial part of the
names of functions and operators that produce \TT{*UNCHECKED*}
results.  For example, the operators `\TT{*UNCHECKED* =}'
and `\TT{*UNCHECKED* <{}<++}' which are distinct from the
operators `\TT{=}' and `\TT{<{}<++}'.
When used thusly, \TT{*UNCHECKED*} is
...
matches an actuall function call variable.
