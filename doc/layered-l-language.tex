...
A block is allocated if the addresses of all its bits are allocated,
and is deallocated if the addresses of all of its bits are deallocated.
A block is unshared if all its bits are unshared.  A block is shared if
there is a set of several blocks (sequences of consecutive addresses)
such that the $n$'th bit of each block in the set is the same.

The address space is divided into pages, which are blocks that
