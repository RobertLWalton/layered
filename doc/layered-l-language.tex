...
type and qualifiers (\pagref{NATURAL_TYPE})
...
compile time and contains \skey{include block}s that generate
        ...
        function T result = min ( T x, T y ):
            result = if:
                ...
                (T) is in floating-types:
                        // Parenthesize T to prevent compiler
                        // from treating it as the beginning
                        // of a result variable declaration.
                    result = NaN
...
Note that variables of type `\TT{type}' with compile-time
values behave as {\em type-names} in the included code.
Similarly variables of types `\TT{qualifier}',
`\TT{qualifier-set}', and `\TT{qualifier-\EOL list}' behave
as {\em qualifier-names}.
