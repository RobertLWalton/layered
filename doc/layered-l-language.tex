...
\emkey{include-for-statement}\label{INCLUDE-FOR-STATEMENT} ::=
    ...
    \TT{include}\ttindex{include for}
        ~ \begin{tabular}[t]{@{}l@{}}
	  {\em include-name}\QMARK{} \\
	  \TT{for} ~ {\em argument-type} ~ {\em argument-name} \\
...
\emkey{include-argument-list} ::=
    \TT{(} {\em argument-name} \{ \TT{,} {\em argument-name} \}\STAR{}
    \TT{)}
\\[0.5ex]
\emkey{argument-name} ::= {\em word} not containing `\TT{.}'
\\[0.5ex]
\emkey{argument-type-name} ::= {\em type-name} referencing an argument type
\\[0.5ex]
\ikey{Argument Type}{argument type} - One of:
    \begin{tabular}[t]{l}
...
{\em arguments} are substituted for the {\em argument-names} of these
arguments.  These values must be known at compile-time within the scope
of the {\em include-statement}, and must have an {\em argument-type}.
The substitutions depend on the {\em argument-type}
...
The argument name is treated as a type name within the included
code, and in that code denotes the type that is the argument value.

\hspace*{-0.3in}\ttmkey{qualifiers}{include argument}
The argument name is treated as a qualifier name within the included
code, and in that code denotes the set of qualifiers specified by
the argument value.
...
of its containing {\em include-statement}.  The values of arguments,
...
The {\em include-for-statement} introduces a new {\em argument-name}
