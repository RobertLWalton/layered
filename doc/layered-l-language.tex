...
The {\em expression} must evaluate to a \TT{type} value at
compile-time.  The simplest {\em expression} is just the name
of another type.
...
(\pagref{TYPE-VALUES}), or can be used
...
The following \underline{unchecked} builtin functions are defined:
\begin{indpar}
{\tt function $T$ r = $T$ ( \TT{*UNCHECKED*} $E$ v )} \\
{\tt function $E$ r = $E$ ( \TT{*UNCHECKED*} $T$ v )}

where $T$ is defined by `{\tt define type $T$ as $e$}' and
$E$ is a {\em type-name} associated with the type that is the
the value of $e$
...
\emkey{enum-assignment}
    ...
    \end{tabular}
\\[0.5ex]
\emkey{enum-type-name} ::= {\em type-name}
\\[0.5ex]
...
{\tt function type r = std \ttmkey{set of} ( type E )} \\
{\tt function type r = std \ttmkey{list of} ( type E )}
\begin{indpar}
