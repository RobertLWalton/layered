...
the implementation.  A reasonable assumption is that the high
order 17 bits are all equal, so only 48-bits of actual address
are used.  This assumption can be used to pack an address and other
data into a 64-bit word.  For example, an address can be inserted
in a 64-bit floating point NaN.  The Layered Languages do
\underline{not} depend upon this assumption, but applications may.

Heavy weight addresses are 128-bits and consist of a 64-bit
middle weight address and a 64-bit signed offset.  Implementations
may have either byte or bit offsets.  For a byte offset the
number stored is the bit offset divided by 8, and only bit offsets
that are exact multiples of 8 may be stored.
