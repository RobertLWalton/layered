...
all values written into that location must have qualifier {\tt Q},
...
A location qualified by `{\tt stack(n)}' can have
a `{\tt stack(m)}' qualified value only if {\tt n}$\geq${\tt m}.

A `{\tt stack(n)}' value can be stored in a location qualified by
`{\tt stack(m)}' only if {\tt n}$\leq${\tt m}.
...
A `{\tt caller}'  value can be stored in a `{\tt stack}' location
but not in a `{\tt caller}' location.
...
{\tt declare calling lifetime qualifier}
    {\em qualifier-name} {\em negated-qualifier-name} [{\em default}\,]
\end{center}
An example is:
\begin{center}
...
a lifetime that extends until an out-of-line function or a code
block that has qualifier {\tt NQ} is called.
The {\em default} in the declaration
is the default qualifier of an out-of-line function, and if not given
is {\tt NQ}.  Inline functions have neither the {\tt Q} or
...
an out-of-line function that did have the `{\tt pure}' qualifier is
