...
\tt // \\
\tt // Evaluate $e2$ - right operand of first "if"\\
\tt bool tmp1 = $e2$ \\
\tt // \\
\tt // Evaluate ($e1$ "if" $e2$) - left operand of first "else" \\
    ...
    if:
        arg1.value:
            r.right op = do ( std call context ):
                r.right op = arg2
            r.value = arg1.right op less than r.right op
        else:
            r.right op = default value
            r.value = false

function bool r = ( T arg1 ) less than ( * T arg2 ):
    // function definition is builtin and omitted here
\end{verbatim}\end{indpar}
...
\begin{tabular}{l}
\tt // Evaluate $e1$ - operand of `implied conversion' below \\
\tt bool tmp1 = $e1$ \\
\end{tabular}

\begin{tabular}{l}
\tt // Evaluate `implied conversion ( $e1$ )' \\
\tt // - left operand of first "<" \\
\tt *COMPARE-VALUE* tmp2, $T$ tmp2.right op, bool tmp2.value = \\
\tt ~~~~do ( implied-conversion-context ): \\
\tt ~~tmp2.right op = tmp1 \\
\tt ~~tmp2.value = true \\
\end{tabular}
...
\tt ~~if: \\
\tt ~~~~tmp2.value: \\
\tt ~~~~~~tmp3.right op = do ( tmp0 ): \\
\tt ~~~~~~~~~tmp3.right op = $e2$ \\
\tt ~~~~~~tmp3.value = tmp2.right op less than tmp3.right op \\
\tt ~~~~else: \\
\tt ~~~~~~tmp3.right op = default value \\
\tt ~~~~~~tmp3.value = false \\
\end{tabular}
...
\tt ~~~~~~~~~tmp4.right op = $e3$ \\
\tt ~~~~~~tmp4.value = tmp3.right op less than tmp4.right op \\
\tt ~~~~else: \\
