...
\begin{indpar}
{\tt define implied variable qualifier \ttkey{std *READ*}} \\
{\tt define abbreviation \ttakey{READ} ---> `std *READ*'} \\
{\tt define implied variable qualifier \ttkey{std *WRITE*}} \\
{\tt define abbreviation \ttakey{WRITE} ---> `std *WRITE*'} \\
{\tt define implied addable value qualifier \ttkey{std *STACK*}} \\
{\tt define abbreviation \ttakey{STACK} ---> `std *STACK*'} \\
{\tt define variable qualifier \ttkey{std *UNCHECKED*}} \\
...
The following functions allocate static memory and compute static addresses.


\begin{indpar}

{\tt function *UNCHECKED* adr r = \begin{tabular}[t]{@{}l@{}}
...
{\tt function *UNCHECKED* adr r = \begin{tabular}[t]{@{}l@{}}
				  ...
				  ~~~~~~unswrd region ?=~0 ) \\
				  \end{tabular} \\
\TS~~~~~~~~| ( underlying type of T ) is numeric |]}
\begin{indpar}
Allocate a vector of {\tt N} elements to
static memory, set each element equal to {\tt v}, and return the
address of the first (lowest addressed) element.
The alignment of the memory is equal to the alignment of $T$.
This alignment must be a power of two not greater than the memory
page size (which is the alignment of regions).
The address viewed as an
integer is an exact multiple of the alignment.
\end{indpar}

{\tt function *UNCHECKED* adr r = \\
\TS~~~~~~~~\ttstdkey{allocate UTF-8 to static} \\
\TS~~~~~~~~~~~~( string s, unsadr region ?=~0 )}
...
into tables in static regions into
...
{\tt L} bits, such that
\begin{center}
\tt extract adr ( embed adr ( T, A, L, S ), L, S ) == A
\end{center}
provided the low order {\tt S} bits of {\tt A} are zero and all of
the non-zero bits of {\tt A} are within the low order {\tt L+S} bits
of {\tt A}.  The high order {\tt 64-L} bits of the value returned
...
\begin{indpar}
Allocate a vector of {\tt N} elements to
dynamic memory, set each element equal to {\tt v}, and store the
address of the first (lowest addressed) element
in the body point that is in the static
memory location whose address is given by the first argument.
The alignment of the memory is equal to the alignment of $T$.
This alignment must be a power of two not greater than the memory
page size (which is the alignment of regions).
...
{\tt function \ttstdkey{deallocate from dynamic}
	      ( adr body pointer )}

\begin{indpar}
Deallocate the dynamic block pointed at by the body pointer,
and set the body pointer to \TT{NULL} (defined below).  If
the body pointer is already \TT{NULL}, do nothing.
\end{indpar}

\end{indpar}

The following functions return constants that are used in
managing dynamic blocks and dynamic pointers.
...
A global area is a dynamic block called a \key{global stack}
...
Specifically, the result variables of a statement in a {\em global-area-stack}
...
Several {\em global-area-blocks} may have the same {\em global-area-name}
provided that
\begin{center}
\begin{tabular}{l}
(1) the blocks have the savem {\em global-area-abbreviation} \\
and \\
(2) either \begin{tabular}[t]{@{}l}
           (2a) the blocks are all top level in the same file \\
	   or \\
	   (2b) the blocks are not top level but have the same
	        smallest containing subblock
	   \end{tabular}
\end{tabular}
\end{center}

As far as allocating variables to the global stack is concerned,
the several block behave as if they were concatenated (but note
that different names may be in scope for different blocks).

In order to be accessed a global area must be named in an
{\em extern-statement}:
\begin{indpar}
...
must be compiled after the {\em global-area-block} that defines
the referenced global variable, and the code containing the
{\em extern-statement} must be initialized after the code containing
the {\em global-area-block} is initialized.
...
a global area, the memory will be allocated to this global area,
...
Global areas are run time by default.  The code of a run time
...
Global area stacks may be moved during execution.  During execution of
a global block, the global area's stack is the current stack.
The main difference between normal execution and global block execution
is that globally visible variables are allocated at the top of the global
stack and after a global block has executed, the global stack is truncated
so it contains just these variables.  Note that truncation could happen
after each statement in the global block, but it does not for debugging
reasons.
...
A global area can be deallocated by the function:
...
An {\em atomic-statement} is a generalization of
...
\emkey{atomic-pointer} ::= {\em expression} evaluatable with \TT{pointer} type
							\\
\emkey{atomic-value} ::= {\em expression} evaluatable with pointer \TT{.type}
...
within the block of the {\em atomic-statement}.
When the {\em atomic-statement} terminates, either
...
In both passes 1 and 2, the \emkey{compare-instruction} reads the value at
the {\em atomic-pointer} designated
...
Re-executing the {\em atomic-statement} will page the offending
...
During passes 1 and 2, the \emkey{write-instruction} performs a
...
the {\em atomic-pointer} designated address.  
...
The returned {\tt count} value returned by the {\em atomic-statement}
...
The returned {\tt atomic code} indicates whether the {\em atomic-statement}
...
in an {\em atomic-statement} block.  Violation of these restrictions
