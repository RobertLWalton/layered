...
value permitted by the base enumeration type according to rules given
below.
...
An enumeration type declaration maps values of the enumeration type
E it is defining onto values of its base type.
...
If an enumeration type \TT{E} has an integer base type \TT{I}
then `\TT{integer of ( e-value )}' returns the same value as
the implicit conversion `\TT{I ( e-value )}'.
...
clauses were omitted from the definitions.

If an enumeration type declaration has no {\em enum-direction}, the
intergers in the {\em enum-range}
...
The {\em enum-range} given in an enum declaration statement
is mapped to a subrange of the
...
applied to the type \TT{Y} (not the type \TT{X}),
...
Builtin integer types are treated as enum types with enum range
equal to the integer type value range.  Thus we have for example:
\begin{indpar}\begin{verbatim}
type non negative int32 is previous int32 with enum range [0 .. 20]
\end{verbatim}\end{indpar}
...
prototype int64 result = ( int64 argument 1 )
                     "+" ( int64 argument 2)
prototype float64 result = ( float64 argument 1 )
                       "+" ( float64 argument 2)
int64 x = ...
int64 y = ...
...
\begin{indpar}\begin{verbatim}
// Prototypes of builtin "+" operator:
prototype int32 result = ( int32 argument 1 )
                     "+" ( int32 argument 2)
prototype float64 result = ( float64 argument 1 )
                       "+" ( float64 argument 2)
int32 x = ...
...
and names of argument variables.
...
\begin{indpar}\begin{verbatim}
( bool v.nonneg ) = test sign ( int32 v ):
    if:
        v >= 0:
            v.nonneg = true
        else:
...
created or used.  The matching prototype variable will
