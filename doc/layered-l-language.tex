...
In the following \key{system memory} refers to the main computer
RAM memory.  Individual processors have caches of this memory,
and the values 

TBD

The following builtin \skey{atomic function}s
can be used to optimistically access a shared database if
there is only one process that writes the database:
...
\begin{indpar}
Return the value of the vector memory element.  The vector length
must be 1 (p.end must equal p.begin + size of uns32).  The type of this
element must be \TT{uns32}, and the address of the element
must a multiple of the element size in bytes.

All reads executed \underline{after} this builtin function will return data
that was written to system memory \underline{after} the value read by
...
All reads executed \underline{before} this builtin function will return data
that was written to system memory \underline{before} the value read by
this function was written to system memory.
\end{indpar}
...
\begin{indpar}
Write the value to the vector memory element.  The vector length
must be 1 (p.end must equal p.begin + size of uns32).  The type of this
element must be \TT{uns32}, and the address of the element
must a multiple of the element size in bytes.
...
An {\em atomic-block} is a generalization of
...
\emkey{atomic-block} ::= \\
\hspace*{0.5in}\begin{tabular}[t]{l}
        {\tt uns32 count, atomic code c, uns32 pass = \ttakey{ATOMIC}:} \\
...
The returned {\tt count} value returned by the {\em atomic-block}
