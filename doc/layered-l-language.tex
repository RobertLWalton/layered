...
The compile time code contains \skey{include block}s that generate
...
`\TT{next V}' is a new variable, but the member of a cluster rooted
at \TT{V} is not changed unless
the inline code that assigned a value to `\TT{next V}'
also assigned a value to that member.
...
stack, `\TT{base}' is the base address of the stack, and
stack, `\TT{base.adr}' is the address of where this
base address is stored in RAM.  The first and last of these
are fixed but `\TT{base}' may change when the stack is
moved.
