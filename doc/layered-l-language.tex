...
{\em name} \begin{tabular}[t]{@{}rl}
    ...
    \end{tabular} \\[1ex]
{\em base-name} ::= {\em simple-name} not beginning with `\TT{.}' \\[1ex]
{\em simple-name} ::= {\em simple-name-item}\PLUS{} \\[1ex]
{\em simple-name-item} ::=
    {\em word} $|$ {\em mark} $|$ {\em quoted-name-item} \\[1ex]
{\em quoted-name-item} ::=
    double quoted (\TT{"~"}) {\em separator}, {\em mark}, or {\em word} \\[1ex]
{\em member-selector} \begin{tabular}[t]{@{}rl}
    ::= & {\em member-name} \\
    $|$ & \TT{[}{\em member-index-list}\TT{]}
    \end{tabular} \\[1ex]
{\em member-name} ::= {\em simple-name} beginning with `\TT{.}' \\[1ex]
...
Name constants are made by \TT{`~'} quoting a {\em name}.
Two {\em name} values can be
...
name n2 = `.husband'
name n3 = `[3]'
name n = n1 + n2 + n3  // n now = `my name .husband [3]'
\end{verbatim}\end{indpar}
...
subsituted for the name.  This is done recursively.  For
example:
\begin{indpar}\begin{verbatim}
int32 i = 5
int32 j = 6
