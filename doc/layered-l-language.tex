...
inline indexint .conversion. ( int32 i ) unchecked:
    assert ( 0 <= i && i < max index )
    return i

inline int32 .conversion. ( indexint i ) unchecked:
    int32 j = i
...
inline unsadr .conversion. ( specialadr a ) unchecked:
    unsadr x = a
    assert ( x < 4096 )
    return x

inline specialadr .conversion. ( unsadr x ) unchecked:
    assert ( x < 4096 )
    adr a = x
    return a
...
In general, if {\tt x} has type `$T1${\tt ->}$T1$ {\tt x}{\tt ->}$T3$'
...
values, as in the following much improved example:
...
Also, conversions of pointer values only affect the pointer part of a type,
so if {\tt x} has type `{\tt int8 ->}~$T$' then {\tt .convert.(x)} has type
`{\tt adr ->}~$T$' for any $T$.
...
location where the real address is stored as `{\tt static}'
lifetime qualifier.
This could all be implemented by the code:

\begin{indpar}[0.2in]\begin{verbatim}
defsubtype ptr unsadr
...
void allocate ( adr V -> ptr -> uns32 )
    // Makes ptr point at uns32 value.
