...
Block data types may be declared by declarations of the form
\begin{center}
...
int function1 ( int * stack * arg1 );
    // The argument is a pointer to a caller allocated location
    // holding a pointer to an int.  The caller need not provide a
    // useful value in this location, and function1 can use
    // `* arg1 = value' to return a pointer to an int value in
    // this location, but cannot use `arg1 = value' to reset the
    // argument.
int function2 ( stack int * stack * arg1 );
    // The argument is a pointer to caller allocated location
    // holding a pointer to a caller allocated location holding
    // an int.  Both pointers must be provided by the caller and
    // cannot be reset by function2.  function2 can use
    // `**arg1 = value' but not `*arg1 = value' or `arg1 = value'.
int function3 ( stack int * * arg1 );
    // Illegal; a pointer not set by the caller cannot point to a
    // pointer that can only be set by the caller.
int * stack * function4 ( int arg1 );
    // The result behaves like an extra argument passed to function4
    // which has the same properties as arg1 in function1.  However
    // the syntax `return value' is illegal in function4 as it would
    // reset the pointer to the stack location.  `* return = value'
    // can be used instead.
int * stack * & function5 ( int arg1 );
    // Same as function4 but `return value' is legal and sets the
    // pointer to int.
stack int * stack * function6 ( int arg1 );
    // The result behaves like an extra argument passed to function6
    // which has the same properties as arg1 in function2.
    // `** return = value' must be used to return an integer.
stack int * * function7 ( int arg1 );
...
qualifier, or vice veras.  An address of a location with one collectable
...
other operations related to garbage collection, such as pushing the
...
int function2 ( stack readonly *& arg1 );
int function3 ( stack *& arg1 );
. . . . . . . . . .
...
\ttkey{noaccess}~~~~~Specifies whether a function execution may read or
write a location.

\hspace*{-1em}\ikey{Protection Qualifiers}%
{protection qualifier}~~~~~Non-builtin protection
qualifiers may be declared by the programmer and used to tag
memory locations that may be accessed only by particular pieces of code.

Note that other qualifiers may also be used to restrict location
access in the same way as declared protection qualifiers.
