...
\emkey{declaration}
    \begin{tabular}[t]{rll}
    ::= & {\em derived-type-declaration}
        & [see \pagref{DERIVED-TYPE-DECLARATION}] \\
    $|$ & {\em tuple-type-declaration }
        & [see \pagref{TUPLE-TYPE-DECLARATION}] \\
    $|$ & {\em pointer-type-declaration }
        & [see \pagref{POINTER-TYPE-DECLARATION}] \\
...
In function and generic calls the
...
type math complex is tuple of float64[2]

function math complex r =
    math ( math complex a1 ) "+" ( math complex a2 ):
    r = math complex ( a1[0] + a2[0], a1[1] + a2[1] )
...
math complex y = complex ( 0, 1 )
    ...
    // so `math' may be omitted.
math complex z = x + y
    // Parses to `math complex z = ( (x) "+" (y) )'.  Could
    // also be `math complex z = math ( x + y )' which would
    // be compiled to `math complex z = ( math (x) "+" (y) )'
\end{verbatim}\end{indpar}
...
names defined by the {\em declaration}
...
The scope of a {\em name}, {\em function-prototype},
or {\em generic-prototype} is the scope of the declaration that defines it.
