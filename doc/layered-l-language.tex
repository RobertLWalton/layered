...
\begin{center}
\begin{tabular}{l}
\TT{function QS-F-result T-F-result F-result =} \\
\TT{~~~~~~~~~F ( QS-F-argument T-F-argument F-argument)} \\
\TT{function QS-G-result T-G-result G-result =} \\
...
that different subexpression/prototype matches introduces different
...
\mkey{more specific}{matching}
than a matching $k$ if and only if \TT{T$_{j,i}$~=>~T$_{k,i}$}
for every $i$.  A matching is \mkey{most specific} if it is
