...
TBD:
If a cluster base \TT{B} with a member \TT{B.M}
is used to create a variable `\TT{next B}',
then `\TT{next B.M}' is \underline{not} a member of `\TT{next B}'.
Instead one must use `\TT{next B.M}' to allocate the corresponding
member of `\TT{next B}'. [Does this work?]
