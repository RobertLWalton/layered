...
{\tt i} names the integer.  `{\tt * ip}' can be used to name the
...
for appropriate stack addresses to be computed, and `{\tt \& i}' and
...
One use of a subtype is to reserve values of the parent type
for special meaning.  For example, an {\tt adr} address value
in the range from 0 through 4095 might be a special code, and not
...
inline specialadr .conversion. ( unsadr a ) unchecked:
    assert ( a < 4096 );
    return a;
...
A second use of a subtype is to make a promise that a value
...
real address of the variable pointed at.  This could be implemented
by the code:

\begin{indpar}\begin{verbatim}
defsubtype ptr ( adr, 4094 );

inline local adr .conversion. ( ptr p ) unchecked:
    adr p2 = p -> adr; return * p2;
inline ptr .conversion. ( adr p -> adr ) unchecked:
    return p;

uns32 x;
stack adr xp = & x -> uns32;
stack ptr p = & xp -> uns32;
    // Implied conversion of stack adr -> stack adr -> uns32
    // to stack ptr -> uns32.
uns32 y = * p;
    // Implied conversion of stack ptr -> uns32 to
    // local adr -> uns32 and then dereference.
\end{verbatim}\end{indpar}
...
and `{\tt constant}' is a location qualifier that says that the
location will never ever be written into.  For example, the
declarations
...
not be stored in a variable whose value is declared to be of
unlimited lifetime.
...
has a lifetime that expires when the
current function execution terminates.
The `{\tt caller}' qualifier
specifies that the value has a lifetime that will \underline{not}
expire until after the end of the function execution of the
caller of the current function.

In addition user lifetime qualifiers may be defined
that expire lifetimes when certain functions are called.  The `{\tt pure}'
qualifier is often defined in this way to specify that a value
has a lifetime that expires when a function that is
\underline{not} `{\tt pure}' is called.  Note that
`{\tt pure}' also becomes both a function access qualifier and
a function body qualifier.
...
Although lifetime qualifiers are not usually applied to numeric values,
it is possible to do so.  For example, if an {\tt int32} value is used
as an open file designator, it may be given a limited lifetime.


The three lifetime qualifiers are all builtin:  `{\tt static}', `{\tt stack}',
and `{\tt caller}'.
Checked code can only allocate to static or stack memory, and addresses
...
\hspace*{-1em}\ttkey{stack}~~~~~Specifies the value expires
when the current function execution terminates.  Equivalent to
{\tt stack(1)}: see the following.

\hspace*{-1em}\ttkey{stack(n)}~~~~~
Specifies the value expires when the current code block of nested level
{\tt n} within the currently executing function terminates.
Unnested code within the function is considered to be at nested level {\tt 1}.
Automatically applied to the addresses of locations allocated to the
current function frame by code in a level {\tt n} block (such locations
are freed when the block of level {\tt n} terminates).

A location with a `{\tt stack(n)}' qualified address can have
a `{\tt stack(m)}' qualified value only if {\tt n}$\geq${\tt m}.

A `{\tt stack(n)}' value can be stored in a location whose value has
a `{\tt stack(m)}' qualifier only if {\tt n}$\leq${\tt m}.



A `{\tt stack(n)}' qualifier \underline{cannot}
be applied to function arguments and return types.
...
Specifies the values lifetime begins during the execution of the caller
of the function and ends after the called function returns.

`{\tt static}' and `{\tt stack(n)}' values are converted to `{\tt caller}'
values when passed as arguments.

The `{\tt caller}' qualifier does not specify precisely when the value
lifetime expires.  Therefore it is not possible to store a `{\tt caller}'
value in a location whose address has the `{\tt caller}' qualifier.
...
int32 function1 ( caller adr arg1 -> static adr -> int32 );
    ...
    // adddress to an int32 value in this location, and can also
    // use `**arg1 = value' to return an int32 value.
...
lifetime qualifier for any argument or return value address value that has no
