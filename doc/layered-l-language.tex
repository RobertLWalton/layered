...
Note that the name of an array us used to name the `{\tt .base}'
of the array, and that unchecked code can use this name to change
the base of the array.  In the above example the `{\tt .void}'
function returns its output parameter `{\tt b}' which because of
the `{\tt *\&}' is a pointer
to a location holding the base of the array.  Thus resetting
`{\tt a.void}' in `{\tt push}' resets the base of the array.
