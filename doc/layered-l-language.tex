...
A name declared inside a block must \underline{not} begin with
a {\em module-abbreviation}.

A declaration not inside a block is said to be `\key{top level}'.
If a top level declaration \underline{begins} with a
{\em module-abbreviation}, the declaration is `\key{external}'.
Other top level declarations are `\key{internal}'.
External declarations are said to be `skey{export}{ed}' to
other modules.
...
module to refer to these names are those associated with the
