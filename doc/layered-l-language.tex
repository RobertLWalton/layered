...
called function starts to execute and last until after that
function returns.
...
`{\tt stack(m)}' values are convertable to
`{\tt stack(n)}' if {\tt n}$\leq${\tt m}.
...
part of the type of a value that is to be passed as an argument or
...
\begin{center}
{\tt declare calling lifetime qualifier}
    \begin{tabular}[t]{@{}l@{}}
    {\em qualifier-name} \\{}%
    [{\tt ,} {\em negated-qualifier-name} [{\tt ,} {\em default}\,] ]
    \end{tabular}
\end{center}
An example is:
\begin{center}
\tt declare calling lifetime qualifier pure, impure
\end{center}

If {\tt Q} is a calling lifetime qualifier and {\tt NQ} is
its negation, then a value with qualifier {\tt Q} will have
a lifetime that extends until a call to an out-of-line function that has
the access qualifier {\tt NQ} or the execution of a code
block that has qualifier {\tt NQ}.

The {\em default} in the declaration
is the default qualifier of an out-of-line function, and if not given
is {\tt NQ}.  In this case where the {\em default} is {\tt NQ},
calls to functions that do not have the {\tt Q} access qualifier
terminate the lifetime.

Inline functions have neither the {\tt Q} or
{\tt NQ} qualifier, and are treated as
if they were copied at the point of their call.  However, 
a code block with qualifier {\tt NQ} can be included inside an inline
function, or {\tt NQ} can be used as the code block qualifier of the
entire inline function body.

Thus in the example, a `{\tt pure}' value would have a lifetime until
an out-of-line function is called that did \underline{not}
have the `{\tt pure}' qualifier is
called, or a code block with the `{\tt impure}' qualifier is executed,
where this code block could be part of an inline function.

If the {\em negated-qualifier-name} is not given, it defaults to
`{\tt \TILDE} {\em qualifier-name}'.

A calling lifetime qualifier is also automatically declared to be
a protection qualifier(\pagref{PROTECTION-QUALIFIER}).
