...
The following builtin functions manipulate error values for any number type $T$:
\begin{indpar}
...
returns {\tt BAD RANGE} if {\tt value} is not
an error value.
\end{indpar}
{\tt function unswrd r = \ttstdkey{MAX ERROR CODE} ( type t == $T$ )}
\begin{indpar}
Returns the maximum allowed error code for type $T$.
This is at least $2^s-1$ where $2s$ is the size in bits of
...
code $c$ not greater than the maximum allowed for $T$:
\begin{center}
\begin{tabular}{l@{~~~~~~~~~~}l}
...
For a floating point number type $T$ whose values are of size
...
The low order $s$ bits are the error code.

The following are builtin error code values for all number types $T$:
\begin{indpar}

{\tt function $T$ r = \ttstdkey{INSUFFICIENT MEMORY}} \hfill [code == 1]
\begin{indpar}
Not enough memory is available.
\end{indpar}
{\tt function $T$ r = \ttstdkey{BAD RANGE}} \hfill [code == 2]
\begin{indpar}
Argument is out of range.
\end{indpar}
{\tt function $T$ r = \ttstdkey{BAD ALIGNMENT}} \hfill [code == 3]
\begin{indpar}
