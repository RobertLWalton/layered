...
The parser begins by parsing expressions according to the syntax:
...
\begin{itemize}
\item[(1)] The sequence begins the expression and consists of
just prefix operators.
\item[(2)] The sequence neither begins nor ends the expression
and the sequence contains a single infix operator optionally
preceeded by just postfix operators and optionally followed by
just prefix operators.
\item[(3)] The sequence ends the expression and consists of
...
After applying these rules, \TT{(}'s are added to the beginning
of the expression and \TT{)}'s to the end as necessary in order
to balance all parentheses.  Also after applying these rules,
...
\begin{center}
\begin{tabular}{lclcl}
\IX{+} > \RX{+} & so & \tt x \IX{+} \RX{+} y \IX{*} z
                & rewrites as
		& \tt x \IX{+} \TT{(} \RX{+} y \IX{*} z \\
\RX{+} < \IX{*} & so & \tt x \IX{+} \TT{(} \RX{+} y \IX{*} z
                & rewrites as
		& \tt x \IX{+} \TT{(} \RX{+} y \TT{)} \IX{*} z \\
\IX{+} > \IX{*} & so 
		& \tt x \IX{+} \TT{(} \RX{+} y \TT{)} \IX{*} z
                & rewrites as
		& \tt x \IX{+} \TT{(} \TT{(} \RX{+} y \TT{)} \IX{*} z \\
\end{tabular}
and adding parentheses at the end to balance we get \\
{\tt x \IX{+} \TT{(} \TT{(} \RX{+} y \TT{)} \IX{*} z \TT{)} }
\end{center}

Notice that the third step above is possible because
{\tt \TT{(} \RX{+} y \TT{)}} became a {\em primary} after
the second step.

As another example, consider
\begin{center}
\tt x \IX{if} \RX{+} y \IX{else} z
\end{center}
we get
\begin{center}
\begin{tabular}{lcl}
\IX{if} >x \RX{+} & so & \tt x \IX{if} \RX{+} y \IX{else} z \\
                & rewrites as
		& \tt x \IX{if} \TT{(}$_x$ \RX{+} y \IX{else} z \\
\RX{+} x< \IX{else} & so & \tt x \IX{+} \TT{(}$_x$ \RX{+} y \IX{else} z \\
                & rewrites as
		& \tt x \IX{if} \TT{(}$_x$ \RX{+} y \TT{)}$_x$ \IX{else} z \\
\IX{if} < \IX{else} & so 
		& \tt x \IX{if} \TT{(}$_x$ \RX{+} y \TT{)}$_x$ \IX{else} z \\
                & rewrites as
		& \tt x \IX{if} \TT{(}$_x$ \RX{+} y \TT{)}$_x$ \TT{)}
		\IX{else} z \\
\end{tabular}
and adding parentheses at the end to balance we get \\
{\tt \TT{(} x \IX{if} \TT{(}$_x$ \RX{+} y \TT{)}$_x$ \TT{)} \IX{else} z } \\
which is a compile error as it contains \IX{if} \TT{(}$_x$
\end{center}

Notice that in the sequence \TT{)}$_x$ \TT{)} \IX{else}
the $_x$ has been hidded from the \IX{else} by a \TT{)} without
the $_x$, so this part of the expression is legal.  The builtin
operator precedence table dictates that the first operand of an \IX{else}
must have \IX{if} as its `topmost' operator or be a {\em primary}
in the initial expression.  This is satisfied by the example.
The builtin operator precedence table also dictates that the
second operand of an \IX{if} must contain a logical or comparison
...
\hline
first & \multicolumn{6}{c|}{second operator} \\
\cline{2-7}
...
\begin{indpar}\begin{verbatim}
define asymetric left associative else operator "else"
define asymetric binary if operator "if"
define right associative and operator "and"
define right associative or operator "or"
define prefix not operator "not"

define else expression as expression
define and expression as expression
define or expression as expression
define not expression as expression
    // Note: if expression is NOT expression

define if expression as else first operand
define expression as else second operand

define expression as if first operand

define and expression as expression
define and expression as if second operand
define or expression as expression
define or expression as if second operand
define not expression as expression
define not expression as if second operand
define not expression as and operand
define not expression as or operand
\end{verbatim}\end{indpar}
...
\emkey{logical-expression}\label{LOGICAL-EXPRESSION} ::=
    {\em and-expression} $|$ {\em or-expression} $|$ {\em not-expression}
\\[0.5ex]
\emkey{and-expression}
    \begin{tabular}[t]{rl}
    ::= & {\em logical-operand} \ttkey{and} {\em logical-operand} \\
    $|$ & {\em and-expression} \ttkey{and} {\em logical-operand} \\
    \end{tabular}
\\[0.5ex]
\emkey{or-expression}
    \begin{tabular}[t]{rl}
    ::= & {\em logical-operand} \ttkey{or} {\em logical-operand} \\
    $|$ & {\em or-expression} \ttkey{or} {\em logical-operand} \\
    \end{tabular}
\\[0.5ex]
