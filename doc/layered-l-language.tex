...
\begin{indpar}
{\tt define protected variable qualifier \ttkey{std *MEM*}} \\
{\tt define abbreviation \ttakey{MEM} ---> `std *MEM*'} \\
{\tt define protected variable qualifier \ttkey{std *WRITABLE*}} \\
{\tt define abbreviation \ttakey{WRITABLE} ---> `std *WRITABLE*'} \\
{\tt define protected addable value qualifier \ttkey{std *STACK*}} \\
...
\mkey{protected}{qualifier}.  These qualifier attributes control
...
the qualifier is \mkey{deletable}{qualifier}.
However, if the qualifier is \mkey{protected}{qualifier}
...
\item[NOTE]
If the assignment is not a result assignment,
then as far as {\tt u} is concerned, the \underline{variable}
qualifiers of {\tt w} are ignored.
\item[NOTE]
If the assignment is a result assignment,
the \underline{variable} qualifiers of {\tt u} must be the same as the
qualifiers of {\tt r}, except that a protected variable qualifier of {\tt w}
need not be explicitly given for {\tt u} but will be implicitly added.
\end{itemize}\end{indpar}
...
*UNCHECKED* uns32 w = uns32 ( z )
    // *UNCHECKED* is an unprotected variable qualifier that
...
Some examples involving the \TT{*STACK*} addable protected value
qualifier are:
\begin{indpar}\begin{verbatim}
// Assume:
//
