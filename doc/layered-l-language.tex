...
Given an enumeration set type \TT{S1} with base type \TT{B1} and
an enumeration set type \TT{S2} with base type \TT{B2}, the following
conversions are defined:
\begin{center}
\begin{tabular}{rl@{~~~~~}l}
{\TT S2} & \TT{(} {\em \TT{S1}-value} \TT{)}
    & if implicit conversion from \TT{B1} to \TT{B2} is defined \\
\TT{*UNCHECKED*} \TT{S2}
	& \TT{(} {\em \TT{S1}-value} \TT{)}
...
\emkey{member-index-list}
    ::= {\em member-index-range} \{ \TT{,} {\em member-index-range} \}\STAR{} \\
\emkey{member-index-range} ::= {\em member-index}
		$|$ {\em member-index}~~\TT{..}~~{\em member-index} \\
\emkey{member-index} ::= \begin{tabular}[t]{l}
                         {\em integer-expression} \\{}
	                 [Must be compile-time when variable allocated.]
			 \end{tabular}
\end{indpar}
...
{\em assignment-statements} in which {\em result-variable-declarations}
...
{\em qualifying-expression}.  If the variable is omitted, this
