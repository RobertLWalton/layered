...
\hspace*{-1em}\ttkey{unique}~~~~~The memory location does not overlap
any other memory location passed to a function as an argument,
return value location, or global location.
...
function and the location is not readonly to the function.

However, if a `{\tt unique}' memory location is addressed via global
memory, it may not be passed as a function argument or return result,
as it will not be unique to the function into which it is passed,
since it can be found through either the argument or return result or
