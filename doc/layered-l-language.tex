...
`\key{implicit assignment}'\label{IMPLICIT-ASSIGNMENT} if possible, or
...
`\key{explicit assignment}'.  Implicit assignment
can only be used if \skey{implicit assignment rule}s are
satisfied, and one of these is
\begin{indpar}\begin{itemize}
\item[(IA1)]\label{IMPLICIT-ASSIGNMENT-TYPE-RULE}
In `{\tt u = w}' the types of {\tt u} and {\tt w}
must be identical.
\end{itemize}\end{indpar}
When implicit assignment is used, the value of {\tt w} is copied
exactly to {\tt u} (or if there is no value because the common
type is derived from \TT{void}, nothing is done).
The other implicit assignment rules involve qualifiers
(\pagref{IMPLICIT-ASSIGNMENT-QUALIFIER-RULES}) and cluster
members (\pagref{IMPLICIT-ASSIGNMENT-OF-CLUSTERS}).

Explicit assignment is very often referred to as
`\key{implied conversion}'.\label{IMPLIED-CONVERSION}
...
Implicit {\tt assign}'s are not cascaded.  More specifically, in
\begin{center}
\tt $Tu$ u = assign ( w )
\end{center}
if {\tt assign} is explcitly written
and its result cannot be implicitly
assigned to {\tt u}, then an implicit {\tt assign} would be added
making the statement
\begin{center}
\tt $Tu$ u = assign ( assign ( w ) )
\end{center}
But this is extra {\tt assign} is \underline{not} added
if the first {\tt assign} is implicit.
...
function \begin{tabular}[t]{@{}l@{}}
	 ...
	 \end{tabular}
\end{center}
(Here the names {\tt Qs}, {\tt r}, and {\tt v} may be different
