...
within the {\em function-prototype}.  The value of the
...
containing the {\em type-argument-declaration}.  It also allows the
...
natural types (\pagref{NATURAL-TYPE}), {\tt T} is assigned the type of
the leftmost of these actual values.
...
which has the form `{\tt qualifiers Qs}' for some {\em variable-name} {\tt Qs}
in an argument list with `\TT{(|~|)}' brackets.
...
natural types (\pagref{NATURAL-TYPE}), {\tt Qs} is assigned the qualifiers
of the leftmost of these actual values.
...
As a special case, if an {\em argument-declaration} ends in
...
{\em variable-name} in the {\em qualifiers-argument-declaration}
is implied.
