...
must be identical, and this type
must not have \TT{void} as its underlying type.
...
It is a compile error for an operation that does not simply copy the
...
\begin{indpar}
{\tt define implied variable qualifier \ttstdkey{*READ*}} \\
{\tt define abbreviation \ttakey{READ} ---> `std *READ*'} \\
{\tt define implied variable qualifier \ttstdkey{*WRITE*}} \\
{\tt define abbreviation \ttakey{WRITE} ---> `std *WRITE*'} \\
{\tt define implied addable value qualifier \ttstdkey{*STACK*}} \\
{\tt define abbreviation \ttakey{STACK} ---> `std *STACK*'} \\
{\tt define variable qualifier \ttstdkey{*UNCHECKED*}} \\
...
assignments, and direct assignments to a result variable); and
(3) result deferred assignments permit value qualifiers
to be added or deleted without restriction.
...
\item[(DA1)]
The types of {\tt u} and {\tt w} are identical and this type
does not have \TT{void} as its underlying type
...
is implied.
\item[NOTE]
Unchecked result deferred assignments ignore qualifiers and are always direct
assignments if (DA1) is satisfied.
\item[NOTE]
If a result variable has \TT{void} as its underlying type, or if it
has the \TT{*READ*} qualifier (see below), it is never
...
to be operators for the purposes of rewritting.  Thus the code
line
\begin{center}
...
\ttamkey{DEFFERED}{call-prototype matching} prototype
...
provide the default
memory regions defined above: one for static data, one for
dynamic data, and one for stacks (which may or may not be the same as the one
for dynamic data).  These default regions are used if no other regions
are created.
...
The following function returns the static memory address
of a large block of memory that will cause a memory fault if accessed:

\begin{indpar}

{\tt function adr r = \ttstdkey{NULL}}\label{NULL}
...
This function returns \TT{NULL} if there is insuffient memory
in the region to allocate the desired memory.  In this case
{\tt s} is set to the maximum {\tt size} argument value that
...
{\tt function unsadr r = \begin{tabular}[t]{@{}l@{}}
...
\underline{must} be set to \TT{NULL}.
If there is insufficient memory in the region to allocated the desired block,
this function does nothing, leaving the body pointer value as \TT{NULL}.
...
Returns {\tt size} on success.  On failure does nothing, but returns
the maximum value of {\tt size} that would be successful (which will
be less than the {\tt size} argument).
...
The number of bytes allocates is returned in {\tt s}.  This equals
the {\tt size} argument on success.  On failure, does nothing except
possibly move dynamic blocks,
and {\tt s} is set equal to the maximum
{\tt size} that would have succeeded.
...
{\tt function body pointer address r =
    \ttstdkey{STATIC BASE}}\label{STATIC BASE}
...
Sets {\tt s} to the value of the {\tt size} argument on success.
On failure does nothing except possibly move dynamic blocks, and returns
in {\tt s} the maximum value of {\tt size} that would be successful
(which will be less than the {\tt size} argument), and returns
\TT{NULL} as the value of {\tt r.base.adr}.
...
being alocated to the current stack.  The `{\tt allocate to global}'
...
In order accessed a global area it must be named in an
{\em extern-statement}:
\begin{indpar}
\emkey{extern-statment} ::= \\
\hspace*{3em}\begin{tabular}[t]{@{}l}
...
The \TT{*COMPILE-TIME*} option indiates both compile-time and run-time.
The \TT{*COMPILE-\EOL TIME-\EOL ONLY*} option indicates compile-time only.
...
{\em global-area-blocks} do not have {\em global-area-options}.
...
\\[1ex]
function adr body pointer = \ttstdkey{dynamic block of} \\
\TS~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~( global area g )
}
...
\begin{indpar}
Add a {\tt size} byte block of to the dynamic block whose body pointer
is located at address {\tt A}.  The offset of the block is returned
and is a multiple of the the given {\tt align\-ment} in bytes.
If necessary padding is added to the dynamic block before the
newly allocated memory to ensure the offset is a multiple of the
{\tt alignment}.  If necessary the dynamic block is moved so that
its body pointer is a multiple of the alignment.
...
\footnote{On modern INTEL X86\_64 processors
...
This buitlin function executes
`{\tt reload *READ*, *READ* dependent}'.
\end{indpar}
...
to system memory \underline{before} the value read by
this function was written to system memory.%
\footnote{On modern INTEL X86\_64 processors
...
All writes executed \underline{after} this builtin function will write
to system memory \underline{after} this function writes its {\tt value}
to system memory.%
\footnote{On modern INTEL X86\_64 processors
...
All writes executed \underline{before} this builtin function will write
to system memory \underline{before} this function writes its {\tt value}
to system memory.%
\footnote{On modern INTEL X86\_64 processors
...
*READABLE* pointer post-version = ...
  // pointer with 2 aligned uns32 elements
...
//
uns32 next-version = (* pre-version) + 1
...
error is an address that is used to write will cause a memory fault.
...
checks that the {\tt pre-version} has not changed since the update
started (if it had, that would mean some other writer updated the
...
Lastly the write instructions are executed as before.

Any locks acquired are released when the {\em atomic-block} terminates
in the reverse order to that in which they were acquired.

Now there are possible failures other than an operation error:
a lock may not be acquireable or a compare may fail.  If this happens
...
However, they are not mean for updates that have a large number of
...
\begin{indpar}
\emkey{atomic-statement} ::= {\em atomic-read-statement}
			 $|$ {\em atomic-write-statement}
			 $|$ {\em atomic-block}
\\[0.5ex]
\emkey{atomic-read-statement} ::= {\em atomic-pre-read-statement}
                              $|$ {\em atomic-post-read-statement} \\
\emkey{atomic-pre-read-statement} ::= \\
\hspace*{0.5in}{\tt uns32 r =
    \ttalkey{ATOMIC}{PRE-READ}} {\em atomic-pointer} \\
\emkey{atomic-post-write-statement} ::= \\
\hspace*{0.5in}{\tt uns32 r =
    \ttalkey{ATOMIC}{POST-READ}} {\em atomic-pointer}
\\[0.5ex]
\emkey{atomic-write-statement} ::= {\em atomic-pre-write-statement}
                              $|$ {\em atomic-post-write-statement} \\
\emkey{atomic-pre-write-statement} ::= \\
\hspace*{0.5in}{\tt uns32 r =
...
\emkey{atomic-write-instruction} ::= {\em atomic-write-statement} \\
...
For {\em compare-instructions}, {\em atomic-write-instructions}, and
{\em atomic-read-statements} this type must be \TT{uns32}, and the
address in the instruction's {\em atomic-pointer} must be aligned on
a 4-byte bounary.
...
define \begin{tabular}[t]{@{}l@{}}
       ...
       \end{tabular} \\
atomic code \ttstdkey{succeed atomic code } = next enum // 0 \\
function std atomic code r = std \ttmkey{succeed}{atomic code}: \\
\hspace*{3em}r = std succeed atomic code \\
...
the lock, other processes in the group cannot.

In pass 1 the \emkey{lock-instruction} reads the lock and succeeds if the lock
was not already set, and otherwise fails.  If the lock is not already set,
it is \underline{not} set or acquired in pass 1.

In pass 2 the \emkey{lock-instruction} sets the lock and succeeds if the lock
...
A {\em lock-instruction} that does not fail in pass 1 may still fail in
pass 2 if another process acquires the lock between when the instruction
executed in pass 1 and the start of pass 2.
...
If the {\em lock-ID} is not valid, the {\em lock-instruction}
has an operation error.

\hspace*{-0.2in}{\em \bf Compare-Instruction}.
In both passes 1 and 2, the \emkey{compare-instruction} reads the \TT{uns32}
value at the {\em atomic-pointer} designated
address and compares it to the {\em atomic-value}.  The instruction
succeeds if the values are equal, and fails otherwise.

During both passes 1 and 2 the \TT{uns32} value read from memory is
read atomically, meaning that
the value read will exactly equal the last value atomically written to system
(non-cache) memory by \underline{any} processor.
The {\em atomic-write-instruction} writes \TT{uns32} values atomically directly
to system memory, bypassing caches.

In pass 3 {\em compare-instructions} do nothing.

The implementation assumes that if the locks before a {\em compare-instruction}
are acquired in pass 2, the locations to be
read by the {\em compare-instruction} cannot be changed by any other
processor.  The program code must enforce this assumption (and is in
this sense unchecked).

It is an operation error if the address pointed at is not readable.
If this happens in pass 1 it is a permanent error.  If it happens
in pass 2 it might be because a page to be read was paged out
between when the instruction executed in pass 1 and the start of pass 2.
Re-executing the {\em atomic-block} will page the offending
page back in during pass 1.

It is a programming error if the pointer does not point at a vector
with at least one element or if the pointer does not have the
{\tt *READABLE*} qualifier.


\hspace*{-0.2in}{\em \bf Write-Instructions}.
During passes 1 and 2, a \emkey{write-instruction} performs a
\key{write-check} of the locations to be written.  This consists
conceptually of reading the values at these locations and writing
the values read back into the locations.

In pass 1 compare-and-set instructions are used.  In pass 2, the
locations are simply read and written using normal instructions.

The implementation assumes that if the locks before a {\em write-instruction}
are acquired in pass 2, the locations to be
written by the {\em write-instruction} cannot be changed by any other
processor.  The program code must enforce this assumption (and is in
this sense unchecked).

During pass 3, the \emkey{write-instruction} writes the {\em atomic-value} to
the {\em atomic-pointer} designated address.  The {\em atomic-write-instruction}
writes the value atomically directly to system memory, bypassing caches.
Furthermore any data written in the {\em atomic-block}
before an {\em atomic-write-instruction} will be written into system memory
before the value written by the {\em atomic-write-instruction} is written.

It is an operation error if the address pointed at is not both readable
and writable.
If this happens in pass 1 it is a permanent error.  If it happens
in pass 2 it might be because a page to be written was paged out
between when the instruction executed in pass 1 and the start of pass 2.
Re-executing the {\em atomic-block} will page the offending
page back in during pass 1.
...
It is a programming error if the pointer does not point at a vector
with at least one element or if the pointer does not have the
{\tt *WRITABLE*} qualifier (it need not have the {\tt *READABLE*}
qualifier though the page with the element must be readable).
\end{indpar}

The returned {\tt index} value returned by the {\em atomic-block}
equals \TT{0} if the statement succeeds, or is the number of the
first failint or operation error detecting
instruction in the statement block.  I.e., if the first instruction
fails, {\tt index} is set to \TT{1}.

The returned {\tt atomic code} indicates whether the {\em atomic-block}
succeeded, failed, or had an operation error.

Programming errors are all detected during pass 1.
...
is a programming error.  At least 32 {\em atomic-instructions} will be
