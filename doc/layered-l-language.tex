...
\begin{center}
\begin{tabular}{l}
(A) the blocks are all top level in the same file \\
or \\
(B) the blocks are not top level but have the same
	        smallest containing subblock
\end{tabular}
\end{center}

As far as allocating variables to the global stack is concerned,
several blocks with the same {\em global-area-name}
behave as if they were concatenated (but note
