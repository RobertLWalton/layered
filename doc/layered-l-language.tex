...
{\tt function adr r = \begin{tabular}[t]{@{}l@{}}
		      \ttstdkey{allocate to static} \\
		      ~~~~( unsadr size, \\
		      ~~~~~~unswrd alignment ?=~0, \\
		      ~~~~~~memory region r ?= \\
		      ~~~~~~~~~~std default static region ) \\
		      \end{tabular}}
...
of a compile-time \TT{string} value can also be allocated by
a pointer version of `\TT{allocate to static}': see
...
Body pointers have type:
\begin{indpar}\tt
define type \ttstdkey{body pointer}~as adr \\
define abbreviation \ttkey{body pointer} \ABV{} `std body pointer' \\
function body pointer r = std default value: \\
\TS~~~~r = NULL
\end{indpar}
...
{\tt function unsadr r = \begin{tabular}[t]{@{}l@{}}
                         \TT{std *UNCHECKED* allocate to dynamic}%
	                 \index{allocate@{\tt *UNCHECKED* allocate}!%
	                        to dynamic@{\tt to dynamic}} \\
			 ~~~~( adr body pointer address, \\
			 ~~~~~~unsadr size, \\
			 ~~~~~~unswrd alignment ?=~0, \\
			 ~~~~~~memory region R ?= \\
...
Returns \TT{size} on success.  On failure does nothing, but returns
the maximum value of {\tt size} that would be successful.
\end{indpar}
...
Note that the body pointer argument given locates the dynamic
block, and the back pointer within that block locates the
body pointer itself.
\end{indpar}
