...
the cluster is the \mkey{base variable} of the cluster, and the
other variables have names that are the name of the base variable
followed by `{\tt .}' followed by another name.  Thus a cluster
is like a structure, but it is a set of variables and not a
piece of memory.  The variables are each allocated separately.
...
The last used element of the array, namely
\begin{center}
{\tt (} $Q$ $T$ {\tt *\&)}
...
last array element).
`\verb|& |$A$\verb|.last|' is {\tt NULL} if there are no
accessible array elements.
...
float64 (readonly *&b) [1 .. a.n] .void ( *& myarray a )
    unchecked = a@24;
...
element of an object, {\tt a.void} is its {\tt .base} pointer, and just enough
...
uses {\tt myarray} is intended to be written by the end user.
...
Also, what about scaling, offsetting, and clipping array values.  I.e.,
\begin{center}
\tt x[i,j] = max ( x.lower, min ( x.upper, y[i,j] * x.scale + x.offset ) )
\end{center}
]
...
Qualifiers may also be applied to code blocks.  A qualifier after the
...
a function.
\ikey{Collectible qualifiers}{collectible qualifier} may be declared that
indicate that the location is garbage collectible and its address needs
to be protected against collection.
Generally a code block
may store the address of a location in the code block's
stack frame as long as the code block knows the location will exist until
the end of the code block's execution.  Addresses of static locations can
...
might contain an encoded value, such as a pointer to a collectible location.
For example, a value possibly containing an encoded pointer to
the collectible location will be restricted as if it were a pointer to
a collectible location.
...
a qualifier may be containing and traceable.

Qualifiers may also be inherited.  For example, in
\begin{indpar}\begin{verbatim}
readonly unsptr *& .n ( *& myarray @ a ) unchecked = a@0;
\end{verbatim}\end{indpar}
...
and `{\tt caller}'.
One kind of lifetime qualifier is declarable: collectible qualifiers.
