...
is can be used to denote the qualifiers of any variable
after it in the {\em argument-variable-list}.
For example:
\begin{indpar}\begin{verbatim}
( Qr pointer result ) =
...
A `\TT{qualifiers}' variable takes as its value a set of
qualifiers.
...
of this are discarded (unless they are part of a cluster whose root
...
\begin{center}
{\em qualifier-name}\STAR{} {\em type-name} \TT{(} {\em inner-call} \TT{)}
\end{center}

This specifies the type and some of the qualifiers of the inner call
temporary result.  This temporary will also be given any protected
...
\item The generic call has no results.
...
\item There is an extra `\TT{include}' block consisting of
      declaration statements that replaces the generic call
      in the code that contains it.  These declarations may
      contain the names of call arguments and computed block variables,
      including abbreviations, and these are replaced by their values
      in the declaration statements that replace the generic call.
\end{enumerate}
