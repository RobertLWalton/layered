...
Memory is organized into two parts: static and dynamic.
Static memory variables are allocated at fixed addresses
that never change.  Dynamic memory is allocated to blocks that
can be moved.  Static memory contains mostly pointers to dynamic
blocks, with these pointers being updated when the blocks they
point at are moved.

Static addresses consist of a single \TT{adr} value.

Dynamic addresses are pairs, each consisting of a static
base location $L$ of type \TT{adr}, and an offset $O$ of type \TT{intadr}.
$L$ is the static memory address of a \TT{adr} value which is the
base address $B$ of the dynamic block containing the dynamic location
being addressed.  $O$ is the offset of the location in that block.
Thus the location address is $B+O$.  If the block is moved, $B$ changes,
but $L$ and $O$ do not change.
