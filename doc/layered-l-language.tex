...
later time.  It is also sometimes important that a value read from
system memory be the same as a value of the same size
stored at the same location by some other processor.
...
\begin{indpar}
Read from system memory and return the value of the vector's only element.
...
\begin{indpar}
Write {\tt value} to the vector's only element in system memory.
The vector length must be 1 (p.end must equal p.begin + size of uns32).
The type of this element must be \TT{uns32}, and the address of the element
must a multiple of the element size in bytes.
