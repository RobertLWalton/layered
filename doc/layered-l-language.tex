...
{\em member-selectors} and a default value (in which case $w$ is
created by the call).
...
A prototype result may be of the form `\TT{next} $v.m$' where $v$ is
...
A cluster member may be used as an actual argument or result if
the corresponding prototype argument or result is
\underline{not} a cluster member.
