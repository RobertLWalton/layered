...
{\tt function unswrd r = std \ttmkey{length of}{enumeration list} ($L$ v)}
\begin{indpar}
Returns the length of the list {\tt v} (number of elements in {\tt v}).
\end{indpar}

{\tt function $E$ r = std ($L$ v) \TT{[} unswrd i \TT{]}}%
\ttmindex{[]}{of enumeration list}
\begin{indpar}
Returns the {\tt i+1}'st list element of {\tt v}.
It is an error if {\tt i >= length of (v)}.
\end{indpar}
...
assumed to produce the same result.  For example:
\begin{indpar}\begin{verbatim}
int16 v1 = ...
int32 v2 = v1
float64 v3 = v2
...
// and v1 ---> v4, it is assumed that v3 == v4.
\end{verbatim}\end{indpar}
...
// (not type safe).
//
function mem int32 r, *UNCHECKED* adr r.adr = read ( adr A ):
    r.adr = A
mem int32 x = read (1000)
    // Creates variable *UNCHECKED* adr x.adr = 1000.
    // The mem variable qualifier is special: x is not given
    // a value by assignment, but automatically gets its
    // value by reading location x.adr.
int32 y = read(2000)
