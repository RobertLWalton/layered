...
The most basic descriptor includes the length, alignment, offset, and
...
can be used for open files, all exception catch data, etc.

\hspace*{-1em}\ttkey{traceable value}~~~~~If a value of a
`{\tt traceable location}' type is stored in a stack or static
...
Traceable value types can be used for data of interest to a garbage
collector.

\end{indpar}

Note that the traceable type flags have no effect when a type
is used for a location that is not stack or static.
...
\subsection{Containers and Aliasing}
\label{CONTAINERS-AND-ALIASING}
...
When two pointers point at the same data, they are said to
`\mkey{aliased}{pointers}'.

The L language uses the concept of container types and type non-overlap
...
Type overlap is represented by two relation. The {\tt T1 <@ T2} relation
...
to the {\tt <@} relation.  The other way of adding to the {\tt <2} and
{\tt \TILDE@} relations are with statement of the form:
\begin{center}
{\tt declare overlap}
{\em type-name}
\{ {\tt @} $|$ {\tt <@} $|$ {\tt <@>} $|$ {\tt \TILDE@} \}
...
where in this statement form {\tt @} is taken to mean {\tt <@} and
