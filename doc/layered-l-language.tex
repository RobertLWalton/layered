...
If {\tt Q} is a calling lifetime qualifier, {\tt CQ} is
...
block that has qualifier {\tt DQ}.  If {\tt DQ} is also the default
qualifier, it is implicitly applied to any out-of-line function that does not
have qualifier {\tt CQ}.

Inline functions have neither the {\tt CQ} or
{\tt DQ} qualifier, and are treated as
if they were copied at the point of their call.  However, 
a code block with qualifier {\tt DQ} can be included inside an inline
function, or {\tt DQ} can be used as the code block qualifier of the
...
have the `{\tt nonrelocating}' qualifier is
...
the value.  Once stored in a single implicit traceable location,
a traceable value can be copied to many other untraceable locations
