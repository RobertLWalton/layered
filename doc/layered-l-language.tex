...
if $L\geq 44$ and $S=0$.\footnote{The I86 64-bit architecture only
uses the low order 48 bits of an address, but \underline{requires}
...
defsubtype specialadr adr;
    // A specialadr is either an adr or an int in the
    // range [0,4096).

inline bool is_int ( specialadr a ) unchecked:
    unsadr x = a;
    return x < 4096;

inline bool is_adr ( specialadr a ) unchecked:
    unsadr x = a;
    return x >= 4096;
...
inline int .conversion. ( specialadr a ) unchecked:
    unsadr x = a;
    assert ( x < 4096 );
    return x;

inline specialadr .conversion. ( int x ) unchecked:
    unsadr x = a;
    assert ( 0 <= x && x < 4096 );
    return a;
