...
The parameters of an array whose element type is $T$ and whose
...
\item[\ttdkey{length}] Type: `{\tt readonly uns64}'.
...
\item[\ttdkey{step}{$[i]$}]
Type: any `{\tt readonly}' integer type.
The multiplier
...
Second, an array is essentually a pointer.  Viewed as the
element of an object, {\tt a.void} is just a pointer (in
our case we are assuming pointers are 8 bytes long and
...
and `{\tt .void}' can always be omitted, as long as the
...
The last subexpression means that the value returned is the base
...
might contains a encoded value, such as a pointer to a collectable location.
...
qualifier may be found code that uses builtin L-language primitives.
For example, if values of a particular type may hold pointers
to garbage collectable locations, making these values
traceable permits all these value allocated to the stack to
...
frame, or in a `{\tt caller}' qualified
argument to another function called by the function
execution.  Furthermore, if used in a function argument or return value type,
`{\tt caller}'
...
or `{\tt static}'
qualifiers, or vice versa.  An address of a location with one collectable
qualifier may \underline{not} be converted to an address of a location with a
different collectable qualifier.
...
\begin{enumerate}
\item
`{\tt constant}' is removed from a location passed to the {\tt .constructor.}
function as the result location.
\item
...
It differs from the `{\tt caller}' lifetime qualifier in that it
promises that the address of the stack location cannot be passed
to a called function except as an argument or as a result location
of the called function, whereas the address of a arbitrary `{\tt caller}'
location might be placed in static memory accessible to any called
function.
...
\hspace*{-1em}\ttkey{unchecked}~~~~~Specifies that an execution may
