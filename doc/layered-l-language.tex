...
returns {\tt unswrd(ERROR(BAD ARGUMENT RANGE))} if {\tt value} is not
an error value.
\end{indpar}
...
is the same and this value designates the error value as a non-signalling NaN.
...
\end{indpar}
{\tt function unswrd r = \ttstdkey{BAD ARGUMENT RANGE}} ~~~ (== 1)
\begin{indpar}
...
different memory allocation/deallocation features.  This document
assumes that stacks may be allocated as dynamic blocks, but an implementation
may choose to implement stacks in static memory and use
function prototypes different from those described in this document
for allocating memory to a stack.
...
This function returns \TT{ERROR(NOMEM)} if there is insuffient memory
...
does not meet these criteria, \TT{ERROR(BADALIGN)} is returned
and {\tt size} is set to the largest legal alignment for the region.

An memory allocated is initialized to zero.
\end{indpar}
