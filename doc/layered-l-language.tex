...
by other code.  The other difference is that the code executed by
a global block to set the block variables can be split among
many files and many locations with each file.

A global area is a free stack with its own dynamic block
(see \itemref{STACKS}).
...
as long as no {\em variable-name} is allocated in more than one of these blocks.
