...
A \ikey{block}{of RAM} of RAM is a sequence of consecutive increasing addresses.
The \ikey{bit address}{of block} of the block is the address of its first bit,
and the \ikey{bit length}{of block} of the block is the number of bits.
An \ikey{alignment}{of block} of the block is any
...
The \ikey{byte address}{of block} of a byte aligned block is the
bit address of the block divided by 8.  The
\ikey{byte length} of such a block is its bit length divided by 8.

A block is \ikey{allocated}{block}
if the addresses of all its bits are allocated,
and is \ikey{deallocated}{block} if the addresses of all of its bits
are deallocated.
A block is \ikey{unshared}{block} if all its bits are unshared.
A block is \ikey{shared}{block} if
...
A \key{register} can hold any of the following:
...
indirect channel & block-size window-size dimensions \\
\end{tabular}
\end{center}
...
and the dimensions parameter the number of dimensions in the window array.
...
address is the byte address of a memory block that holds a light weight
address.  This light weight address is called a reference pointer.
When a middle weight address is loaded into a virtual register,
...
data structure called a memory channel.
These complex structures permit pipelined
assess to memory, and are described in their own section
(\secref{MEMORY-CHANNELS}).
