...
and the scope of this declaration includes any block that may
consitute the {\em assignment-right-side}.
Result variables are not allowed to hide (\pagref{HIDE})
other {\em name} declarations except as specified in \itemref{SCOPE}.
...
The {\em result-variables} may be completely omitted in most some
...
If the {\em result-variable-list} is omitted, the
{\em assignment-right-side} must be a {\em function-call} that
matches a {\em function-prototype} with no result variables.
