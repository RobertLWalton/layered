...
\begin{indpar}[0.6in]
\hspace*{-0.3in}{\tt unsadr \ttdmkey{begin}{of pointer}} \\
The offset of the first element relative to the {\tt base}.

\hspace*{-0.3in}{\tt unsadr \ttdmkey{end}{of pointer}} \\
...
~~~~\ttstdkey{assign} \\
~~~~~~~~(| qualifiers Qs |) \\
~~~~~~~~( Qs pointer p, unsadr p.begin, unsadr p.end, \\
...
~~~~\ttstdkey{read only} \\
~~~~~~~~(| qualifiers Qps |) \\
~~~~~~~~( Qps pointer p, unsadr p.begin, unsadr p.end, \\
~~~~~~~~~~adr p.base.adr, type p.type, qualifiers p.qualifiers ) \\
~~~~~~~~[| Qrs = Qps - \{ *WRITABLE* \} |] \\
\end{tabular}}\label{POINTER-ASSIGN}

\begin{indpar}
Copies a pointer as per an assignment statement whic removing the
...
\begin{indpar}\begin{verbatim}
define type std pointer as void
define abbreviation pointer ---> `std pointer'

    // if p is a pointer then:
    //   unsadr p.begin is the offset of the first element
    //   unsadr p.end is the offset just AFTER the
    //                   last element
    //   *READ* adr p.base is the base address that
    //                      is added to the offsets
    //   adr p.base.adr is the address of pp.base
    //   type p.type is type of elements pointed at
    //   qualifiers p.qualifiers are the qualifiers
    //              of elements pointed at
    //
    // A pointer is *READABLE* if it can be used to read
    // elements and *WRITABLE* if it can be used to write
    // elements.
\end{verbatim}\end{indpar}

Pointers may be copied
