...
either shared or unshared.  The only way for an abitrary RAM block to
...
Usually they are garbage collectable.

All the heap blocks that are garbage collectable by the
...
A qualifier immediately preceeding a \verb|{ }| bracketted code block
...
cannonical by distributing \verb/|/ over \verb|>>| as in
...
Intuitively a cannonical {\em type-overlap-expression} $e_1$\verb/|/$e_2$
describes a data block that can be described by either of the
{\em type-inclusion-sequences} $e_1$ or $e_2$.  Formally,
two cannonical {\em type-overlap-expressions} overlap if only
