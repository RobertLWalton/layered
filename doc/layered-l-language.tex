...
\newcommand{\HSP}[1]{\hspace*{0in}\phantom{{\tt #1}}}
...
function \ldots~Qs~$T2$ r =
		      \begin{tabular}[t]{@{}l@{}}
                      ...
                      \HSP{~~~~}(| qualifiers Qs |) ( Qs~$T1$ v )
                      \end{tabular}
\end{center}
is defined, where the `\ldots{}' denote arbitrary variable qualifiers
(see \itemref{QUALIFIERS}) and {\tt Qs} is an implicit argument that
copies the value qualifiers of the assignment result variable to the function
...
There can be several {\tt implied conversion} functions from $T1$
...
function Qs~uns32 r =
		      \begin{tabular}[t]{@{}l@{}}
                      ...
                      \HSP{~~~~}(| qualifiers Qs |) ( Qs~character c )
                      \end{tabular}
\begin{indpar}
...
generic call.  Note that variables qualifiers cannot be attached
...
may also be propagated by assignments or implicit conversions operations.

An assignment statement of the form
\begin{center}
\tt $T$ v = w
\end{center}
where {\tt w} is of type $T$ behaves as if it had the form
\begin{center}
\tt $T$ v = assign ( w )
\end{center}
where {\tt assign} has the function prototype
\begin{center} \tt
function ~Qs~$T$ r = assign (| qualifiers Qs |) ( Qs~$T$ v )
\end{center}
Implicit conversions have a similar prototype of the form
\begin{center} \tt
function \ldots~Qs~$T2$ r =
		      \begin{tabular}[t]{@{}l@{}}
                      \ttkey{implied conversion} \\
...
As a consequence, assignment statements and implicit conversions
may remove required value qualifiers but not add them, and
may add prohibited value qualifiers
but not remove them.  Assignment statements and implicit conversions
...
To compile a {\em function-call} or {\em generic-call} it must be matched to a
...
to the {\em function-pattern} in the prototype,
...
\item
The {\em function-term-names} in the call are located by
...
are reordered so the {\em function-term-names} in the call,
...
these must be implied, and they are added to the call
...
\item
The modified call now matches the {\em function-pattern}
...
the call statement.  The statement is then
replaced by:
\begin{indpar}
