...
type by the
\underline{unchecked} (\itemref{UNCHECKED-EXPRESSIONS-AND-BLOCKS})
operations:
\begin{center}
\begin{tabular}{rcl}
...
\section{Unchecked Expressions and Blocks}
\label{UNCHECKED-EXPRESSIONS-AND-BLOCKS}

Unchecked operations, such as certain type conversions mentioned above,
can only be executed in an
\mkey{unchecked}{expression}\index{unchecked!code block}
expression or code block.

An unchecked expression has the form

\begin{indpar}
\emkey{unchecked-expression}
    ::= \TT{*UNCHECKED*} \TT{(} {\em expression} \TT{)}
\end{indpar}

as if \TT{*UNCHECKED*} were a function.  The meaning
is that the argument {\em expression} of this \TT{*UNCHECKED*} pseudo-function
is unchecked, i.e., can execute unchecked operations.

An unchecked code block that does not assign any variables outside
of itself has the form:
...
Other code blocks, which are describe below, can have the
`\TT{*UNCHECKED*}' block qualifier.  In an unchecked code block
all the {\em executable-statements} are unchecked.
...
\emkey{value-assignment-statement}
    ::= \{ {\em variable-list} \TT{=} \}\QMARK{} \TT{*UNCHECKED*}\QMARK{}
        {\em expression}
\\[0.5ex]
\emkey{variable-list}
    \begin{tabular}[t]{@{}rl}
...
where \TT{block} may be omitted as long as either \TT{=} or
\TT{*UNCHECKED*} is not omitted.
...
executes after another subblock executes if and only if
...
of if the \TT{*NO-MATCH*} option is given, there is no matching of
...
top to bottom, left to right.  However, in this case the
...
Stacks of function block executions are free, which is implemented by
not allowing the variables in the {\em variable-list}
of a {\em call-assignment-statement} to have the `\TT{stack}' qualifier.

Stacks of loop iterations are free, which is implemented by not
allowing the variables in the {\em iteration-variable-list} 
f a {\em loop-assignment-statement} to have the `\TT{stack}'
qualifier.
...
The \TT{*THROW*} statement cause its containing block to
...
If an \TT{*UNWIND*} block that is abnormally terminating
executes a \TT{*CATCH*} statement, the termination of the block
is changed from abnormal to normal.  This means, in particular,
that the block's containing block will resume when the now
normally terminating block finishes terminating.
