...
\begin{indpar}
{\em name} \begin{tabular}[t]{@{}rl}
    ::=  & {\em base-name} {\em member-selector}\STAR{} \\
    $|$  & {\em member-selector}\PLUS{} \\
    \end{tabular} \\[1ex]
{\em name-component} ::= {\em base-name} $|$ {\em member-selector} \\
{\em base-name} ::= {\em simple-name} not containing any `\TT{.}'s \\
{\em member-name} ::=
    {\em simple-name} containing just one `\TT{.}' at its beginning \\[1ex]
{\em member-selector} ::= {\em member-name} {\em member-subscript}\STAR{} \\
{\em member-subscript} ::= \TT{[}{\em member-index-list}\TT{]} \\
{\em member-index-list} \begin{tabular}[t]{@{}rl}
    ::= & {\em member-index} \\
    $|$ & {\em member-index} \TT{,} {\em member-index-list}
    \end{tabular} \\
{\em member-index} ::= {\em integer-constant-expression} \\[1ex]
{\em simple-name} ::= {\em simple-name-item}\PLUS{} \\
{\em simple-name-item} \begin{tabular}[t]{@{}rl}
...
at the end of the {\em name} or just before a {\em member-selector}
in the name (i.e., the part must be a contiguous sequence of the
{\em name-components} in the {\em name}).
If there is more than one contiguous part
that might be replaced, one beginning first, and among these
...
name nnj = `nj'
abbreviation ni
int32 p = ni  // same as p = i
int32 q = nj  // Illegal type conversion: same as q = `j'.
abbreviation nj
int32 r = nj  // same as r = j (now nj => j)
int32 s = nnj // same as s = j (nnj => nj => j)
\end{verbatim}\end{indpar}
...
names are in scope, where a variable name is in scope from the point
where the variable is assigned a value until the end of the block
containing the variable assignment.  The abbreviation scope ends when
the variable name scope ends.
