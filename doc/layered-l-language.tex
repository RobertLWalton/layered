...
The parser itself does \underline{not} have this convention builtin.
For example, the user added parser instructions
...
would allow one to write `{\tt ++ x ++}' which would parse as
`{\tt ++ (x ++)}', that is, postfix operators would be executed before
prefix operators.  However this seems confusing, so the builtin
parser instructions avoid this.
...
Prototype arguments with member names
\underline{must} be \underline{omitted} in the call itself.
They are given the values taken from members of their root values
given in the call.
...
Cluster member names may be used as prototype result
names, if the root of these names is either a prototype result or
a prototype argument.
Prototype results with member names
\underline{must} be \underline{omitted} in the call itself.
They will be created by the call.
A prototype cluster member result that is \underline{not} of the form
`\TT{next} \ldots' must \underline{not}
have a matching actual cluster member that
pre-exists the call, else the prototype-call match fails.
Note that qualifiers of cluster members created by the call are
