...
contain information necessary to do this.  These qualifiers are
built into the language: they are `{\tt local}', `{\tt stack}',
`{\tt global}', and `{\tt constant}'.
...
\ikey{Alias qualifiers}{alias qualifier} indicate that memory locations
may overlap.
These qualifiers are declared in the program and not built into the
language.
...
be given the same alias qualifier.  Aliased locations
may not be able to use software caching.

\ikey{Parallel qualifiers}{parallel qualifier} should be attached
to memory locations whose software \underline{and hardware}
caches are to be flushed by explicit cache flushing statements in the
L-Language.
These qualifiers are declared in the program and not built into the
language.
They are used for memory locations shared between different hardware
...
The caching qualifiers are all builtin.  They are `{\tt constant}',
`{\tt stack}', and `{\tt global}'.
...
This qualifier imposes no restrictions on where an address to the
...
\hspace*{-1em}\ttkey{stack}~~The memory location is or may be allocated
to a function frame, and it will or may be accessed by the function
execution of that frame and all functions called by that function
execution.

The memory location may be cached in a register, but this cache must
be flushed when the address of the location is passed as an argument
to a function.
...
When used in an argument or return value type, a `{\tt stack}' memory location
may be either located in a function frame or may be `{\tt global}'.
