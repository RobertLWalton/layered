...
The two kinds of qualifiers may be declared by:
\begin{center}
...
A variable with a protection qualifier can only be assigned its
value by being matched with a function result variable
that also has the protection qualifier.

As an example of the above rules, the \TT{mem} qualifier is
a protection qualifier, so if \TT{t} is a type and \TT{v} a
variable, an {\em value-assignment-statement} for \TT{v} must have the
form `\TT{mem t v = }{\em expression}' where \TT{v} is matched
with some result variable that has the \TT{mem} qualifier of some
function called in {\em expression}.
...
has all the qualifiers of \TT{v} and was originally computed first,
it will have been recomputed before \TT{v} is reloaded.
