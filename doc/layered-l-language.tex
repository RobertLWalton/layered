...
Implicit conversions may also be allows, such as the conversion of
...
Qualifier declarations are described later.  \ikey{Qualifiers}{qualifier}
have the syntax:
...
can be used for open files, exception catch data, etc.
...
Recall that within a function
\begin{indpar}\begin{verbatim}
int8 x -> float64 y
\end{verbatim}\end{indpar}
is shorthand for
\begin{indpar}\begin{verbatim}
constant stack adr -> int8 x -> float64 y
\end{verbatim}\end{indpar}
so `{\tt .get.(\&x)}' can be simplified to `{\tt x}'.
...
// Out of line function to allocate a vector of n T's.
//
( pointer pair pp -> T,
  unchecked adr b == pp.begin -> T,
  unchecked adr e == pp.end -> T )
    allocate ( type T, basic(T) descriptor, uns32 n )

// Prefix operator to dereference a pointer pair.
//
( adr a -> T ) "*"
        ( type T, pointer pair pp -> T,
          adr -> unchecked adr b == pp.begin -> T,
          adr -> unchecked adr e == pp.end   -> T )
    unchecked:
...
void my function ( void ):
    ...
    //
    pointer pair pp -> int32 = allocate ( 2 )
    * pp = 100
    * ++ pp = 101
    . . . .
