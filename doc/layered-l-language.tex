...
The buitin protection qualifiers are `{\tt pure}', `{\tt impure}'
and `{\tt unchecked}'.
...
a function with access qualifier $X$ can only be called by functions
whose bodies have qualifier $X$.  The builtin access
qualifier `{\tt unchecked}' permits access to various
builtin operations that explicitly or implicitly involve type
conversion.  The builtin access qualifier `{\tt impure}' permits
access to operations that allocate and reallocate memory
...
The `{\tt temp}' qualifier makes use of the `{\tt impure}' protection qualifier.
...
{protection qualifier}~~~~~Protection
...
The window of a memory channel is organized in the same way
for all standard memory channels, though user defined memory channels
may have a different organization.  In the standard organization,
which described here, the window appears to be an array with
\ttdmkey{dimensions}{of memory channel},
\ttdmkey{lower\_bound}{of memory channel}{\tt [}$i${\tt ]},
and \ttdmkey{upper\_bound}{of memory channel}{\tt [}$i${\tt ]}
being defined members.  The elements of the window each have a
value and an associated \mkey{block descriptor}{of memory channel}.
...
shifted, or when a neighboring element is read from memory.
...
Some memory channels access arrays stored in memory.  For these
the block descriptors can be computed from the array coordinates
