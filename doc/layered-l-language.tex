...
\emkey{qualifier-attribute}
    \begin{tabular}[t]{rl}
    ::= & \ttkey{required protection} \\
    $|$ & \ttkey{prohibited protection} \\
    $|$ & \ttkey{implied protection} \\
    $|$ & \ttkey{inherited} \\
    \end{tabular}
\end{indpar}

If a function prototype argument variable has a required
protection qualifier, any actual variable matched to this
must have the protection qualifier.  Thus in the above example,
any actual variable matched to the argument of \TT{F} or \TT{G}
must have the `\TT{bar}' qualifier.

If a function prototype result variable has a protection
qualifier (required, prohibited, or implied),
any actual variable matched to
this will be implicitly given the protection qualifier.
Furthermore, this is the \underline{only way} a variable may
acquire a protection qualifier.  In the above example,
variables matched to the result prototype variable of \TT{F}
are given the `\TT{bar}' qualifier, but this is not true of
variables matched to the result prototype variable of \TT{G}.

In the above example there are no rules regarding the `TT{foo}'
qualifier, which can be attached to any variable as the
user wishes, but is not required or prohibited of argument variables or
implied for result variables.

If a function prototype argument variable has a prohibited
protection qualifier, any actual variable matched to this
must \underline{not} have the protection qualifier.  Thus in the above example,
any actual variable matched to the argument of \TT{H}
must \underline{not} have the `\TT{off}' qualifier.

Note that the presence of implied protection qualifiers on prototype
argument variables has no affect on actual argument variables.
...
\begin{indpar}
\TT{implied~protection~qualifier~}\ttkey{mem} \\
\TT{prohibited~protection~qualifier~}\ttkey{stack} \\
\TT{qualifier~}\ttkey{*UNCHECKED*} \\
