...
\ttikey{operability}{of block}
    & \ttikey{integer}{block operability},
      ...
      \ttikey{address}{block operability},
      \ttikey{container}{block operability}, or
      ...
      blocks also have operability in case their contents are moved
      to a register.

      Note that if the {\tt operability} is `{\tt address}', the
      {\tt size} of the block determines the weight of the address
      (see~\pagref{WEIGHT}).
\end{tabular}\end{indpar}
\begin{indpar}\begin{tabular}{p{1.0in}p{4.0in}}
...
A value all of whose objects are constant is called a
\key{constant value}.  Other values are called \skey{variable value}s.
...
Various operations on values are classified as \ikey{compile-time}{operation}
or \ikey{run-time}{operation}.  An operation may be both.
A value is called \key{run-time} if it can be input to a run-time operation.
All values can be input to compile-time operations.

A value whose value proper is unknown is represented for practical
purposes by its type.  Such values are not run-time values.

A run-time value must have a constant exportable type.
...
Commonly a run-time value is a sequence of just two objects,
...
Both objects must be known.

Non-run-time types are commonly a sequence of two
objects, the first being a constant that indicates that the
second is a variable, with the second indicating that any
value proper associated with the type is unknown.  Such a type
is translated to be a run-time value by an information monotonic
operation known as \key{linking}.  This discards the first
object and makes the second into a constant object that indicates
any associated value proper is known.
