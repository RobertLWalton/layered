...
has a forwarding address stored at a fixed location that
...
A \key{pointer} is a block from which a block bit address can be
computed by a function whose only input is the bit sequence value of
the pointer.  The block whose bit address is computed is the
\key{target} of the pointer.  A pointer can be a light weight address
or it can be an index into some data structure in which light weight
addresses are stored.  A heavy weight address contains a pointer
...
A forwarding address has the same format as a light weight address.
...
can be used to remove all references so the old forwarding address to
...
A \key{pointer} is a block (typically holding just an address or an integer
index) from which a block bit address can be computed by a function whose
only input is the bit sequence value of the pointer.  The block whose
bit address is computed is the \key{target} of the pointer.
...
\item For each of these blocks, a (possibly empty) set of pointers to blocks
      ...
      in RAM memory, but blocks that are not targets can be free.
...
A \key{clique} is a subgraph of a block graph that consists of
a single root block and all blocks reachable from this root
by following graph pointers whose targets are not root blocks.
Thus there is only one root block in a clique, which is called
the \ikey{root}{of clique} of the clique.

A clique points at another clique if some block of the first
clique sources a graph pointer whose target is the root of
the second clique.  A set of cliques is \ikey{closed}{set of cliques}
if no clique in the set points at a clique outside the set.

Block graph blocks that are not stored in RAM memory cannot have bit
addresses or be targets of pointers.  Roots of cliques can be such blocks.
All non-root blocks must be have bit addresses and be stored in RAM memory,
...
An \key{object} is a clique in a block graph whose root is stored in
RAM memory and has a bit address.
The bit address of this root is the
\ikey{bit address}{of object} of the object.

A block graph is said to be \key{non-overlapping} if no two blocks
in the graph overlap and no two cliques in the graph overlap.
...
of the block, if all its cliques are objects (all roots have bit addresses),
if a closed set of object (cliques) can be copied to another
...
A \key{value} is a sequence of cliques.  The last clique is
the \key{value proper}.
The initial subsequence of all cliques except the last
is the \key{type} of the value, and is itself a value.

Note that the roots of the cliques need not be stored in RAM memory,
so the cliques need not be objects.  Also, the block graph of each clique
in a value can be \underline{different}.
...
the block size of the root.  The type must also specify the block graph
to which the value proper belongs.

Since a type is a value, its last clique is a value proper for the type,
and the sequence of preceding cliques is the type of the type.
...
produce outputs with more information.
...
that are constants, and cannot be changed.  They may, however,
contain pointers to changeable objects.

Types are frequently computed at compile time with
...
Normally at run-time a value is a sequence of just two cliques,
...
cliques, the first begin a constant type that indicates that the
...
discarding its first clique and making its second into a constant
object.
